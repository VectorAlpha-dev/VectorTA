// medprice.cu  — CUDA 13+
// Drop-in optimized kernels for MEDPRICE = 0.5f * (high + low)

#include <cuda_runtime.h>
#include <stdint.h>
#include <math.h>
#include <math_constants.h>
#include <math_functions.h> // __isnanf

#ifndef ROW_TILE
#define ROW_TILE 8       // tile height for one-series × many-params
#endif
...
static __device__ __forceinline__ float medprice_scalar(float h, float l) {
    // Propagates NaN if any input is NaN or if +inf + -inf
    const float s = h + l;
    return __isnanf(s) ? CUDART_NAN_F : 0.5f * s;
}

// -----------------------------------------
// 1) One series × one "param" (scalar path)
// -----------------------------------------
                         ...
                         float* __restrict__ out)
{
    ...
    // Stage A: prefix fill for indices before first valid
    for (int i = tid; i < len && i < fv; i += stride) {
        out[i] = CUDART_NAN_F;
    }

    // Stage B: main region [fv, len)
    const bool do_vec = is_aligned_16(high) && is_aligned_16(low) && is_aligned_16(out);
    if (do_vec) {
        // Align vecStart up to a multiple of 4 so float4 loads are aligned
        ...
        if (vecStart < len) {
            const int totalVec = (len - vecStart) >> 2; // number of float4s
        ...
        // Scalar fallback (unaligned pointers, unusual slicing)
...
// ----------------------------------------------------------------------
// 2) One series × many params (rows). We load high/low ONCE per index i
//    and broadcast medprice across a tile of rows (ROW_TILE) efficiently.
// ----------------------------------------------------------------------
extern "C" __global__ __launch_bounds__(256, 2)
                        ...
                        float* __restrict__ out)
{
    ...
    const bool do_vec = is_aligned_16(high) && is_aligned_16(low) && is_aligned_16(out);
    if (do_vec) {
        const int totalVec = len >> 2; // whole float4s
        ...
        for (int v = tid; v < totalVec; v += stride) {
            ...
            // Precompute medprice for this i..i+3 once
...
// -----------------------------------------------------------------
// 3) Many series × one param (time-major). Threads map to 'series'.
//    Loop over t is unrolled a bit; coalesced at each t across series.
// -----------------------------------------------------------------
extern "C" __global__ __launch_bounds__(256, 2)
                                        ...
                                        float* __restrict__ out_tm)
{
    ...
    // Iterate time-major; across a warp, s is contiguous => coalesced accesses per t.
    #pragma unroll 4
