...
impl<'a> TradjemaInput<'a> {
    ...
    #[inline]
    pub fn from_slices(high: &'a [f64], low: &'a [f64], close: &'a [f64], params: TradjemaParams) -> Self {
        Self {
...
impl TradjemaBuilder {
    ...
    #[inline(always)]
    pub fn apply_slices(self, high: &[f64], low: &[f64], close: &[f64]) -> Result<TradjemaOutput, TradjemaError> {
        let p = TradjemaParams {
            length: self.length,
            mult: self.mult,
        };
...
fn tradjema_prepare<'a>(
    ...
    let (high, low, close) = match &input.data {
        TradjemaData::Candles { candles } => {
            let h = candles.select_candle_field("high").map_err(|_| TradjemaError::EmptyInputData)?;
            let l = candles.select_candle_field("low").map_err(|_| TradjemaError::EmptyInputData)?;
            let c = candles.select_candle_field("close").map_err(|_| TradjemaError::EmptyInputData)?;
            (h, l, c)
        }
    ...
    let len = close.len();
    if len == 0 { return Err(TradjemaError::EmptyInputData); }

    let first = close.iter().position(|v| !v.is_nan()).ok_or(TradjemaError::AllValuesNaN)?;
    let length = input.get_length();
    if length < 2 || length > len {
        return Err(TradjemaError::InvalidLength { length, data_len: len });
    }
    if len - first < length {
        return Err(TradjemaError::NotEnoughValidData { needed: length, valid: len - first });
    }
...
pub fn tradjema_with_kernel(input: &TradjemaInput, kernel: Kernel) -> Result<TradjemaOutput, TradjemaError> {
...
pub fn tradjema_into_slice(
    ...
    if dst.len() != c.len() {
        return Err(TradjemaError::InvalidLength { length: dst.len(), data_len: c.len() });
    }
...
fn tradjema_compute_into_scalar(
    ...
    for &v in &tr_buf[1..] {
        if v < tr_low { tr_low = v; }
        if v > tr_high { tr_high = v; }
    }
    ...
    let tr_adj0 = if tr_high != tr_low { (current_tr0 - tr_low) / (tr_high - tr_low) } else { 0.0 };
    ...
    for i in (warm + 1)..out.len() {
        ...
        if tr_old <= tr_low || tr_old >= tr_high {
            ...
            if tr_new < tr_low { tr_low = tr_new; }
            if tr_new > tr_high { tr_high = tr_new; }
        }

        let tr_adj = if tr_high != tr_low { (tr_new - tr_low) / (tr_high - tr_low) } else { 0.0 };
...
impl TradjemaStream {
    ...
    pub fn update(&mut self, high: f64, low: f64, close: f64) -> Option<f64> {
        ...
        if !self.filled && self.idx == 0 {
            ...
            let src = if self.prev_close.is_nan() { close } else { self.prev_close }; // 1-bar lag
...
pub fn tradjema_py<'py>(
    ...
    if h.len() != l.len() || l.len() != c.len() {
        return Err(PyValueError::new_err("All OHLC arrays must have the same length"));
    }
    let kern = validate_kernel(kernel, false)?;
    let input = TradjemaInput::from_slices(h, l, c, TradjemaParams { length: Some(length), mult: Some(mult) });
...
pub fn tradjema_batch_py<'py>(
    ...
    let sweep = TradjemaBatchRange { length: length_range, mult: mult_range };
    ...
    let first = c.iter().position(|v| !v.is_nan())
    ...
    for (row, prm) in combos.iter().enumerate() {
        ...
        for v in &mut row_slice[..warm] { *v = f64::NAN; }
    }
    ...
    dict.set_item("lengths", combos.iter().map(|p| p.length.unwrap_or(40) as u64).collect::<Vec<_>>().into_pyarray(py))?;
    dict.set_item("mults",   combos.iter().map(|p| p.mult.unwrap_or(10.0)).collect::<Vec<_>>().into_pyarray(py))?;
    Ok(dict)
}

#[cfg(feature = "python")]
#[pyclass(name = "TradjemaStream")]
pub struct TradjemaStreamPy { inner: TradjemaStream }
...
impl TradjemaStreamPy {
    ...
    fn new(length: usize, mult: f64) -> PyResult<Self> {
        TradjemaStream::try_new(TradjemaParams { length: Some(length), mult: Some(mult) })
            .map(|inner| Self { inner })
            .map_err(|e| PyValueError::new_err(e.to_string()))
    }
    fn update(&mut self, high: f64, low: f64, close: f64) -> Option<f64> { self.inner.update(high, low, close) }
}
...
pub fn tradjema_into(
    ...
    if [high_ptr, low_ptr, close_ptr, out_ptr].iter().any(|p| p.is_null()) {
    ...
    unsafe {
        ...
        let params = TradjemaParams { length: Some(length), mult: Some(mult) };
        let input = TradjemaInput::from_slices(h, l, c, params);

        if (out_ptr as *const f64) == close_ptr || (out_ptr as *const f64) == high_ptr || (out_ptr as *const f64) == low_ptr {
            // aliasing: compute into temp then copy
            let mut tmp = vec![f64::NAN; len];
            tradjema_into_slice(&mut tmp, &input, detect_best_kernel()).map_err(|e| JsValue::from_str(&e.to_string()))?;
            std::slice::from_raw_parts_mut(out_ptr, len).copy_from_slice(&tmp);
        } else {
            let out = std::slice::from_raw_parts_mut(out_ptr, len);
            tradjema_into_slice(out, &input, detect_best_kernel()).map_err(|e| JsValue::from_str(&e.to_string()))?;
        }
        Ok(())
    }
}
...
pub fn tradjema_batch_into(
    ...
    unsafe {
        ...
        let first = close.iter().position(|v| !v.is_nan())
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn tradjema_js(high: &[f64], low: &[f64], close: &[f64], length: usize, mult: f64) -> Result<Vec<f64>, JsValue> {
    ...
    let first = close.iter().position(|v| !v.is_nan()).ok_or_else(|| JsValue::from_str("All values are NaN"))?;
    ...
    // Compute in place
    tradjema_compute_into(high, low, close, length, mult, first, detect_best_kernel(), &mut out);
...
pub struct TradjemaBatchConfig { pub length_range: (usize,usize,usize), pub mult_range: (f64,f64,f64) }

#[cfg(feature = "wasm")]
#[derive(Serialize, Deserialize)]
pub struct TradjemaBatchJsOutput { pub values: Vec<f64>, pub combos: Vec<TradjemaParams>, pub rows: usize, pub cols: usize }

#[cfg(feature = "wasm")]
#[wasm_bindgen(js_name = "tradjema_batch")]
pub fn tradjema_batch_unified_js(high: &[f64], low: &[f64], close: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
    let cfg: TradjemaBatchConfig = serde_wasm_bindgen::from_value(config).map_err(|e| JsValue::from_str(&format!("Invalid config: {e}")))?;
    let sweep = TradjemaBatchRange { 
    ...
    let js = TradjemaBatchJsOutput { values: out.values, combos: out.combos, rows: out.rows, cols: out.cols };
...
impl TradjemaBatchBuilder {
    ...
    pub fn with_default_candles(c: &Candles) -> Result<TradjemaBatchOutput, TradjemaError> {
        TradjemaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c)
    }
    ...
    pub fn apply_slices(self, high: &[f64], low: &[f64], close: &[f64]) -> Result<TradjemaBatchOutput, TradjemaError> {
    ...
    pub fn apply_candles(self, c: &Candles) -> Result<TradjemaBatchOutput, TradjemaError> {
        ...
        let low = c.select_candle_field("low")
            .map_err(|_| TradjemaError::EmptyInputData)?;
        let close = c.select_candle_field("close")
            .map_err(|_| TradjemaError::EmptyInputData)?;
...
fn tradjema_batch_inner_into(
    ...
    let combos = expand_grid(sweep);
    if combos.is_empty() {
        return Err(TradjemaError::InvalidLength { length: 0, data_len: 0 });
    }
    if high.len() != low.len() || low.len() != close.len() {
        return Err(TradjemaError::MissingData);
    }
    ...
    if parallel {
        ...
        for (row, slice) in out.chunks_mut(cols).enumerate() { do_row(row, slice); }
    } else {
        for (row, slice) in out.chunks_mut(cols).enumerate() { do_row(row, slice); }
    }
...
fn tradjema_batch_inner(
    ...
    let first = close.iter().position(|v| !v.is_nan()).ok_or(TradjemaError::AllValuesNaN)?;
    let warms: Vec<usize> = combos.iter().map(|p| first + p.length.unwrap_or(40) - 1).collect();
    ...
    Ok(TradjemaBatchOutput { values, combos, rows, cols })
}
...
mod tests {
    ...
    fn check_tradjema_partial_params(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
    ...
    fn check_tradjema_accuracy(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for i in 0..warmup {
            assert!(result.values[i].is_nan(), "[{}] Expected NaN during warmup at index {}", test_name, i);
        }
        
        // Check that we have valid values after warmup
        for i in warmup..result.values.len() {
            assert!(!result.values[i].is_nan(), "[{}] Expected valid value after warmup at index {}", test_name, i);
        }
    ...
    fn check_tradjema_default_candles(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
    ...
    fn check_tradjema_zero_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        assert!(res.is_err(), "[{}] TRADJEMA should fail with zero length", test_name);
        ...
        let input = TradjemaInput::from_slices(&input_data, &input_data, &input_data, params);
        let res = tradjema_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] TRADJEMA should fail with length=1 (minimum is 2)", test_name);
        
        Ok(())
    }
    
    fn check_tradjema_length_exceeds_data(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_tradjema_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        ...
        let res = tradjema_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] TRADJEMA should fail with insufficient data", test_name);
        Ok(())
    }
    ...
    fn check_tradjema_no_poison(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let test_params = vec![
            ...
            TradjemaParams { length: Some(10), mult: Some(5.0) },
            TradjemaParams { length: Some(20), mult: Some(7.5) },
            TradjemaParams { length: Some(50), mult: Some(15.0) },
            TradjemaParams { length: Some(100), mult: Some(20.0) },
        ];
    ...
    fn check_tradjema_property(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        use proptest::prelude::*;
        skip_if_unsupported!(kernel, test_name);
        ...
        proptest::test_runner::TestRunner::default()
            .run(&strat, |(data, length, mult)| {
                ...
                let TradjemaOutput { values: ref_out } = tradjema_with_kernel(&input, Kernel::Scalar).unwrap();
    ...
    fn check_tradjema_into_slice(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let (h,l,cl) = (c.select_candle_field("high")?, c.select_candle_field("low")?, c.select_candle_field("close")?);
        ...
        assert!(dst[..warm].iter().all(|v| v.is_nan()), "[{}] warmup prefix must be NaN", test_name);
        Ok(())
    }
    ...
    fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let output = TradjemaBatchBuilder::new().kernel(kernel).apply_candles(&c)?;
    ...
    fn check_batch_no_poison(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for (cfg_idx, &(l_start, l_end, l_step, m_start, m_end, m_step)) in test_configs.iter().enumerate() {
            let output = TradjemaBatchBuilder::new()
                .kernel(kernel)
    ...
    fn test_expand_grid_single_value() {
        ...
        assert!(!combos.is_empty(), "expand_grid should not return empty for single value");
        ...
        assert!(!combos2.is_empty(), "expand_grid should not return empty for single value (40,40,0)");
        assert_eq!(combos2.len(), 1, "Should have exactly one combo for (40,40,0)");
