...
impl CudaEr {
    ...
    pub fn er_batch_dev(
        ...
        let d_periods =
            DeviceBuffer::from_slice(&periods).map_err(|e| CudaErError::Cuda(e.to_string()))?;
        ...
        if !want_f32 && !want_prefix && self.module.get_function("er_batch_f64").is_ok() {
            let func = self
                .module
                .get_function("er_batch_f64")
                .map_err(|e| CudaErError::Cuda(e.to_string()))?;
            let data_f64: Vec<f64> = data_f32.iter().map(|&v| v as f64).collect();
            let d_data64 = DeviceBuffer::from_slice(&data_f64)
                .map_err(|e| CudaErError::Cuda(e.to_string()))?;
            let block_x: u32 = 256;
            ...
            && self.module.get_function("er_batch_f32").is_ok()
            && (want_f32 || len < 8192 || n_combos < 64)
        {
            let func = self
                .module
            ...
            let d_prefix =
                DeviceBuffer::from_slice(&prefix).map_err(|e| CudaErError::Cuda(e.to_string()))?;
            let func = self
                .module
