...
impl CudaAtr {
    ...
    pub fn set_policy(&mut self, policy: CudaAtrPolicy) { self.policy = policy; }
    pub fn synchronize(&self) -> Result<(), CudaAtrError> { self.stream.synchronize().map_err(|e| CudaAtrError::Cuda(e.to_string())) }
    ...
    pub fn atr_batch_dev(
        ...
        let len = close.len();
        if len == 0 { return Err(CudaAtrError::InvalidInput("empty input".into())); }
        ...
        if start == 0 { return Err(CudaAtrError::InvalidInput("period must be > 0".into())); }
        let periods: Vec<usize> = if step == 0 || start == end {
            vec![start]
        } else {
            (start..=end).step_by(step).collect()
        };
        if periods.is_empty() { return Err(CudaAtrError::InvalidInput("no parameter combos".into())); }
        for &p in &periods {
        ...
        for i in 0..len { prefix[i + 1] = prefix[i] + (tr[i] as f64); }

        let d_tr = DeviceBuffer::from_slice(&tr).map_err(|e| CudaAtrError::Cuda(e.to_string()))?;
        let d_prefix = DeviceBuffer::from_slice(&prefix).map_err(|e| CudaAtrError::Cuda(e.to_string()))?;
        ...
        let mut h_warms: Vec<i32> = periods.iter().map(|&p| (first_valid + p - 1) as i32).collect();
        let d_periods = DeviceBuffer::from_slice(&h_periods_i32).map_err(|e| CudaAtrError::Cuda(e.to_string()))?;
        let d_alphas  = DeviceBuffer::from_slice(&h_alphas).map_err(|e| CudaAtrError::Cuda(e.to_string()))?;
        let d_warms   = DeviceBuffer::from_slice(&h_warms).map_err(|e| CudaAtrError::Cuda(e.to_string()))?;
        ...
        let func = match self.module.get_function("atr_batch_from_tr_prefix_f32") {
            Ok(f) => f,
            Err(_) => self.module.get_function("atr_batch_f32").map_err(|e| CudaAtrError::Cuda(e.to_string()))?,
        };
        ...
        while launched < n_combos {
            ...
            unsafe {
                ...
                self.stream.launch(&func, grid, block, 0, args).map_err(|e| CudaAtrError::Cuda(e.to_string()))?;
            }
    ...
    fn first_valids_time_major(high_tm: &[f32], low_tm: &[f32], close_tm: &[f32], cols: usize, rows: usize) -> Result<Vec<i32>, CudaAtrError> {
        let n = cols.checked_mul(rows).ok_or_else(|| CudaAtrError::InvalidInput("rows*cols overflow".into()))?;
        if high_tm.len() != n || low_tm.len() != n || close_tm.len() != n {
            return Err(CudaAtrError::InvalidInput("time-major input length mismatch".into()));
        }
    ...
    pub fn atr_many_series_one_param_time_major_dev(
        ...
        if period == 0 { return Err(CudaAtrError::InvalidInput("period must be > 0".into())); }
        let first_valids = Self::first_valids_time_major(high_tm, low_tm, close_tm, cols, rows)?;
        if rows < period { return Err(CudaAtrError::InvalidInput("not enough rows for period".into())); }

        let mut d_high = DeviceBuffer::from_slice(high_tm).map_err(|e| CudaAtrError::Cuda(e.to_string()))?;
        let mut d_low  = DeviceBuffer::from_slice(low_tm).map_err(|e| CudaAtrError::Cuda(e.to_string()))?;
        let mut d_close= DeviceBuffer::from_slice(close_tm).map_err(|e| CudaAtrError::Cuda(e.to_string()))?;
        let d_fv   = DeviceBuffer::from_slice(&first_valids).map_err(|e| CudaAtrError::Cuda(e.to_string()))?;
        let mut d_out: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(cols * rows) }
            .map_err(|e| CudaAtrError::Cuda(e.to_string()))?;

        let func = self.module.get_function("atr_many_series_one_param_f32")
            .map_err(|e| CudaAtrError::Cuda(e.to_string()))?;
...
pub mod benches {
    ...
    impl CudaBenchState for AtrBatchState { fn launch(&mut self) { let _ = self.cuda.atr_batch_dev(&self.high, &self.low, &self.close, &self.sweep).unwrap(); } }
    ...
    impl CudaBenchState for AtrManyState { fn launch(&mut self) { let _ = self.cuda.atr_many_series_one_param_time_major_dev(&self.high_tm, &self.low_tm, &self.close_tm, self.cols, self.rows, self.period).unwrap(); } }

    struct BatchPrepCfg;
    fn prep_one_series_many_params() -> Box<dyn CudaBenchState> {
        let len = ONE_SERIES_LEN;
        let pstart = 5usize; let pend = 64usize; let pstep = 5usize;
        ...
        Box::new(AtrBatchState {
            ...
            low,
            close,
            sweep: AtrBatchRange { length: (pstart, pend, pstep) },
        })
    }
    ...
    fn prep_many_series_one_param() -> Box<dyn CudaBenchState> {
        ...
        Box::new(AtrManyState { cuda: CudaAtr::new(0).unwrap(), high_tm, low_tm, close_tm, cols, rows, period })
    }
    ...
    pub fn bench_profiles() -> Vec<CudaBenchScenario> {
        ...
        .with_mem_required((3 * cols * rows + cols * rows) * std::mem::size_of::<f32>() + 64 * 1024 * 1024);
