...
#[derive(Clone, Copy, Debug)]
pub struct CudaSmiPolicy { pub batch: BatchKernelPolicy, pub many_series: ManySeriesKernelPolicy }
impl Default for CudaSmiPolicy { fn default() -> Self { Self { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::Auto } } }
...
#[derive(Clone, Debug)]
struct SmCombo { lbb: usize, mbb: f32, lkc: usize, mkc: f32 }
...
impl CudaSqueezeMomentum {
    ...
    pub fn set_policy(&mut self, policy: CudaSmiPolicy) { self.policy = policy; }
    pub fn policy(&self) -> &CudaSmiPolicy { &self.policy }
    pub fn selected_batch_kernel(&self) -> Option<BatchKernelSelected> { self.last_batch }
    pub fn selected_many_series_kernel(&self) -> Option<ManySeriesKernelSelected> { self.last_many }
    pub fn synchronize(&self) -> Result<(), CudaSmiError> {
        self.stream.synchronize().map_err(|e| CudaSmiError::Cuda(e.to_string()))
    }
    ...
    fn expand_grid(sweep: &SqueezeMomentumBatchRange) -> Vec<SmCombo> {
        ...
        for &a in &lbb { for &b in &mbb { for &c in &lkc { for &d in &mkc { out.push(SmCombo { lbb: a, mbb: b as f32, lkc: c, mkc: d as f32 }); }}}}
        out
    }

    fn prepare_batch_inputs(
        high: &[f32], low: &[f32], close: &[f32], sweep: &SqueezeMomentumBatchRange,
    ) -> Result<(Vec<SmCombo>, usize, usize), CudaSmiError> {
        if high.len() != low.len() || low.len() != close.len() {
            return Err(CudaSmiError::InvalidInput("inconsistent array lengths".into()));
        }
        if close.is_empty() { return Err(CudaSmiError::InvalidInput("empty data".into())); }
        let len = close.len();
        let first_valid = (0..len)
            .find(|&i| !(high[i].is_nan() || low[i].is_nan() || close[i].is_nan()))
            .ok_or_else(|| CudaSmiError::InvalidInput("all values are NaN".into()))?;
        let combos = Self::expand_grid(sweep);
        if combos.is_empty() { return Err(CudaSmiError::InvalidInput("no parameter combos".into())); }
        // Feasibility: require tail >= max(lbb, lkc)
        let mut need = 0usize;
        for c in &combos { need = need.max(c.lbb.max(c.lkc)); }
        let tail = len - first_valid;
        if tail < need {
            return Err(CudaSmiError::InvalidInput(format!(
                "not enough valid data: needed {}, valid {}", need, tail
            )));
        }
    ...
    fn launch_batch_kernel(
        ...
        d_lbb: &DeviceBuffer<i32>, d_mbb: &DeviceBuffer<f32>, d_lkc: &DeviceBuffer<i32>, d_mkc: &DeviceBuffer<f32>,
        len: usize,
        n_combos: usize,
        first_valid: usize,
        d_sq: &mut DeviceBuffer<f32>, d_mo: &mut DeviceBuffer<f32>, d_si: &mut DeviceBuffer<f32>,
    ) -> Result<(), CudaSmiError> {
        ...
        let block_x: u32 = match self.policy.batch { BatchKernelPolicy::Auto => 256, BatchKernelPolicy::OneD { block_x } => block_x.max(32) };
        ...
        unsafe { (*(self as *const _ as *mut CudaSqueezeMomentum)).last_batch = Some(BatchKernelSelected::OneD { block_x }); }
        Ok(())
    }
    ...
    pub fn squeeze_momentum_batch_dev(
        ...
        let params_bytes = combos.len() * (2 * std::mem::size_of::<i32>() + 2 * std::mem::size_of::<f32>());
        ...
        // Upload inputs
        let d_h = DeviceBuffer::from_slice(high_f32).map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let d_l = DeviceBuffer::from_slice(low_f32).map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let d_c = DeviceBuffer::from_slice(close_f32).map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        ...
        let d_lbb = DeviceBuffer::from_slice(&v_lbb).map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let d_mbb = DeviceBuffer::from_slice(&v_mbb).map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let d_lkc = DeviceBuffer::from_slice(&v_lkc).map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let d_mkc = DeviceBuffer::from_slice(&v_mkc).map_err(|e| CudaSmiError::Cuda(e.to_string()))?;

        // Allocate outputs
        let elems = combos.len() * len;
        let mut d_sq: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let mut d_mo: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let mut d_si: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaSmiError::Cuda(e.to_string()))?;

        self.launch_batch_kernel(&d_h, &d_l, &d_c, &d_lbb, &d_mbb, &d_lkc, &d_mkc, len, combos.len(), first_valid, &mut d_sq, &mut d_mo, &mut d_si)?;
        self.stream.synchronize().map_err(|e| CudaSmiError::Cuda(e.to_string()))?;

        Ok((
            DeviceArrayF32 { buf: d_sq, rows: combos.len(), cols: len },
            DeviceArrayF32 { buf: d_mo, rows: combos.len(), cols: len },
            DeviceArrayF32 { buf: d_si, rows: combos.len(), cols: len },
        ))
    }
    ...
    pub fn squeeze_momentum_many_series_one_param_time_major_dev(
        ...
        if cols == 0 || rows == 0 { return Err(CudaSmiError::InvalidInput("cols or rows is zero".into())); }
        let expected = cols * rows;
        if high_tm_f32.len() != expected || low_tm_f32.len() != expected || close_tm_f32.len() != expected {
            return Err(CudaSmiError::InvalidInput("time-major arrays length mismatch".into()));
        }
        ...
        for s in 0..cols {
            ...
            for r in 0..rows {
                let idx = r * cols + s;
                let h = high_tm_f32[idx]; let l = low_tm_f32[idx]; let c = close_tm_f32[idx];
                if !(h.is_nan() || l.is_nan() || c.is_nan()) { found = Some(r); break; }
            }
            let fv_s = found.ok_or_else(|| CudaSmiError::InvalidInput(format!("series {} all NaN", s)))?;
            if rows - fv_s < lbb.max(lkc) {
                return Err(CudaSmiError::InvalidInput(format!(
                    "series {} not enough valid data (needed {}, valid {})", s, lbb.max(lkc), rows - fv_s
                )));
            }
            fv[s] = fv_s as i32;
        }

        let d_h = DeviceBuffer::from_slice(high_tm_f32).map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let d_l = DeviceBuffer::from_slice(low_tm_f32).map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let d_c = DeviceBuffer::from_slice(close_tm_f32).map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let d_fv = DeviceBuffer::from_slice(&fv).map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let mut d_sq_tm: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(expected) }.map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let mut d_mo_tm: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(expected) }.map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        let mut d_si_tm: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(expected) }.map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        ...
        let block_x: u32 = match self.policy.many_series { ManySeriesKernelPolicy::Auto => 1, ManySeriesKernelPolicy::OneD { block_x } => block_x.max(1) };
        ...
        self.stream.synchronize().map_err(|e| CudaSmiError::Cuda(e.to_string()))?;
        unsafe { (*(self as *const _ as *mut CudaSqueezeMomentum)).last_many = Some(ManySeriesKernelSelected::OneD { block_x }); }

        Ok((
            DeviceArrayF32 { buf: d_sq_tm, rows, cols },
            DeviceArrayF32 { buf: d_mo_tm, rows, cols },
            DeviceArrayF32 { buf: d_si_tm, rows, cols },
        ))
    }
}
...
pub mod benches {
    ...
    struct SmiBatchState { cuda: CudaSqueezeMomentum, h: Vec<f32>, l: Vec<f32>, c: Vec<f32>, sweep: SqueezeMomentumBatchRange }
    impl CudaBenchState for SmiBatchState {
        fn launch(&mut self) { let _ = self.cuda.squeeze_momentum_batch_dev(&self.h, &self.l, &self.c, &self.sweep).unwrap(); }
    }
    fn prep_one_series_many_params() -> Box<dyn CudaBenchState> {
        ...
        let sweep = SqueezeMomentumBatchRange { length_bb: (10, 10 + PARAM_SWEEP - 1, 1), mult_bb: (2.0, 2.0, 0.0), length_kc: (10, 10, 0), mult_kc: (1.5, 1.5, 0.0) };
        Box::new(SmiBatchState { cuda, h, l, c, sweep })
    }
