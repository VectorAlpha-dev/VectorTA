...
impl CudaStc {
    pub fn new(device_id: usize) -> Result<Self, CudaStcError> {
        ...
        Ok(Self { module, stream, _context: context, policy: CudaStcPolicy::default() })
    }

    pub fn set_policy(&mut self, policy: CudaStcPolicy) { self.policy = policy; }
    pub fn policy(&self) -> &CudaStcPolicy { &self.policy }
    pub fn synchronize(&self) -> Result<(), CudaStcError> { self.stream.synchronize().map_err(|e| CudaStcError::Cuda(e.to_string())) }
    ...
    fn expand_grid(sweep: &StcBatchRange) -> Vec<StcParams> {
        ...
        for &f in &fs { for &s in &ss { for &k in &ks { for &d in &ds {
            out.push(StcParams { fast_period: Some(f), slow_period: Some(s), k_period: Some(k), d_period: Some(d), fast_ma_type: None, slow_ma_type: None });
        }}}}
        out
    }

    fn validate_first_valid(data: &[f32], max_needed: usize) -> Result<usize, CudaStcError> {
        if data.is_empty() { return Err(CudaStcError::InvalidInput("empty data".into())); }
    ...
    pub fn stc_batch_dev(
        ...
        if combos.is_empty() { return Err(CudaStcError::InvalidInput("empty sweep".into())); }

        let len = data.len();
        let max_needed = combos.iter().map(|c| {
            c.fast_period.unwrap().max(c.slow_period.unwrap()).max(c.k_period.unwrap()).max(c.d_period.unwrap())
        }).max().unwrap();
        ...
        if !Self::device_mem_ok(req) {
            return Err(CudaStcError::InvalidInput("insufficient device memory".into()));
        }

        // Upload input
        let h = LockedBuffer::from_slice(data).map_err(|e| CudaStcError::Cuda(e.to_string()))?;
        let mut d_prices: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized_async(len, &self.stream) }
            .map_err(|e| CudaStcError::Cuda(e.to_string()))?;
        unsafe { d_prices.async_copy_from(&h, &self.stream).map_err(|e| CudaStcError::Cuda(e.to_string()))?; }
        ...
        let func = self.module.get_function("stc_batch_f32").map_err(|e| CudaStcError::Cuda(e.to_string()))?;
        let block_x = match self.policy.batch { BatchKernelPolicy::Auto => 256, BatchKernelPolicy::Plain { block_x } => block_x.max(32) };
        let shmem_bytes = (2 * max_k * std::mem::size_of::<f32>()) + (2 * max_k * std::mem::size_of::<u8>());

        for (start, count) in Self::grid_x_chunks(rows) {
            let mut d_f = DeviceBuffer::from_slice(&fasts[start..start + count]).map_err(|e| CudaStcError::Cuda(e.to_string()))?;
            let mut d_s = DeviceBuffer::from_slice(&slows[start..start + count]).map_err(|e| CudaStcError::Cuda(e.to_string()))?;
            let mut d_k = DeviceBuffer::from_slice(&ks[start..start + count]).map_err(|e| CudaStcError::Cuda(e.to_string()))?;
            let mut d_d = DeviceBuffer::from_slice(&ds[start..start + count]).map_err(|e| CudaStcError::Cuda(e.to_string()))?;
            ...
            unsafe {
                ...
                let mut o_ptr = d_out.as_device_ptr().as_raw().wrapping_add((start * len * std::mem::size_of::<f32>()) as u64);
                ...
                self.stream.launch(&func, grid, block, shmem_bytes.try_into().unwrap_or(u32::MAX), &mut args)
                    .map_err(|e| CudaStcError::Cuda(e.to_string()))?;
            }
        }

        self.stream.synchronize().map_err(|e| CudaStcError::Cuda(e.to_string()))?;
    ...
    pub fn stc_many_series_one_param_time_major_dev(
        ...
        if cols == 0 || rows == 0 { return Err(CudaStcError::InvalidInput("empty matrix".into())); }
        if data_tm.len() != cols * rows { return Err(CudaStcError::InvalidInput("matrix shape mismatch".into())); }
        ...
        for s in 0..cols {
            for t in 0..rows {
                let idx = t * cols + s;
                if data_tm[idx].is_finite() { first_valids[s] = t as i32; break; }
            }
            let fv = first_valids[s] as usize;
            let warm = fv + fast.max(slow).max(k).max(d) - 1;
            if warm >= rows { return Err(CudaStcError::InvalidInput("not enough valid data for at least one series".into())); }
        }

        // VRAM estimate: inputs + first_valids + outputs
        let req = (cols * rows + cols + cols * rows) * std::mem::size_of::<f32>();
        if !Self::device_mem_ok(req) { return Err(CudaStcError::InvalidInput("insufficient device memory".into())); }

        // Upload inputs
        let mut d_data = DeviceBuffer::from_slice(data_tm).map_err(|e| CudaStcError::Cuda(e.to_string()))?;
        let mut d_first = DeviceBuffer::from_slice(&first_valids).map_err(|e| CudaStcError::Cuda(e.to_string()))?;
        let mut d_out: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized_async(cols * rows, &self.stream) }
            .map_err(|e| CudaStcError::Cuda(e.to_string()))?;

        // Launch kernel
        let func = self.module.get_function("stc_many_series_one_param_f32").map_err(|e| CudaStcError::Cuda(e.to_string()))?;
        ...
        unsafe {
            ...
            let mut c_i = cols as i32; let mut r_i = rows as i32;
            let mut fast_i = fast as i32; let mut slow_i = slow as i32;
            let mut k_i = k as i32; let mut d_i = d as i32;
            ...
            self.stream.launch(&func, grid, block, 0, &mut args).map_err(|e| CudaStcError::Cuda(e.to_string()))?;
        }
...
pub mod benches {
    ...
    pub fn bench_profiles() -> Vec<CudaBenchScenario> {
        ...
        v.push(CudaBenchScenario::new(
            ...
            "100k x 64",
            || {
                struct State { cuda: CudaStc, data: Vec<f32>, sweep: StcBatchRange }
                impl CudaBenchState for State { fn launch(&mut self) { let _ = self.cuda.stc_batch_dev(&self.data, &self.sweep); } }
                let n = 100_000usize; let mut data = vec![f32::NAN; n];
                for i in 200..n { let x = i as f32; data[i] = (x * 0.0013).sin() + 0.0002 * x; }
                let sweep = StcBatchRange { fast_period: (10, 25, 5), slow_period: (30, 60, 10), k_period: (10, 10, 0), d_period: (3, 3, 0) };
                Box::new(State { cuda: CudaStc::new(0).unwrap(), data, sweep })
            }
        ).with_sample_size(20));
        ...
        v.push(CudaBenchScenario::new(
            ...
            "2048r x 512c",
            || {
                struct State { cuda: CudaStc, data_tm: Vec<f32>, cols: usize, rows: usize, params: StcParams }
                impl CudaBenchState for State { fn launch(&mut self) { let _ = self.cuda.stc_many_series_one_param_time_major_dev(&self.data_tm, self.cols, self.rows, &self.params); } }
                ...
                let params = StcParams { fast_period: Some(23), slow_period: Some(50), k_period: Some(10), d_period: Some(3), fast_ma_type: None, slow_ma_type: None };
                Box::new(State { cuda: CudaStc::new(0).unwrap(), data_tm: tm, cols, rows, params })
            }
        ).with_sample_size(20));
