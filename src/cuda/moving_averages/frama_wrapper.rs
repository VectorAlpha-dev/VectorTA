...
impl CudaFrama {
    ...
    pub fn frama_batch_dev_from_device(
        ...
        let combos = expand_grid(sweep);
        if combos.is_empty() {
            return Err(CudaFramaError::InvalidInput("no parameter combinations".into()));
        }
        ...
        Ok((
            DeviceArrayF32 { buf: d_out, rows: combos.len(), cols: series_len },
            combos,
        ))
    }
    ...
    pub fn frama_many_series_one_param_time_major_dev_from_device(
        ...
        let window = params
            .window
            .ok_or_else(|| CudaFramaError::InvalidInput("window parameter must be provided".into()))?
            as i32;
        let sc = params
            .sc
            .ok_or_else(|| CudaFramaError::InvalidInput("sc parameter must be provided".into()))? as i32;
        let fc = params
            .fc
            .ok_or_else(|| CudaFramaError::InvalidInput("fc parameter must be provided".into()))? as i32;
