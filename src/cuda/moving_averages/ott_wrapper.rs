...
impl CudaOtt {
    ...
    fn memset_nan32_async(&self, dst_ptr_raw: u64, n_elems: usize) -> Result<(), CudaOttError> {
        ...
        unsafe {
            ...
            let err = cuMemsetD32Async(dst_ptr_raw as cust::sys::CUdeviceptr, QNAN_BITS, n_elems, self.stream.as_inner());
    ...
    fn maybe_log_batch_debug(&self) {
        ...
        if std::env::var("BENCH_DEBUG").ok().as_deref() == Some("1") {
            if let Some(sel) = self.last_batch {
                ...
                unsafe { (*(self as *const _ as *mut CudaOtt)).debug_batch_logged = true; }
            }
        }
    }
    ...
    fn maybe_log_many_debug(&self) {
        ...
        if std::env::var("BENCH_DEBUG").ok().as_deref() == Some("1") {
            if let Some(sel) = self.last_many {
                ...
                unsafe { (*(self as *const _ as *mut CudaOtt)).debug_many_logged = true; }
            }
        }
    }
    ...
    pub fn ott_batch_dev(&self, prices_f32: &[f32], sweep: &OttBatchRange) -> Result<DeviceArrayF32, CudaOttError> {
        ...
        if combos.is_empty() {
            return Err(CudaOttError::InvalidInput("no parameter combinations".into()));
        }
        ...
        if !Self::will_fit(bytes, headroom) {
            return Err(CudaOttError::InvalidInput("insufficient free VRAM for OTT batch".into()));
        }

        // Stage prices once
        let mut d_prices = unsafe { DeviceBuffer::<f32>::uninitialized(cols) }.map_err(|e| CudaOttError::Cuda(e.to_string()))?;
        unsafe { d_prices.async_copy_from(prices_f32, &self.stream) }.map_err(|e| CudaOttError::Cuda(e.to_string()))?;

        // Allocate output and prefill qNaN
        let mut d_out = unsafe { DeviceBuffer::<f32>::uninitialized(rows * cols) }.map_err(|e| CudaOttError::Cuda(e.to_string()))?;
        ...
        if f_apply.is_none() {
            return Err(CudaOttError::Cuda("missing ott_apply_single_f32 symbol".into()));
        }

        // Reusable 1-element device buffers for var path
        let mut d_period = unsafe { DeviceBuffer::<i32>::uninitialized(1) }.map_err(|e| CudaOttError::Cuda(e.to_string()))?;
        let mut d_percent = unsafe { DeviceBuffer::<f32>::uninitialized(1) }.map_err(|e| CudaOttError::Cuda(e.to_string()))?;
        ...
        for (row_idx, p) in combos.iter().enumerate() {
            ...
            if ma_type.eq_ignore_ascii_case("VAR") {
                ...
                if let Some(ref mut func) = f_var {
                    // Copy scalars
                    unsafe { d_period.async_copy_from(&[period as i32], &self.stream) }.map_err(|e| CudaOttError::Cuda(e.to_string()))?;
                    unsafe { d_percent.async_copy_from(&[percent], &self.stream) }.map_err(|e| CudaOttError::Cuda(e.to_string()))?;
                    ...
                    unsafe {
                        ...
                        self.stream.launch(func, grid, block, 0, args).map_err(|e| CudaOttError::Cuda(e.to_string()))?;
                    }
                } else {
                    ...
                    let dev = selector
                        ...
                        .map_err(|e| CudaOttError::Cuda(e.to_string()))?;
                    self.launch_apply_single(&mut f_apply.as_mut().unwrap(), &dev.buf, cols, percent, out_row_ptr.as_raw())?;
                    // Ensure kernel completion before dropping dev buffer
                    self.stream.synchronize().map_err(|e| CudaOttError::Cuda(e.to_string()))?;
                }
            } else {
    ...
    pub fn ott_many_series_one_param_time_major_dev(
        ...
        if cols == 0 || rows == 0 { return Err(CudaOttError::InvalidInput("empty input".into())); }
        if data_tm_f32.len() != cols * rows { return Err(CudaOttError::InvalidInput("shape mismatch".into())); }
        ...
        // Output buffer
        let mut d_out = unsafe { DeviceBuffer::<f32>::uninitialized(cols * rows) }.map_err(|e| CudaOttError::Cuda(e.to_string()))?;
        self.memset_nan32_async(d_out.as_device_ptr().as_raw() as u64, cols * rows)?;

        if ma_type.eq_ignore_ascii_case("VAR") {
            // VAR-integrated path
            let mut d_in = unsafe { DeviceBuffer::<f32>::uninitialized(cols * rows) }.map_err(|e| CudaOttError::Cuda(e.to_string()))?;
            unsafe { d_in.async_copy_from(data_tm_f32, &self.stream) }.map_err(|e| CudaOttError::Cuda(e.to_string()))?;
            let mut func = self.module.get_function("ott_from_var_many_series_one_param_f32").map_err(|e| CudaOttError::Cuda(e.to_string()))?;
            unsafe {
                ...
                let block: BlockSize = (1, 1, 1).into();
                self.stream.launch(&mut func, grid, block, 0, args).map_err(|e| CudaOttError::Cuda(e.to_string()))?;
            }
        } else {
            ...
            let ma_dev = if ma_type.eq_ignore_ascii_case("EMA") {
                let p = crate::indicators::moving_averages::ema::EmaParams { period: Some(period) };
                CudaEma::new(self.device_id as usize).map_err(|e| CudaOttError::Cuda(e.to_string()))?
                    .ema_many_series_one_param_time_major_dev(data_tm_f32, cols, rows, &p)
                    .map_err(|e| CudaOttError::Cuda(e.to_string()))?
            } else if ma_type.eq_ignore_ascii_case("SMA") {
                let p = crate::indicators::moving_averages::sma::SmaParams { period: Some(period) };
                CudaSma::new(self.device_id as usize).map_err(|e| CudaOttError::Cuda(e.to_string()))?
                    .sma_multi_series_one_param_time_major_dev(data_tm_f32, cols, rows, &p)
                    .map_err(|e| CudaOttError::Cuda(e.to_string()))?
            } else if ma_type.eq_ignore_ascii_case("ZLEMA") {
                let p = crate::indicators::moving_averages::zlema::ZlemaParams { period: Some(period) };
                CudaZlema::new(self.device_id as usize).map_err(|e| CudaOttError::Cuda(e.to_string()))?
                    .zlema_many_series_one_param_time_major_dev(data_tm_f32, cols, rows, &p)
                    .map_err(|e| CudaOttError::Cuda(e.to_string()))?
            } else if ma_type.eq_ignore_ascii_case("WILDERS") || ma_type.eq_ignore_ascii_case("WWMA") {
                let p = crate::indicators::moving_averages::wilders::WildersParams { period: Some(period) };
                CudaWilders::new(self.device_id as usize).map_err(|e| CudaOttError::Cuda(e.to_string()))?
                    .wilders_many_series_one_param_time_major_dev(data_tm_f32, cols, rows, &p)
                    .map_err(|e| CudaOttError::Cuda(e.to_string()))?
            } else if ma_type.eq_ignore_ascii_case("KAMA") {
                let p = crate::indicators::moving_averages::kama::KamaParams { period: Some(period) };
                CudaKama::new(self.device_id as usize).map_err(|e| CudaOttError::Cuda(e.to_string()))?
                    .kama_many_series_one_param_time_major_dev(data_tm_f32, cols, rows, &p)
                    .map_err(|e| CudaOttError::Cuda(e.to_string()))?
            } else if ma_type.eq_ignore_ascii_case("VWMA") {
                return Err(CudaOttError::Unsupported("vwma requires candles+volume; not supported in this path".into()));
            } else if ma_type.eq_ignore_ascii_case("VPWMA") {
                // Needs power; use default power=0.382 for parity with selector
                let p = crate::indicators::moving_averages::vpwma::VpwmaParams { period: Some(period), power: Some(0.382) };
                CudaVpwma::new(self.device_id as usize).map_err(|e| CudaOttError::Cuda(e.to_string()))?
                    .vpwma_multi_series_one_param_time_major_dev(data_tm_f32, cols, rows, &p)
                    .map_err(|e| CudaOttError::Cuda(e.to_string()))?
            } else if ma_type.eq_ignore_ascii_case("NAMA") {
                let p = crate::indicators::moving_averages::nama::NamaParams { period: Some(period), ..Default::default() };
                CudaNama::new(self.device_id as usize).map_err(|e| CudaOttError::Cuda(e.to_string()))?
                ...
                return Err(CudaOttError::Unsupported("volume/anchor-based MA not supported in ott_many_series path".into()));
            } else {
                return Err(CudaOttError::Unsupported(format!("unsupported ma_type '{}' for OTT CUDA many-series", ma_type)));
            };

            // Launch apply-many over rows
            let mut func = self.module.get_function("ott_many_series_one_param_f32").map_err(|e| CudaOttError::Cuda(e.to_string()))?;
            unsafe {
...
pub mod benches {
    ...
    define_ma_period_benches!(
        ...
        crate::indicators::ott::OttParams { period: Some(16), percent: Some(1.4), ma_type: Some("VAR".to_string()) },
        "ott",
        "ott"
    );
...
fn expand_combos(range: &OttBatchRange) -> Vec<OttParams> {
    fn axis_usize(axis: (usize, usize, usize)) -> Vec<usize> {
        ...
        let mut out = Vec::new();
        let mut v = start;
        while v <= end { out.push(v); v = v.saturating_add(step); }
        out
    }
    fn axis_f64(axis: (f64, f64, f64)) -> Vec<f64> {
        ...
        let mut out = Vec::new();
        let mut v = start;
        while v <= end + 1e-12 { out.push(v); v += step; }
        out
    }
    let periods = axis_usize(range.period);
    let percents = axis_f64(range.percent);
    let types = if range.ma_types.is_empty() { vec!["VAR".to_string()] } else { range.ma_types.clone() };
    let mut combos = Vec::with_capacity(periods.len() * percents.len() * types.len());
    for &p in &periods { for &q in &percents { for t in &types {
        combos.push(OttParams { period: Some(p), percent: Some(q), ma_type: Some(t.clone()) });
    }}}
    combos
}
