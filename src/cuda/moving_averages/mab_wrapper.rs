...
impl CudaMab {
    pub fn new(device_id: usize) -> Result<Self, CudaMabError> {
        ...
        let device = Device::get_device(device_id as u32).map_err(|e| CudaMabError::Cuda(e.to_string()))?;
        ...
        let stream = Stream::new(StreamFlags::NON_BLOCKING, None).map_err(|e| CudaMabError::Cuda(e.to_string()))?;
    ...
    fn compute_ma_host(ma_type: &str, prices_f32: &[f32], period: usize) -> Result<Vec<f32>, CudaMabError> {
        ...
        let n = prices.len();
        if period == 0 || period > n { return Err(CudaMabError::InvalidInput("invalid period".into())); }
        let out_f64 = match ma_type.to_ascii_lowercase().as_str() {
            "ema" => ema(&EmaInput::from_slice(&prices, EmaParams{ period: Some(period) }))
                        .map_err(|e| CudaMabError::InvalidInput(e.to_string()))?
                        .values,
            _ => sma(&SmaInput::from_slice(&prices, SmaParams{ period: Some(period) }))
                        .map_err(|e| CudaMabError::InvalidInput(e.to_string()))?
                        .values,
        };
    ...
    fn compute_ma_host_time_major(
        ...
        use crate::indicators::moving_averages::ema::{ema, EmaInput, EmaParams};
        use crate::indicators::moving_averages::sma::{sma, SmaInput, SmaParams};
        if data_tm_f32.len() != cols * rows { return Err(CudaMabError::InvalidInput("time-major dims mismatch".into())); }
        let mut out_tm = vec![f32::NAN; cols * rows];
        for s in 0..cols {
            // extract column
            let mut col = vec![f64::NAN; rows];
            for r in 0..rows { col[r] = data_tm_f32[r * cols + s] as f64; }
            let vals = match ma_type.to_ascii_lowercase().as_str() {
                "ema" => ema(&EmaInput::from_slice(&col, EmaParams{ period: Some(period) }))
                            .map_err(|e| CudaMabError::InvalidInput(e.to_string()))?
                            .values,
                _ => sma(&SmaInput::from_slice(&col, SmaParams{ period: Some(period) }))
                            .map_err(|e| CudaMabError::InvalidInput(e.to_string()))?
                            .values,
            };
            for r in 0..rows { out_tm[r * cols + s] = vals[r] as f32; }
        }
        Ok(out_tm)
    }
    ...
    pub fn mab_batch_dev(
        ...
        let combos = {
            ...
            if sweep.fast_period.2 == 0 { fasts.push(sweep.fast_period.0); } else {
                let mut p = sweep.fast_period.0; while p <= sweep.fast_period.1 { fasts.push(p); p += sweep.fast_period.2; }
            }
            let mut slows = Vec::new();
            if sweep.slow_period.2 == 0 { slows.push(sweep.slow_period.0); } else {
                let mut p = sweep.slow_period.0; while p <= sweep.slow_period.1 { slows.push(p); p += sweep.slow_period.2; }
            }
            let mut ups = Vec::new();
            if sweep.devup.2 == 0.0 { ups.push(sweep.devup.0); } else {
                let mut d = sweep.devup.0; while d <= sweep.devup.1 { ups.push(d); d += sweep.devup.2; }
            }
            let mut dns = Vec::new();
            if sweep.devdn.2 == 0.0 { dns.push(sweep.devdn.0); } else {
                let mut d = sweep.devdn.0; while d <= sweep.devdn.1 { dns.push(d); d += sweep.devdn.2; }
            }
            for &f in &fasts { for &s in &slows { for &u in &ups { for &d in &dns {
                v.push(P{ fast_period: Some(f), slow_period: Some(s), devup: Some(u), devdn: Some(d),
                           fast_ma_type: Some(sweep.fast_ma_type.0.clone()), slow_ma_type: Some(sweep.slow_ma_type.0.clone())});
            }}}}
            v
        };
        if combos.is_empty() {
            return Err(CudaMabError::InvalidInput("no parameter combinations".into()))
        }
        ...
        if all_same_ma {
            ...
            let fast_ma_host = Self::compute_ma_host(
                p0.fast_ma_type.as_deref().unwrap_or("sma"), prices_f32, p0.fast_period.unwrap(),
            )?;
            let slow_ma_host = Self::compute_ma_host(
                p0.slow_ma_type.as_deref().unwrap_or("sma"), prices_f32, p0.slow_period.unwrap(),
            )?;
            let d_fast = DeviceBuffer::from_slice(&fast_ma_host).map_err(|e| CudaMabError::Cuda(e.to_string()))?;
            let d_slow = DeviceBuffer::from_slice(&slow_ma_host).map_err(|e| CudaMabError::Cuda(e.to_string()))?;
            ...
            let mut f_dev: Function = self.module
            ...
            unsafe {
                ...
                self.stream
                    .launch(&mut f_dev, GridSize::xyz(1, 1, 1), BlockSize::xyz(1, 1, 1), 0, args)
                    .map_err(|e| CudaMabError::Cuda(e.to_string()))?;
            }

            // Upload per-row multipliers
            let h_ups = LockedBuffer::from_slice(&devups).map_err(|e| CudaMabError::Cuda(e.to_string()))?;
            let h_dns = LockedBuffer::from_slice(&devdns).map_err(|e| CudaMabError::Cuda(e.to_string()))?;
            ...
            let mut d_dns = unsafe { DeviceBuffer::<f32>::uninitialized_async(rows, &self.stream) }
                .map_err(|e| CudaMabError::Cuda(e.to_string()))?;
            unsafe {
                d_ups.async_copy_from(&h_ups, &self.stream).map_err(|e| CudaMabError::Cuda(e.to_string()))?;
                d_dns.async_copy_from(&h_dns, &self.stream).map_err(|e| CudaMabError::Cuda(e.to_string()))?;
            }

            // Apply shared dev to outputs for all rows
            let mut f_apply: Function = self.module
            ...
            let mut f_row: Function = self.module
                .get_function("mab_single_row_from_ma_f32")
                .map_err(|e| CudaMabError::Cuda(e.to_string()))?;
            for (row, p) in combos.iter().enumerate() {
                let fast_ma_host = Self::compute_ma_host(
                    p.fast_ma_type.as_deref().unwrap_or("sma"), prices_f32, p.fast_period.unwrap(),
                )?;
                let slow_ma_host = Self::compute_ma_host(
                    p.slow_ma_type.as_deref().unwrap_or("sma"), prices_f32, p.slow_period.unwrap(),
                )?;
                let d_fast = DeviceBuffer::from_slice(&fast_ma_host).map_err(|e| CudaMabError::Cuda(e.to_string()))?;
                let d_slow = DeviceBuffer::from_slice(&slow_ma_host).map_err(|e| CudaMabError::Cuda(e.to_string()))?;
                ...
                unsafe {
                    ...
                    self.stream
                        .launch(&mut f_row, GridSize::xyz(1, 1, 1), BlockSize::xyz(1, 1, 1), 0, args)
                        .map_err(|e| CudaMabError::Cuda(e.to_string()))?;
                }
            }
        }

        self.stream.synchronize().map_err(|e| CudaMabError::Cuda(e.to_string()))?;

        let trip = DeviceArrayF32Triplet {
            upper: DeviceArrayF32 { buf: d_upper, rows, cols: len },
            middle: DeviceArrayF32 { buf: d_middle, rows, cols: len },
    ...
    pub fn mab_many_series_one_param_time_major_dev(
        ...
        if cols == 0 || rows == 0 { return Err(CudaMabError::InvalidInput("invalid series dims".into())); }
        if data_tm_f32.len() != cols * rows {
            return Err(CudaMabError::InvalidInput("time-major length mismatch".into()));
        }
        let fast = params.fast_period.unwrap_or(0);
        let slow = params.slow_period.unwrap_or(0);
        if fast == 0 || slow == 0 { return Err(CudaMabError::InvalidInput("periods must be >=1".into())); }
        ...
        for s in 0..cols {
            let mut fv = None;
            for r in 0..rows {
                if !data_tm_f32[r * cols + s].is_nan() { fv = Some(r as i32); break; }
            }
            let fv = fv.ok_or_else(|| CudaMabError::InvalidInput(format!("series {} all-NaN", s)))?;
            // require enough tail for both MAs and dev window
            let need_total = (fast.max(slow) + fast - 1) as i32;
            if (rows as i32) - fv < need_total {
                return Err(CudaMabError::InvalidInput(format!(
                    "series {} insufficient valid tail for fast={}, slow={}", s, fast, slow
                )));
            }
            first_valids[s] = fv;
        }
        ...
        let fast_tm_host = Self::compute_ma_host_time_major(fast_type, data_tm_f32, cols, rows, fast)?;
        let slow_tm_host = Self::compute_ma_host_time_major(slow_type, data_tm_f32, cols, rows, slow)?;
        let fast_dev = DeviceArrayF32 { buf: DeviceBuffer::from_slice(&fast_tm_host).map_err(|e| CudaMabError::Cuda(e.to_string()))?, rows, cols };
        let slow_dev = DeviceArrayF32 { buf: DeviceBuffer::from_slice(&slow_tm_host).map_err(|e| CudaMabError::Cuda(e.to_string()))?, rows, cols };

        // Outputs
        let elems = cols * rows;
        let mut d_upper: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaMabError::Cuda(e.to_string()))?;
        let mut d_middle: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaMabError::Cuda(e.to_string()))?;
        let mut d_lower: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaMabError::Cuda(e.to_string()))?;
        ...
        self.stream.synchronize().map_err(|e| CudaMabError::Cuda(e.to_string()))?;
        Ok(DeviceArrayF32Triplet { upper: DeviceArrayF32 { buf: d_upper, rows, cols },
...
pub mod benches {
    ...
    pub fn bench_profiles() -> Vec<CudaBenchScenario> {
        vec![
            CudaBenchScenario::new(
                "mab", "batch_dev", "mab_cuda_batch_dev", "60k_x_49combos", prep_mab_batch_box,
            ).with_inner_iters(4),
            CudaBenchScenario::new(
                "mab", "many_series_one_param", "mab_cuda_many_series_one_param", "128x1m", prep_mab_many_series_box,
            ).with_inner_iters(2),
        ]
    }
    ...
    impl CudaBenchState for MabBatchState {
        fn launch(&mut self) {
            let _ = self.cuda.mab_batch_dev(&self.price, &self.combos).expect("mab batch");
    ...
    fn prep_mab_batch() -> MabBatchState {
        ...
        for i in 10..len { let x = i as f32; price[i] = (x*0.001).sin() + 0.001*x; }
        let combos = MabBatchRange{
            fast_period: (10, 22, 4), slow_period: (50, 74, 12),
            devup: (1.0, 1.0, 0.0), devdn: (1.0, 1.0, 0.0),
        ...
        MabBatchState { cuda, d_up, d_mid, d_lo, price, combos, rows, len }
    }
    fn prep_mab_batch_box() -> Box<dyn CudaBenchState> { Box::new(prep_mab_batch()) }

    struct MabManySeriesState { cuda: CudaMab, tm: Vec<f32>, cols: usize, rows: usize, p: MabParams }
    impl CudaBenchState for MabManySeriesState { fn launch(&mut self){
        let _ = self.cuda.mab_many_series_one_param_time_major_dev(&self.tm, self.cols, self.rows, &self.p).unwrap();
        self.cuda.stream.synchronize().unwrap();
    }}
    fn prep_mab_many_series() -> MabManySeriesState {
        let cuda = CudaMab::new(0).expect("cuda mab");
        let cols = 128usize; let rows = 1_000_000usize;
        let mut tm = vec![f32::NAN; cols*rows];
        for s in 0..cols { for r in s..rows { let x = (r as f32)+0.1*(s as f32); tm[r*cols+s] = (x*0.002).sin()+0.0005*x; }}
        let p = MabParams{ fast_period: Some(10), slow_period: Some(50), devup: Some(1.0), devdn: Some(1.0), fast_ma_type: Some("sma".into()), slow_ma_type: Some("sma".into()) };
        MabManySeriesState{ cuda, tm, cols, rows, p }
    }
    fn prep_mab_many_series_box() -> Box<dyn CudaBenchState> { Box::new(prep_mab_many_series()) }
}
