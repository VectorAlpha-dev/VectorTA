//! CUDA wrapper for Correlation Cycle (Ehlers) — real/imag/angle/state.
//!
//! Parity with ALMA wrapper:
//! - PTX loaded via include_str!(.../correlation_cycle_kernel.ptx) with DetermineTargetFromContext + O2
//! - NON_BLOCKING stream
//! - Policy enums (plain 1D for batch; 2D for many-series optional), introspection, BENCH_DEBUG logging
//! - VRAM checks with ~64MB headroom and grid.y chunking for large combo counts
//! - Public device entry points return VRAM-backed arrays (DeviceArrayF32) without staging to host

#![cfg(feature = "cuda")]

use super::alma_wrapper::DeviceArrayF32;
use crate::indicators::correlation_cycle::{
    CorrelationCycleBatchRange, CorrelationCycleParams,
};
...
pub enum BatchKernelPolicy {
    Auto,
    /// Plain 1D over time, grid.y over combos
    Plain { block_x: u32 },
}
...
/// Four VRAM-backed arrays returned by the CUDA kernels.
...
impl DeviceCorrelationCycleQuad {
    #[inline]
    pub fn rows(&self) -> usize { self.real.rows }
    #[inline]
    pub fn cols(&self) -> usize { self.real.cols }
}
...
impl CudaCorrelationCycle {
    pub fn new(device_id: usize) -> Result<Self, CudaCorrelationCycleError> {
        cust::init(CudaFlags::empty()).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let device = Device::get_device(device_id as u32)
            .map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let context = Context::new(device).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        ...
        let module = match Module::from_ptx(ptx, jit_opts) {
            Ok(m) => m,
            Err(_) => {
                if let Ok(m) = Module::from_ptx(ptx, &[ModuleJitOption::DetermineTargetFromContext]) { m }
                else { Module::from_ptx(ptx, &[]).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))? }
            }
        };
        let stream = Stream::new(StreamFlags::NON_BLOCKING, None)
            .map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        Ok(Self {
            module, stream, _context: context, device_id: device_id as u32,
            policy: CudaCorrelationCyclePolicy::default(),
            last_batch: None, last_many: None,
            debug_batch_logged: false, debug_many_logged: false,
        })
    }

    pub fn set_policy(&mut self, p: CudaCorrelationCyclePolicy) { self.policy = p; }
    pub fn policy(&self) -> &CudaCorrelationCyclePolicy { &self.policy }
    pub fn selected_batch_kernel(&self) -> Option<BatchKernelSelected> { self.last_batch }
    pub fn selected_many_series_kernel(&self) -> Option<ManySeriesKernelSelected> { self.last_many }
    pub fn synchronize(&self) -> Result<(), CudaCorrelationCycleError> {
        self.stream.synchronize().map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))
    }

    #[inline] fn maybe_log_batch_debug(&self) {
    ...
    #[inline] fn maybe_log_many_debug(&self) {
        static ONCE: AtomicBool = AtomicBool::new(false);
    ...
    #[inline]
    fn pick_launch_2d(&self, cols: usize, rows: usize, tx_ty: Option<(u32, u32)>) -> (GridSize, BlockSize, (u32,u32)) {
    ...
    fn compute_trig_weights_and_consts(period: usize) -> (Vec<f32>, Vec<f32>, f32, f32, f32, f32) {
        ...
        if period == 0 { return (wcos, wsin, 0.0, 0.0, 0.0, 0.0); }
        ...
        for j in 0..period {
            ...
            wcos[j] = c as f32;
            wsin[j] = ys as f32;
            sum_c += c; sum_s += ys; sum_c2 += c * c; sum_s2 += ys * ys;
        }
    ...
    fn expand_grid(r: &CorrelationCycleBatchRange) -> Vec<CorrelationCycleParams> {
        fn axis_usize(a: (usize, usize, usize)) -> Vec<usize> {
            let (start,end,step) = a; if step==0 || start==end { return vec![start]; }
            (start..=end).step_by(step).collect()
        }
        fn axis_f64(a: (f64, f64, f64)) -> Vec<f64> {
            let (start,end,step) = a; if step.abs() < 1e-12 || (start-end).abs() < 1e-12 { return vec![start]; }
            let mut v=Vec::new(); let mut x=start; while x <= end + 1e-12 { v.push(x); x += step; } v
        }
        let periods = axis_usize(r.period);
        let thresholds = axis_f64(r.threshold);
        let mut out = Vec::with_capacity(periods.len()*thresholds.len());
        for &p in &periods { for &t in &thresholds { out.push(CorrelationCycleParams { period: Some(p), threshold: Some(t) }); } }
        out
    }
    ...
    pub fn correlation_cycle_batch_dev(
        ...
        if series_len == 0 { return Err(CudaCorrelationCycleError::InvalidInput("empty input".into())); }
        let first_valid = data_f32.iter().position(|x| x.is_finite()).unwrap_or(series_len);
        if first_valid >= series_len { return Err(CudaCorrelationCycleError::InvalidInput("all NaN".into())); }

        let combos = Self::expand_grid(sweep);
        let n_combos = combos.len();
        if n_combos == 0 { return Err(CudaCorrelationCycleError::InvalidInput("empty sweep".into())); }
        let max_period = combos.iter().map(|c| c.period.unwrap_or(0)).max().unwrap_or(0);
        if max_period == 0 { return Err(CudaCorrelationCycleError::InvalidInput("period=0".into())); }
        ...
        for (i, prm) in combos.iter().enumerate() {
            ...
            periods_i32[i] = p as i32; thresholds_f32[i] = t;
            ...
            sum_cos[i] = sc; sum_sin[i] = ss; sqrt_t2[i] = st2; sqrt_t4[i] = st4;
        }

        // VRAM estimate: inputs + params + weights + 4 outputs + headroom
        let prices_bytes = series_len * 4;
        let params_bytes = (n_combos * (4 + 4)) + (n_combos * (4*4)) + (2 * n_combos * max_period * 4);
        ...
        let d_prices = unsafe { DeviceBuffer::from_slice_async(data_f32, &self.stream) }
            .map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let d_cos   = DeviceBuffer::from_slice(&cos_flat).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let d_sin   = DeviceBuffer::from_slice(&sin_flat).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let d_periods = DeviceBuffer::from_slice(&periods_i32).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let d_thresholds = DeviceBuffer::from_slice(&thresholds_f32).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let d_sum_cos = DeviceBuffer::from_slice(&sum_cos).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let d_sum_sin = DeviceBuffer::from_slice(&sum_sin).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let d_sqrt_t2 = DeviceBuffer::from_slice(&sqrt_t2).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let d_sqrt_t4 = DeviceBuffer::from_slice(&sqrt_t4).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let mut d_real: DeviceBuffer<f32>  = unsafe { DeviceBuffer::uninitialized(n_combos * series_len) }.map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let mut d_imag: DeviceBuffer<f32>  = unsafe { DeviceBuffer::uninitialized(n_combos * series_len) }.map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let mut d_angle: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(n_combos * series_len) }.map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let mut d_state: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(n_combos * series_len) }.map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;

        // Kernel selection
        let (grid, block, bx) = match self.policy.batch {
            BatchKernelPolicy::Auto | BatchKernelPolicy::Plain { .. } => self.pick_launch_1d(series_len, Some(256)),
        };
        self.last_batch = Some(BatchKernelSelected::Plain { block_x: bx });
        self.maybe_log_batch_debug();

        // RIA pass
        let func_ria = self.module
        ...
        Ok(DeviceCorrelationCycleQuad {
            real: DeviceArrayF32 { buf: d_real, rows: n_combos, cols: series_len },
            imag: DeviceArrayF32 { buf: d_imag, rows: n_combos, cols: series_len },
            angle: DeviceArrayF32 { buf: d_angle, rows: n_combos, cols: series_len },
            state: DeviceArrayF32 { buf: d_state, rows: n_combos, cols: series_len },
        })
    }
    ...
    pub fn correlation_cycle_many_series_one_param_time_major_dev(
        ...
        if cols == 0 || rows == 0 { return Err(CudaCorrelationCycleError::InvalidInput("empty dims".into())); }
        let period = params.period.unwrap_or(20);
        let threshold = params.threshold.unwrap_or(9.0) as f32;
        if period == 0 { return Err(CudaCorrelationCycleError::InvalidInput("period=0".into())); }
        if prices_tm_f32.len() != cols * rows { return Err(CudaCorrelationCycleError::InvalidInput("flat data size mismatch".into())); }

        // first_valid per series (host)
        let mut first_valids = vec![0i32; cols];
        for s in 0..cols {
            let mut fv = 0usize; let mut found = false;
            for t in 0..rows { let v = prices_tm_f32[t * cols + s]; if v.is_finite() { fv = t; found = true; break; } }
        ...
        // VRAM estimate
        let elems = cols * rows;
        let required = elems*4 + (period*2*4) + (cols*4) + (4*elems*4) + 64*1024*1024;
        if !Self::will_fit(required, 0) {
            return Err(CudaCorrelationCycleError::InvalidInput("insufficient VRAM".into()));
        }

        // Upload
        let d_prices_tm = unsafe { DeviceBuffer::from_slice_async(prices_tm_f32, &self.stream) }
            .map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let d_first_valids = DeviceBuffer::from_slice(&first_valids).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let d_wcos = DeviceBuffer::from_slice(&wcos).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let d_wsin = DeviceBuffer::from_slice(&wsin).map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let mut d_real: DeviceBuffer<f32>  = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let mut d_imag: DeviceBuffer<f32>  = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let mut d_angle: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;
        let mut d_state: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaCorrelationCycleError::Cuda(e.to_string()))?;

        // Kernel selection
        let (grid, block, (tx, ty)) = match self.policy.many_series {
            ManySeriesKernelPolicy::Auto | ManySeriesKernelPolicy::Tiled2D { .. } => self.pick_launch_2d(cols, rows, Some((128, 4))),
        };
        ...
        Ok(DeviceCorrelationCycleQuad {
            real: DeviceArrayF32 { buf: d_real, rows, cols },
            imag: DeviceArrayF32 { buf: d_imag, rows, cols },
            angle: DeviceArrayF32 { buf: d_angle, rows, cols },
            state: DeviceArrayF32 { buf: d_state, rows, cols },
        })
    }
}
...
pub mod benches {
    ...
    impl CudaBenchState for CcBatchState {
        fn launch(&mut self) {
            ...
            let func_ria = self.cuda.module.get_function("correlation_cycle_batch_f32_ria").unwrap();
            ...
            while processed < self.n_combos {
                ...
                unsafe {
                    ...
                    let func_state = self.cuda.module.get_function("correlation_cycle_state_batch_f32").unwrap();
    ...
    fn prep_batch() -> Box<dyn CudaBenchState> {
        ...
        let start_p = 16usize; let end_p = start_p + PARAM_SWEEP - 1;
        let sweep = CorrelationCycleBatchRange { period: (start_p, end_p, 1), threshold: (9.0, 9.0, 0.0) };
        ...
        for (i, prm) in combos.iter().enumerate() {
            ...
            periods[i] = p as i32; th[i] = prm.threshold.unwrap() as f32;
            let base = i * max_p; cos_flat[base..base+p].copy_from_slice(&wc); sin_flat[base..base+p].copy_from_slice(&ws);
            sumc[i]=sc; sums[i]=ss; st2[i]=t2; st4[i]=t4;
        }
        ...
        Box::new(CcBatchState { cuda, d_prices, d_cos, d_sin, d_periods, d_th, d_sumc, d_sums, d_st2, d_st4, d_real, d_imag, d_ang, d_st: d_state, first_valid, max_period: max_p, series_len: ONE_SERIES_LEN, n_combos })
    }
    ...
    struct CcManyState {
        ...
        period: usize, threshold: f32,
        sumc: f32, sums: f32, st2: f32, st4: f32,
    }
    impl CudaBenchState for CcManyState {
        fn launch(&mut self) {
            let (grid, block, _cfg) = self.cuda.pick_launch_2d(self.cols, self.rows, Some((128,4)));
            let func_ria = self.cuda.module.get_function("correlation_cycle_many_series_one_param_f32_ria").unwrap();
            ...
            unsafe {
                ...
                let func_st = self.cuda.module.get_function("correlation_cycle_state_many_series_one_param_f32").unwrap();
    ...
    fn prep_many() -> Box<dyn CudaBenchState> {
        ...
        for s in 0..cols { let mut fv=0usize; for t in 0..rows { if prices_tm[t*cols+s].is_finite() { fv=t; break; } } fvs[s]=fv as i32; }
        let period = 32usize; let threshold = 9.0f32;
        let (wcos, wsin, sumc, sums, st2, st4) = CudaCorrelationCycle::compute_trig_weights_and_consts(period);
        let d_prices_tm = unsafe { DeviceBuffer::from_slice_async(&prices_tm, &cuda.stream) }.unwrap();
        ...
        Box::new(CcManyState { cuda, d_prices_tm, d_first_valids, d_wcos, d_wsin, d_real, d_imag, d_ang, d_st, cols, rows, period, threshold, sumc, sums, st2, st4 })
    }

    pub fn bench_profiles() -> Vec<CudaBenchScenario> {
        vec![
            CudaBenchScenario::new("correlation_cycle", "one_series_many_params", "correlation_cycle_cuda_batch_dev", "250k_x_64", prep_batch),
            CudaBenchScenario::new("correlation_cycle", "many_series_one_param", "correlation_cycle_cuda_many_series_one_param_dev", "128x500k", prep_many),
        ]
    }
}
