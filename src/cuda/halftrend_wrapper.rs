...
impl CudaHalftrend {
    ...
    pub fn halftrend_batch_dev(
        ...
        if std::env::var("BENCH_DEBUG").ok().as_deref() == Some("1") && !self.debug_batch_logged {
            eprintln!(
                "[halftrend] batch kernel: block_x={} rows={} len={} first_valid={}",
        ...
        unsafe {
            ...
            self.stream
                .launch(&func, grid, block, 0, all.as_mut_slice())
                .map_err(|e| CudaHalftrendError::Cuda(e.to_string()))?;
        }
    ...
    pub fn halftrend_many_series_one_param_time_major_dev(
        ...
        let d_close = unsafe { DeviceBuffer::from_slice_async(close_tm, &self.stream) }
            .map_err(|e| CudaHalftrendError::Cuda(e.to_string()))?;
        let d_atr = unsafe { DeviceBuffer::from_slice_async(&atr_tm, &self.stream) }
            .map_err(|e| CudaHalftrendError::Cuda(e.to_string()))?;
        let d_hma = unsafe { DeviceBuffer::from_slice_async(&hma_tm, &self.stream) }
            .map_err(|e| CudaHalftrendError::Cuda(e.to_string()))?;
        let d_lma = unsafe { DeviceBuffer::from_slice_async(&lma_tm, &self.stream) }
            .map_err(|e| CudaHalftrendError::Cuda(e.to_string()))?;
        let d_rhi = unsafe { DeviceBuffer::from_slice_async(&rhi_tm, &self.stream) }
            .map_err(|e| CudaHalftrendError::Cuda(e.to_string()))?;
        let d_rlo = unsafe { DeviceBuffer::from_slice_async(&rlo_tm, &self.stream) }
            .map_err(|e| CudaHalftrendError::Cuda(e.to_string()))?;
        let d_warms = DeviceBuffer::from_slice(&warms)
            .map_err(|e| CudaHalftrendError::Cuda(e.to_string()))?;
