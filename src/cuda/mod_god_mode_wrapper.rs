...
impl CudaModGodMode {
    ...
    pub fn mod_god_mode_batch_dev(
        ...
        // Upload inputs
        let d_high =
            DeviceBuffer::from_slice(high).map_err(|e| CudaModGodModeError::Cuda(e.to_string()))?;
        let d_low =
            DeviceBuffer::from_slice(low).map_err(|e| CudaModGodModeError::Cuda(e.to_string()))?;
        ...
        let d_n1s =
            DeviceBuffer::from_slice(&n1s).map_err(|e| CudaModGodModeError::Cuda(e.to_string()))?;
        let d_n2s =
            DeviceBuffer::from_slice(&n2s).map_err(|e| CudaModGodModeError::Cuda(e.to_string()))?;
        let d_n3s =
            DeviceBuffer::from_slice(&n3s).map_err(|e| CudaModGodModeError::Cuda(e.to_string()))?;
        let d_modes = DeviceBuffer::from_slice(&modes)
            .map_err(|e| CudaModGodModeError::Cuda(e.to_string()))?;

        let mut d_wt: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(rows * n) }
            .map_err(|e| CudaModGodModeError::Cuda(e.to_string()))?;
        let mut d_sig: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(rows * n) }
            .map_err(|e| CudaModGodModeError::Cuda(e.to_string()))?;
        let mut d_hist: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(rows * n) }
            .map_err(|e| CudaModGodModeError::Cuda(e.to_string()))?;
        ...
        if !self.debug_logged && std::env::var("BENCH_DEBUG").ok().as_deref() == Some("1") {
            eprintln!(
                "[mod_god_mode] batch policy selected: block_x={} grid_x={}",
                block_x, grid_x
            );
    ...
    pub fn mod_god_mode_many_series_one_param_time_major_dev(
        ...
        let bx = match self.policy.many_series {
            ManySeriesKernelPolicy::Auto | ManySeriesKernelPolicy::OneD { .. } => 32,
        };
