...
impl CudaRvi {
    pub fn new(device_id: usize) -> Result<Self, CudaRviError> {
        ...
        let stream = Stream::new(StreamFlags::NON_BLOCKING, None)
            .map_err(|e| CudaRviError::Cuda(e.to_string()))?;
        Ok(Self { module, stream, _context: context, policy: CudaRviPolicy::default(), debug_batch_logged: false, debug_many_logged: false })
    }

    pub fn set_policy(&mut self, policy: CudaRviPolicy) { self.policy = policy; }
    pub fn policy(&self) -> &CudaRviPolicy { &self.policy }
    pub fn synchronize(&self) -> Result<(), CudaRviError> { self.stream.synchronize().map_err(|e| CudaRviError::Cuda(e.to_string())) }
    ...
    fn expand_grid(sweep: &RviBatchRange) -> Vec<RviParams> {
        ...
        for &p in &periods { for &m in &ma_lens { for &t in &matypes { for &d in &devtypes {
            out.push(RviParams { period: Some(p), ma_len: Some(m), matype: Some(t), devtype: Some(d) });
        }}}}
        out
    }

    fn prepare_batch(data: &[f32], sweep: &RviBatchRange) -> Result<(Vec<RviParams>, usize, usize, usize, usize), CudaRviError> {
        if data.is_empty() { return Err(CudaRviError::InvalidInput("empty data".into())); }
        let len = data.len();
        let first_valid = data.iter().position(|v| !v.is_nan())
            .ok_or_else(|| CudaRviError::InvalidInput("all values are NaN".into()))?;
        let combos = Self::expand_grid(sweep);
        if combos.is_empty() { return Err(CudaRviError::InvalidInput("no parameter combinations".into())); }

        // Reject unsupported devtype=2 for now (median abs dev)
        if combos.iter().any(|c| c.devtype.unwrap_or(0) == 2) {
            return Err(CudaRviError::InvalidInput("devtype=2 (median abs dev) not supported by CUDA kernel yet".into()));
        }
        ...
        if max_period == 0 || max_ma_len == 0 { return Err(CudaRviError::InvalidInput("invalid period/ma_len".into())); }
        if len - first_valid <= (max_period - 1) + (max_ma_len - 1) {
            return Err(CudaRviError::InvalidInput("not enough valid data for warmup".into()));
        }
        Ok((combos, first_valid, len, max_period, max_ma_len))
    }

    pub fn rvi_batch_dev(&self, data: &[f32], sweep: &RviBatchRange) -> Result<(DeviceArrayF32, Vec<RviParams>), CudaRviError> {
        let (combos, first_valid, len, max_period, max_ma_len) = Self::prepare_batch(data, sweep)?;
        let rows = combos.len();

        // VRAM estimate: inputs + param arrays + outputs
        let req = (len + rows * len) * std::mem::size_of::<f32>()
            + rows * 4 * std::mem::size_of::<i32>();
        if !Self::device_mem_ok(req) {
            return Err(CudaRviError::InvalidInput("insufficient device memory".into()));
        }
        ...
        for (start, count) in Self::grid_y_chunks(rows) {
            ...
            let mut d_p = DeviceBuffer::from_slice(p).map_err(|e| CudaRviError::Cuda(e.to_string()))?;
            let mut d_m = DeviceBuffer::from_slice(m).map_err(|e| CudaRviError::Cuda(e.to_string()))?;
            let mut d_t = DeviceBuffer::from_slice(t).map_err(|e| CudaRviError::Cuda(e.to_string()))?;
            let mut d_d = DeviceBuffer::from_slice(d).map_err(|e| CudaRviError::Cuda(e.to_string()))?;

            self.launch_batch(&d_data, &mut d_out, &mut d_p, &mut d_m, &mut d_t, &mut d_d, len, first_valid, count, max_period, max_ma_len, start, shmem_bytes)?;
        }
    ...
    fn launch_batch(
        ...
        let func = self.module.get_function("rvi_batch_f32").map_err(|e| CudaRviError::Cuda(e.to_string()))?;
        ...
        if std::env::var("BENCH_DEBUG").ok().as_deref() == Some("1") && !self.debug_batch_logged {
            eprintln!("[rvi] batch kernel: block_x={} rows_chunk={} len={} max_p={} max_m={}", block_x, n_rows, len, max_period, max_ma_len);
            unsafe { (*(self as *const _ as *mut CudaRvi)).debug_batch_logged = true; }
        }
        unsafe {
            ...
            let mut o_ptr = d_out.as_device_ptr().as_raw().wrapping_add((row_offset * len * std::mem::size_of::<f32>()) as u64);
    ...
    pub fn rvi_many_series_one_param_time_major_dev(&self, data_tm: &[f32], cols: usize, rows: usize, params: &RviParams) -> Result<DeviceArrayF32, CudaRviError> {
        ...
        // Upload buffers
        let h_data = LockedBuffer::from_slice(data_tm).map_err(|e| CudaRviError::Cuda(e.to_string()))?;
        let h_firsts = LockedBuffer::from_slice(&firsts).map_err(|e| CudaRviError::Cuda(e.to_string()))?;
        let mut d_data = unsafe { DeviceBuffer::<f32>::uninitialized_async(cols * rows, &self.stream) }.map_err(|e| CudaRviError::Cuda(e.to_string()))?;
        let mut d_firsts = unsafe { DeviceBuffer::<i32>::uninitialized_async(cols, &self.stream) }.map_err(|e| CudaRviError::Cuda(e.to_string()))?;
        let mut d_out = unsafe { DeviceBuffer::<f32>::uninitialized_async(cols * rows, &self.stream) }.map_err(|e| CudaRviError::Cuda(e.to_string()))?;
        unsafe {
            d_data.async_copy_from(&h_data, &self.stream).map_err(|e| CudaRviError::Cuda(e.to_string()))?;
            d_firsts.async_copy_from(&h_firsts, &self.stream).map_err(|e| CudaRviError::Cuda(e.to_string()))?;
        }

        self.launch_many_series(&d_data, &d_firsts, cols, rows, period, ma_len, matype, devtype, &mut d_out)?;
        self.stream.synchronize().map_err(|e| CudaRviError::Cuda(e.to_string()))?;
    ...
    fn launch_many_series(
        ...
        let func = self.module.get_function("rvi_many_series_one_param_f32").map_err(|e| CudaRviError::Cuda(e.to_string()))?;
        ...
        if std::env::var("BENCH_DEBUG").ok().as_deref() == Some("1") && !self.debug_many_logged {
            ...
            unsafe { (*(self as *const _ as *mut CudaRvi)).debug_many_logged = true; }
        }
        unsafe {
            ...
            self.stream.launch(&func, grid, block, 0, &mut args).map_err(|e| CudaRviError::Cuda(e.to_string()))?;
        }
        Ok(())
    }
}
...
pub mod benches {
    ...
    pub fn bench_profiles() -> Vec<CudaBenchScenario> {
        ...
        v.push(CudaBenchScenario::new(
            ...
            "100k x 128",
            || {
                struct State { cuda: CudaRvi, data: Vec<f32>, sweep: RviBatchRange }
                impl CudaBenchState for State { fn launch(&mut self) { let _ = self.cuda.rvi_batch_dev(&self.data, &self.sweep); } }
                let n = 100_000usize;
                let mut data = vec![f32::NAN; n];
                for i in 500..n { let x = i as f32; data[i] = (x * 0.00123).sin() + 0.0002 * x; }
                let sweep = RviBatchRange { period: (10, 25, 1), ma_len: (14, 14, 0), matype: (1, 1, 0), devtype: (0, 0, 0) };
                Box::new(State { cuda: CudaRvi::new(0).unwrap(), data, sweep })
            }
        ).with_sample_size(20));
        ...
        v.push(CudaBenchScenario::new(
            ...
            "2048r x 512c",
            || {
                struct State { cuda: CudaRvi, data_tm: Vec<f32>, cols: usize, rows: usize, params: RviParams }
                impl CudaBenchState for State { fn launch(&mut self) { let _ = self.cuda.rvi_many_series_one_param_time_major_dev(&self.data_tm, self.cols, self.rows, &self.params); } }
                let cols = 512usize; let rows = 2048usize; let mut tm = vec![f32::NAN; cols*rows];
                ...
                Box::new(State { cuda: CudaRvi::new(0).unwrap(), data_tm: tm, cols, rows, params })
            }
        ).with_sample_size(20));
