...
impl CudaDti {
    ...
    pub fn dti_batch_dev(
        ...
        let hx =
            LockedBuffer::from_slice(&x_host).map_err(|e| CudaDtiError::Cuda(e.to_string()))?;
        let hax =
            LockedBuffer::from_slice(&ax_host).map_err(|e| CudaDtiError::Cuda(e.to_string()))?;
    ...
    pub fn dti_many_series_one_param_time_major_dev(
        ...
        if !Self::will_fit(bytes, 64 * 1024 * 1024) {
            return Err(CudaDtiError::InvalidInput(
                "insufficient VRAM for DTI many-series".into(),
            ));
        }

        let d_high =
            DeviceBuffer::from_slice(high_tm_f32).map_err(|e| CudaDtiError::Cuda(e.to_string()))?;
        let d_low =
            DeviceBuffer::from_slice(low_tm_f32).map_err(|e| CudaDtiError::Cuda(e.to_string()))?;
