...
impl CudaQqe {
    ...
    pub fn qqe_batch_dev(
        ...
        let fast_f32: Vec<f32> = combos
            ...
            .collect();
        let d_rsi =
            DeviceBuffer::from_slice(&rsi_i32).map_err(|e| CudaQqeError::Cuda(e.to_string()))?;
        let d_ema =
            DeviceBuffer::from_slice(&ema_i32).map_err(|e| CudaQqeError::Cuda(e.to_string()))?;
        let d_fast =
            DeviceBuffer::from_slice(&fast_f32).map_err(|e| CudaQqeError::Cuda(e.to_string()))?;
        ...
        while base < rows {
            ...
            unsafe {
                ...
                let mut f_rsi =
                    d_rsi.as_device_ptr().as_raw() + ((base * std::mem::size_of::<i32>()) as u64);
                let mut f_ema =
                    d_ema.as_device_ptr().as_raw() + ((base * std::mem::size_of::<i32>()) as u64);
                let mut f_fast =
                    d_fast.as_device_ptr().as_raw() + ((base * std::mem::size_of::<f32>()) as u64);
                let mut series_len_i = len as i32;
                let mut n_combos_i = take as i32;
                let mut first_i = first_valid as i32;
                let mut f_out = d_out.as_device_ptr().as_raw()
                    + ((2 * base * len) * std::mem::size_of::<f32>()) as u64;
    ...
    pub fn qqe_many_series_one_param_time_major_dev(
        ...
        if !Self::device_mem_ok(req) {
            return Err(CudaQqeError::InvalidInput("insufficient VRAM".into()));
        }
        let d_prices: DeviceBuffer<f32> =
        ...
        let d_first = DeviceBuffer::from_slice(&first_valids)
            .map_err(|e| CudaQqeError::Cuda(e.to_string()))?;
        let mut d_out: DeviceBuffer<f32> =
            unsafe { DeviceBuffer::uninitialized_async(rows * (2 * cols), &self.stream) }
                .map_err(|e| CudaQqeError::Cuda(e.to_string()))?;

        let block_x = match self.policy_many {
        ...
        unsafe {
            ...
            let func = self
                ...
                .map_err(|e| CudaQqeError::Cuda(e.to_string()))?;
            let grid: GridSize = (
                ((rows as u32 + block_x - 1) / block_x).max(1),
                cols as u32,
                1,
            )
                .into();
