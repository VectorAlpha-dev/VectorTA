...
#[derive(Clone, Copy, Debug)]
pub struct CudaReverseRsiPolicy { pub batch: BatchKernelPolicy, pub many_series: ManySeriesKernelPolicy }
...
impl CudaReverseRsi {
    ...
    pub fn set_policy(&mut self, policy: CudaReverseRsiPolicy) { self.policy = policy; }
    pub fn policy(&self) -> &CudaReverseRsiPolicy { &self.policy }
    pub fn selected_batch_kernel(&self) -> Option<BatchKernelSelected> { self.last_batch }
    pub fn selected_many_series_kernel(&self) -> Option<ManySeriesKernelSelected> { self.last_many }
    pub fn synchronize(&self) -> Result<(), CudaReverseRsiError> {
        self.stream.synchronize().map_err(|e| CudaReverseRsiError::Cuda(e.to_string()))
    }
    ...
    fn maybe_log_batch_debug(&self) {
        ...
        if std::env::var("BENCH_DEBUG").ok().as_deref() == Some("1") {
            if let Some(sel) = self.last_batch {
                ...
                unsafe { (*(self as *const _ as *mut CudaReverseRsi)).debug_batch_logged = true; }
            }
        }
    }
    #[inline]
    fn maybe_log_many_debug(&self) {
        ...
        if std::env::var("BENCH_DEBUG").ok().as_deref() == Some("1") {
            if let Some(sel) = self.last_many {
                ...
                unsafe { (*(self as *const _ as *mut CudaReverseRsi)).debug_many_logged = true; }
            }
        }
    }
    ...
    fn expand_grid(sweep: &ReverseRsiBatchRange) -> Vec<ReverseRsiParams> {
        ...
        let lengths: Vec<usize> = if lp == 0 { vec![ls] } else { (ls..=le).step_by(lp).collect() };
        let mut levels: Vec<f64> = Vec::new();
        if vp == 0.0 { levels.push(vs) } else { let mut x = vs; while x <= ve { levels.push(x); x += vp; } }
        let mut combos = Vec::with_capacity(lengths.len() * levels.len());
        for &l in &lengths { for &v in &levels { combos.push(ReverseRsiParams { rsi_length: Some(l), rsi_level: Some(v) }); } }
        combos
    }
    ...
    fn prepare_batch_inputs(
        ...
        if prices.is_empty() { return Err(CudaReverseRsiError::InvalidInput("empty data".into())); }
        let len = prices.len();
        let first_valid = prices.iter().position(|v| !v.is_nan()).ok_or_else(|| CudaReverseRsiError::InvalidInput("all values are NaN".into()))?;
        let combos = Self::expand_grid(sweep);
        if combos.is_empty() { return Err(CudaReverseRsiError::InvalidInput("no parameter combos".into())); }
        // Validate feasibility for at least max length
        let max_len = combos.iter().map(|p| p.rsi_length.unwrap_or(14)).max().unwrap_or(14);
        let ema_len = (2 * max_len).saturating_sub(1);
        if len - first_valid <= ema_len {
            return Err(CudaReverseRsiError::InvalidInput(format!(
                "not enough valid data: needed > {}, have {}", ema_len, len - first_valid
            )));
        }
    ...
    fn launch_batch_kernel(
        ...
        unsafe { (*(self as *const _ as *mut CudaReverseRsi)).last_batch = Some(BatchKernelSelected::OneD { block_x }); }
    ...
    pub fn reverse_rsi_batch_dev(
        ...
        if !Self::will_fit(bytes, headroom) {
            return Err(CudaReverseRsiError::InvalidInput("insufficient VRAM for reverse_rsi batch".into()));
        }

        let lengths_i32: Vec<i32> = combos.iter().map(|c| c.rsi_length.unwrap_or(14) as i32).collect();
        let levels_f32: Vec<f32> = combos.iter().map(|c| c.rsi_level.unwrap_or(50.0) as f32).collect();

        // Pinned host buffers + async H2D
        let h_prices = LockedBuffer::from_slice(prices).map_err(|e| CudaReverseRsiError::Cuda(e.to_string()))?;
        let h_lens = LockedBuffer::from_slice(&lengths_i32).map_err(|e| CudaReverseRsiError::Cuda(e.to_string()))?;
        let h_lvls = LockedBuffer::from_slice(&levels_f32).map_err(|e| CudaReverseRsiError::Cuda(e.to_string()))?;
        ...
        unsafe {
            d_prices.async_copy_from(&h_prices, &self.stream).map_err(|e| CudaReverseRsiError::Cuda(e.to_string()))?;
            d_lengths.async_copy_from(&h_lens, &self.stream).map_err(|e| CudaReverseRsiError::Cuda(e.to_string()))?;
            d_levels.async_copy_from(&h_lvls, &self.stream).map_err(|e| CudaReverseRsiError::Cuda(e.to_string()))?;
        }

        self.launch_batch_kernel(&d_prices, &d_lengths, &d_levels, len, rows, first_valid, &mut d_out)?;
    ...
    fn prepare_many_series_inputs(
        ...
        if prices_tm.len() != cols * rows {
            return Err(CudaReverseRsiError::InvalidInput("time-major input has wrong size".into()));
        }
        let period = params.rsi_length.unwrap_or(14) as i32;
        let level = params.rsi_level.unwrap_or(50.0) as f32;
        if !(level > 0.0 && level < 100.0) || period <= 0 { return Err(CudaReverseRsiError::InvalidInput("invalid params".into())); }
        let mut first_valids = vec![0i32; cols];
        for s in 0..cols {
            let mut fv = -1i32;
            for r in 0..rows {
                let v = prices_tm[r * cols + s];
                if !v.is_nan() { fv = r as i32; break; }
            }
            if fv < 0 { fv = 0; }
            first_valids[s] = fv;
        }
    ...
    fn launch_many_series_kernel(
        ...
        unsafe { (*(self as *const _ as *mut CudaReverseRsi)).last_many = Some(ManySeriesKernelSelected::OneD { block_x }); }
    ...
    pub fn reverse_rsi_many_series_one_param_time_major_dev(
        ...
        let (first_valids, period, level) = Self::prepare_many_series_inputs(prices_tm, cols, rows, params)?;
        ...
        if !Self::will_fit(bytes, headroom) {
            return Err(CudaReverseRsiError::InvalidInput("insufficient VRAM for reverse_rsi many-series".into()));
        }

        let d_prices_tm = DeviceBuffer::from_slice(prices_tm).map_err(|e| CudaReverseRsiError::Cuda(e.to_string()))?;
        let d_first = DeviceBuffer::from_slice(&first_valids).map_err(|e| CudaReverseRsiError::Cuda(e.to_string()))?;
        let mut d_out_tm = unsafe { DeviceBuffer::<f32>::uninitialized(elems) }.map_err(|e| CudaReverseRsiError::Cuda(e.to_string()))?;

        self.launch_many_series_kernel(&d_prices_tm, &d_first, cols, rows, period, level, &mut d_out_tm)?;
        self.stream.synchronize().map_err(|e| CudaReverseRsiError::Cuda(e.to_string()))?;
...
pub mod benches {
    ...
    struct BatchState { cuda: CudaReverseRsi, price: Vec<f32>, sweep: ReverseRsiBatchRange }
    impl CudaBenchState for BatchState {
        fn launch(&mut self) {
            let _ = self.cuda.reverse_rsi_batch_dev(&self.price, &self.sweep).expect("reverse_rsi_batch_dev");
        }
    }
    fn prep_one_series_many_params() -> Box<dyn CudaBenchState> {
        let cuda = CudaReverseRsi::new(0).expect("cuda");
        let price = gen_series(ONE_SERIES_LEN);
        let sweep = ReverseRsiBatchRange { rsi_length_range: (5, 5 + PARAM_SWEEP_L as usize - 1, 1), rsi_level_range: (10.0, 10.0 + PARAM_SWEEP_V as f64 - 1.0, 1.0) };
        Box::new(BatchState { cuda, price, sweep })
    }

    struct ManyState { cuda: CudaReverseRsi, data_tm: Vec<f32>, cols: usize, rows: usize, params: ReverseRsiParams }
    impl CudaBenchState for ManyState {
        fn launch(&mut self) {
            let _ = self.cuda.reverse_rsi_many_series_one_param_time_major_dev(&self.data_tm, self.cols, self.rows, &self.params).expect("reverse_rsi_many_series_one_param_time_major_dev");
        }
    }
