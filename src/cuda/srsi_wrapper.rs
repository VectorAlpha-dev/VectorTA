...
impl CudaSrsi {
    ...
    pub fn srsi_batch_dev(
        ...
        if len == 0 { return Err(CudaSrsiError::InvalidInput("empty series".into())); }
        ...
        if combos.is_empty() {
            return Err(CudaSrsiError::InvalidInput("no parameter combinations".into()));
        }
        let max_need = combos
            .iter()
            .map(|c| {
        ...
        for (rp, idxs) in groups {
            ...
            let mut max_sp = 0usize; let mut max_k = 0usize; let mut max_d = 0usize;
            for &row in &idxs {
                let p = &combos[row];
                let s = p.stoch_period.unwrap(); let k = p.k.unwrap(); let d = p.d.unwrap();
                sp.push(s as i32); kp.push(k as i32); dp.push(d as i32);
                max_sp = max_sp.max(s); max_k = max_k.max(k); max_d = max_d.max(d);
            }
            let d_sp = DeviceBuffer::from_slice(&sp).map_err(|e| CudaSrsiError::Cuda(e.to_string()))?;
    ...
    pub fn srsi_many_series_one_param_time_major_dev(
        ...
        if cols == 0 || rows == 0 { return Err(CudaSrsiError::InvalidInput("empty matrix".into())); }
        if prices_tm.len() != cols * rows { return Err(CudaSrsiError::InvalidInput("size mismatch".into())); }
        ...
        if rp <= 0 || sp <= 0 || kp <= 0 || dp <= 0 { return Err(CudaSrsiError::InvalidInput("non-positive periods".into())); }
        ...
        for s in 0..cols {
            for t in 0..rows {
                ...
                if v == v { firsts[s] = t as i32; break; }
            }
        }

        // VRAM estimate
        if let Ok((free, _)) = mem_get_info() {
            let n = cols * rows; let bytes = n * std::mem::size_of::<f32>() * 3 + cols * std::mem::size_of::<i32>() + 64 * 1024 * 1024;
            if bytes > free { return Err(CudaSrsiError::InvalidInput("estimated device memory exceeds free VRAM".into())); }
        }

        let d_prices = DeviceBuffer::from_slice(prices_tm).map_err(|e| CudaSrsiError::Cuda(e.to_string()))?;
        let d_firsts = DeviceBuffer::from_slice(&firsts).map_err(|e| CudaSrsiError::Cuda(e.to_string()))?;
        let mut d_k: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(cols * rows).map_err(|e| CudaSrsiError::Cuda(e.to_string()))? };
        let mut d_d: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(cols * rows).map_err(|e| CudaSrsiError::Cuda(e.to_string()))? };
        ...
        Ok(DeviceSrsiPair { k: DeviceArrayF32 { buf: d_k, rows, cols }, d: DeviceArrayF32 { buf: d_d, rows, cols } })
    }
}
...
pub mod benches {
    ...
    fn bytes_many_series() -> usize {
        let n = MANY_COLS * MANY_ROWS; let in_b = n * std::mem::size_of::<f32>(); let out_b = n * std::mem::size_of::<f32>() * 2; in_b + out_b + 64 * 1024 * 1024
    }

    struct SrsiBatchState { cuda: CudaSrsi, prices: Vec<f32>, sweep: SrsiBatchRange, rows: usize }
    impl CudaBenchState for SrsiBatchState { fn launch(&mut self) { let _ = self.cuda.srsi_batch_dev(&self.prices, &self.sweep).expect("srsi batch"); } }
    fn prep_one_series_many_params() -> Box<dyn CudaBenchState> {
        let cuda = CudaSrsi::new(0).expect("cuda srsi");
        let mut prices = gen_series(ONE_SERIES_LEN);
        for i in 0..16 { prices[i] = f32::NAN; }
        for i in 16..ONE_SERIES_LEN { let x = i as f32 * 0.0031; prices[i] += 0.001 * x.sin(); }
        let sweep = SrsiBatchRange { rsi_period: (2, 50, 1), stoch_period: (2, 32, 1), k: (3, 5, 1), d: (3, 5, 1) };
    ...
    struct SrsiManyState { cuda: CudaSrsi, prices_tm: Vec<f32> }
    impl CudaBenchState for SrsiManyState { fn launch(&mut self) { let _ = self.cuda.srsi_many_series_one_param_time_major_dev(&self.prices_tm, MANY_COLS, MANY_ROWS, &SrsiParams::default()).expect("srsi many"); } }
    fn prep_many_series() -> Box<dyn CudaBenchState> {
        let cuda = CudaSrsi::new(0).expect("cuda srsi");
        let n = MANY_COLS * MANY_ROWS; let mut base = gen_series(n); let mut prices = vec![f32::NAN; n];
        for s in 0..MANY_COLS { for t in s..MANY_ROWS { let idx = t * MANY_COLS + s; let x = (t as f32) * 0.002 + (s as f32) * 0.01; prices[idx] = base[idx] + 0.02 * x.cos(); } }
    ...
    pub fn bench_profiles() -> Vec<CudaBenchScenario> {
        ...
        vec![
            CudaBenchScenario::new("srsi", "one_series_many_params", "srsi_cuda_batch_dev", "1m_param_sweep", prep_one_series_many_params)
                .with_sample_size(10)
                .with_mem_required(bytes_one_series_many_params(rows)),
            CudaBenchScenario::new("srsi", "many_series_one_param", "srsi_cuda_many_series_one_param_dev", "1024x8192", prep_many_series)
                .with_sample_size(10)
