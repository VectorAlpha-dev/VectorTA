...
impl CudaDevStop {
    ...
    pub fn devstop_batch_dev(
        ...
        let d_low = DeviceBuffer::from_slice(&low[..len])
            .map_err(|e| CudaDevStopError::Cuda(e.to_string()))?;
        let d_p1 =
            DeviceBuffer::from_slice(&p1).map_err(|e| CudaDevStopError::Cuda(e.to_string()))?;
        let d_p2 =
            DeviceBuffer::from_slice(&p2).map_err(|e| CudaDevStopError::Cuda(e.to_string()))?;
        let d_pc =
            DeviceBuffer::from_slice(&pc).map_err(|e| CudaDevStopError::Cuda(e.to_string()))?;
        ...
        for (period, mults_host) in groups.into_iter() {
            ...
            let shmem_bytes =
                (period * (std::mem::size_of::<f32>() + std::mem::size_of::<i32>())) as u32;
    ...
    pub fn devstop_many_series_one_param_time_major_dev(
        ...
        let shmem_bytes =
            (period * (2 * std::mem::size_of::<f32>() + std::mem::size_of::<i32>())) as u32;
