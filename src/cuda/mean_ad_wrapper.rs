...
#[derive(Clone, Copy, Debug)]
pub struct CudaMeanAdPolicy { pub batch: BatchKernelPolicy, pub many_series: ManySeriesKernelPolicy }
...
impl CudaMeanAd {
    pub fn new(device_id: usize) -> Result<Self, CudaMeanAdError> {
        ...
        let device = Device::get_device(device_id as u32).map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;
        ...
        let stream = Stream::new(StreamFlags::NON_BLOCKING, None)
            .map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;
        Ok(Self { module, stream, _context: context, policy: CudaMeanAdPolicy::default() })
    }
    ...
    fn prepare_batch_inputs(
        ...
        if data_f32.is_empty() { return Err(CudaMeanAdError::InvalidInput("empty data".into())); }
        let first_valid = data_f32.iter().position(|x| !x.is_nan())
        ...
        let combos: Vec<MeanAdParams> = {
            ...
            if st == 0 || s == e {
                ...
                let mut v = Vec::new();
                let mut p = s;
                while p <= e { v.push(MeanAdParams { period: Some(p) }); p += st; }
                v
            }
        };
        if combos.is_empty() { return Err(CudaMeanAdError::InvalidInput("no parameter combinations".into())); }
        let len = data_f32.len();
        let mut max_period = 0usize;
        for prm in &combos {
            let p = prm.period.unwrap_or(0);
            if p == 0 { return Err(CudaMeanAdError::InvalidInput("period must be > 0".into())); }
            if p > len { return Err(CudaMeanAdError::InvalidInput("period exceeds data length".into())); }
            if len - first_valid < p {
                return Err(CudaMeanAdError::InvalidInput("not enough valid data for period".into()));
            }
    ...
    pub fn mean_ad_batch_dev(
        ...
        let prices_bytes = series_len.checked_mul(std::mem::size_of::<f32>())
            .ok_or_else(|| CudaMeanAdError::InvalidInput("size overflow".into()))?;
        let periods_bytes = n_combos.checked_mul(std::mem::size_of::<i32>())
            .ok_or_else(|| CudaMeanAdError::InvalidInput("size overflow".into()))?;
        let warms_bytes = n_combos.checked_mul(std::mem::size_of::<i32>())
            .ok_or_else(|| CudaMeanAdError::InvalidInput("size overflow".into()))?;
        ...
        if let Ok((free, _)) = mem_get_info() {
            ...
            if required.saturating_add(headroom) > free {
                return Err(CudaMeanAdError::InvalidInput("insufficient free VRAM".into()));
            }
        }
        ...
        let d_prices = DeviceBuffer::from_slice(data_f32).map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;
        let d_periods = DeviceBuffer::from_slice(&periods_i32).map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;
        let d_warms = DeviceBuffer::from_slice(&warms_i32).map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;
        let mut d_out: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(n_combos * series_len) }
            .map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;

        let mut func = self.module.get_function("mean_ad_batch_f32")
            .map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;
        ...
        unsafe {
            ...
            self.stream.launch(&mut func, grid, block, shared_bytes, &mut args)
                .map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;
        }

        self.stream.synchronize().map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;
    ...
    fn prepare_many_series_inputs(
        ...
        if cols == 0 || rows == 0 { return Err(CudaMeanAdError::InvalidInput("empty grid".into())); }
        if data_tm_f32.len() != cols * rows { return Err(CudaMeanAdError::InvalidInput("data length mismatch".into())); }
        let period = params.period.unwrap_or(5);
        if period == 0 { return Err(CudaMeanAdError::InvalidInput("period must be > 0".into())); }
        if period > rows { return Err(CudaMeanAdError::InvalidInput("period exceeds series length".into())); }
        // first_valid per series (column)
        let mut firsts = vec![0i32; cols];
        for s in 0..cols { let mut f = -1; for t in 0..rows { let v = data_tm_f32[t * cols + s]; if !v.is_nan() { f = t as i32; break; } } firsts[s] = f; }
        Ok((firsts, period))
    }
    ...
    pub fn mean_ad_many_series_one_param_time_major_dev(
        ...
        let mut block_x = match self.policy.many_series { ManySeriesKernelPolicy::OneD { block_x } if block_x > 0 => block_x as usize, _ => 128 };
        block_x = block_x.min(max_shmem / (period * std::mem::size_of::<f32>())).max(1);
        ...
        let d_prices_tm = DeviceBuffer::from_slice(data_tm_f32).map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;
        let d_firsts = DeviceBuffer::from_slice(&firsts).map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;
        let mut d_out_tm: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(cols * rows) }
            .map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;

        let mut func = self.module.get_function("mean_ad_many_series_one_param_f32")
            .map_err(|e| CudaMeanAdError::Cuda(e.to_string()))?;
...
pub mod benches {
    ...
    define_ma_period_benches!(
        ...
        crate::indicators::mean_ad::MeanAdBatchRange { period: (10, 10 + PARAM_SWEEP - 1, 1) },
