...
impl DeviceKstPair {
    #[inline] pub fn rows(&self) -> usize { self.line.rows }
    #[inline] pub fn cols(&self) -> usize { self.line.cols }
}
...
impl CudaKst {
    pub fn new(device_id: usize) -> Result<Self, CudaKstError> {
        ...
        let module = match Module::from_ptx(ptx, jit_opts) {
            ...
            Err(_) => {
                ...
                { m } else { Module::from_ptx(ptx, &[]).map_err(|e| CudaKstError::Cuda(e.to_string()))? }
            }
        };
        let stream = Stream::new(StreamFlags::NON_BLOCKING, None)
            .map_err(|e| CudaKstError::Cuda(e.to_string()))?;
        Ok(Self { module, stream, _context: context, policy: CudaKstPolicy::default() })
    }

    #[inline] pub fn set_policy(&mut self, policy: CudaKstPolicy) { self.policy = policy; }
    #[inline] pub fn policy(&self) -> &CudaKstPolicy { &self.policy }
    #[inline] pub fn synchronize(&self) -> Result<(), CudaKstError> {
        self.stream.synchronize().map_err(|e| CudaKstError::Cuda(e.to_string()))
    }
    ...
    fn expand_grid(range: &KstBatchRange) -> Vec<KstParams> {
        fn axis(t: (usize, usize, usize)) -> Vec<usize> {
            if t.2 == 0 || t.0 == t.1 { vec![t.0] } else { (t.0..=t.1).step_by(t.2).collect() }
        }
        ...
        let mut out = Vec::with_capacity(s1.len()*s2.len()*s3.len()*s4.len()*r1.len()*r2.len()*r3.len()*r4.len()*sg.len());
        for &a in &s1 {
            for &b in &s2 {
                for &c in &s3 {
                    for &d in &s4 {
                            ...
                            for &f in &r2 {
                                for &g in &r3 {
                                    for &h in &r4 {
                                        for &q in &sg {
                                            out.push(KstParams {
                                                sma_period1: Some(a), sma_period2: Some(b), sma_period3: Some(c), sma_period4: Some(d),
                                                roc_period1: Some(e), roc_period2: Some(f), roc_period3: Some(g), roc_period4: Some(h),
    ...
    pub fn kst_batch_dev(&self, prices: &[f32], sweep: &KstBatchRange) -> Result<(DeviceKstPair, Vec<KstParams>), CudaKstError> {
        if prices.is_empty() { return Err(CudaKstError::InvalidInput("empty price input".into())); }
        let combos = Self::expand_grid(sweep);
        if combos.is_empty() { return Err(CudaKstError::InvalidInput("empty parameter sweep".into())); }
        let len = prices.len();
        let first_valid = (0..len).find(|&i| !prices[i].is_nan()).ok_or_else(|| CudaKstError::InvalidInput("all values are NaN".into()))?;

        // Validate: need at least warm_sig samples available
        let mut max_warm_line = 0usize; let mut max_sig = 0usize;
        for c in &combos {
            ...
            if wl > max_warm_line { max_warm_line = wl; }
            if ws > max_sig { max_sig = ws; }
        }
        if len - first_valid <= max_warm_line { return Err(CudaKstError::InvalidInput("not enough valid data for KST warmup".into())); }

        let rows = combos.len();
        let req_bytes = len*4 + 9*rows*4 + 2*rows*len*4 + 64*1024*1024; // prices + periods + outputs + headroom
        if !Self::device_mem_ok(req_bytes) { return Err(CudaKstError::InvalidInput("insufficient device memory".into())); }

        let d_prices: DeviceBuffer<f32> = unsafe { DeviceBuffer::from_slice_async(prices, &self.stream) }
            .map_err(|e| CudaKstError::Cuda(e.to_string()))?;
        // Compact period arrays
        let to_i32 = |v: usize| -> i32 { v as i32 };
        let s1: Vec<i32> = combos.iter().map(|c| to_i32(c.sma_period1.unwrap())).collect();
        let s2: Vec<i32> = combos.iter().map(|c| to_i32(c.sma_period2.unwrap())).collect();
        let s3: Vec<i32> = combos.iter().map(|c| to_i32(c.sma_period3.unwrap())).collect();
        let s4: Vec<i32> = combos.iter().map(|c| to_i32(c.sma_period4.unwrap())).collect();
        let r1: Vec<i32> = combos.iter().map(|c| to_i32(c.roc_period1.unwrap())).collect();
        let r2: Vec<i32> = combos.iter().map(|c| to_i32(c.roc_period2.unwrap())).collect();
        let r3: Vec<i32> = combos.iter().map(|c| to_i32(c.roc_period3.unwrap())).collect();
        let r4: Vec<i32> = combos.iter().map(|c| to_i32(c.roc_period4.unwrap())).collect();
        let sg: Vec<i32> = combos.iter().map(|c| to_i32(c.signal_period.unwrap())).collect();
        ...
        let mut d_line: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized_async(rows*len, &self.stream) }
            .map_err(|e| CudaKstError::Cuda(e.to_string()))?;
        let mut d_signal: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized_async(rows*len, &self.stream) }
            .map_err(|e| CudaKstError::Cuda(e.to_string()))?;

        self.launch_batch(&d_prices, &d_s1, &d_s2, &d_s3, &d_s4, &d_r1, &d_r2, &d_r3, &d_r4, &d_sg,
                          len, rows, first_valid, &mut d_line, &mut d_signal)?;
        self.synchronize()?;

        let pair = DeviceKstPair {
            line:   DeviceArrayF32 { buf: d_line,   rows, cols: len },
    ...
    fn launch_batch(
        ...
        let func = self.module.get_function("kst_batch_f32")
            .map_err(|e| CudaKstError::Cuda(e.to_string()))?;
        let block_x = match self.policy.batch { BatchKernelPolicy::Auto => 256, BatchKernelPolicy::Plain { block_x } => block_x.max(64) };
    ...
    pub fn kst_many_series_one_param_time_major_dev(
        ...
        if cols == 0 || rows == 0 { return Err(CudaKstError::InvalidInput("cols/rows must be positive".into())); }
        if data_tm_f32.len() != cols * rows { return Err(CudaKstError::InvalidInput("time-major buffer size mismatch".into())); }
        ...
        for s in 0..cols {
            ...
            if fv < 0 { fv = rows as i32; }
            first_valids[s] = fv;
        }

        let req = data_tm_f32.len()*4 + cols*4 + 2*data_tm_f32.len()*4 + 64*1024*1024;
        if !Self::device_mem_ok(req) { return Err(CudaKstError::InvalidInput("insufficient device memory".into())); }

        let d_prices_tm: DeviceBuffer<f32> = unsafe { DeviceBuffer::from_slice_async(data_tm_f32, &self.stream) }
            .map_err(|e| CudaKstError::Cuda(e.to_string()))?;
        let d_first = DeviceBuffer::from_slice(&first_valids).map_err(|e| CudaKstError::Cuda(e.to_string()))?;
        let mut d_line_tm: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized_async(cols*rows, &self.stream) }
            .map_err(|e| CudaKstError::Cuda(e.to_string()))?;
        let mut d_sig_tm: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized_async(cols*rows, &self.stream) }
            .map_err(|e| CudaKstError::Cuda(e.to_string()))?;

        self.launch_many_series(&d_prices_tm, cols, rows, s1, s2, s3, s4, r1, r2, r3, r4, sig, &d_first, &mut d_line_tm, &mut d_sig_tm)?;
        self.synchronize()?;

        Ok(DeviceKstPair {
            line:   DeviceArrayF32 { buf: d_line_tm, rows, cols },
            signal: DeviceArrayF32 { buf: d_sig_tm, rows, cols },
        })
    }
    ...
    fn launch_many_series(
        ...
        let func = self.module.get_function("kst_many_series_one_param_f32")
            .map_err(|e| CudaKstError::Cuda(e.to_string()))?;
        let block_x = match self.policy.many_series { ManySeriesKernelPolicy::Auto => 256, ManySeriesKernelPolicy::OneD { block_x } => block_x.max(64) };
        ...
        unsafe {
            ...
            let mut s1_ = s1 as i32; let mut s2_ = s2 as i32; let mut s3_ = s3 as i32; let mut s4_ = s4 as i32;
            let mut r1_ = r1 as i32; let mut r2_ = r2 as i32; let mut r3_ = r3 as i32; let mut r4_ = r4 as i32;
...
pub mod benches {
    ...
    struct BatchState { cuda: CudaKst, price: Vec<f32>, sweep: KstBatchRange }
    impl CudaBenchState for BatchState {
        fn launch(&mut self) { let _ = self.cuda.kst_batch_dev(&self.price, &self.sweep).expect("kst batch"); }
    }
    ...
    struct ManyState { cuda: CudaKst, data_tm: Vec<f32>, cols: usize, rows: usize, params: KstParams }
    impl CudaBenchState for ManyState {
        fn launch(&mut self) { let _ = self.cuda.kst_many_series_one_param_time_major_dev(&self.data_tm, self.cols, self.rows, &self.params).expect("kst many"); }
    }
    ...
    pub fn bench_profiles() -> Vec<CudaBenchScenario> {
        vec![
            CudaBenchScenario::new("kst", "one_series_many_params", "kst_cuda_batch_dev", "1m_x_125", prep_one_series_many_params)
                .with_sample_size(10)
                .with_mem_required(bytes_one_series_many_params()),
            CudaBenchScenario::new("kst", "many_series_one_param", "kst_cuda_many_series_one_param", "250x1m", prep_many_series_one_param)
                .with_sample_size(6)
