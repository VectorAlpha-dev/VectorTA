...
impl CudaDamianiVolatmeter {
    ...
    pub fn damiani_volatmeter_batch_dev(
        ...
        let d_ss_ = DeviceBuffer::from_slice(&sed_std)
            .map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
        let d_th =
            DeviceBuffer::from_slice(&thresh).map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
        let d_s = DeviceBuffer::from_slice(&s_prefix)
            .map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
        let d_ss = DeviceBuffer::from_slice(&ss_prefix)
            .map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
        let d_tr =
            DeviceBuffer::from_slice(&tr).map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
        let mut d_out: DeviceBuffer<f32> =
            unsafe { DeviceBuffer::uninitialized_async(2 * rows * series_len, &self.stream) }
                .map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
    ...
    pub fn damiani_volatmeter_many_series_one_param_time_major_dev(
        ...
        let d_first = DeviceBuffer::from_slice(&first_valids)
            .map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
        let d_s =
            DeviceBuffer::from_slice(&s_tm).map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
        let d_ss =
            DeviceBuffer::from_slice(&ss_tm).map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
        let mut d_out: DeviceBuffer<f32> =
