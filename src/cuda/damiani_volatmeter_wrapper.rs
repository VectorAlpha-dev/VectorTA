...
impl CudaDamianiVolatmeter {
    ...
    fn maybe_log_batch_debug(&self) {
        ...
        if std::env::var("BENCH_DEBUG").ok().as_deref() == Some("1") {
            if let Some(sel) = self.last_batch {
                ...
                unsafe { (*(self as *const _ as *mut CudaDamianiVolatmeter)).debug_batch_logged = true; }
            }
        }
    }
    #[inline]
    fn maybe_log_many_debug(&self) {
        ...
        if std::env::var("BENCH_DEBUG").ok().as_deref() == Some("1") {
            if let Some(sel) = self.last_many {
                ...
                unsafe { (*(self as *const _ as *mut CudaDamianiVolatmeter)).debug_many_logged = true; }
            }
        }
    }
    ...
    fn compute_tr_close_only(prices: &[f32], first_valid: usize) -> Vec<f32> {
        ...
        for i in first_valid..prices.len() {
            let c = prices[i];
            let t = if have_prev && c.is_finite() { (c - prev_close).abs() } else { 0.0 };
            tr[i] = t;
    ...
    fn compute_prefix_sums(prices: &[f32], first_valid: usize) -> (Vec<f64>, Vec<f64>) {
        ...
        let mut acc = 0f64; let mut acc2 = 0f64;
        for i in 0..prices.len() {
            if i >= first_valid {
                let v = if prices[i].is_nan() { 0.0 } else { prices[i] as f64 };
                acc += v; acc2 += v * v;
            }
            s[i] = acc; ss[i] = acc2;
        }
    ...
    fn compute_prefix_sums_time_major(close_tm: &[f32], cols: usize, rows: usize, first_valids: &[i32]) -> (Vec<f64>, Vec<f64>) {
        ...
        for series in 0..cols {
            ...
            for t in 0..rows {
                ...
                if t >= fv {
                    let v = if close_tm[idx].is_nan() { 0.0 } else { close_tm[idx] as f64 };
                    acc += v; acc2 += v * v;
                }
                s[idx] = acc; ss[idx] = acc2;
            }
        }
        (s, ss)
    }
        ...
        d_out: &mut DeviceBuffer<f32>) -> Result<(), CudaDamianiError>
    {
        ...
        unsafe {
            ...
            self.stream.launch(&func, grid, block, 0, &mut args)
                .map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
            (*(self as *const _ as *mut CudaDamianiVolatmeter)).last_batch = Some(BatchKernelSelected::Plain { block_x });
        }
    ...
    pub fn damiani_volatmeter_batch_dev(
        ...
        if series_len == 0 { return Err(CudaDamianiError::InvalidInput("empty series".into())); }
        let combos = Self::expand_grid(sweep);
        if combos.is_empty() { return Err(CudaDamianiError::InvalidInput("no combinations".into())); }
        let first_valid = Self::first_valid_close(prices)?;

        // Validate feasibility
        for prm in &combos {
            let needed = *[
                prm.vis_atr.unwrap(), prm.vis_std.unwrap(), prm.sed_atr.unwrap(), prm.sed_std.unwrap(), 3
            ].iter().max().unwrap();
            if series_len - first_valid < needed {
                return Err(CudaDamianiError::InvalidInput(format!(
                    "not enough valid data (need >= {}, have {})",
                    needed, series_len - first_valid
                )));
            }
        }
        ...
        d_out_tm: &mut DeviceBuffer<f32>) -> Result<(), CudaDamianiError>
    {
        ...
        unsafe {
            ...
            self.stream.launch(&func, grid, block, 0, &mut args)
                .map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
            (*(self as *const _ as *mut CudaDamianiVolatmeter)).last_many = Some(ManySeriesKernelSelected::OneD { block_x });
        }
    ...
    pub fn damiani_volatmeter_many_series_one_param_time_major_dev(
        ...
        if cols == 0 || rows == 0 { return Err(CudaDamianiError::InvalidInput("empty matrix".into())); }
        if high_tm.len() != low_tm.len() || low_tm.len() != close_tm.len() {
            return Err(CudaDamianiError::InvalidInput("matrix length mismatch".into()));
        }
        if high_tm.len() != cols * rows { return Err(CudaDamianiError::InvalidInput("matrix shape mismatch".into())); }
        ...
        for series in 0..cols {
            ...
            for t in 0..rows {
                let idx = t * cols + series;
                if close_tm[idx].is_finite() { fv = Some(t as i32); break; }
            }
            let val = fv.ok_or_else(|| CudaDamianiError::InvalidInput(format!("series {} all NaN", series)))?;
            let need = *[vis_atr, vis_std, sed_atr, sed_std, 3].iter().max().unwrap();
            if rows - (val as usize) < need {
                return Err(CudaDamianiError::InvalidInput(format!(
                    "series {} lacks data: need >= {}, valid = {}", series, need, rows - (val as usize)
                )));
            }
        ...
        if !Self::will_fit(req, headroom) {
            return Err(CudaDamianiError::InvalidInput("insufficient VRAM for Damiani many-series".into()));
        }
        ...
        self.launch_many_series(&d_high, &d_low, &d_close, cols, rows, vis_atr, vis_std, sed_atr, sed_std, threshold, &d_first, &d_s, &d_ss, &mut d_out)?;
        self.stream.synchronize().map_err(|e| CudaDamianiError::Cuda(e.to_string()))?;
...
pub mod benches {
    ...
    struct BatchState { cuda: CudaDamianiVolatmeter, close: Vec<f32>, sweep: DamianiVolatmeterBatchRange }
    impl CudaBenchState for BatchState { fn launch(&mut self) { let _ = self.cuda.damiani_volatmeter_batch_dev(&self.close, &self.sweep).unwrap(); } }

    struct ManySeriesState { cuda: CudaDamianiVolatmeter, high_tm: Vec<f32>, low_tm: Vec<f32>, close_tm: Vec<f32>, cols: usize, rows: usize, params: DamianiVolatmeterParams }
    impl CudaBenchState for ManySeriesState { fn launch(&mut self) { let _ = self.cuda.damiani_volatmeter_many_series_one_param_time_major_dev(&self.high_tm, &self.low_tm, &self.close_tm, self.cols, self.rows, &self.params).unwrap(); } }
    ...
    fn prep_many() -> Box<dyn CudaBenchState> {
        ...
        Box::new(ManySeriesState { cuda, high_tm, low_tm, close_tm, cols, rows, params })
    }
    ...
    pub fn bench_profiles() -> Vec<CudaBenchScenario> {
        vec![
            CudaBenchScenario::new("damiani_volatmeter", "batch", "damiani_cuda_batch", "1m", prep_batch)
                .with_mem_required(bytes_batch()),
            CudaBenchScenario::new("damiani_volatmeter", "many_series_one_param", "damiani_cuda_many_series", "8k x 256", prep_many)
