...
pub struct CudaVpciPolicy { pub batch: BatchKernelPolicy, pub many_series: ManySeriesKernelPolicy }
...
impl CudaVpci {
    ...
    pub fn new_with_policy(device_id: usize, policy: CudaVpciPolicy) -> Result<Self, CudaVpciError> {
        cust::init(CudaFlags::empty()).map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let device = Device::get_device(device_id as u32).map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        ...
        let stream = Stream::new(StreamFlags::NON_BLOCKING, None).map_err(|e| CudaVpciError::Cuda(e.to_string()))?;

        Ok(Self { module, stream, _context: context, policy })
    }

    #[inline] pub fn set_policy(&mut self, policy: CudaVpciPolicy) { self.policy = policy; }
    #[inline] pub fn policy(&self) -> &CudaVpciPolicy { &self.policy }
    #[inline] pub fn synchronize(&self) -> Result<(), CudaVpciError> { self.stream.synchronize().map_err(|e| CudaVpciError::Cuda(e.to_string())) }

    #[inline]
    fn mem_ok(bytes: usize, headroom: usize) -> bool {
        if std::env::var("CUDA_MEM_CHECK").ok().filter(|v| v == "0" || v.eq_ignore_ascii_case("false")).is_some() { return true; }
        mem_get_info().map(|(free, _)| bytes.saturating_add(headroom) <= free).unwrap_or(true)
    }

    // --------------- Prefix sums (single series) ---------------
    fn build_prefix_single(&self, close: &[f32], volume: &[f32]) -> Result<(usize, Vec<f64>, Vec<f64>, Vec<f64>), CudaVpciError> {
        ...
        -> Result<(Vec<i32>, Vec<f64>, Vec<f64>, Vec<f64>), CudaVpciError>
    {
        ...
        for s in 0..cols {
            ...
            for r in 0..rows {
                let idx = r * cols + s;
                if close_tm[idx].is_finite() && volume_tm[idx].is_finite() { first = Some(r); break; }
            }
            if let Some(fv) = first {
                ...
                for r in (fv + 1)..rows {
                    let idx = r * cols + s; let prev = (r - 1) * cols + s;
    ...
    pub fn vpci_batch_dev(
        ...
        if close_f32.len() != volume_f32.len() { return Err(CudaVpciError::InvalidInput("length mismatch".into())); }
        let len = close_f32.len(); if len == 0 { return Err(CudaVpciError::InvalidInput("empty input".into())); }
        ...
        for &s in &shorts { for &l in &longs { combos.push(VpciParams { short_range: Some(s), long_range: Some(l) }); } }
        if combos.is_empty() { return Err(CudaVpciError::InvalidInput("no parameter combinations".into())); }
        let max_long = combos.iter().map(|p| p.long_range.unwrap()).max().unwrap();

        let (first_valid, pfx_c, pfx_v, pfx_cv) = self.build_prefix_single(close_f32, volume_f32)?;
        ...
        if !Self::mem_ok(bytes, headroom) { return Err(CudaVpciError::InvalidInput("insufficient VRAM for VPCI batch".into())); }

        // Upload inputs
        let h_pfx_c  = LockedBuffer::from_slice(&pfx_c).map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let h_pfx_v  = LockedBuffer::from_slice(&pfx_v).map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let h_pfx_cv = LockedBuffer::from_slice(&pfx_cv).map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let d_pfx_c  = unsafe { DeviceBuffer::from_slice_async(h_pfx_c.as_slice(), &self.stream) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let d_pfx_v  = unsafe { DeviceBuffer::from_slice_async(h_pfx_v.as_slice(), &self.stream) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let d_pfx_cv = unsafe { DeviceBuffer::from_slice_async(h_pfx_cv.as_slice(), &self.stream) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let d_vol    = unsafe { DeviceBuffer::from_slice_async(volume_f32, &self.stream) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        ...
        let d_shorts = unsafe { DeviceBuffer::from_slice_async(&shorts_i32, &self.stream) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let d_longs  = unsafe { DeviceBuffer::from_slice_async(&longs_i32,  &self.stream) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;

        // Outputs
        let mut d_vpci:  DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(rows * len) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let mut d_vpcis: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(rows * len) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        ...
        self.stream.synchronize().map_err(|e| CudaVpciError::Cuda(e.to_string()))?;

        Ok((DeviceArrayF32Pair {
            a: DeviceArrayF32 { buf: d_vpci, rows, cols: len },
            b: DeviceArrayF32 { buf: d_vpcis, rows, cols: len },
        }, combos))
    }
    ...
    pub fn vpci_many_series_one_param_time_major_dev(
        ...
        if short_p == 0 || long_p == 0 || short_p > long_p { return Err(CudaVpciError::InvalidInput("invalid params".into())); }
        if cols == 0 || rows == 0 { return Err(CudaVpciError::InvalidInput("invalid dims".into())); }
        if close_tm_f32.len() != rows * cols || volume_tm_f32.len() != rows * cols { return Err(CudaVpciError::InvalidInput("dims do not match data length".into())); }

        let (first_valids, pfx_c, pfx_v, pfx_cv) = self.build_prefix_tm(close_tm_f32, volume_tm_f32, cols, rows)?;
        ...
        let d_pfx_v  = unsafe { DeviceBuffer::from_slice_async(h_pfx_v.as_slice(), &self.stream) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let d_pfx_cv = unsafe { DeviceBuffer::from_slice_async(h_pfx_cv.as_slice(), &self.stream) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let d_firsts = unsafe { DeviceBuffer::from_slice_async(h_firsts.as_slice(), &self.stream) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let d_vol    = unsafe { DeviceBuffer::from_slice_async(volume_tm_f32, &self.stream) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;

        // Outputs
        let total = rows * cols;
        let mut d_vpci:  DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(total) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        let mut d_vpcis: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(total) }.map_err(|e| CudaVpciError::Cuda(e.to_string()))?;
        ...
        unsafe {
            ...
            let mut short_i = short_p as i32; let mut long_i = long_p as i32;
            let mut out_vpci_ptr  = d_vpci.as_device_ptr().as_raw();
            let mut out_vpcis_ptr = d_vpcis.as_device_ptr().as_raw();
            let mut args: [*mut c_void; 12] = [
                &mut pfx_c_ptr as *mut _ as *mut c_void,
...
pub mod benches {
    ...
    impl CudaBenchState for BatchState { fn launch(&mut self) { let _ = self.cuda.vpci_batch_dev(&self.c, &self.v, &self.sweep); } }
    ...
    pub fn bench_profiles() -> Vec<CudaBenchScenario> {
        ...
        v.push(CudaBenchScenario::new(
            ...
            || {
                ...
                for i in 0..1024 { close[i] = f32::NAN; vol[i] = f32::NAN; }
                Box::new(BatchState {
                    cuda: CudaVpci::new(0).unwrap(),
                    c: close,
                    v: vol,
                    sweep: VpciBatchRange { short_range: (5, 20, 1), long_range: (25, 60, 5) },
                })
            },
