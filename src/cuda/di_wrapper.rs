...
impl CudaDi {
    ...
    pub fn di_batch_dev(
        ...
        let d_periods = DeviceBuffer::from_slice(&periods_i32).map_err(|e| CudaDiError::Cuda(e.to_string()))?;
        let d_warms = DeviceBuffer::from_slice(&warms_i32).map_err(|e| CudaDiError::Cuda(e.to_string()))?;

        // Allocate outputs (combos x len)
        let elems = n_combos * len;
        let mut d_plus: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaDiError::Cuda(e.to_string()))?;
        let mut d_minus: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaDiError::Cuda(e.to_string()))?;
    ...
    pub fn di_many_series_one_param_time_major_dev(
        ...
        let d_high = DeviceBuffer::from_slice(high_tm).map_err(|e| CudaDiError::Cuda(e.to_string()))?;
        let d_low  = DeviceBuffer::from_slice(low_tm).map_err(|e| CudaDiError::Cuda(e.to_string()))?;
        let d_close= DeviceBuffer::from_slice(close_tm).map_err(|e| CudaDiError::Cuda(e.to_string()))?;
        let d_first= DeviceBuffer::from_slice(&first_valids).map_err(|e| CudaDiError::Cuda(e.to_string()))?;

        let mut d_plus_tm: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(cols * rows) }.map_err(|e| CudaDiError::Cuda(e.to_string()))?;
        let mut d_minus_tm: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(cols * rows) }.map_err(|e| CudaDiError::Cuda(e.to_string()))?;
