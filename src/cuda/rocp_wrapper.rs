...
impl CudaRocp {
    ...
    pub fn rocp_batch_dev(
        ...
        // Upload inputs
        let h_data =
            LockedBuffer::from_slice(data).map_err(|e| CudaRocpError::Cuda(e.to_string()))?;
        let h_inv =
            LockedBuffer::from_slice(&inv_host).map_err(|e| CudaRocpError::Cuda(e.to_string()))?;
        ...
        unsafe {
            d_data
                ...
                .map_err(|e| CudaRocpError::Cuda(e.to_string()))?;
            d_inv
                .async_copy_from(&h_inv, &self.stream)
                .map_err(|e| CudaRocpError::Cuda(e.to_string()))?;
        }
    ...
    pub fn rocp_many_series_one_param_time_major_dev(
        ...
        // Upload buffers
        let h_data =
            LockedBuffer::from_slice(data_tm).map_err(|e| CudaRocpError::Cuda(e.to_string()))?;
        let h_firsts =
            LockedBuffer::from_slice(&firsts).map_err(|e| CudaRocpError::Cuda(e.to_string()))?;
        let mut d_data =
