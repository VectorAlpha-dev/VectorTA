...
impl CudaVoss {
    ...
    pub fn synchronize(&self) -> Result<(), CudaVossError> {
        self.stream.synchronize().map_err(|e| CudaVossError::Cuda(e.to_string()))
    }
    ...
    pub fn voss_batch_dev(
        ...
        if data_f32.is_empty() { return Err(CudaVossError::InvalidInput("empty input".into())); }
        let len = data_f32.len();
        let first = data_f32.iter().position(|x| !x.is_nan()).ok_or_else(|| CudaVossError::InvalidInput("all values are NaN".into()))?;

        let combos = expand_grid_voss(sweep);
        if combos.is_empty() { return Err(CudaVossError::InvalidInput("no parameter combinations".into())); }
        for prm in &combos {
            let p = prm.period.unwrap_or(0);
            let q = prm.predict.unwrap_or(0);
            let order = 3 * q;
            let min_index = p.max(5).max(order);
            if p == 0 || p > len { return Err(CudaVossError::InvalidInput("invalid period".into())); }
            if len - first < min_index { return Err(CudaVossError::InvalidInput("not enough valid data".into())); }
            let b = prm.bandwidth.unwrap_or(0.25);
            if !b.is_finite() || b <= 0.0 || b > 1.0 { return Err(CudaVossError::InvalidInput("invalid bandwidth".into())); }
        }
        ...
        let d_p = DeviceBuffer::from_slice(&periods).map_err(|e| CudaVossError::Cuda(e.to_string()))?;
        let d_q = DeviceBuffer::from_slice(&predicts).map_err(|e| CudaVossError::Cuda(e.to_string()))?;
        let d_bw = DeviceBuffer::from_slice(&bws).map_err(|e| CudaVossError::Cuda(e.to_string()))?;
        ...
        self.stream.synchronize().map_err(|e| CudaVossError::Cuda(e.to_string()))?;
        Ok((DeviceArrayF32 { buf: d_voss, rows, cols: len }, DeviceArrayF32 { buf: d_filt, rows, cols: len }, combos))
    }
    ...
    pub fn voss_many_series_one_param_time_major_dev(
        ...
        if cols == 0 || rows == 0 { return Err(CudaVossError::InvalidInput("empty matrix".into())); }
        let elems = cols.checked_mul(rows).ok_or_else(|| CudaVossError::InvalidInput("overflow".into()))?;
        if data_tm_f32.len() != elems { return Err(CudaVossError::InvalidInput("data must be time-major cols*rows".into())); }
        ...
        self.stream.synchronize().map_err(|e| CudaVossError::Cuda(e.to_string()))?;
        Ok((DeviceArrayF32 { buf: d_voss, rows, cols }, DeviceArrayF32 { buf: d_filt, rows, cols }))
    }
}
...
pub mod benches {
    ...
    impl CudaBenchState for VossBatchState {
        fn launch(&mut self) {
            let _ = self.cuda.voss_batch_dev(&self.data, &self.sweep).expect("voss batch");
        }
    }
    ...
    fn prep_batch() -> Box<dyn CudaBenchState> {
        ...
        for i in 0..4 { data[i] = f32::NAN; }
        let sweep = VossBatchRange { period: (10, 34, 2), predict: (1, 4, 1), bandwidth: (0.1, 0.4, 0.05) };
        Box::new(VossBatchState { cuda, data, sweep })
    }

    struct VossManyState { cuda: CudaVoss, data_tm: Vec<f32>, cols: usize, rows: usize, params: VossParams }
    impl CudaBenchState for VossManyState {
        fn launch(&mut self) {
            let _ = self
                .cuda
                .voss_many_series_one_param_time_major_dev(&self.data_tm, self.cols, self.rows, &self.params)
    ...
    fn prep_many() -> Box<dyn CudaBenchState> {
        ...
        for s in 0..MANY_SERIES_COLS { tm[s] = f32::NAN; tm[s + MANY_SERIES_COLS] = f32::NAN; }
        let params = VossParams { period: Some(20), predict: Some(3), bandwidth: Some(0.25) };
        Box::new(VossManyState { cuda, data_tm: tm, cols: MANY_SERIES_COLS, rows: MANY_SERIES_ROWS, params })
    }
