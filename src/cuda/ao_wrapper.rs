...
impl CudaAo {
    ...
    pub fn ao_batch_dev(
        ...
        // Device buffers
        let d_prefix =
            DeviceBuffer::from_slice(&prefix).map_err(|e| CudaAoError::Cuda(e.to_string()))?;
        ...
        if fits && rows <= 65_535 {
            let d_shorts =
                DeviceBuffer::from_slice(&shorts).map_err(|e| CudaAoError::Cuda(e.to_string()))?;
            let d_longs =
                DeviceBuffer::from_slice(&longs).map_err(|e| CudaAoError::Cuda(e.to_string()))?;
            unsafe {
        ...
        let d_out =
            DeviceBuffer::from_slice(&host_out).map_err(|e| CudaAoError::Cuda(e.to_string()))?;
        Ok(DeviceArrayF32 {
            buf: d_out,
            rows,
            cols: len,
