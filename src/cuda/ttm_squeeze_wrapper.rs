...
#[derive(Clone, Copy, Debug)]
pub struct CudaTtmSqueezePolicy { pub batch: BatchKernelPolicy, pub many_series: ManySeriesKernelPolicy }
impl Default for CudaTtmSqueezePolicy { fn default() -> Self { Self { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::Auto } } }
...
#[derive(Clone, Debug)]
struct Combo { length: i32, bb_mult: f32, kc_high: f32, kc_mid: f32, kc_low: f32 }
...
impl CudaTtmSqueeze {
    pub fn new(device_id: usize) -> Result<Self, CudaTtmSqueezeError> {
        ...
        let device = Device::get_device(device_id as u32).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        ...
        Ok(Self { module, stream, _context: context, policy: CudaTtmSqueezePolicy::default(), last_batch: None, last_many: None })
    }
    ...
    #[inline]
    fn mem_check_enabled() -> bool { match std::env::var("CUDA_MEM_CHECK") { Ok(v) => v != "0" && !v.eq_ignore_ascii_case("false"), Err(_) => true } }
    #[inline]
    ...
    fn expand_grid(range: &TtmSqueezeBatchRange) -> Vec<Combo> {
        ...
        fn axis_f64((s, e, st): (f64, f64, f64)) -> Vec<f64> {
            if (st.abs() < 1e-12) || ((s - e).abs() < 1e-12) { vec![s] } else { let mut v=Vec::new(); let mut x=s; while x <= e + 1e-12 { v.push(x); x += st; } v }
        }
        ...
        for &l in &lengths { for &b in &bb { for &h in &kh { for &m in &km { for &lo in &kl {
            out.push(Combo { length: l as i32, bb_mult: b as f32, kc_high: h as f32, kc_mid: m as f32, kc_low: lo as f32 });
        }}}}}
        out
    }

    fn prepare_batch_inputs(
        high_f32: &[f32], low_f32: &[f32], close_f32: &[f32], sweep: &TtmSqueezeBatchRange,
        ...
        if high_f32.len() != low_f32.len() || low_f32.len() != close_f32.len() {
            return Err(CudaTtmSqueezeError::InvalidInput(format!(
                "inconsistent lengths: high={}, low={}, close={}", high_f32.len(), low_f32.len(), close_f32.len()
            )));
        }
        if close_f32.is_empty() { return Err(CudaTtmSqueezeError::InvalidInput("empty series".into())); }
        let len = close_f32.len();
        let first_valid = close_f32.iter().position(|v| !v.is_nan()).ok_or_else(|| CudaTtmSqueezeError::InvalidInput("all values are NaN".into()))?;
        let combos = Self::expand_grid(sweep);
        if combos.is_empty() { return Err(CudaTtmSqueezeError::InvalidInput("no parameter combinations".into())); }
        for c in &combos { if c.length <= 0 || (len - first_valid) < (c.length as usize) { return Err(CudaTtmSqueezeError::InvalidInput("invalid length or insufficient data".into())); } }
    ...
    fn launch_batch_kernel(
        ...
        unsafe {
            ...
            self.stream.launch(&func, grid, block, 0, &mut args).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        }
        unsafe { (*(self as *const _ as *mut CudaTtmSqueeze)).last_batch = Some(BatchKernelSelected::OneD { block_x }); }
        Ok(())
    }

    pub fn ttm_squeeze_batch_dev(
        &self,
        high_f32: &[f32], low_f32: &[f32], close_f32: &[f32],
        ...
        let params_bytes = combos.len() * (std::mem::size_of::<i32>() + 4 * std::mem::size_of::<f32>());
        let out_bytes = 2 * combos.len() * len * std::mem::size_of::<f32>();
        let required = in_bytes + params_bytes + out_bytes;
        if !Self::will_fit(required, 64 * 1024 * 1024) {
            return Err(CudaTtmSqueezeError::InvalidInput(format!(
                "estimated device memory {:.2} MB exceeds free VRAM", (required as f64) / (1024.0 * 1024.0)
            )));
        }

        // Upload inputs
        let d_h = DeviceBuffer::from_slice(high_f32).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        let d_l = DeviceBuffer::from_slice(low_f32).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        let d_c = DeviceBuffer::from_slice(close_f32).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        ...
        let d_len = DeviceBuffer::from_slice(&v_len).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        let d_bb  = DeviceBuffer::from_slice(&v_bb ).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        let d_kh  = DeviceBuffer::from_slice(&v_kh ).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        let d_km  = DeviceBuffer::from_slice(&v_km ).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        let d_kl  = DeviceBuffer::from_slice(&v_kl ).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;

        // Allocate outputs
        let elems = combos.len() * len;
        let mut d_mo: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        let mut d_sq: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;

        self.launch_batch_kernel(&d_h, &d_l, &d_c, &d_len, &d_bb, &d_kh, &d_km, &d_kl, len, combos.len(), first_valid, &mut d_mo, &mut d_sq)?;
    ...
    fn launch_many_series_kernel(
        ...
        d_first: &DeviceBuffer<i32>, rows: usize, cols: usize, length: usize, bb_mult: f32, kh: f32, km: f32, kl: f32,
        ...
        let _ = func.set_cache_config(CacheConfig::PreferL1);
        let block_x = match self.policy.many_series { ManySeriesKernelPolicy::Auto => 64, ManySeriesKernelPolicy::OneD { block_x } => block_x.max(32) };
        ...
        unsafe { (*(self as *const _ as *mut CudaTtmSqueeze)).last_many = Some(ManySeriesKernelSelected::OneD { block_x }); }
        Ok(())
    }

    pub fn ttm_squeeze_many_series_one_param_time_major_dev(
        &self,
        high_tm_f32: &[f32], low_tm_f32: &[f32], close_tm_f32: &[f32],
        cols: usize, rows: usize,
        length: usize, bb_mult: f32, kc_high: f32, kc_mid: f32, kc_low: f32,
    ) -> Result<(DeviceArrayF32, DeviceArrayF32), CudaTtmSqueezeError> {
        if high_tm_f32.len() != low_tm_f32.len() || low_tm_f32.len() != close_tm_f32.len() {
            return Err(CudaTtmSqueezeError::InvalidInput("inconsistent time-major inputs".into()));
        }
        if cols == 0 || rows == 0 { return Err(CudaTtmSqueezeError::InvalidInput("zero dims".into())); }
        if high_tm_f32.len() != cols * rows { return Err(CudaTtmSqueezeError::InvalidInput("dims mismatch with buffer length".into())); }

        // Compute per-series first_valid indices on host (time-major layout)
        let mut first_valids: Vec<i32> = vec![0; cols];
        for s in 0..cols {
            let mut fv = -1i32;
            for t in 0..rows { let v = close_tm_f32[t * cols + s]; if !v.is_nan() { fv = t as i32; break; } }
            if fv < 0 { return Err(CudaTtmSqueezeError::InvalidInput(format!("series {} all NaN", s))); }
            if (rows as i32 - fv) < (length as i32) { return Err(CudaTtmSqueezeError::InvalidInput("insufficient valid data for length".into())); }
            first_valids[s] = fv;
        }
        ...
        // Upload inputs
        let d_h = DeviceBuffer::from_slice(high_tm_f32).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        let d_l = DeviceBuffer::from_slice(low_tm_f32 ).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        let d_c = DeviceBuffer::from_slice(close_tm_f32).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        let d_fv = DeviceBuffer::from_slice(&first_valids).map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;

        let mut d_mo: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;
        let mut d_sq: DeviceBuffer<f32> = unsafe { DeviceBuffer::uninitialized(elems) }.map_err(|e| CudaTtmSqueezeError::Cuda(e.to_string()))?;

        self.launch_many_series_kernel(&d_h, &d_l, &d_c, &d_fv, cols, rows, length, bb_mult, kc_high, kc_mid, kc_low, &mut d_mo, &mut d_sq)?;
        Ok((DeviceArrayF32 { buf: d_mo, rows, cols }, DeviceArrayF32 { buf: d_sq, rows, cols }))
    }
}
...
pub mod benches {
    ...
    struct TtmBatchState { cuda: CudaTtmSqueeze, h: Vec<f32>, l: Vec<f32>, c: Vec<f32>, sweep: TtmSqueezeBatchRange }
    impl CudaBenchState for TtmBatchState { fn launch(&mut self) { let _ = self.cuda.ttm_squeeze_batch_dev(&self.h, &self.l, &self.c, &self.sweep).unwrap(); } }
    fn prep_one_series_many_params() -> Box<dyn CudaBenchState> {
        let cuda = CudaTtmSqueeze::new(0).expect("cuda ttm squeeze");
        let h = gen_series(ONE_SERIES_LEN);
        let mut l = h.clone(); for v in &mut l { *v -= 0.5; }
        let mut c = h.clone(); for v in &mut c { *v -= 0.25; }
        let sweep = TtmSqueezeBatchRange { length: (10, 10 + PARAM_SWEEP - 1, 1), bb_mult: (2.0, 2.0, 0.0), kc_high: (1.0, 1.0, 0.0), kc_mid: (1.5, 1.5, 0.0), kc_low: (2.0, 2.0, 0.0) };
