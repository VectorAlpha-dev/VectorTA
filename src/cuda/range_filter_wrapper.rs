...
impl CudaRangeFilter {
    pub fn new(device_id: usize) -> Result<Self, CudaRangeFilterError> {
        ...
        let context = Context::new(device).map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))?;
    ...
    pub fn set_policy(&mut self, p: CudaRangeFilterPolicy) { self.policy = p; }
    pub fn synchronize(&self) -> Result<(), CudaRangeFilterError> {
        self.stream.synchronize().map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))
    }

    #[inline]
    fn headroom_bytes() -> usize {
        env::var("CUDA_MEM_HEADROOM").ok().and_then(|s| s.parse::<usize>().ok()).unwrap_or(64 * 1024 * 1024)
    }
    #[inline]
    ...
    pub fn range_filter_batch_dev(
        ...
        if data_f32.is_empty() { return Err(CudaRangeFilterError::InvalidInput("empty data".into())); }
        let len = data_f32.len();
        let first_valid = data_f32.iter().position(|v| v.is_finite())
            .ok_or_else(|| CudaRangeFilterError::InvalidInput("all values are NaN".into()))?;

        let combos = expand_grid(sweep);
        if combos.is_empty() { return Err(CudaRangeFilterError::InvalidInput("no parameter combinations".into())); }
        ...
        for p in &combos {
            ...
            if !rs.is_finite() || rs <= 0.0 {
                return Err(CudaRangeFilterError::InvalidInput("invalid range_size".into()));
            }
            let rp = p.range_period.unwrap_or(14);
            if rp == 0 || rp > len { return Err(CudaRangeFilterError::InvalidInput("invalid range_period".into())); }
            let sr = p.smooth_range.unwrap_or(true);
            let sp = p.smooth_period.unwrap_or(27);
            if sr && (sp == 0 || sp > len) { return Err(CudaRangeFilterError::InvalidInput("invalid smooth_period".into())); }
        }
        ...
        if !Self::will_fit(required, Self::headroom_bytes()) {
            return Err(CudaRangeFilterError::InvalidInput("insufficient device memory for range_filter batch".into()));
        }

        let d_prices = DeviceBuffer::from_slice(data_f32).map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))?;
        let range_sizes_f32: Vec<f32> = combos.iter().map(|c| c.range_size.unwrap_or(2.618) as f32).collect();
        let range_periods_i32: Vec<i32> = combos.iter().map(|c| c.range_period.unwrap_or(14) as i32).collect();
        let smooth_flags_i32: Vec<i32> = combos.iter().map(|c| if c.smooth_range.unwrap_or(true) { 1 } else { 0 }).collect();
        let smooth_periods_i32: Vec<i32> = combos.iter().map(|c| c.smooth_period.unwrap_or(27) as i32).collect();
        let d_rs  = DeviceBuffer::from_slice(&range_sizes_f32).map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))?;
        let d_rp  = DeviceBuffer::from_slice(&range_periods_i32).map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))?;
        let d_sf  = DeviceBuffer::from_slice(&smooth_flags_i32).map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))?;
        let d_sp  = DeviceBuffer::from_slice(&smooth_periods_i32).map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))?;
        ...
        let func = self.module.get_function("range_filter_batch_f32")
            .map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))?;
        let block_x = match self.policy.batch { BatchKernelPolicy::Plain { block_x } => block_x.max(1), _ => 1 };
        ...
        Ok((DeviceRangeFilterTrio { filter: d_f, high: d_h, low: d_l, rows, cols: len }, combos))
    }
    ...
    pub fn range_filter_many_series_one_param_time_major_dev(
        ...
        if rows == 0 || cols == 0 { return Err(CudaRangeFilterError::InvalidInput("empty dims".into())); }
        if data_tm_f32.len() != rows * cols {
            return Err(CudaRangeFilterError::InvalidInput("time-major input must be rows*cols".into()));
        }
        let rs_f64 = params.range_size.unwrap_or(2.618);
        if !rs_f64.is_finite() || rs_f64 <= 0.0 { return Err(CudaRangeFilterError::InvalidInput("invalid range_size".into())); }
        ...
        if rp <= 0 || (sr && sp <= 0) { return Err(CudaRangeFilterError::InvalidInput("invalid period".into())); }
        ...
        for s in 0..cols {
            for t in 0..rows {
                let idx = t * cols + s;
                if data_tm_f32[idx].is_finite() { first_valids[s] = t as i32; break; }
            }
        }
        ...
        if !Self::will_fit(in_bytes + out_bytes + aux_bytes, Self::headroom_bytes()) {
            return Err(CudaRangeFilterError::InvalidInput("insufficient device memory".into()));
        }

        let d_data = DeviceBuffer::from_slice(data_tm_f32).map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))?;
        let d_first = DeviceBuffer::from_slice(&first_valids).map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))?;
        ...
        let func = self.module.get_function("range_filter_many_series_one_param_f32")
            .map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))?;
        let block_x = match self.policy.many_series { ManySeriesKernelPolicy::OneD { block_x } => block_x.max(1), _ => 1 };
        ...
        Ok(DeviceRangeFilterTrio { filter: d_f, high: d_h, low: d_l, rows, cols })
    }
}
...
pub mod benches {
    ...
    struct RfBatchState { cuda: CudaRangeFilter }
    impl CudaBenchState for RfBatchState { fn launch(&mut self) { let _ = &self.cuda; } }

    fn prep_rf_batch() -> Box<dyn CudaBenchState> {
        let len = 120_000usize;
        let mut data = vec![f32::NAN; len];
        for i in 5..len { let x = i as f32; data[i] = (x*0.0021).sin() + 0.00021*x; }
        let sweep = RangeFilterBatchRange { range_size: (2.0, 4.0, 0.2), range_period: (8, 64, 8), smooth_range: Some(true), smooth_period: Some(27) };
        let cuda = CudaRangeFilter::new(0).unwrap();
        let _ = cuda.range_filter_batch_dev(&data, &sweep).unwrap();
        Box::new(RfBatchState { cuda })
    }

    struct RfManySeriesState { cuda: CudaRangeFilter }
    impl CudaBenchState for RfManySeriesState { fn launch(&mut self) { let _ = &self.cuda; } }
    ...
    fn prep_rf_many_series() -> Box<dyn CudaBenchState> {
        ...
        for s in 0..cols { for t in s..rows { let idx = t*cols + s; let x = t as f32 + s as f32*0.01; tm[idx] = (x*0.0013).sin() + 0.00011*x; } }
        let params = RangeFilterParams { range_size: Some(2.618), range_period: Some(14), smooth_range: Some(true), smooth_period: Some(27) };
        let cuda = CudaRangeFilter::new(0).unwrap();
        let _ = cuda.range_filter_many_series_one_param_time_major_dev(&tm, cols, rows, &params).unwrap();
        Box::new(RfManySeriesState { cuda })
    }

    pub fn bench_profiles() -> Vec<CudaBenchScenario> {
        vec![
            CudaBenchScenario::new("range_filter", "one_series_many_params", "cuda/range_filter", "batch", prep_rf_batch)
                .with_sample_size(25),
            CudaBenchScenario::new("range_filter", "many_series_one_param", "cuda/range_filter", "many_series", prep_rf_many_series)
                .with_sample_size(15),
        ]
    }
}
