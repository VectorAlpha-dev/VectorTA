...
pub struct CudaRangeFilter {
    module: Module,
    stream: Stream,
    device: Device, // keep a handle for launch sizing
    _context: Context,
...
impl CudaRangeFilter {
    pub fn new(device_id: usize) -> Result<Self, CudaRangeFilterError> {
        ...
        // Re-acquire a device handle for storage if Context::new consumed it
        let device = Device::get_device(device_id as u32)
            .map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))?;
    ...
    fn pick_1d_launch_for_batch(
        ...
        use cust::device::DeviceAttribute as DA;
        let max_threads_per_block =
            self.device
                .get_attribute(DA::MaxThreadsPerBlock)
                .map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))? as u32;
        let sm_count = self
            .device
            .get_attribute(DA::MultiprocessorCount)
            .map_err(|e| CudaRangeFilterError::Cuda(e.to_string()))? as u32;
        ...
        let blocks_needed = ((n as u32) + block_x - 1) / block_x;
