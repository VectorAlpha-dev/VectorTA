...
fn main() {
    ...
    let candles = read_candles_from_csv("src/data/1MillionCandles.csv")
    ...
    // Warmup
    for _ in 0..10 {
        let _ = sma_with_kernel(&SmaInput::from_slice(data, SmaParams { period: Some(14) }), Kernel::Auto);
        let _ = alma_with_kernel(&AlmaInput::from_slice(data, AlmaParams {
            period: Some(9),
            offset: Some(0.85),
            sigma: Some(6.0),
        }), Kernel::Auto);
    ...
    println!("SMA (Auto kernel): {:.2} ms", sma_avg.as_secs_f64() * 1000.0);
    ...
    println!("ALMA (Auto kernel): {:.2} ms", alma_avg.as_secs_f64() * 1000.0);
