...
mod tests {
	...
	fn check_medium_ad_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let period = 5;
		let input = MediumAdInput::from_candles(&candles, "close", MediumAdParams { period: Some(period) });
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let output = MediumAdBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
impl MediumAdStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = MediumAdParams { period: Some(period) };
		let stream = MediumAdStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn medium_ad_batch_into(
	...
	if in_ptr.is_null() || out_ptr.is_null() {
		return Err(JsValue::from_str("null pointer passed to medium_ad_batch_into"));
	}
