...
impl TrixBuilder {
	...
	pub fn apply(self, c: &Candles) -> Result<TrixOutput, TrixError> {
		let p = TrixParams { period: self.period };
...
impl TrixBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<TrixBatchOutput, TrixError> {
		TrixBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}

pub fn trix_batch_with_kernel(data: &[f64], sweep: &TrixBatchRange, k: Kernel) -> Result<TrixBatchOutput, TrixError> {
	let kernel = match k {
...
#[inline(always)]
pub fn trix_batch_slice(data: &[f64], sweep: &TrixBatchRange, kern: Kernel) -> Result<TrixBatchOutput, TrixError> {
	trix_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn trix_batch_par_slice(data: &[f64], sweep: &TrixBatchRange, kern: Kernel) -> Result<TrixBatchOutput, TrixError> {
...
impl TrixStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		...
		let stream = TrixStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
mod tests {
	...
	fn check_trix_accuracy(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let expected_last_five = [-16.03736447, -15.92084231, -15.76171478, -15.53571033, -15.34967155];
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = TrixBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
