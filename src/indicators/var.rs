...
#[inline(always)]
pub unsafe fn var_row_scalar(data: &[f64], first: usize, period: usize, _stride: usize, nbdev: f64, out: &mut [f64]) {
	let len = data.len();
	let nbdev2 = nbdev * nbdev;
	let inv_p = 1.0 / (period as f64);
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn var_row_avx2(data: &[f64], first: usize, period: usize, stride: usize, nbdev: f64, out: &mut [f64]) {
	var_row_scalar(data, first, period, stride, nbdev, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn var_row_avx512(data: &[f64], first: usize, period: usize, stride: usize, nbdev: f64, out: &mut [f64]) {
	if period <= 32 {
...
impl VarBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<VarBatchOutput, VarError> {
		VarBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
#[inline(always)]
pub fn var_batch_slice(data: &[f64], sweep: &VarBatchRange, kern: Kernel) -> Result<VarBatchOutput, VarError> {
	var_batch_inner(data, sweep, kern, false)
}
#[inline(always)]
pub fn var_batch_par_slice(data: &[f64], sweep: &VarBatchRange, kern: Kernel) -> Result<VarBatchOutput, VarError> {
...
pub fn var_batch_with_kernel(data: &[f64], sweep: &VarBatchRange, k: Kernel) -> Result<VarBatchOutput, VarError> {
	let kernel = match k {
