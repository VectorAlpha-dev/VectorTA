...
impl<'a> PmaInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: PmaParams) -> Self {
		Self {
			data: PmaData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
...
pub fn pma_scalar(data: &[f64], first_valid_idx: usize) -> Result<PmaOutput, PmaError> {
	...
	for j in (first_valid_idx + 6)..data.len() {
		...
		let trigger_j = ((4.0 * predict_j) + (3.0 * predict[j - 1]) + (2.0 * predict[j - 2]) + predict[j - 3]) / 10.0;
		trigger[j] = trigger_j;
	}
...
#[inline]
pub fn pma_batch_with_kernel(data: &[f64], sweep: &PmaBatchRange, k: Kernel) -> Result<PmaBatchOutput, PmaError> {
	let kernel = match k {
...
impl PmaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<PmaBatchOutput, PmaError> {
		PmaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
#[inline(always)]
pub fn pma_batch_par_slice(data: &[f64], sweep: &PmaBatchRange, kern: Kernel) -> Result<PmaBatchOutput, PmaError> {
...
mod tests {
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let output = PmaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
