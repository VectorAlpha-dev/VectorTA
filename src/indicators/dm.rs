...
impl<'a> DmInput<'a> {
    ...
    pub fn from_candles(candles: &'a Candles, params: DmParams) -> Self {
        Self { data: DmData::Candles { candles }, params }
    }
    #[inline]
    pub fn from_slices(high: &'a [f64], low: &'a [f64], params: DmParams) -> Self {
        Self { data: DmData::Slices { high, low }, params }
    }
    #[inline]
    pub fn with_default_candles(candles: &'a Candles) -> Self {
        Self { data: DmData::Candles { candles }, params: DmParams::default() }
    }
    #[inline]
    pub fn get_period(&self) -> usize {
        self.params.period.unwrap_or_else(|| DmParams::default().period.unwrap())
    }
}
...
pub fn dm_with_kernel(input: &DmInput, kernel: Kernel) -> Result<DmOutput, DmError> {
    ...
    let first_valid_idx = high.iter().zip(low.iter()).position(|(&h, &l)| !h.is_nan() && !l.is_nan())
    ...
    unsafe {
        match chosen {
            ...
            Kernel::Avx2 | Kernel::Avx2Batch => {
                dm_avx2(high, low, period, first_valid_idx)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
...
impl DmBatchOutput {
    ...
    pub fn values_for(&self, p: &DmParams) -> Option<(&[f64], &[f64])> {
        self.row_for_params(p).map(|row| {
            ...
            (&self.plus[start..start + self.cols], &self.minus[start..start + self.cols])
        })
    }
}
...
fn dm_batch_inner(
    ...
    let first = high.iter().zip(low.iter()).position(|(&h, &l)| !h.is_nan() && !l.is_nan())
        .ok_or(DmError::AllValuesNaN)?;
...
#[inline(always)]
fn expand_grid_dm(_r: &DmBatchRange) -> Vec<DmParams> { expand_grid(_r) }
