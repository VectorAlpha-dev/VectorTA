...
impl<'a> CciInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: CciParams) -> Self {
		Self {
			data: CciData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
...
impl CciBuilder {
	...
	pub fn apply(self, c: &Candles) -> Result<CciOutput, CciError> {
		let p = CciParams { period: self.period };
...
impl CciBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<CciBatchOutput, CciError> {
		CciBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "hlc3")
	}
}
...
pub fn cci_batch_with_kernel(data: &[f64], sweep: &CciBatchRange, k: Kernel) -> Result<CciBatchOutput, CciError> {
	let kernel = match k {
		Kernel::Auto => detect_best_batch_kernel(),
		other if other.is_batch() => other,
		_ => return Err(CciError::InvalidPeriod { period: 0, data_len: 0 }),
	};
...
#[inline(always)]
pub fn cci_batch_slice(data: &[f64], sweep: &CciBatchRange, kern: Kernel) -> Result<CciBatchOutput, CciError> {
	cci_batch_inner(data, sweep, kern, false)
}
#[inline(always)]
pub fn cci_batch_par_slice(data: &[f64], sweep: &CciBatchRange, kern: Kernel) -> Result<CciBatchOutput, CciError> {
...
fn cci_batch_inner_into(
	...
	let combos = expand_grid(sweep);
	if combos.is_empty() {
		return Err(CciError::InvalidPeriod { period: 0, data_len: 0 });
	}
...
mod tests {
	...
	fn check_cci_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		assert!(res.is_err(), "[{}] CCI should fail with zero period", test_name);
		Ok(())
	}

	fn check_cci_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_cci_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let res = cci_with_kernel(&input, kernel);
		assert!(res.is_err(), "[{}] CCI should fail with insufficient data", test_name);
		Ok(())
	}
	...
	fn check_cci_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let period = 14;
		let input = CciInput::from_candles(&candles, "close", CciParams { period: Some(period) });
		let batch_output = cci_with_kernel(&input, kernel)?.values;

		let mut stream = CciStream::try_new(CciParams { period: Some(period) })?;
	...
	fn check_cci_empty_input(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let res = cci_with_kernel(&input, kernel);
		assert!(res.is_err(), "[{}] CCI should fail with empty input data", test_name);
		if let Err(e) = res {
			match e {
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = CciBatchBuilder::new().kernel(kernel).apply_candles(&c, "hlc3")?;
...
impl CciStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = CciParams { period: Some(period) };
		let stream = CciStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn cci_batch_metadata_js(period_start: usize, period_end: usize, period_step: usize) -> Result<Vec<f64>, JsValue> {
	let sweep = CciBatchRange {
		period: (period_start, period_end, period_step),
	};
