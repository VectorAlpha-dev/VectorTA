...
pub unsafe fn srsi_scalar(
    ...
    let rsi_input = RsiInput::from_slice(data, RsiParams { period: Some(rsi_period) });
...
fn srsi_batch_inner(
    ...
    let max_period = combos
        .iter()
        .map(|c| c.rsi_period.unwrap().max(c.stoch_period.unwrap()).max(c.k.unwrap()).max(c.d.unwrap()))
        .max()
        .unwrap();
...
mod tests {
    ...
    fn check_srsi_accuracy(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for i in 0..5 {
            ...
            assert!(diff_k < 1e-6, "Mismatch in SRSI K at index {}: got {}, expected {}", i, k_slice[i], last_five_k[i]);
            assert!(diff_d < 1e-6, "Mismatch in SRSI D at index {}: got {}, expected {}", i, d_slice[i], last_five_d[i]);
        }
        Ok(())
    }
