...
pub fn kaufmanstop_with_kernel(
    ...
    let mut hl_diff = AVec::<f64>::with_capacity(CACHELINE_ALIGN, high.len());
    hl_diff.resize(high.len(), f64::NAN);
    ...
    let hl_diff_ma =
        ma(ma_type, ma_input, period).map_err(|_| KaufmanstopError::AllValuesNaN)?;
    ...
    pub fn row_for_params(&self, p: &KaufmanstopParams) -> Option<usize> {
        self.combos.iter().position(|c| {
            c.period.unwrap_or(22) == p.period.unwrap_or(22)
                ...
                && c.direction.as_ref().unwrap_or(&"long".to_string()) == p.direction.as_ref().unwrap_or(&"long".to_string())
...
fn expand_grid(r: &KaufmanstopBatchRange) -> Vec<KaufmanstopParams> {
    ...
    fn axis_string((start, end, step): (String, String, f64)) -> Vec<String> {
        if start == end {
...
fn kaufmanstop_batch_inner(
    ...
    let rows = combos.len();
    let cols = high.len();
    let mut range_buf = AVec::<f64>::with_capacity(CACHELINE_ALIGN, high.len());
    range_buf.resize(high.len(), f64::NAN);
    ...
    let do_row = |row: usize, out_row: &mut [f64]| unsafe {
        ...
        match kern {
            Kernel::Scalar => kaufmanstop_row_scalar(
                high,
                low,
                &hl_diff_ma,
                period,
            ...
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 => kaufmanstop_row_avx2(
                high,
                low,
                &hl_diff_ma,
                period,
            ...
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 => kaufmanstop_row_avx512(
                high,
                low,
                &hl_diff_ma,
                period,
