...
pub fn ht_trendmode(input: &HtTrendModeInput) -> Result<HtTrendModeOutput, HtTrendModeError> {
	...
	fn do_hilbert_even(vars: &mut [f64; 3], input: f64, idx: &mut usize) -> f64 {
		let temp = 0.0962 * input + 0.5769 * vars[(*idx + 2) % 3] - 0.5769 * vars[(*idx + 1) % 3] - 0.0962 * vars[*idx];
		vars[*idx] = temp;
		*idx = (*idx + 2) % 3;
		temp
	}
...
mod tests {
	...
	fn test_ht_trendmode_partial_params() {
		...
		let output_default = ht_trendmode(&input_default).expect("Failed HT_TRENDMODE with defaults");
	...
	fn test_ht_trendmode_accuracy() {
		...
		assert_eq!(result.values.len(), close_prices.len(), "HT_TRENDMODE length mismatch");
	...
	fn test_ht_trendmode_slice_data_reinput() {
		...
		let second_input = HtTrendModeInput::from_slice(&first_result.values, HtTrendModeParams::default());
