...
impl<'a> AoInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: AoParams) -> Self {
		Self {
			data: AoData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub unsafe fn ao_avx512_short(data: &[f64], short: usize, long: usize, first: usize, out: &mut [f64]) {
	ao_scalar(data, short, long, first, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub unsafe fn ao_avx512_long(data: &[f64], short: usize, long: usize, first: usize, out: &mut [f64]) {
	ao_scalar(data, short, long, first, out)
}
...
impl AoBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<AoBatchOutput, AoError> {
		AoBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "hl2")
	}
}
...
#[inline(always)]
pub fn ao_batch_par_slice(data: &[f64], sweep: &AoBatchRange, kern: Kernel) -> Result<AoBatchOutput, AoError> {
...
#[inline(always)]
fn ao_batch_inner(data: &[f64], sweep: &AoBatchRange, kern: Kernel, parallel: bool) -> Result<AoBatchOutput, AoError> {
	let combos = expand_grid(sweep);
	let rows = combos.len();
	let cols = data.len();
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn ao_row_avx512(data: &[f64], first: usize, short: usize, long: usize, out: &mut [f64]) {
	if long <= 32 {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn ao_row_avx512_short(data: &[f64], first: usize, short: usize, long: usize, out: &mut [f64]) {
	ao_scalar(data, short, long, first, out)
}
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn ao_row_avx512_long(data: &[f64], first: usize, short: usize, long: usize, out: &mut [f64]) {
	ao_scalar(data, short, long, first, out)
}
...
mod tests {
	...
	fn check_ao_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = AoBatchBuilder::new().kernel(kernel).apply_candles(&c, "hl2")?;
...
pub fn ao_batch_unified_js(high: &[f64], low: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let output = ao_batch_inner(&hl2, &sweep, Kernel::Scalar, false).map_err(|e| JsValue::from_str(&e.to_string()))?;
