...
impl<'a> RviInput<'a> {
    ...
    pub fn from_candles(c: &'a Candles, s: &'a str, p: RviParams) -> Self {
        Self { data: RviData::Candles { candles: c, source: s }, params: p }
    }
    #[inline]
    pub fn from_slice(sl: &'a [f64], p: RviParams) -> Self {
        Self { data: RviData::Slice(sl), params: p }
    }
    #[inline]
    ...
    #[inline]
    pub fn get_period(&self) -> usize { self.params.period.unwrap_or(10) }
    #[inline]
    pub fn get_ma_len(&self) -> usize { self.params.ma_len.unwrap_or(14) }
    #[inline]
    pub fn get_matype(&self) -> usize { self.params.matype.unwrap_or(1) }
    #[inline]
...
impl RviBuilder {
    ...
    #[inline(always)]
    pub fn ma_len(mut self, n: usize) -> Self { self.ma_len = Some(n); self }
    #[inline(always)]
    pub fn matype(mut self, n: usize) -> Self { self.matype = Some(n); self }
    #[inline(always)]
    pub fn devtype(mut self, n: usize) -> Self { self.devtype = Some(n); self }
    #[inline(always)]
...
pub enum RviError {
    ...
    InvalidPeriod { period: usize, ma_len: usize, data_len: usize },
...
pub fn rvi_with_kernel(input: &RviInput, kernel: Kernel) -> Result<RviOutput, RviError> {
    ...
    if data.is_empty() { return Err(RviError::EmptyData); }
    ...
    if period == 0 || ma_len == 0 || period > data.len() || ma_len > data.len() {
        return Err(RviError::InvalidPeriod { period, ma_len, data_len: data.len() });
    }
    ...
    if (data.len() - first) <= max_needed {
        return Err(RviError::NotEnoughValidData { needed: max_needed + 1, valid: data.len() - first });
    }
...
impl RviBatchBuilder {
    ...
    #[inline]
    pub fn ma_len_range(mut self, start: usize, end: usize, step: usize) -> Self { self.range.ma_len = (start, end, step); self }
    #[inline]
    pub fn ma_len_static(mut self, p: usize) -> Self { self.range.ma_len = (p, p, 0); self }
    #[inline]
    pub fn matype_range(mut self, start: usize, end: usize, step: usize) -> Self { self.range.matype = (start, end, step); self }
    #[inline]
    pub fn matype_static(mut self, p: usize) -> Self { self.range.matype = (p, p, 0); self }
    #[inline]
    pub fn devtype_range(mut self, start: usize, end: usize, step: usize) -> Self { self.range.devtype = (start, end, step); self }
    #[inline]
    pub fn devtype_static(mut self, p: usize) -> Self { self.range.devtype = (p, p, 0); self }
...
pub fn rvi_batch_with_kernel(
    ...
    let kernel = match k {
        ...
        _ => return Err(RviError::InvalidPeriod { period: 0, ma_len: 0, data_len: 0 }),
    };
...
fn rvi_batch_inner(
    ...
    if combos.is_empty() {
        return Err(RviError::InvalidPeriod { period: 0, ma_len: 0, data_len: 0 });
    }
    let first = data.iter().position(|x| !x.is_nan()).ok_or(RviError::AllValuesNaN)?;
    ...
    Ok(RviBatchOutput { values, combos, rows, cols })
}

#[inline(always)]
unsafe fn rvi_row_scalar(
    data: &[f64],
    first: usize,
    params: &RviParams,
    out: &mut [f64],
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
unsafe fn rvi_row_avx512_short(
    data: &[f64],
    first: usize,
    params: &RviParams,
    out: &mut [f64],
...
fn rolling_std_dev(data: &[f64], period: usize) -> Vec<f64> {
    let mut out = vec![f64::NAN; data.len()];
    if period == 0 || period > data.len() { return out; }
    ...
    for i in 0..data.len() {
        ...
        if x.is_nan() {
            ...
            if window.len() > period {
                if let Some(old) = window.pop_front() { current_sum -= old; }
            }
            if window.len() == period {
    ...
    for i in 0..data.len() {
        ...
        if x.is_nan() {
            ...
            if window.len() == period {
                ...
                let mut abs_sum = 0.0;
                for &val in &tmp { abs_sum += (val - median).abs(); }
                out[i] = abs_sum / (period as f64);
            }
        }
    }
...
fn compute_rolling_ma(data: &[f64], period: usize, matype: usize) -> Vec<f64> {
    match matype { 0 => rolling_sma(data, period), _ => rolling_ema(data, period) }
}
...
mod tests {
    ...
    fn check_rvi_partial_params(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let partial_params = RviParams { period: Some(10), ma_len: None, matype: None, devtype: None };
    ...
    fn check_rvi_error_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let params = RviParams { period: Some(0), ma_len: Some(14), matype: Some(1), devtype: Some(0) };
    ...
    fn check_rvi_error_zero_ma_len(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        skip_if_unsupported!(kernel, test_name);
        let data = [10.0, 20.0, 30.0, 40.0];
        let params = RviParams { period: Some(10), ma_len: Some(0), matype: Some(1), devtype: Some(0) };
        let input = RviInput::from_slice(&data, params);
        let result = rvi_with_kernel(&input, kernel);
        assert!(result.is_err());
        Ok(())
    ...
    fn check_rvi_error_period_exceeds_data_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_rvi_not_enough_valid_data(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let params = RviParams { period: Some(3), ma_len: Some(5), matype: Some(1), devtype: Some(0) };
        let input = RviInput::from_slice(&data, params);
        let result = rvi_with_kernel(&input, kernel);
        assert!(result.is_err());
        Ok(())
    ...
    fn check_rvi_example_values(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (i, &val) in last_five.iter().enumerate() {
            ...
            assert!(diff < 1e-1, "Mismatch at index {} -> got: {}, expected: {}, diff: {}", i, val, exp, diff);
        }
        Ok(())
    }
