...
/// In-place native API: writes outputs into caller-provided buffers without allocating.
///
/// Preserves NaN warmups exactly like `dvdiqqe()` and `dvdiqqe_with_kernel()`; all output
/// slices must have length equal to the input length.
...
pub fn dvdiqqe_into(
    ...
    // Use Kernel::Auto to match the Vec-returning API's selection behavior.
    dvdiqqe_into_slices(dvdi_dst, fast_tl_dst, slow_tl_dst, center_dst, input, Kernel::Auto)
}
