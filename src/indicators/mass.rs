    ...
    unsafe {
        match chosen {
            Kernel::Scalar | Kernel::ScalarBatch => {
                mass_scalar(high, low, period, first_valid_idx, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 | Kernel::Avx2Batch => {
                mass_avx2(high, low, period, first_valid_idx, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                mass_avx512(high, low, period, first_valid_idx, &mut out)
            }
            _ => unreachable!(),
        }
    }
...
fn mass_batch_inner(
    ...
    let first = (0..high.len()).find(|&i| !high[i].is_nan() && !low[i].is_nan()).ok_or(MassError::AllValuesNaN)?;
