...
pub fn gatorosc_with_kernel(
    ...
    let first = data.iter().position(|x| !x.is_nan()).ok_or(GatorOscError::AllValuesNaN)?;
    ...
    if jaws_length == 0 || jaws_shift == 0 ||
       teeth_length == 0 || teeth_shift == 0 ||
       lips_length == 0 || lips_shift == 0 {
    ...
    unsafe {
        match chosen {
            Kernel::Scalar | Kernel::ScalarBatch => {
                gatorosc_scalar(data, jaws_length, jaws_shift, teeth_length, teeth_shift, lips_length, lips_shift, first, &mut upper, &mut lower, &mut upper_change, &mut lower_change)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 | Kernel::Avx2Batch => {
                gatorosc_avx2(data, jaws_length, jaws_shift, teeth_length, teeth_shift, lips_length, lips_shift, first, &mut upper, &mut lower, &mut upper_change, &mut lower_change)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                gatorosc_avx512(data, jaws_length, jaws_shift, teeth_length, teeth_shift, lips_length, lips_shift, first, &mut upper, &mut lower, &mut upper_change, &mut lower_change)
            }
            _ => unreachable!(),
        }
    }
...
impl GatorOscStream {
    ...
    pub fn update(&mut self, value: f64) -> Option<(f64, f64, f64, f64)> {
        ...
        let upper_change = if !prev_upper.is_nan() { upper - prev_upper } else { f64::NAN };
        let lower_change = if !prev_lower.is_nan() { -(lower - prev_lower) } else { f64::NAN };
...
impl GatorOscBatchBuilder {
    ...
    pub fn jaws_length_range(mut self, start: usize, end: usize, step: usize) -> Self { self.range.jaws_length = (start, end, step); self }
    pub fn jaws_shift_range(mut self, start: usize, end: usize, step: usize) -> Self { self.range.jaws_shift = (start, end, step); self }
    pub fn teeth_length_range(mut self, start: usize, end: usize, step: usize) -> Self { self.range.teeth_length = (start, end, step); self }
    pub fn teeth_shift_range(mut self, start: usize, end: usize, step: usize) -> Self { self.range.teeth_shift = (start, end, step); self }
    pub fn lips_length_range(mut self, start: usize, end: usize, step: usize) -> Self { self.range.lips_length = (start, end, step); self }
    pub fn lips_shift_range(mut self, start: usize, end: usize, step: usize) -> Self { self.range.lips_shift = (start, end, step); self }
...
fn expand_grid_gatorosc(r: &GatorOscBatchRange) -> Vec<GatorOscParams> {
    ...
    let mut out = Vec::with_capacity(
        jaws_lengths.len() * jaws_shifts.len() * teeth_lengths.len() *
        teeth_shifts.len() * lips_lengths.len() * lips_shifts.len());
...
fn gatorosc_batch_inner(
    ...
    let needed = max_jl.max(max_tl).max(max_ll)
...
mod tests {
    ...
    fn check_gatorosc_zero_setting(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let params = GatorOscParams { jaws_length: Some(0), ..Default::default() };
    ...
    fn check_gatorosc_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let res = gatorosc_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] GatorOsc should fail with insufficient data", test_name);
        Ok(())
    }
    ...
    fn check_gatorosc_batch_default_row(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let row = output.combos.iter().position(|p| {
    ...
    fn check_batch_multi_param_sweep(test: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let some_upper = output.upper.chunks(output.cols).any(|row| row.iter().any(|&x| !x.is_nan()));
        assert!(some_upper);
