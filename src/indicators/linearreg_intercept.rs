...
#[inline]
pub fn linearreg_intercept(input: &LinearRegInterceptInput) -> Result<LinearRegInterceptOutput, LinearRegInterceptError> {
    linearreg_intercept_with_kernel(input, Kernel::Auto)
}

pub fn linearreg_intercept_with_kernel(input: &LinearRegInterceptInput, kernel: Kernel) -> Result<LinearRegInterceptOutput, LinearRegInterceptError> {
    let data: &[f64] = input.as_ref();

    let first = data.iter().position(|x| !x.is_nan()).ok_or(LinearRegInterceptError::AllValuesNaN)?;
    let len = data.len();
    ...
    if (len - first) < period {
        return Err(LinearRegInterceptError::NotEnoughValidData { needed: period, valid: len - first });
    }
...
#[inline]
pub fn linearreg_intercept_scalar(
    data: &[f64],
    period: usize,
    first_val: usize,
    out: &mut [f64],
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn linearreg_intercept_avx512(
    data: &[f64],
    period: usize,
    first_val: usize,
    out: &mut [f64],
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn linearreg_intercept_avx2(
    data: &[f64],
    period: usize,
    first_val: usize,
    out: &mut [f64],
...
impl LinearRegInterceptBatchBuilder {
    ...
    pub fn with_default_slice(data: &[f64], k: Kernel) -> Result<LinearRegInterceptBatchOutput, LinearRegInterceptError> {
        LinearRegInterceptBatchBuilder::new().kernel(k).apply_slice(data)
    }
    pub fn apply_candles(self, c: &Candles, src: &str) -> Result<LinearRegInterceptBatchOutput, LinearRegInterceptError> {
        let slice = source_type(c, src);
        self.apply_slice(slice)
    }
    pub fn with_default_candles(c: &Candles) -> Result<LinearRegInterceptBatchOutput, LinearRegInterceptError> {
        LinearRegInterceptBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
    }
}
...
fn expand_grid(r: &LinearRegInterceptBatchRange) -> Vec<LinearRegInterceptParams> {
    ...
    for &p in &periods {
        out.push(LinearRegInterceptParams {
            period: Some(p),
        });
    }
    out
...
fn linearreg_intercept_batch_inner(
    ...
    if data.len() - first < max_p {
        return Err(LinearRegInterceptError::NotEnoughValidData { needed: max_p, valid: data.len() - first });
    }
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
unsafe fn linearreg_intercept_row_avx2(
    data: &[f64],
    first: usize,
    period: usize,
    out: &mut [f64],
...
mod tests {
    ...
    fn check_linreg_nan_handling(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
        let input = LinearRegInterceptInput::from_candles(&candles, "close", LinearRegInterceptParams { period: Some(14) });
        let res = linearreg_intercept_with_kernel(&input, kernel)?;
