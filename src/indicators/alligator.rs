...
fn alligator_batch_inner(
	...
	let jaw: &mut [f64] =
		unsafe { core::slice::from_raw_parts_mut(jaw_guard.as_mut_ptr() as *mut f64, jaw_guard.len()) };
	let teeth: &mut [f64] =
		unsafe { core::slice::from_raw_parts_mut(teeth_guard.as_mut_ptr() as *mut f64, teeth_guard.len()) };
	let lips: &mut [f64] =
...
impl AlligatorStreamPy {
	...
	fn new(
		...
		let stream = AlligatorStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
