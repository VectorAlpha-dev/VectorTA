...
mod tests {
	...
	fn test_ht_dcphase_partial_params() {
		...
		let input_custom = HtDcPhaseInput::from_candles(&candles, "hlc3", HtDcPhaseParams::default());
		let output_custom = ht_dcphase(&input_custom).expect("Failed ht_dcphase custom source=hlc3");
	...
	fn test_ht_dcphase_accuracy() {
		...
		assert_eq!(result.values.len(), close_prices.len(), "ht_dcphase length mismatch");
	...
	fn test_ht_dcphase_reinput() {
		...
		let first_input = HtDcPhaseInput::from_candles(&candles, "close", HtDcPhaseParams::default());
		let first_result = ht_dcphase(&first_input).expect("Failed to calculate ht_dcphase first");

		let second_input = HtDcPhaseInput::from_slice(&first_result.values, HtDcPhaseParams::default());
		let second_result = ht_dcphase(&second_input).expect("Failed to calculate ht_dcphase second");
