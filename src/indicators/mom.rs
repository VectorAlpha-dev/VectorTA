...
pub fn mom_with_kernel(input: &MomInput, kernel: Kernel) -> Result<MomOutput, MomError> {
    ...
    if (len - first) < period {
        return Err(MomError::NotEnoughValidData { needed: period, valid: len - first });
    }
    ...
    unsafe {
        match chosen {
            Kernel::Scalar | Kernel::ScalarBatch => {
                mom_scalar(data, period, first, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 | Kernel::Avx2Batch => {
                mom_avx2(data, period, first, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                mom_avx512(data, period, first, &mut out)
            }
            _ => unreachable!(),
        }
    }
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
unsafe fn mom_row_avx512_short(
    data: &[f64],
    first: usize,
    period: usize,
    out: &mut [f64],
