...
unsafe fn fvg_ts_avx2(
    ...
    fvg_ts_scalar(high, low, close, lookback, smoothing_len, reset_on_cross, upper, lower, upper_ts, lower_ts);
}
...
fn fvg_ts_prepare<'a>(
    ...
    if smoothing_len == 0 {
        return Err(FvgTrailingStopError::InvalidSmoothingLength { smoothing: smoothing_len });
    }
    
    // need at least 2 (FVG) + (smoothing_len - 1)
    let need = 2 + smoothing_len.saturating_sub(1);
    if len - first < need {
        return Err(FvgTrailingStopError::NotEnoughValidData { needed: need, valid: len - first });
    }
    ...
    unsafe {
        #[cfg(all(target_arch = "wasm32", target_feature = "simd128"))]
        {
            if matches!(kernel, Kernel::Scalar | Kernel::ScalarBatch) {
                fvg_ts_simd128(high, low, close, lookback, smoothing_len, reset_on_cross, upper, lower, upper_ts, lower_ts);
                return;
            }
        }
        ...
        match kernel {
            ...
            Kernel::Avx2 | Kernel::Avx2Batch => {
                fvg_ts_avx2(high, low, close, lookback, smoothing_len, reset_on_cross, upper, lower, upper_ts, lower_ts)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                fvg_ts_avx512(high, low, close, lookback, smoothing_len, reset_on_cross, upper, lower, upper_ts, lower_ts)
            }
...
pub fn fvg_trailing_stop_with_kernel(
    ...
    fvg_ts_compute_into(h, l, c, lookback, smoothing_len, reset_on_cross,
                        &mut upper, &mut lower, &mut upper_ts, &mut lower_ts, chosen);
...
pub fn fvg_trailing_stop_into_slices(
    upper: &mut [f64], lower: &mut [f64], upper_ts: &mut [f64], lower_ts: &mut [f64],
    input: &FvgTrailingStopInput, kernel: Kernel,
) -> Result<(), FvgTrailingStopError> {
    let (h, l, c, lookback, smoothing_len, reset_on_cross, first) = fvg_ts_prepare(input)?;
    let len = h.len();
    if [upper.len(), lower.len(), upper_ts.len(), lower_ts.len()].iter().any(|&n| n != len) {
        return Err(FvgTrailingStopError::InvalidPeriod { period: len, data_len: len });
    }
    let chosen = match kernel {
        Kernel::Auto => detect_best_kernel(),
    ...
    fvg_ts_compute_into(h, l, c, lookback, smoothing_len, reset_on_cross,
                        upper, lower, upper_ts, lower_ts, chosen);
...
impl FvgTsBatchOutput {
    ...
    pub fn values_for(&self, p: &FvgTrailingStopParams) -> Option<(&[f64], &[f64], &[f64], &[f64])> {
...
fn expand_grid_ts(r: &FvgTsBatchRange) -> Vec<FvgTrailingStopParams> {
    ...
    if resets.is_empty() { resets.push(false); } // Default to false if neither is selected
...
pub fn fvg_ts_batch_inner_into(
    h: &[f64], l: &[f64], c: &[f64],
    sweep: &FvgTsBatchRange, kern: Kernel, parallel: bool,
    ...
    if combos.is_empty() { return Err(FvgTrailingStopError::InvalidPeriod { period: 0, data_len: 0 }); }
    let len = h.len();
    let rows = combos.len();
    let cols = len;
    ...
    let first = first_valid_ohlc(h, l, c);
    if first == usize::MAX { return Err(FvgTrailingStopError::AllValuesNaN); }
    ...
    let do_one = |row: usize, dst: &mut [f64]| {
        ...
        fvg_ts_compute_into(
            ...
            u_block, l_block, uts_block, lts_block,
            chosen
        );
        for buf in [u_block, l_block, uts_block, lts_block] {
            for v in &mut buf[..warm] { *v = f64::NAN; }
        }
    };
    ...
    if parallel {
        use rayon::prelude::*;
        out.par_chunks_mut(4 * cols).enumerate().for_each(|(row, dst)| do_one(row, dst));
    } else {
        out.chunks_mut(4 * cols).enumerate().for_each(|(row, dst)| do_one(row, dst));
    }
...
pub fn fvg_trailing_stop_batch_with_kernel(
    high: &[f64], low: &[f64], close: &[f64], sweep: &FvgTsBatchRange, kernel: Kernel
    ...
    Ok(FvgTsBatchOutput { values, combos: used, rows, cols })
}
...
impl FvgTsBatchBuilder {
    ...
    pub fn apply_slices(self, high: &[f64], low: &[f64], close: &[f64]) -> Result<FvgTsBatchOutput, FvgTrailingStopError> {
    ...
    pub fn with_default_candles(c: &Candles) -> Result<FvgTsBatchOutput, FvgTrailingStopError> {
        FvgTsBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c)
    }

    pub fn with_default_slices(high: &[f64], low: &[f64], close: &[f64]) -> Result<FvgTsBatchOutput, FvgTrailingStopError> {
        FvgTsBatchBuilder::new().kernel(Kernel::Auto).apply_slices(high, low, close)
    }
}
...
impl FvgTrailingStopStream {
    ...
    pub fn update(&mut self, high: f64, low: f64, close: f64) -> Option<(f64, f64, f64, f64)> {
        ...
        if self.bar_count >= 2 && !self.prev_high2.is_nan() && !self.prev_low2.is_nan() && !self.prev_close.is_nan() {
        ...
        let (mut upper, mut lower, mut upper_ts, mut lower_ts) = (f64::NAN, f64::NAN, f64::NAN, f64::NAN);
...
pub fn fvg_trailing_stop_batch_py<'py>(
    ...
    dict.set_item("lookbacks",
        combos.iter().map(|p| p.unmitigated_fvg_lookback.unwrap() as u64).collect::<Vec<_>>().into_pyarray(py))?;
    dict.set_item("smoothings",
        combos.iter().map(|p| p.smoothing_length.unwrap() as u64).collect::<Vec<_>>().into_pyarray(py))?;
    dict.set_item("resets",
        combos.iter().map(|p| p.reset_on_cross.unwrap_or(false)).collect::<Vec<_>>().into_pyarray(py))?;
    Ok(dict)
}
...
pub fn fvg_trailing_stop_js(
    ...
    if high.is_empty() || low.is_empty() || close.is_empty() {
        return Err(JsValue::from_str("fvg_trailing_stop: Input data slice is empty."));
    }
    ...
    let _ = unsafe { Vec::from_raw_parts(guard.as_mut_ptr() as *mut f64, guard.len(), guard.capacity()) };
    core::mem::forget(guard);
...
pub fn fvg_trailing_stop_batch_js(
    high:&[f64], low:&[f64], close:&[f64],
    lookback_start: usize, lookback_end: usize, lookback_step: usize,
    smoothing_start: usize, smoothing_end: usize, smoothing_step: usize,
    reset_include_false: bool, reset_include_true: bool,
) -> Result<JsValue, JsValue> {
    ...
    for prm in &combos {
        let sm = prm.smoothing_length.unwrap_or(9);
        let w = if first == usize::MAX { 0 } else { (first + 2 + sm.saturating_sub(1)).min(cols) };
        warms.extend_from_slice(&[w, w, w, w]);
    }
    init_matrix_prefixes(&mut buf_mu, cols, &warms);
    ...
    fvg_ts_batch_inner_into(high, low, close, &sweep, detect_best_batch_kernel(), false, flat)
        .map_err(|e| JsValue::from_str(&e.to_string()))?;

    let values = unsafe { Vec::from_raw_parts(guard.as_mut_ptr() as *mut f64, guard.len(), guard.capacity()) };
    core::mem::forget(guard);

    let out = FvgTsBatchJsOutput { values, combos, rows, cols };
...
pub fn fvg_trailing_stop_zero_copy_js(
    ...
    fvg_trailing_stop_into_slices(upper, lower, upper_ts, lower_ts, &input, detect_best_kernel())
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
...
impl FvgTrailingStopBuilder {
    ...
    pub fn apply_slice(&self, high: &[f64], low: &[f64], close: &[f64]) -> Result<FvgTrailingStopOutput, FvgTrailingStopError> {
        let params = FvgTrailingStopParams {
            unmitigated_fvg_lookback: self.unmitigated_fvg_lookback,
            smoothing_length: self.smoothing_length,
            reset_on_cross: self.reset_on_cross,
...
mod tests {
            ...
            for i in (n-5)..n {
                if !result.lower[i].is_nan() {
                    ...
                    assert!(
                        ...
                        test_name, i, expected_lower, result.lower[i], diff
                    );
                }
                if !result.lower_ts[i].is_nan() {
                    ...
                    assert!(
                        ...
                        test_name, i, expected_lower_ts, result.lower_ts[i], diff
                    );
                }
            }
        }
        ...
        #[cfg(debug_assertions)]
        {
            ...
            for (name, row) in [
                ...
                for (i, &v) in row.iter().enumerate() {
                    ...
                    let b = v.to_bits();
                    assert_ne!(b, 0x1111_1111_1111_1111, "[{}] alloc poison in {} at {}", test_name, name, i);
                    assert_ne!(b, 0x2222_2222_2222_2222, "[{}] matrix poison in {} at {}", test_name, name, i);
                    assert_ne!(b, 0x3333_3333_3333_3333, "[{}] uninit poison in {} at {}", test_name, name, i);
                }
            }
        }
        Ok(())
    ...
    fn check_fvg_ts_batch_default(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let output = FvgTsBatchBuilder::new().kernel(kernel).apply_candles(&candles)?;
    ...
    fn check_fvg_ts_builder_apply_slice(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_fvg_ts_into_slices_warm_nan(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let h = vec![100.0,101.0,102.0,103.0,104.0,105.0,106.0,107.0,108.0,109.0];
        let l = vec![ 99.0, 99.5,100.0,101.0,102.0,103.0,104.0,105.0,106.0,107.0];
        let c = vec![ 99.5,100.5,101.5,102.5,103.5,104.5,105.5,106.5,107.5,108.5];
        let params = FvgTrailingStopParams::default();
        let input = FvgTrailingStopInput::from_slices(&h,&l,&c,params);
        ...
        for v in [&u,&d,&uts,&lts] {
            for i in 0..expected_warm.min(h.len()) {
                assert!(v[i].is_nan(), "[{}] Expected NaN at index {} but got {}", test_name, i, v[i]);
            }
        }
        Ok(())
    }

    fn check_fvg_ts_invalid_smoothing(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let h = vec![1.0; 20];
        let l = vec![0.0; 20];
        let c = vec![0.5; 20];
        ...
        let res = fvg_trailing_stop_with_kernel(&input, kernel);
        assert!(
            matches!(res, Err(FvgTrailingStopError::InvalidSmoothingLength{..})),
    ...
    fn check_fvg_ts_invalid_lookback(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let h = vec![1.0; 20];
        let l = vec![0.0; 20];
        let c = vec![0.5; 20];
    ...
    fn check_fvg_ts_batch_values_for(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let h = vec![100.0; 64];
        let l = vec![90.0; 64];
        let c = vec![95.0; 64];
