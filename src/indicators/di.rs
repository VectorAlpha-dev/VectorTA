...
impl<'a> DiInput<'a> {
    ...
    pub fn from_candles(candles: &'a Candles, params: DiParams) -> Self {
        Self { data: DiData::Candles { candles }, params }
    }
    #[inline(always)]
    pub fn from_slices(high: &'a [f64], low: &'a [f64], close: &'a [f64], params: DiParams) -> Self {
        Self { data: DiData::Slices { high, low, close }, params }
    }
    #[inline(always)]
    pub fn with_default_candles(candles: &'a Candles) -> Self {
        Self { data: DiData::Candles { candles }, params: DiParams::default() }
    }
    #[inline(always)]
...
pub fn di_with_kernel(input: &DiInput, kernel: Kernel) -> Result<DiOutput, DiError> {
    ...
    let n = high.len();
    if n == 0 || low.len() != n || close.len() != n { return Err(DiError::EmptyData); }
    let period = input.get_period();
    if period == 0 || period > n { return Err(DiError::InvalidPeriod { period, data_len: n }); }
    let first_valid_idx = (0..n).find(|&i| !(high[i].is_nan() || low[i].is_nan() || close[i].is_nan()));
    let first_idx = match first_valid_idx { Some(idx) => idx, None => return Err(DiError::AllValuesNaN), };
    if (n - first_idx) < period { return Err(DiError::NotEnoughValidData { needed: period, valid: n - first_idx }); }
...
pub unsafe fn di_scalar(
    ...
    for i in (first_idx + 1)..(first_idx + period) {
        ...
        prev_close = close[i];
        if diff_p > 0.0 && diff_p > diff_m { plus_dm_sum += diff_p; }
        if diff_m > 0.0 && diff_m > diff_p { minus_dm_sum += diff_m; }
        tr_sum += tr;
    }
    ...
    plus_di[idx] = if current_tr == 0.0 { 0.0 } else { (current_plus_dm / current_tr) * 100.0 };
    minus_di[idx] = if current_tr == 0.0 { 0.0 } else { (current_minus_dm / current_tr) * 100.0 };
    idx += 1;
    ...
    Ok(DiOutput { plus: plus_di, minus: minus_di })
}
...
impl DiStream {
    pub fn try_new(params: DiParams) -> Result<Self, DiError> {
        ...
        if period == 0 { return Err(DiError::InvalidPeriod { period, data_len: 0 }); }
        Ok(Self {
            period,
    ...
    pub fn update(&mut self, high: f64, low: f64, close: f64) -> Option<(f64, f64)> {
        ...
        if !self.filled { self.prev_high = high; self.prev_low = low; self.prev_close = close; return None; }
...
pub fn di_batch_slice(
    high: &[f64], low: &[f64], close: &[f64],
    sweep: &DiBatchRange, kern: Kernel,
) -> Result<DiBatchOutput, DiError> {
...
fn di_batch_inner(
    high: &[f64], low: &[f64], close: &[f64],
    sweep: &DiBatchRange, kern: Kernel, parallel: bool,
) -> Result<DiBatchOutput, DiError> {
    let combos = expand_grid(sweep);
    if combos.is_empty() { return Err(DiError::InvalidPeriod { period: 0, data_len: 0 }); }
    let n = high.len();
    if n == 0 || low.len() != n || close.len() != n { return Err(DiError::EmptyData); }
    let first = (0..n).find(|&i| !(high[i].is_nan() || low[i].is_nan() || close[i].is_nan()))
        .ok_or(DiError::AllValuesNaN)?;
    let max_p = combos.iter().map(|c| c.period.unwrap()).max().unwrap();
    if n - first < max_p { return Err(DiError::NotEnoughValidData { needed: max_p, valid: n - first }); }
    ...
    Ok(DiBatchOutput { plus, minus, combos, rows, cols })
}

#[inline(always)]
pub unsafe fn di_row_scalar(
    high: &[f64], low: &[f64], close: &[f64],
    period: usize, first: usize,
    out_plus: &mut [f64], out_minus: &mut [f64],
) -> Result<(), DiError> {
    let n = high.len();
    ...
    out_plus[idx] = if current_tr == 0.0 { 0.0 } else { (current_plus_dm / current_tr) * 100.0 };
    out_minus[idx] = if current_tr == 0.0 { 0.0 } else { (current_minus_dm / current_tr) * 100.0 };
    idx += 1;
        ...
        if di_result.plus.len() > 5 {
            ...
            for i in 0..5 {
                assert!(
                    ...
                    i, test_plus[i], plus_tail[i]
                );
                assert!(
                    (minus_tail[i] - test_minus[i]).abs() < 1e-6,
                    "Mismatch in -DI at tail index {}: expected {}, got {}",
                    i, test_minus[i], minus_tail[i]
                );
            }
        }
        Ok(())
