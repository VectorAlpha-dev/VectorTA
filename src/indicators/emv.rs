...
impl<'a> EmvInput<'a> {
	...
	#[inline(always)]
	pub fn from_slices(high: &'a [f64], low: &'a [f64], close: &'a [f64], volume: &'a [f64]) -> Self {
		Self {
			data: EmvData::Slices {
				high,
				low,
...
impl EmvBuilder {
	...
	#[inline(always)]
	pub fn apply_slices(self, high: &[f64], low: &[f64], close: &[f64], volume: &[f64]) -> Result<EmvOutput, EmvError> {
...
pub unsafe fn emv_avx512_short(high: &[f64], low: &[f64], volume: &[f64], first: usize, out: &mut [f64]) {
	emv_scalar(high, low, volume, first, out);
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f")]
pub unsafe fn emv_avx512_long(high: &[f64], low: &[f64], volume: &[f64], first: usize, out: &mut [f64]) {
	emv_scalar(high, low, volume, first, out);
}
