...
mod tests {
	...
	fn test_ht_trendline_slice_data_reinput() {
		...
		let first_result = ht_trendline(&first_input).expect("Failed to calculate first HT_TRENDLINE");
		...
		let second_result = ht_trendline(&second_input).expect("Failed to calculate second HT_TRENDLINE");
