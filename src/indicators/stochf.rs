...
impl<'a> StochfInput<'a> {
    ...
    pub fn from_candles(candles: &'a Candles, params: StochfParams) -> Self {
        Self { data: StochfData::Candles { candles }, params }
    }
    #[inline]
    pub fn from_slices(high: &'a [f64], low: &'a [f64], close: &'a [f64], params: StochfParams) -> Self {
        Self { data: StochfData::Slices { high, low, close }, params }
    }
    #[inline]
...
impl StochfBuilder {
    ...
    #[inline]
    pub fn fastk_period(mut self, n: usize) -> Self { self.fastk_period = Some(n); self }
    #[inline]
    pub fn fastd_period(mut self, n: usize) -> Self { self.fastd_period = Some(n); self }
    #[inline]
    pub fn fastd_matype(mut self, t: usize) -> Self { self.fastd_matype = Some(t); self }
    #[inline]
...
pub enum StochfError {
    ...
    InvalidPeriod { fastk: usize, fastd: usize, data_len: usize },
...
pub fn stochf_with_kernel(input: &StochfInput, kernel: Kernel) -> Result<StochfOutput, StochfError> {
    let (high, low, close) = match &input.data {
        StochfData::Candles { candles } => {
            let high = candles.select_candle_field("high").map_err(|_| StochfError::EmptyData)?;
            let low = candles.select_candle_field("low").map_err(|_| StochfError::EmptyData)?;
            let close = candles.select_candle_field("close").map_err(|_| StochfError::EmptyData)?;
            (high, low, close)
        }
    ...
    if fastk_period == 0 || fastd_period == 0 || fastk_period > len || fastd_period > len {
        return Err(StochfError::InvalidPeriod { fastk: fastk_period, fastd: fastd_period, data_len: len });
    }
    ...
    if (len - first_valid_idx) < fastk_period {
        return Err(StochfError::NotEnoughValidData { needed: fastk_period, valid: len - first_valid_idx });
    }
    ...
    unsafe {
        match chosen {
            Kernel::Scalar | Kernel::ScalarBatch => {
                stochf_scalar(high, low, close, fastk_period, fastd_period, matype, first_valid_idx, &mut k_vals, &mut d_vals)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 | Kernel::Avx2Batch => {
                stochf_avx2(high, low, close, fastk_period, fastd_period, matype, first_valid_idx, &mut k_vals, &mut d_vals)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                stochf_avx512(high, low, close, fastk_period, fastd_period, matype, first_valid_idx, &mut k_vals, &mut d_vals)
            }
            _ => unreachable!(),
        }
    }
...
pub unsafe fn stochf_scalar(
    ...
    fastk_period: usize, fastd_period: usize, matype: usize,
    first_valid_idx: usize, k_vals: &mut [f64], d_vals: &mut [f64]
) {
    let len = high.len();
    for i in first_valid_idx..len {
        if i < first_valid_idx + fastk_period - 1 { continue; }
...
pub unsafe fn stochf_avx2(
    ...
    stochf_scalar(high, low, close, fastk_period, fastd_period, matype, first_valid_idx, k_vals, d_vals);
}
...
impl StochfStream {
    pub fn try_new(params: StochfParams) -> Result<Self, StochfError> {
        ...
        if fastk_period == 0 || fastd_period == 0 {
            return Err(StochfError::InvalidPeriod { fastk: fastk_period, fastd: fastd_period, data_len: 0 });
        }
    ...
    pub fn update(&mut self, high: f64, low: f64, close: f64) -> Option<(f64, f64)> {
        ...
        if self.count < self.fastk_period { self.count += 1; }
        if self.count < self.fastk_period { return None; }
        ...
        let k = if hh == ll {
            if self.close_buffer[(self.head + self.fastk_period - 1) % self.fastk_period] == hh { 100.0 } else { 0.0 }
        } else {
        ...
        if self.k_count < self.fastd_period { self.k_count += 1; }
...
impl StochfBatchBuilder {
    pub fn new() -> Self { Self::default() }
    pub fn kernel(mut self, k: Kernel) -> Self { self.kernel = k; self }
    pub fn fastk_range(mut self, start: usize, end: usize, step: usize) -> Self { self.range.fastk_period = (start, end, step); self }
    pub fn fastk_static(mut self, p: usize) -> Self { self.range.fastk_period = (p, p, 0); self }
    pub fn fastd_range(mut self, start: usize, end: usize, step: usize) -> Self { self.range.fastd_period = (start, end, step); self }
    pub fn fastd_static(mut self, p: usize) -> Self { self.range.fastd_period = (p, p, 0); self }
    ...
    pub fn with_default_slices(high: &[f64], low: &[f64], close: &[f64], k: Kernel) -> Result<StochfBatchOutput, StochfError> {
...
pub fn stochf_batch_with_kernel(
    high: &[f64], low: &[f64], close: &[f64],
    sweep: &StochfBatchRange, k: Kernel
) -> Result<StochfBatchOutput, StochfError> {
    let kernel = match k {
        Kernel::Auto => detect_best_batch_kernel(),
        other if other.is_batch() => other,
        _ => return Err(StochfError::InvalidPeriod { fastk: 0, fastd: 0, data_len: 0 }),
    };
...
pub fn stochf_batch_slice(
    high: &[f64], low: &[f64], close: &[f64],
    sweep: &StochfBatchRange, kern: Kernel
) -> Result<StochfBatchOutput, StochfError> {
...
fn stochf_batch_inner(
    high: &[f64], low: &[f64], close: &[f64],
    sweep: &StochfBatchRange, kern: Kernel, parallel: bool
) -> Result<StochfBatchOutput, StochfError> {
    let combos = expand_grid(sweep);
    if combos.is_empty() { return Err(StochfError::InvalidPeriod { fastk: 0, fastd: 0, data_len: 0 }); }
    let first = (0..high.len()).find(|&i| !high[i].is_nan() && !low[i].is_nan() && !close[i].is_nan())
        .ok_or(StochfError::AllValuesNaN)?;
    let max_k = combos.iter().map(|c| c.fastk_period.unwrap()).max().unwrap();
    if high.len() - first < max_k {
        return Err(StochfError::NotEnoughValidData { needed: max_k, valid: high.len() - first });
    }
    let rows = combos.len();
    let cols = high.len();
    ...
    let do_row = |row: usize, kout: &mut [f64], dout: &mut [f64]| unsafe {
        ...
        match kern {
            Kernel::Scalar => stochf_row_scalar(high, low, close, first, fastk_period, fastd_period, matype, kout, dout),
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
    ...
    Ok(StochfBatchOutput { k: k_out, d: d_out, combos, rows, cols })
}

#[inline(always)]
unsafe fn stochf_row_scalar(
    high: &[f64], low: &[f64], close: &[f64],
    first: usize, fastk_period: usize, fastd_period: usize, matype: usize,
    k_out: &mut [f64], d_out: &mut [f64]
) {
    let len = high.len();
    for i in first..len {
        if i < first + fastk_period - 1 { continue; }
        let start = i + 1 - fastk_period;
        let (mut hh, mut ll) = (f64::NEG_INFINITY, f64::INFINITY);
        for j in start..=i {
            let h = high[j];
...
unsafe fn stochf_row_avx2(
    ...
    stochf_row_scalar(high, low, close, first, fastk_period, fastd_period, matype, k_out, d_out);
}
...
mod tests {
    ...
    fn check_stochf_partial_params(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let params = StochfParams { fastk_period: None, fastd_period: None, fastd_matype: None };
    ...
    fn check_stochf_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let params = StochfParams { fastk_period: Some(0), fastd_period: Some(3), fastd_matype: Some(0) };
    ...
    fn check_stochf_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let params = StochfParams { fastk_period: Some(10), fastd_period: Some(3), fastd_matype: Some(0) };
        let input = StochfInput::from_slices(&data, &data, &data, params);
        let res = stochf_with_kernel(&input, kernel);
        assert!(res.is_err());
        Ok(())
    ...
    fn check_stochf_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let data = [42.0];
        let params = StochfParams { fastk_period: Some(9), fastd_period: Some(3), fastd_matype: Some(0) };
        let input = StochfInput::from_slices(&data, &data, &data, params);
        let res = stochf_with_kernel(&input, kernel);
        assert!(res.is_err());
        Ok(())
