...
impl<'a> QqeInput<'a> {
    ...
    pub fn from_candles(c: &'a Candles, s: &'a str, p: QqeParams) -> Self {
        Self {
            data: QqeData::Candles { candles: c, source: s },
            params: p,
        }
    }
...
unsafe fn qqe_avx2(data: &[f64], rsi_p: usize, ema_p: usize, fast_k: f64, first: usize, fast_warm: usize) -> Result<QqeOutput, QqeError> {
...
unsafe fn qqe_avx512(data: &[f64], rsi_p: usize, ema_p: usize, fast_k: f64, first: usize, fast_warm: usize) -> Result<QqeOutput, QqeError> {
    // For now, fallback to scalar
    qqe_scalar(data, rsi_p, ema_p, fast_k, first, fast_warm)
}
...
pub fn qqe_into_slices(
    ...
    if dst_fast.len() != data.len() || dst_slow.len() != data.len() {
        return Err(QqeError::InvalidPeriod { period: dst_fast.len(), data_len: data.len() });
    }
    let warm = first + rsi_p + ema_p - 2;

    // Temporary row buffer for RSI (no copies of existing data)
    let mut tmp_mu = make_uninit_matrix(1, data.len());
    let tmp: &mut [f64] = unsafe {
        core::slice::from_raw_parts_mut(tmp_mu.as_mut_ptr() as *mut f64, data.len())
    };

    // RSI → tmp
    let rsi_in = RsiInput::from_slice(data, RsiParams { period: Some(rsi_p) });
    rsi_into_slice(tmp, &rsi_in, chosen)
        .map_err(|e| QqeError::DependentIndicatorError { message: e.to_string() })?;

    // EMA(tmp) → dst_fast
    let ema_in = EmaInput::from_slice(tmp, EmaParams { period: Some(ema_p) });
    ema_into_slice(dst_fast, &ema_in, chosen)
        .map_err(|e| QqeError::DependentIndicatorError { message: e.to_string() })?;

    // Enforce NaN warm prefixes for both outputs
    for v in &mut dst_fast[..warm] { *v = f64::NAN; }
    for v in &mut dst_slow[..warm] { *v = f64::NAN; }
...
fn qqe_prepare<'a>(
    ...
    Ok((data, rsi_period, smoothing_factor, fast_factor, first, chosen))
}

/// Core computation for slow line (QQES) - optimized version
#[inline]
fn qqe_compute_slow_from(
    qqef: &[f64],
    fast_factor: f64,
    start: usize,
    qqes: &mut [f64],
...
impl QqeBatchBuilder {
    ...
    pub fn with_default_candles(c: &Candles) -> Result<QqeBatchOutput, QqeError> {
        QqeBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
    }
}
...
pub fn qqe_batch_with_kernel(
    ...
    if combos.is_empty() { return Err(QqeError::InvalidPeriod { period: 0, data_len: 0 }); }
    let cols = data.len();
    if cols == 0 { return Err(QqeError::EmptyInputData); }

    let first = data.iter().position(|x| !x.is_nan()).ok_or(QqeError::AllValuesNaN)?;
    let worst_needed = combos.iter()
        .map(|c| c.rsi_period.unwrap() + c.smoothing_factor.unwrap())
        .max().unwrap();
    if cols - first < worst_needed {
        return Err(QqeError::NotEnoughValidData { needed: worst_needed, valid: cols - first });
    }
    ...
    let actual = match k {
        ...
        _ => return Err(QqeError::InvalidPeriod { period: 0, data_len: 0 }),
    };
    ...
    let fast_out: &mut [f64] = unsafe {
        core::slice::from_raw_parts_mut(fast_mu.as_mut_ptr() as *mut f64, rows * cols)
    };
    let slow_out: &mut [f64] = unsafe {
        core::slice::from_raw_parts_mut(slow_mu.as_mut_ptr() as *mut f64, rows * cols)
    };

    // Reusable tmp buffer for RSI
    let mut tmp_mu = make_uninit_matrix(1, cols);
    let tmp: &mut [f64] = unsafe {
        core::slice::from_raw_parts_mut(tmp_mu.as_mut_ptr() as *mut f64, cols)
    };
    ...
    for (row, combo) in combos.iter().enumerate() {
        ...
        // RSI → tmp
        let rsi_in = RsiInput::from_slice(data, RsiParams { period: Some(rsi_p) });
        rsi_into_slice(tmp, &rsi_in, simd)
            .map_err(|e| QqeError::DependentIndicatorError { message: e.to_string() })?;

        // EMA(tmp) → dst_fast
        let ema_in = EmaInput::from_slice(tmp, EmaParams { period: Some(ema_p) });
        ema_into_slice(dst_fast, &ema_in, simd)
            .map_err(|e| QqeError::DependentIndicatorError { message: e.to_string() })?;
    ...
    let fast_values = unsafe {
        Vec::from_raw_parts(fast_mu.as_mut_ptr() as *mut f64, rows * cols, rows * cols)
    };
    let slow_values = unsafe {
        Vec::from_raw_parts(slow_mu.as_mut_ptr() as *mut f64, rows * cols, rows * cols)
    };
    core::mem::forget(fast_mu);
    core::mem::forget(slow_mu);

    Ok(QqeBatchOutput { fast_values, slow_values, combos, rows, cols })
}
...
fn qqe_batch_inner(
    ...
    let do_row = |row: usize, f_mu: &mut [MaybeUninit<f64>], s_mu: &mut [MaybeUninit<f64>]| {
        ...
        let dst_fast = unsafe { core::slice::from_raw_parts_mut(f_mu.as_mut_ptr() as *mut f64, cols) };
        let dst_slow = unsafe { core::slice::from_raw_parts_mut(s_mu.as_mut_ptr() as *mut f64, cols) };
    ...
    if parallel {
        ...
        {
            for (row, (f_mu, s_mu)) in fast_mu.chunks_mut(cols).zip(slow_mu.chunks_mut(cols)).enumerate() {
    ...
    let fast_values = unsafe { Vec::from_raw_parts(fast_mu.as_mut_ptr() as *mut f64, rows * cols, rows * cols) };
    let slow_values = unsafe { Vec::from_raw_parts(slow_mu.as_mut_ptr() as *mut f64, rows * cols, rows * cols) };
    core::mem::forget(fast_mu);
    core::mem::forget(slow_mu);
...
#[inline(always)]
pub fn qqe_batch_slice(data: &[f64], sweep: &QqeBatchRange, kern: Kernel) -> Result<QqeBatchOutput, QqeError> {
    qqe_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn qqe_batch_par_slice(data: &[f64], sweep: &QqeBatchRange, kern: Kernel) -> Result<QqeBatchOutput, QqeError> {
...
pub fn qqe_py<'py>(
    ...
    Ok((
        result.fast.into_pyarray(py),
        result.slow.into_pyarray(py),
    ))
}
...
impl QqeStreamPy {
    ...
    fn new(rsi_period: usize, smoothing_factor: usize, fast_factor: f64) -> PyResult<Self> {
        ...
        let stream = QqeStream::try_new(params)
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn qqe_batch_py<'py>(
    ...
    let combos = expand_grid(&sweep);
    if combos.is_empty() { return Err(PyValueError::new_err("Empty parameter combination")); }
    let rows = combos.len();
    ...
    py.allow_threads(|| -> PyResult<()> {
        for (row, combo) in combos.iter().enumerate() {
            ...
            // RSI → tmp
            rsi_into_slice(tmp, &RsiInput::from_slice(slice_in, RsiParams { period: Some(rsi_p) }), simd)
                .map_err(|e| PyValueError::new_err(e.to_string()))?;
            // EMA(tmp) → dst_fast
            ema_into_slice(dst_fast, &EmaInput::from_slice(tmp, EmaParams { period: Some(ema_p) }), simd)
                .map_err(|e| PyValueError::new_err(e.to_string()))?;

            // warm NaNs
            for v in &mut dst_fast[..start] { *v = f64::NAN; }
            for v in &mut dst_slow[..start] { *v = f64::NAN; }
    ...
    dict.set_item("fast_factors", combos.iter().map(|c| c.fast_factor.unwrap()).collect::<Vec<_>>().into_pyarray(py))?;
    Ok(dict)
}
...
pub fn qqe_batch_into(
    ...
    unsafe {
        ...
        let combos = expand_grid(&sweep);
        let rows = combos.len();
        if rows == 0 { return Err(JsValue::from_str("Empty parameter combination")); }
        ...
        for (row, combo) in combos.iter().enumerate() {
            ...
            // RSI → tmp
            rsi_into_slice(tmp, &RsiInput::from_slice(data, RsiParams { period: Some(rsi_p) }), simd)
                .map_err(|e| JsValue::from_str(&e.to_string()))?;
            // EMA(tmp) → fast row
            ema_into_slice(dst_fast, &EmaInput::from_slice(tmp, EmaParams { period: Some(ema_p) }), simd)
                .map_err(|e| JsValue::from_str(&e.to_string()))?;
...
mod tests {
    ...
    fn check_qqe_accuracy(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for (i, (&fast_val, &slow_val)) in result.fast[start..].iter()
    ...
    fn check_qqe_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        assert!(res.is_err(), "[{}] QQE should fail with zero period", test_name);
        Ok(())
    }
    
    fn check_qqe_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_qqe_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        assert!(!results.is_empty(), "[{}] Should have streaming results", test_name);
        
        // Verify results are valid
        for (fast, slow) in &results {
            assert!(!fast.is_nan(), "[{}] Fast value should not be NaN", test_name);
            assert!(!slow.is_nan(), "[{}] Slow value should not be NaN", test_name);
        }
    ...
    fn check_qqe_into_slices(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for i in 0..data.len() {
            if dst_fast[i].is_nan() && regular.fast[i].is_nan() {
                ...
                assert_eq!(dst_fast[i], regular.fast[i], "[{}] Fast mismatch at {}", test_name, i);
            }
            
            if dst_slow[i].is_nan() && regular.slow[i].is_nan() {
                // Both NaN is ok
            } else {
                assert_eq!(dst_slow[i], regular.slow[i], "[{}] Slow mismatch at {}", test_name, i);
            }
        }
        ...
        // Test single computation
        {
            ...
            for (i, &val) in fast.iter().enumerate() {
                assert!(
                    ...
                    "[{}] Uninitialized memory detected in fast at index {}: {:?}", test_name, i, val
                );
            }
            ...
            for (i, &val) in slow.iter().enumerate() {
                assert!(
                    val.is_nan() || (val.is_finite() && val != POISON),
                    "[{}] Uninitialized memory detected in slow at index {}: {:?}", test_name, i, val
                );
            }
        }
        ...
        // Test batch computation
        {
            ...
            for (i, &val) in batch_out.fast_values.iter().enumerate() {
                assert!(
                    ...
                    "[{}] Invalid value in batch fast at index {}: {:?}", test_name, i, val
                );
            }
            ...
            for (i, &val) in batch_out.slow_values.iter().enumerate() {
                assert!(
                    val.is_nan() || val.is_finite(),
                    "[{}] Invalid value in batch slow at index {}: {:?}", test_name, i, val
                );
            }
        }
    ...
    fn check_batch_default_row(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let out = QqeBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
        let def = QqeParams::default();
        let row = out.row_for_params(&def).expect("default row missing");

        let start = row * out.cols;
        assert_eq!(out.fast_values[start..start+out.cols].len(), c.close.len());
        assert_eq!(out.slow_values[start..start+out.cols].len(), c.close.len());
        Ok(())
    }
    ...
    fn check_batch_no_poison(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for (idx, &v) in out.fast_values.iter().enumerate() {
            ...
            let b = v.to_bits();
            assert!(b != 0x1111_1111_1111_1111 && b != 0x2222_2222_2222_2222 && b != 0x3333_3333_3333_3333,
                    "[{}] poison in fast @ {}", test_name, idx);
        }
        for (idx, &v) in out.slow_values.iter().enumerate() {
            if v.is_nan() { continue; }
            let b = v.to_bits();
            assert!(b != 0x1111_1111_1111_1111 && b != 0x2222_2222_2222_2222 && b != 0x3333_3333_3333_3333,
                    "[{}] poison in slow @ {}", test_name, idx);
        }
        Ok(())
    }
    ...
    #[cfg(feature = "proptest")]
    fn check_qqe_property(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        use proptest::prelude::*;
        skip_if_unsupported!(kernel, test_name);
        ...
        proptest::test_runner::TestRunner::default()
            .run(&strat, |(data, rsi_p, ema_p, fast_k)| {
                let p = QqeParams {
                    rsi_period: Some(rsi_p),
                    smoothing_factor: Some(ema_p),
                    fast_factor: Some(fast_k),
                ...
                for i in 0..data.len() {
                    ...
                    let b = f[i];
                    if a.is_nan() { prop_assert!(b.is_nan()); } else { prop_assert!((a - b).abs() <= 1e-9); }

                    let c = ref_out.slow[i];
                    let d = s[i];
                    if c.is_nan() { prop_assert!(d.is_nan()); } else { prop_assert!((c - d).abs() <= 1e-9); }

                    if !a.is_nan() { prop_assert!(a >= 0.0 && a <= 100.0); }
                }
                Ok(())
            })?;
        Ok(())
