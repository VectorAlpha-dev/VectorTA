...
impl<'a> NetMyrsiInput<'a> {
    ...
    pub fn from_candles(c: &'a Candles, s: &'a str, p: NetMyrsiParams) -> Self {
        Self {
            data: NetMyrsiData::Candles { candles: c, source: s },
            params: p,
        }
    }
...
fn net_myrsi_prepare<'a>(
    ...
    let first = data.iter().position(|x| !x.is_nan()).ok_or(NetMyrsiError::AllValuesNaN)?;
    let period = input.get_period();
    
    if period == 0 || period > len {
        return Err(NetMyrsiError::InvalidPeriod { period, data_len: len });
    }
...
pub fn net_myrsi_with_kernel(input: &NetMyrsiInput, kernel: Kernel) -> Result<NetMyrsiOutput, NetMyrsiError> {
...
#[inline]
pub fn net_myrsi_into_slice(dst: &mut [f64], input: &NetMyrsiInput, kern: Kernel) -> Result<(), NetMyrsiError> {
    let (data, period, first, chosen) = net_myrsi_prepare(input, kern)?;
    
    if dst.len() != data.len() {
        return Err(NetMyrsiError::InvalidPeriod { period: dst.len(), data_len: data.len() });
    }
...
impl NetMyrsiStream {
    pub fn try_new(params: NetMyrsiParams) -> Result<Self, NetMyrsiError> {
        ...
        if period == 0 {
            return Err(NetMyrsiError::InvalidPeriod { period, data_len: 0 });
        }
...
impl NetMyrsiBatchBuilder {
    ...
    pub fn apply_candles(self, c: &Candles, src: &str) -> Result<NetMyrsiBatchOutput, NetMyrsiError> {
        self.apply_slice(source_type(c, src))
    }
    
    pub fn with_default_candles(c: &Candles) -> Result<NetMyrsiBatchOutput, NetMyrsiError> {
        NetMyrsiBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
    }
}
...
pub fn net_myrsi_batch_with_kernel(
    ...
    let kernel = match k {
        ...
        _ => return Err(NetMyrsiError::InvalidPeriod { period: 0, data_len: 0 }),
    };
    
    let periods = expand_grid_period(sweep.period);
    let combos: Vec<NetMyrsiParams> = periods.into_iter().map(|p| NetMyrsiParams { period: Some(p) }).collect();
...
fn net_myrsi_batch_inner(
    ...
    let max_needed = combos.iter().map(|c| c.period.unwrap_or(14) + 1).max().unwrap();
    if cols - first < max_needed {
        return Err(NetMyrsiError::NotEnoughValidData { needed: max_needed, valid: cols - first });
    }
    ...
    let warms: Vec<usize> = combos.iter().map(|c| first + c.period.unwrap_or(14) - 1).collect();
    ...
    Ok(NetMyrsiBatchOutput { values, combos: combos.to_vec(), rows, cols })
}
...
fn net_myrsi_batch_inner_into(
    ...
    if data.is_empty() { return Err(NetMyrsiError::EmptyInputData); }
    let first = data.iter().position(|x| !x.is_nan()).ok_or(NetMyrsiError::AllValuesNaN)?;
    let cols = data.len();
    let rows = combos.len();
...
impl NetMyrsiStreamPy {
    ...
    pub fn new(period: usize) -> PyResult<Self> {
        ...
        let stream = NetMyrsiStream::try_new(params)
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
        Ok(Self { stream })
    }
...
pub fn net_myrsi_js(
    ...
    let input = NetMyrsiInput::from_slice(data, NetMyrsiParams { period: Some(period) });
...
pub fn net_myrsi_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
    let cfg: NetMyrsiBatchConfig =
        serde_wasm_bindgen::from_value(config).map_err(|e| JsValue::from_str(&format!("Invalid config: {}", e)))?;
    let sweep = NetMyrsiBatchRange { period: cfg.period_range };
...
pub fn net_myrsi_batch_into(
    ...
    if in_ptr.is_null() || out_ptr.is_null() {
        return Err(JsValue::from_str("null pointer passed to net_myrsi_batch_into"));
    }
    unsafe {
        let data = std::slice::from_raw_parts(in_ptr, len);
...
mod tests {
    ...
    fn check_net_myrsi_partial_params(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
        
        let default_params = NetMyrsiParams {
            period: None,
        };
    ...
    fn check_net_myrsi_default_candles(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
    ...
    fn check_net_myrsi_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        assert!(res.is_err(), "[{}] NET_MYRSI should fail with zero period", test_name);
        Ok(())
    }
    
    fn check_net_myrsi_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_net_myrsi_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_net_myrsi_empty_input(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let res = net_myrsi_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] NET_MYRSI should fail with empty input", test_name);
        Ok(())
    }
    ...
    fn check_net_myrsi_all_nan(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let input = NetMyrsiInput::from_slice(&data, params);
        let res = net_myrsi_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] NET_MYRSI should fail with all NaN values", test_name);
        Ok(())
    }
    ...
    fn check_net_myrsi_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let first_params = NetMyrsiParams {
            period: Some(14),
        };
        ...
        let second_params = NetMyrsiParams {
            period: Some(14),
        };
        ...
        assert!(non_nan_count > 0, "[{}] Second pass should produce non-NaN values", test_name);
    ...
    fn check_net_myrsi_warmup_nans(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let p = 14;
        let out = net_myrsi_with_kernel(&NetMyrsiInput::from_candles(&candles, "close", NetMyrsiParams{period:Some(p)}),
                                        kernel)?.values;
        let warm = first + p - 1;
        assert!(out[..warm].iter().all(|v| v.is_nan()), 
                "[{}] Warmup NaNs should not be overwritten", test_name);
        Ok(())
    }
    ...
    fn check_net_myrsi_nan_handling(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        assert!(non_nan_count > 0, "[{}] Should produce some non-NaN values despite NaN input", test_name);
    ...
    fn check_net_myrsi_streaming(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let batch = NetMyrsiInput::from_candles(&c, "close", NetMyrsiParams { period: Some(period) });
        let b = net_myrsi_with_kernel(&batch, kernel)?.values;
        
        let mut s = NetMyrsiStream::try_new(NetMyrsiParams { period: Some(period) })?;
        ...
        for i in first_valid..b.len() {
            if !b[i].is_nan() && !stream[i].is_nan() {
                assert!((b[i] - stream[i]).abs() < 1e-10, "[{test}] idx {i}: {} vs {}", b[i], stream[i]);
            }
        }
    ...
    fn check_net_myrsi_property(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        use proptest::prelude::*;
        skip_if_unsupported!(kernel, test_name);
        ...
        proptest::test_runner::TestRunner::default()
            .run(&strat, |(data, period)| {
                ...
                let NetMyrsiOutput { values: ref_out } = net_myrsi_with_kernel(&input, Kernel::Scalar).unwrap();
                
                assert_eq!(out.len(), ref_out.len());
                assert_eq!(out.len(), data.len());
                
                // Find first valid index
                let first_valid = ref_out.iter().position(|v| !v.is_nan()).unwrap_or(ref_out.len());
    ...
    fn check_net_myrsi_property(_test_name: &str, _kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        Ok(())
    }
    ...
    fn check_batch_no_poison(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for (idx, &v) in out.values.iter().enumerate() {
            ...
            let bits = v.to_bits();
            assert_ne!(bits, 0x1111_1111_1111_1111, "[{test}] alloc_with_nan_prefix poison at {idx}");
            assert_ne!(bits, 0x2222_2222_2222_2222, "[{test}] init_matrix_prefixes poison at {idx}");
            assert_ne!(bits, 0x3333_3333_3333_3333, "[{test}] make_uninit_matrix poison at {idx}");
        }
        Ok(())
    }
