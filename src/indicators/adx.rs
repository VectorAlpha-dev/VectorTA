...
//!
//! ## Decision Log
//! SIMD enabled (AVX2/AVX512 warmup-only) with scalar main loop for determinism; CUDA wrapper returns typed errors and VRAM handles; Python interop provides CAI v3 + DLPack v1.x. Numerical outputs and NaN warmup semantics match the scalar reference.
...
impl DeviceArrayF32AdxPy {
    ...
    fn __cuda_array_interface__<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyDict>> {
        ...
        // Stream omitted: kernels synchronize before returning the handle (CAI v3).
        d.set_item("version", 3)?;
        Ok(d)
    }

    fn __dlpack_device__(&self) -> PyResult<(i32, i32)> {
        // Prefer allocation device from pointer attributes; fall back to current context.
        unsafe {
            ...
            let mut dev: i32 = self._device_id as i32;
    ...
    fn __dlpack__<'py>(
        ...
        // Choose versioned vs legacy capsule based on negotiation.
        let use_versioned = max_version.map(|(maj, _)| maj >= 1).unwrap_or(false);

        // Ignore consumer stream: producer is synchronized (no pending work).
        let _ = stream;

        // Move VRAM buffer into capsule-owned manager.
        ...
        // Determine allocation device and retain its primary context.
        ...
        unsafe extern "C" fn deleter_v1(p: *mut DLManagedTensorVersioned) {
            ...
            if !mgr.is_null() {
                ...
                let _ = cust::sys::cuDevicePrimaryCtxRelease(
                    (*p).dl_tensor.device.device_id as i32,
                );
            }
        ...
        unsafe extern "C" fn capsule_dtor(capsule: *mut pyo3::ffi::PyObject) {
            ...
            let ptr =
                pyo3::ffi::PyCapsule_GetPointer(capsule, vname.as_ptr());
            if !ptr.is_null() {
            ...
            let ptr2 =
                pyo3::ffi::PyCapsule_GetPointer(capsule, legacy.as_ptr());
            if !ptr2.is_null() {
