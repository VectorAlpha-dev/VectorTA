...
impl<'a> StdDevInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: StdDevParams) -> Self {
		Self {
			data: StdDevData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
...
pub fn stddev_with_kernel(input: &StdDevInput, kernel: Kernel) -> Result<StdDevOutput, StdDevError> {
	...
	unsafe {
		match chosen {
			Kernel::Scalar | Kernel::ScalarBatch => stddev_scalar(data, period, first, nbdev, &mut out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
unsafe fn stddev_avx512_short(data: &[f64], period: usize, first: usize, nbdev: f64, out: &mut [f64]) {
	stddev_scalar(data, period, first, nbdev, out);
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
unsafe fn stddev_avx512_long(data: &[f64], period: usize, first: usize, nbdev: f64, out: &mut [f64]) {
	stddev_scalar(data, period, first, nbdev, out);
}
...
impl StdDevBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<StdDevBatchOutput, StdDevError> {
		StdDevBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
#[inline(always)]
unsafe fn stddev_row_scalar(data: &[f64], first: usize, period: usize, nbdev: f64, out: &mut [f64]) {
	stddev_scalar(data, period, first, nbdev, out)
}
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
unsafe fn stddev_row_avx512(data: &[f64], first: usize, period: usize, nbdev: f64, out: &mut [f64]) {
	if period <= 32 {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
unsafe fn stddev_row_avx512_short(data: &[f64], first: usize, period: usize, nbdev: f64, out: &mut [f64]) {
	stddev_scalar(data, period, first, nbdev, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
unsafe fn stddev_row_avx512_long(data: &[f64], first: usize, period: usize, nbdev: f64, out: &mut [f64]) {
	stddev_scalar(data, period, first, nbdev, out)
}
...
impl StdDevStreamPy {
	...
	fn new(period: usize, nbdev: f64) -> PyResult<Self> {
		...
		let stream = StdDevStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
mod tests {
	...
	fn check_stddev_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let res = stddev_with_kernel(&input, kernel);
		assert!(res.is_err(), "[{}] STDDEV should fail with zero period", test_name);
		Ok(())
	}

	fn check_stddev_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_stddev_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = StdDevBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
