    ...
    unsafe {
        match chosen {
            Kernel::Scalar | Kernel::ScalarBatch => {
                lrsi_scalar(&price, alpha, first_valid_idx, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 | Kernel::Avx2Batch => {
                lrsi_avx2(&price, alpha, first_valid_idx, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                lrsi_avx512(&price, alpha, first_valid_idx, &mut out)
            }
            _ => unreachable!(),
        }
    }
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
unsafe fn lrsi_row_avx512_short(
    price: &[f64],
    first: usize,
    alpha: f64,
    out: &mut [f64],
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
unsafe fn lrsi_row_avx512_long(
    price: &[f64],
    first: usize,
    alpha: f64,
    out: &mut [f64],
