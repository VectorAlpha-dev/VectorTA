...
pub fn efi_with_kernel(input: &EfiInput, kernel: Kernel) -> Result<EfiOutput, EfiError> {
    ...
    if (len - first_valid_idx) < 2 {
        return Err(EfiError::NotEnoughValidData { needed: 2, valid: len - first_valid_idx });
    }
    ...
    unsafe {
        match chosen {
            Kernel::Scalar | Kernel::ScalarBatch => {
                efi_scalar(price, volume, period, first_valid_idx, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 | Kernel::Avx2Batch => {
                efi_avx2(price, volume, period, first_valid_idx, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                efi_avx512(price, volume, period, first_valid_idx, &mut out)
            }
            _ => unreachable!(),
        }
    }
...
impl EfiBatchOutput {
    pub fn row_for_params(&self, p: &EfiParams) -> Option<usize> {
        self.combos.iter().position(|c| {
            c.period.unwrap_or(13) == p.period.unwrap_or(13)
        })
    }
...
fn efi_batch_inner(
    ...
    if price.len() - first < max_p {
        return Err(EfiError::NotEnoughValidData { needed: max_p, valid: price.len() - first });
    }
    let rows = combos.len();
    let cols = price.len();
...
mod tests {
    ...
    fn check_efi_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let input = EfiInput::from_slices(&price, &volume, params);
        let res = efi_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] EFI should fail with period exceeding length", test_name);
        Ok(())
    }
    ...
    fn check_efi_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for (i, (&b, &s)) in batch_output.iter().zip(stream_values.iter()).enumerate() {
            ...
            let diff = (b - s).abs();
            assert!(diff < 1.0, "[{}] EFI streaming mismatch at idx {}: batch={}, stream={}", test_name, i, b, s);
        }
        Ok(())
    }
