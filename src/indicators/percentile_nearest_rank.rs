...
impl<'a> PercentileNearestRankInput<'a> {
    ...
    pub fn from_candles(c: &'a Candles, s: &'a str, p: PercentileNearestRankParams) -> Self {
        Self {
            data: PercentileNearestRankData::Candles { candles: c, source: s },
            params: p,
        }
    }
...
fn pnr_prepare<'a>(
    ...
    let len = data.len();
    if len == 0 { return Err(PercentileNearestRankError::EmptyInputData); }
    ...
    if length == 0 || length > len {
        return Err(PercentileNearestRankError::InvalidPeriod { period: length, data_len: len });
    }
    ...
    if len - first < length {
        return Err(PercentileNearestRankError::NotEnoughValidData { needed: length, valid: len - first });
    }
...
fn pnr_compute_into(
    ...
    for i in start_i..data.len() {
        ...
        for j in 0..length {
            let idx = i - j;
            let v = data[idx];
            if !v.is_nan() { window.push(v); }
        }

        if window.is_empty() { out[i] = f64::NAN; continue; }
...
#[inline]
pub fn percentile_nearest_rank(input: &PercentileNearestRankInput)
...
pub fn percentile_nearest_rank_into_slice(
    ...
    let (data, length, percentage, first, chosen) = pnr_prepare(input, kernel)?;
    if dst.len() != data.len() {
        return Err(PercentileNearestRankError::InvalidPeriod { period: dst.len(), data_len: data.len() });
    }
...
impl PercentileNearestRankBuilder {
    ...
    pub fn build(self, data: &[f64]) -> Result<PercentileNearestRankOutput, PercentileNearestRankError> {
    ...
    pub fn build_candles(self, candles: &Candles, source: &str) -> Result<PercentileNearestRankOutput, PercentileNearestRankError> {
        let params = PercentileNearestRankParams {
            length: self.length,
            percentage: self.percentage,
        };
    ...
    #[inline(always)]
    pub fn apply(self, c: &Candles) -> Result<PercentileNearestRankOutput, PercentileNearestRankError> {
        let p = PercentileNearestRankParams { length: self.length, percentage: self.percentage };
        let i = PercentileNearestRankInput::from_candles(c, "close", p);
        percentile_nearest_rank_with_kernel(&i, self.kernel)
    }
    
    #[inline(always)]
    pub fn apply_slice(self, d: &[f64]) -> Result<PercentileNearestRankOutput, PercentileNearestRankError> {
        let p = PercentileNearestRankParams { length: self.length, percentage: self.percentage };
...
impl PercentileNearestRankStream {
    pub fn try_new(params: PercentileNearestRankParams) -> Result<Self, PercentileNearestRankError> {
    ...
    pub fn update(&mut self, value: f64) -> Option<f64> {
        ...
        let mut window: Vec<f64> = self.buffer.iter().copied().filter(|x| !x.is_nan()).collect();
        if window.is_empty() { return Some(f64::NAN); }
        window.sort_by(|a,b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
...
pub fn percentile_nearest_rank_batch_py<'py>(
    ...
    dict.set_item(
        "lengths",
        combos.iter().map(|p| p.length.unwrap_or(15) as u64).collect::<Vec<_>>().into_pyarray(py),
    )?;
    dict.set_item(
        "percentages",
        combos.iter().map(|p| p.percentage.unwrap_or(50.0)).collect::<Vec<_>>().into_pyarray(py),
    )?;
    Ok(dict.into())
}
...
impl PercentileNearestRankBatchBuilder {
    ...
    pub fn apply(self, data: &[f64]) -> Result<PercentileNearestRankBatchOutput, PercentileNearestRankError> {
        pnr_batch_slice(data, &self.range, self.kernel)
    }
    
    pub fn apply_candles(self, candles: &Candles, source: &str) -> Result<PercentileNearestRankBatchOutput, PercentileNearestRankError> {
    ...
    pub fn with_default_slice(data: &[f64], k: Kernel)
        ...
        PercentileNearestRankBatchBuilder::new().kernel(k).apply(data)
    }

    pub fn with_default_candles(c: &Candles)
        -> Result<PercentileNearestRankBatchOutput, PercentileNearestRankError> {
        PercentileNearestRankBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
    }
}
...
pub fn pnr_batch_with_kernel(
    ...
    let kernel = match k {
        ...
        _ => return Err(PercentileNearestRankError::InvalidPeriod { period: 0, data_len: 0 }),
    };
...
fn pnr_batch_inner(
    ...
    if data.is_empty() { return Err(PercentileNearestRankError::EmptyInputData); }
    let combos = expand_grid_pnr(sweep);
    if combos.is_empty() {
        return Err(PercentileNearestRankError::InvalidPeriod { period: 0, data_len: 0 });
    }
    let rows = combos.len();
    let cols = data.len();
    ...
    if data.len() - first < max_len {
        return Err(PercentileNearestRankError::NotEnoughValidData { needed: max_len, valid: data.len() - first });
    }

    // allocate uninit matrix and set NaN prefixes per row
    let mut buf_mu = make_uninit_matrix(rows, cols);
    let warm: Vec<usize> = combos.iter().map(|c| first + c.length.unwrap() - 1).collect();
    ...
    Ok(PercentileNearestRankBatchOutput { values, combos, rows, cols })
}
...
fn pnr_batch_inner_into(
    ...
    let cols = data.len();
    if cols == 0 { return Err(PercentileNearestRankError::EmptyInputData); }
    ...
    if parallel {
        ...
        {
            use rayon::prelude::*;
            out.par_chunks_mut(cols).enumerate().for_each(|(row, s)| do_row(row, s));
        }
        #[cfg(target_arch = "wasm32")]
        for (row, s) in out.chunks_mut(cols).enumerate() { do_row(row, s); }
    } else {
        for (row, s) in out.chunks_mut(cols).enumerate() { do_row(row, s); }
    }
    Ok(())
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn percentile_nearest_rank_js(data: &[f64], length: usize, percentage: f64) -> Result<Vec<f64>, JsValue> {
    let params = PercentileNearestRankParams { length: Some(length), percentage: Some(percentage) };
    let input = PercentileNearestRankInput::from_slice(data, params);
    percentile_nearest_rank(&input).map(|o| o.values).map_err(|e| JsValue::from_str(&e.to_string()))
}
...
pub fn percentile_nearest_rank_free(ptr: *mut f64, n: usize) {
    unsafe { let _ = Vec::from_raw_parts(ptr, n, n); }
}
...
pub fn percentile_nearest_rank_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
    let cfg: PercentileNearestRankBatchConfig =
        serde_wasm_bindgen::from_value(config).map_err(|e| JsValue::from_str(&format!("Invalid config: {}", e)))?;
    let sweep = PercentileNearestRankBatchRange { length: cfg.length_range, percentage: cfg.percentage_range };
    let out = pnr_batch_inner(data, &sweep, detect_best_batch_kernel(), false)
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
    let js = PercentileNearestRankBatchJsOutput { values: out.values, combos: out.combos, rows: out.rows, cols: out.cols };
...
mod tests {
    ...
    fn check_pnr_accuracy(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for i in 0..14 {
            assert!(result.values[i].is_nan(), "[{}] Expected NaN at index {}", test_name, i);
        }
        
        // Check that we have valid values after warmup
        assert!(!result.values[14].is_nan(), "[{}] Expected valid value at index 14", test_name);
        ...
        for (i, (&actual, &expected)) in actual_last_5.iter().zip(expected_last_5.iter()).enumerate() {
    ...
    fn check_pnr_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        assert!(matches!(result, Err(PercentileNearestRankError::InvalidPeriod { .. })));
        Ok(())
    }
    
    fn check_pnr_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_pnr_empty_input(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        assert!(matches!(result, Err(PercentileNearestRankError::EmptyInputData)));
        Ok(())
    }
    ...
    fn check_pnr_invalid_percentage(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let input = PercentileNearestRankInput::from_slice(&data, params);
        let result = percentile_nearest_rank_with_kernel(&input, kernel);
        assert!(matches!(result, Err(PercentileNearestRankError::InvalidPercentage { .. })));
    ...
    fn check_pnr_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let second_input = PercentileNearestRankInput::from_slice(&first_result.values, second_params);
    ...
    fn check_pnr_nan_handling(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let data = vec![
            ...
            f64::NAN, 7.0, 8.0, 9.0, 10.0,
            11.0, 12.0, 13.0, f64::NAN, 15.0,
        ];
    ...
    fn check_pnr_no_poison(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let test_params = vec![
            ...
            PercentileNearestRankParams { length: Some(5), percentage: Some(25.0) },
            PercentileNearestRankParams { length: Some(10), percentage: Some(75.0) },
            PercentileNearestRankParams { length: Some(20), percentage: Some(50.0) },
            PercentileNearestRankParams { length: Some(50), percentage: Some(90.0) },
        ];
    ...
    fn check_batch_no_poison(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for &val in &output.values {
            ...
            if bits == 0x11111111_11111111 || bits == 0x22222222_22222222 || bits == 0x33333333_33333333 {
                panic!("[{}] Found poison value {} (0x{:016X})", test_name, val, bits);
            }
        }
    ...
    fn test_percentile_nearest_rank_all_nan() {
        ...
        assert!(matches!(result, Err(PercentileNearestRankError::AllValuesNaN)));
    }
    ...
    fn test_percentile_nearest_rank_with_candles() {
        let close_data = vec![1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5, 10.5,
