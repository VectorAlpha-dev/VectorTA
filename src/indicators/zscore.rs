...
pub fn zscore_with_kernel(input: &ZscoreInput, kernel: Kernel) -> Result<ZscoreOutput, ZscoreError> {
    ...
    if (len - first) < period {
        return Err(ZscoreError::NotEnoughValidData { needed: period, valid: len - first });
    }
...
pub unsafe fn zscore_scalar(
    ...
    let means = ma(ma_type, MaData::Slice(data), period)
        .unwrap_or_else(|_| vec![f64::NAN; data.len() - first]);
    ...
    let mut sigmas = deviation(&dev_input)
        .unwrap_or_else(|_| DeviationOutput { values: vec![f64::NAN; data.len() - first] })
        .values;
    for v in &mut sigmas {
        *v *= nbdev;
    }
    ...
    for i in (first + period - 1)..data.len() {
        ...
        let value = data[i];
        out[i] = if sigma == 0.0 || sigma.is_nan() { f64::NAN } else { (value - mean) / sigma };
    }
...
impl ZscoreStream {
    ...
    fn compute_zscore(&self) -> f64 {
        ...
        let means = ma(&self.ma_type, MaData::Slice(&ordered), self.period)
            .unwrap_or_else(|_| vec![f64::NAN; self.period]);
        let dev_input = DevInput {
            data: &ordered,
            params: DevParams {
        ...
        let mut sigmas = deviation(&dev_input)
            .unwrap_or_else(|_| DeviationOutput { values: vec![f64::NAN; self.period] })
            .values;
        for s in &mut sigmas {
            *s *= self.nbdev;
        }
...
fn zscore_batch_inner(
    ...
    if data.len() - first < max_p {
        return Err(ZscoreError::NotEnoughValidData { needed: max_p, valid: data.len() - first });
    }
    ...
    Ok(ZscoreBatchOutput { values, combos, rows, cols })
}
...
unsafe fn zscore_row_scalar(
    ...
    let dev_input = DevInput {
        data,
        params: DevParams { period: Some(period), devtype: Some(devtype) },
    };
    let mut sigmas = deviation(&dev_input)
        .unwrap_or_else(|_| DeviationOutput { values: vec![f64::NAN; data.len() - first] })
        .values;
...
mod tests {
    ...
    fn check_zscore_partial_params(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let default_params = ZscoreParams {
            period: None, ma_type: None, nbdev: None, devtype: None
        };
    ...
    fn check_zscore_with_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let params = ZscoreParams {
            period: Some(0), ma_type: None, nbdev: None, devtype: None
        };
    ...
    fn check_zscore_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let params = ZscoreParams {
            period: Some(10), ma_type: None, nbdev: None, devtype: None
        };
        let input = ZscoreInput::from_slice(&data_small, params);
        let res = zscore_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] Zscore should fail with period exceeding length", test_name);
        Ok(())
    }
    fn check_zscore_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let single_point = [42.0];
        let params = ZscoreParams { period: Some(14), ma_type: None, nbdev: None, devtype: None };
        let input = ZscoreInput::from_slice(&single_point, params);
        let res = zscore_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] Zscore should fail with insufficient data", test_name);
        Ok(())
    }
    fn check_zscore_all_nan(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let input = ZscoreInput::from_slice(&input_data, params);
        let res = zscore_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] Zscore should fail when all values are NaN", test_name);
        Ok(())
    }
