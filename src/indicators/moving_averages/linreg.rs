...
pub fn linreg_compute_into(input: &LinRegInput, kernel: Kernel, out: &mut [f64]) -> Result<(), LinRegError> {
	let data: &[f64] = input.as_ref();
	let first = data.iter().position(|x| !x.is_nan()).ok_or(LinRegError::AllValuesNaN)?;
	let len = data.len();
	let period = input.get_period();
...
impl LinRegBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<LinRegBatchOutput, LinRegError> {
		LinRegBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
mod tests {
	...
	fn check_linreg_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let candles = read_candles_from_csv(file_path)?;
		let period = 14;
		let input = LinRegInput::from_candles(&candles, "close", LinRegParams { period: Some(period) });
	...
	fn check_linreg_no_poison(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		for period in &test_periods {
			for source in &test_sources {
				let input = LinRegInput::from_candles(&candles, source, LinRegParams { period: Some(*period) });
				let output = linreg_with_kernel(&input, kernel)?;
...
pub fn linreg_batch_py<'py>(
	...
	let sweep = LinRegBatchRange { period: period_range };
...
pub fn linreg_js(data: &[f64], period: usize) -> Result<Vec<f64>, JsValue> {
	...
	linreg_into_slice(&mut output, &input, Kernel::Scalar).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn linreg_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let output = linreg_batch_slice(data, &sweep, Kernel::Scalar).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn linreg_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, period: usize) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr {
			...
			linreg_into_slice(&mut temp, &input, Kernel::Scalar).map_err(|e| JsValue::from_str(&e.to_string()))?;
			...
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			linreg_into_slice(out, &input, Kernel::Scalar).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
