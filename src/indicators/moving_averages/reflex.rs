...
impl ReflexStream {
	...
	pub fn update(&mut self, value: f64) -> Option<f64> {
		...
		if t >= period {
			...
			let period_f = period as f64;
			let my_sum =
				ssf_t + ((ssf_t_period - ssf_t) * (period_f + 1.0) / (2.0 * period_f)) - (self.ssf_sum / period_f);
...
impl ReflexBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<ReflexBatchOutput, ReflexError> {
		ReflexBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
impl ReflexStreamPy {
	...
	pub fn new(period: usize) -> PyResult<Self> {
		let params = ReflexParams { period: Some(period) };
		let inner =
			ReflexStream::try_new(params).map_err(|e| PyValueError::new_err(format!("reflex stream error: {}", e)))?;
		Ok(Self { inner })
	}
...
pub fn reflex_js(data: &[f64], period: usize) -> Result<Vec<f64>, JsValue> {
	...
	reflex_into_slice(&mut output, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn reflex_batch_metadata_js(period_start: usize, period_end: usize, period_step: usize) -> Vec<usize> {
	let range = ReflexBatchRange {
		period: (period_start, period_end, period_step),
	};
	let combos = expand_grid(&range);
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn reflex_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, period: usize) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr {
			...
			reflex_into_slice(&mut temp, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			out.copy_from_slice(&temp);
		} else {
			// No aliasing: compute directly into output
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			reflex_into_slice(out, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
		Ok(())
	}
}
...
mod tests {
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = ReflexBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
