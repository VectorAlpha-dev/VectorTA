...
impl<'a> VwmaInput<'a> {
	...
	pub fn from_candles_plus_prices(candles: &'a Candles, prices: &'a [f64], params: VwmaParams) -> Self {
		Self {
...
pub fn vwma_with_kernel(input: &VwmaInput, kernel: Kernel) -> Result<VwmaOutput, VwmaError> {
	let (price, volume): (&[f64], &[f64]) = match &input.data {
		VwmaData::Candles { candles, source } => (source_type(candles, source), source_type(candles, "volume")),
	...
	unsafe {
		match chosen {
			Kernel::Scalar | Kernel::ScalarBatch => vwma_scalar(price, volume, period, first, &mut out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx2 | Kernel::Avx2Batch => vwma_avx2(price, volume, period, first, &mut out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx512 | Kernel::Avx512Batch => vwma_avx512(price, volume, period, first, &mut out),
			_ => unreachable!(),
		}
	}
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub unsafe fn vwma_avx512_short(price: &[f64], volume: &[f64], period: usize, first: usize, out: &mut [f64]) {
	vwma_scalar(price, volume, period, first, out)
}
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub unsafe fn vwma_avx512_long(price: &[f64], volume: &[f64], period: usize, first: usize, out: &mut [f64]) {
	vwma_scalar(price, volume, period, first, out)
}
...
pub fn vwma_batch_with_kernel(
	...
	let chosen = match kernel {
		...
		_ => return Err(VwmaError::InvalidPeriod { period: 0, data_len: 0 }),
	};
...
#[inline(always)]
pub unsafe fn vwma_row_scalar(price: &[f64], volume: &[f64], first: usize, period: usize, out: &mut [f64]) {
	vwma_scalar(price, volume, period, first, out);
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn vwma_row_avx2(price: &[f64], volume: &[f64], first: usize, period: usize, out: &mut [f64]) {
	vwma_scalar(price, volume, period, first, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn vwma_row_avx512(price: &[f64], volume: &[f64], first: usize, period: usize, out: &mut [f64]) {
	if period <= 32 {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn vwma_row_avx512_short(price: &[f64], volume: &[f64], first: usize, period: usize, out: &mut [f64]) {
	vwma_scalar(price, volume, period, first, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn vwma_row_avx512_long(price: &[f64], volume: &[f64], first: usize, period: usize, out: &mut [f64]) {
	vwma_scalar(price, volume, period, first, out)
}
...
mod tests {
	...
	fn check_vwma_no_poison(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		for &period in &test_periods {
			...
			let input = VwmaInput::from_candles(&candles, "close", VwmaParams { period: Some(period) });
	...
	mod batch_tests {
		...
		fn check_batch_no_poison(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
			...
			let batch_configs = vec![
				(1, 10, 1),    // Edge case: starting from minimum period
				(5, 25, 5),    // Small range with step 5
				(10, 30, 10),  // Medium range with larger step
				(20, 100, 10), // Default start with step 10
				(50, 200, 50), // Very large periods
				(1, 5, 1),     // Very small range to test edge cases
			];
...
pub fn vwma_py<'py>(
	...
	py.allow_threads(|| -> Result<(), VwmaError> {
		...
		unsafe {
			match chosen {
				Kernel::Scalar | Kernel::ScalarBatch => vwma_scalar(price, volume, period, first, slice_out),
				#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
				Kernel::Avx2 | Kernel::Avx2Batch => vwma_avx2(price, volume, period, first, slice_out),
				#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
				Kernel::Avx512 | Kernel::Avx512Batch => vwma_avx512(price, volume, period, first, slice_out),
				_ => unreachable!(),
			}
		}
...
impl VwmaStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = VwmaParams { period: Some(period) };
		let stream = VwmaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn vwma_batch_py<'py>(
	...
	py.allow_threads(|| {
		...
		let out_uninit =
			unsafe { std::slice::from_raw_parts_mut(slice_out.as_mut_ptr() as *mut MaybeUninit<f64>, slice_out.len()) };
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn vwma_batch_metadata_js(period_start: usize, period_end: usize, period_step: usize) -> Result<Vec<f64>, JsValue> {
	let sweep = VwmaBatchRange {
		period: (period_start, period_end, period_step),
	};
