...
impl<'a> TrimaInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: TrimaParams) -> Self {
		Self {
			data: TrimaData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
...
impl TrimaBuilder {
	...
	pub fn apply(self, c: &Candles) -> Result<TrimaOutput, TrimaError> {
		let p = TrimaParams { period: self.period };
...
> {
	...
	if period == 0 || period > len {
		return Err(TrimaError::InvalidPeriod { period, data_len: len });
	}
	if period <= 3 {
...
fn trima_compute_into(
	...
	unsafe {
		match kernel {
			...
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx512 | Kernel::Avx512Batch => trima_scalar_optimized(data, period, m1, m2, first, out),
...
impl TrimaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<TrimaBatchOutput, TrimaError> {
		TrimaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
#[inline(always)]
pub fn trima_batch_slice(data: &[f64], sweep: &TrimaBatchRange, kern: Kernel) -> Result<TrimaBatchOutput, TrimaError> {
...
impl TrimaStreamPy {
	...
	fn new(period: Option<usize>) -> PyResult<Self> {
		...
		let stream = TrimaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
mod tests {
	...
	fn check_trima_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		assert!(res.is_err(), "[{}] TRIMA should fail with zero period", test_name);
		Ok(())
	}
	...
	fn check_trima_period_too_small(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let input = TrimaInput::from_slice(&input_data, params);
		let res = trima_with_kernel(&input, kernel);
		assert!(res.is_err(), "[{}] TRIMA should fail with period <= 3", test_name);
		Ok(())
	}
	...
	fn check_trima_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let res = trima_with_kernel(&input, kernel);
		assert!(res.is_err(), "[{}] TRIMA should fail with insufficient data", test_name);
		Ok(())
	}
	...
	fn check_trima_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let input = TrimaInput::from_candles(&candles, "close", TrimaParams { period: Some(period) });
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let output = TrimaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
