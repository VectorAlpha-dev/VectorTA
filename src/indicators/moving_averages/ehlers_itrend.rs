...
impl<'a> EhlersITrendInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: EhlersITrendParams) -> Self {
		Self {
			data: EhlersITrendData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline(always)]
...
pub fn ehlers_itrend_into_slice(
	...
	if dst.len() != data.len() {
		return Err(EhlersITrendError::InvalidOutputLen { expected: data.len(), got: dst.len() });
	}
	// prefill warmup prefix without copying the whole buffer
	for v in &mut dst[..warm] { *v = f64::from_bits(0x7ff8_0000_0000_0000); }
	ehlers_itrend_compute_into(data, warmup_bars, max_dc, first, chosen, dst);
	Ok(())
}

#[inline]
pub fn ehlers_itrend_scalar(src: &[f64], warmup_bars: usize, max_dc: usize, first_valid: usize, out: &mut [f64]) {
	debug_assert_eq!(src.len(), out.len());
...
pub unsafe fn ehlers_itrend_unsafe_scalar(
	...
	for (idx, &x0) in src.iter().enumerate() {
		...
		*det.get_unchecked_mut(rp) = det_val;
		...
		let i1v = r7(&det, rp, 3);
		let q1v = (C0 * (r7(&det, rp, 0) - r7(&det, rp, 6)) + C1 * (r7(&det, rp, 2) - r7(&det, rp, 4))) * period_mult;
		*i1.get_unchecked_mut(rp) = i1v;
		*q1.get_unchecked_mut(rp) = q1v;

		let j_i = (C0 * (r7(&i1, rp, 0) - r7(&i1, rp, 6)) + C1 * (r7(&i1, rp, 2) - r7(&i1, rp, 4))) * period_mult;
		let j_q = (C0 * (r7(&q1, rp, 0) - r7(&q1, rp, 6)) + C1 * (r7(&q1, rp, 2) - r7(&q1, rp, 4))) * period_mult;
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub fn ehlers_itrend_avx2(data: &[f64], warmup_bars: usize, max_dc: usize, first_valid: usize, out: &mut [f64]) {
	ehlers_itrend_scalar(data, warmup_bars, max_dc, first_valid, out);
}
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub fn ehlers_itrend_avx512(data: &[f64], warmup_bars: usize, max_dc: usize, first_valid: usize, out: &mut [f64]) {
	ehlers_itrend_scalar(data, warmup_bars, max_dc, first_valid, out);
}
...
impl EhlersITrendBatchBuilder {
	...
	pub fn apply_candles(self, c: &Candles, src: &str) -> Result<EhlersITrendBatchOutput, EhlersITrendError> {
...
fn ehlers_itrend_batch_inner(
	...
	let first = data
		...
		.position(|x| !x.is_nan())
		.ok_or(EhlersITrendError::AllValuesNaN)?;
	let warm: Vec<usize> = combos.iter().map(|c| first + c.warmup_bars.unwrap()).collect();
...
fn ehlers_itrend_batch_inner_into(
	...
	let raw = unsafe { core::slice::from_raw_parts_mut(out.as_mut_ptr() as *mut MaybeUninit<f64>, out.len()) };
...
pub fn ehlers_itrend_row_scalar_tail(
	data: &[f64], warmup_bars: usize, max_dc: usize, first: usize, out: &mut [f64]
) {
	let warm = warm_index(first, warmup_bars);
	ehlers_itrend_scalar_tail(data, warmup_bars, max_dc, first, warm, out);
}
...
pub fn ehlers_itrend_py<'py>(
	...
	let vec_out: Vec<f64> = py.allow_threads(|| ehlers_itrend_with_kernel(&input, kern)
...
pub fn ehlers_itrend_batch_py<'py>(
	...
	let slice_in = data.as_slice()?;
	let sweep = EhlersITrendBatchRange { warmup_bars: warmup_range, max_dc_period: max_dc_range };
	...
	dict.set_item("warmups", combos.iter().map(|p| p.warmup_bars.unwrap() as u64).collect::<Vec<_>>().into_pyarray(py))?;
	dict.set_item("max_dcs", combos.iter().map(|p| p.max_dc_period.unwrap() as u64).collect::<Vec<_>>().into_pyarray(py))?;
	Ok(dict)
}
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn ehlers_itrend_js(data: &[f64], warmup_bars: usize, max_dc_period: usize) -> Result<Vec<f64>, JsValue> {
	let input = EhlersITrendInput::from_slice(data, EhlersITrendParams {
		warmup_bars: Some(warmup_bars), max_dc_period: Some(max_dc_period)
	});
	let mut out = vec![0.0; data.len()];
	ehlers_itrend_into_slice(&mut out, &input, detect_best_kernel()).map_err(|e| JsValue::from_str(&e.to_string()))?;
	Ok(out)
}
...
pub fn ehlers_itrend_into(
	...
	unsafe {
		...
		if core::ptr::eq(in_ptr, out_ptr) {
			...
			ehlers_itrend_into_slice(&mut tmp, &input, detect_best_kernel()).map_err(|e| JsValue::from_str(&e.to_string()))?;
			core::slice::from_raw_parts_mut(out_ptr, len).copy_from_slice(&tmp);
		} else {
			let out = core::slice::from_raw_parts_mut(out_ptr, len);
			ehlers_itrend_into_slice(out, &input, detect_best_kernel()).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
	}
	Ok(())
}
...
pub fn ehlers_itrend_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let sweep = EhlersITrendBatchRange { warmup_bars: cfg.warmup_range, max_dc_period: cfg.max_dc_range };
	let out = ehlers_itrend_batch_inner(data, &sweep, detect_best_kernel(), false)
		.map_err(|e| JsValue::from_str(&e.to_string()))?;
	let js = EhlersITrendBatchJsOutput { values: out.values, combos: out.combos, rows: out.rows, cols: out.cols };
...
impl EhlersITrendStreamPy {
	...
	fn new(warmup_bars: Option<usize>, max_dc_period: Option<usize>) -> PyResult<Self> {
		...
		let stream = EhlersITrendStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
mod tests {
	...
	fn check_itrend_no_data(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		assert!(res.is_err(), "[{}] EIT should fail with empty data", test_name);
		Ok(())
	}
	...
	fn check_itrend_all_nan_data(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let input = EhlersITrendInput::from_slice(&data, params);
		let res = ehlers_itrend_with_kernel(&input, kernel);
		assert!(res.is_err(), "[{}] EIT should fail with all-NaN data", test_name);
		Ok(())
	}

	fn check_itrend_small_data_for_warmup(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
		let data = [42.0; 5];
		let params = EhlersITrendParams {
			warmup_bars: Some(12),
	...
	fn check_itrend_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_itrend_zero_warmup(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let input = EhlersITrendInput::from_slice(&data, params);
		let res = ehlers_itrend_with_kernel(&input, kernel);
		assert!(matches!(res, Err(EhlersITrendError::InvalidWarmupBars { .. })));
		Ok(())
	}
	...
	fn check_itrend_invalid_max_dc(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let input = EhlersITrendInput::from_slice(&data, params);
		let res = ehlers_itrend_with_kernel(&input, kernel);
		assert!(matches!(res, Err(EhlersITrendError::InvalidMaxDcPeriod { .. })));
		Ok(())
	}
	...
	fn check_itrend_property(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
			...
			.run(&strat, |(data, warmup, max_dc, a, b)| {
				...
				match (fast, slow) {
					...
					(Ok(fast), Ok(reference)) => {
						...
						let t_out = ehlers_itrend(&EhlersITrendInput::from_slice(&transformed, params))?.values;
		...
		assert!(ehlers_itrend(&EhlersITrendInput::from_slice(&[], EhlersITrendParams::default())).is_err());
