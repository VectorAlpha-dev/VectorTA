...
impl<'a> EhlersEcemaInput<'a> {
    ...
    pub fn from_candles(c: &'a Candles, s: &'a str, p: EhlersEcemaParams) -> Self {
        Self {
            data: EhlersEcemaData::Candles { candles: c, source: s },
            params: p,
        }
    }
...
fn calculate_pine_ema_into(
    ...
    let len = data.len();
    for v in &mut dst[..first.min(len)] { *v = f64::NAN; }
    if first >= len { return; }
...
fn ehlers_ecema_prepare<'a>(
    ...
    let first = data.iter().position(|x| !x.is_nan()).ok_or(EhlersEcemaError::AllValuesNaN)?;
    let length = input.get_length();
    let gain_limit = input.get_gain_limit();
    
    if length == 0 || length > len {
        return Err(EhlersEcemaError::InvalidPeriod { period: length, data_len: len });
    }
    ...
    unsafe {
        match kernel {
            Kernel::Scalar | Kernel::ScalarBatch => 
                ehlers_ecema_scalar_into_with_mode(data, ema_values, length, gain_limit, first, alpha, beta, pine_compatible, confirmed_only, out),
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 | Kernel::Avx2Batch => 
                ehlers_ecema_avx2_into_with_mode(data, ema_values, length, gain_limit, first, alpha, beta, pine_compatible, confirmed_only, out),
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => 
                ehlers_ecema_avx512_into_with_mode(data, ema_values, length, gain_limit, first, alpha, beta, pine_compatible, confirmed_only, out),
...
pub fn ehlers_ecema_with_kernel(input: &EhlersEcemaInput, kernel: Kernel) -> Result<EhlersEcemaOutput, EhlersEcemaError> {
    let (data, length, gain_limit, first, alpha, beta, chosen) = ehlers_ecema_prepare(input, kernel)?;
    let pine_compatible = input.get_pine_compatible();
    let confirmed_only = input.get_confirmed_only();
    
    // Build EMA in-place
    let mut ema_buf = alloc_with_nan_prefix(data.len(), if pine_compatible { first } else { first + length - 1 });
    if pine_compatible {
        calculate_pine_ema_into(&mut ema_buf, data, length, alpha, beta, first);
    } else {
        let ema_input = EmaInput::from_slice(data, EmaParams { period: Some(length) });
        ema_into_slice(&mut ema_buf, &ema_input, chosen).map_err(|e| EhlersEcemaError::EmaError(e.to_string()))?;
    }
    
    // Allocate output once
    let mut out = alloc_with_nan_prefix(data.len(), if pine_compatible { first } else { first + length - 1 });
    
    ehlers_ecema_compute_into_with_mode(
        data, &ema_buf, length, gain_limit, first, alpha, beta,
        chosen, pine_compatible, confirmed_only, &mut out
    );
...
pub fn ehlers_ecema_into_slice(
    ...
    if dst.len() != data.len() {
        return Err(EhlersEcemaError::InvalidPeriod { period: dst.len(), data_len: data.len() });
    }
    let pine_compatible = input.get_pine_compatible();
    let confirmed_only = input.get_confirmed_only();
    ...
    let warmup_end = if pine_compatible { first } else { first + length - 1 };
    let dst_len = dst.len();
    for v in &mut dst[..warmup_end.min(dst_len)] { *v = f64::NAN; }
    
    ehlers_ecema_compute_into_with_mode(
        data, &ema_buf, length, gain_limit, first, alpha, beta,
        chosen, pine_compatible, confirmed_only, dst
    );
    Ok(())
}
...
impl EhlersEcemaBatchBuilder {
    ...
    pub fn apply_candles(self, c: &Candles, src: &str) -> Result<EhlersEcemaBatchOutput, EhlersEcemaError> {
        let slice = source_type(c, src);
        self.apply_slice(slice)
    }
    
    pub fn with_default_slice(data: &[f64], k: Kernel) -> Result<EhlersEcemaBatchOutput, EhlersEcemaError> {
    ...
    pub fn with_default_candles(c: &Candles) -> Result<EhlersEcemaBatchOutput, EhlersEcemaError> {
        EhlersEcemaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
    }
}
...
pub fn ehlers_ecema_batch_with_kernel(data: &[f64], sweep: &EhlersEcemaBatchRange, k: Kernel) -> Result<EhlersEcemaBatchOutput, EhlersEcemaError> {
    let kernel = match k {
        Kernel::Auto => detect_best_batch_kernel(),
        other if other.is_batch() => other,
        _ => return Err(EhlersEcemaError::InvalidPeriod { period: 0, data_len: 0 }),
    };
...
fn ehlers_ecema_batch_inner(
    ...
    if combos.is_empty() {
        return Err(EhlersEcemaError::InvalidPeriod { period: 0, data_len: 0 });
    }
    ...
    let first = data.iter().position(|x| !x.is_nan()).ok_or(EhlersEcemaError::AllValuesNaN)?;
    let max_len = combos.iter().map(|p| p.length.unwrap_or(20)).max().unwrap_or(1);
    if cols - first < max_len {
        return Err(EhlersEcemaError::NotEnoughValidData { needed: max_len, valid: cols - first });
    }
    ...
    Ok(EhlersEcemaBatchOutput { values, combos, rows, cols })
}
...
fn ehlers_ecema_batch_inner_into(
    ...
    let unique_lengths: std::collections::HashSet<usize> = combos.iter()
        .map(|p| p.length.unwrap_or(20))
        .collect();
    
    for &length in &unique_lengths {
        let mut buf = alloc_with_nan_prefix(cols, first + length - 1);
        let ema_in = EmaInput::from_slice(data, EmaParams { period: Some(length) });
        ema_into_slice(&mut buf, &ema_in, kern).map_err(|e| EhlersEcemaError::EmaError(e.to_string()))?;
    ...
    let do_row = |row: usize, row_out: &mut [f64]| -> Result<(), EhlersEcemaError> {
        ...
        unsafe { ehlers_ecema_scalar_into_with_mode(data, ema_vals, length, gain_limit, first, alpha, beta, false, false, row_out); }
        Ok(())
    };
...
#[inline(always)]
pub fn ehlers_ecema_batch_slice(data: &[f64], sweep: &EhlersEcemaBatchRange, kern: Kernel)
    -> Result<EhlersEcemaBatchOutput, EhlersEcemaError>
{
    ehlers_ecema_batch_inner(data, sweep, match kern {
...
#[inline(always)]
pub fn ehlers_ecema_batch_par_slice(data: &[f64], sweep: &EhlersEcemaBatchRange, kern: Kernel)
    -> Result<EhlersEcemaBatchOutput, EhlersEcemaError>
{
    ehlers_ecema_batch_inner(data, sweep, match kern {
        Kernel::Auto => detect_best_kernel(),
...
impl EhlersEcemaStream {
    ...
    pub fn next(&mut self, value: f64) -> f64 {
        ...
        let ema_value = if self.pine_compatible {
            ...
            if !self.ema_filled {
                self.ema_mean = ((self.count as f64 - 1.0) * self.ema_mean + src) / self.count as f64;
        ...
        for gain_int in -(self.gain_limit as i32)..=(self.gain_limit as i32) {
            let gain = gain_int as f64 / 10.0;
            let test_ec_ema = self.alpha * (ema_value + gain * (src - self.prev_ecema)) + self.beta * self.prev_ecema;
            let error = (src - test_ec_ema).abs();
        ...
        let ec_ema = self.alpha * (ema_value + best_gain * (src - self.prev_ecema)) + self.beta * self.prev_ecema;
...
pub fn ehlers_ecema_batch_py<'py>(
    ...
    let slice_in = data.as_slice()?;
    let sweep = EhlersEcemaBatchRange { length: length_range, gain_limit: gain_limit_range };
    ...
    dict.set_item("lengths",
        combos.iter().map(|p| p.length.unwrap_or(20) as u64).collect::<Vec<_>>().into_pyarray(py)
    )?;
    dict.set_item("gain_limits",
        combos.iter().map(|p| p.gain_limit.unwrap_or(50) as u64).collect::<Vec<_>>().into_pyarray(py)
    )?;
...
impl EhlersEcemaStreamPy {
    ...
    pub fn new(length: usize, gain_limit: usize, pine_compatible: bool, confirmed_only: bool) -> PyResult<Self> {
        let params = EhlersEcemaParams {
            length: Some(length),
            gain_limit: Some(gain_limit),
            pine_compatible: Some(pine_compatible),
        ...
        let stream = EhlersEcemaStream::try_new(params)
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn ehlers_ecema_js(data: &[f64], length: usize, gain_limit: usize) -> Result<Vec<f64>, JsValue> {
    let params = EhlersEcemaParams {
        length: Some(length),
        gain_limit: Some(gain_limit),
        pine_compatible: Some(false),
...
pub fn ehlers_ecema_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
    let cfg: EhlersEcemaBatchConfig =
...
pub fn ehlers_ecema_into(
    ...
    if in_ptr.is_null() || out_ptr.is_null() {
        return Err(JsValue::from_str("null pointer passed to ehlers_ecema_into"));
    }
...
pub fn ehlers_ecema_into_ex(
    ...
    if in_ptr.is_null() || out_ptr.is_null() {
        return Err(JsValue::from_str("null pointer passed to ehlers_ecema_into_ex"));
    }
    if length == 0 || length > len { return Err(JsValue::from_str("Invalid length")); }
    if gain_limit == 0 { return Err(JsValue::from_str("Invalid gain limit")); }
...
mod tests {
    ...
    fn check_ehlers_ecema_partial_params(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
    ...
    fn check_ehlers_ecema_accuracy(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        assert!(first_valid.is_some(), "[{}] No valid values found", test_name);
    ...
    fn check_ehlers_ecema_pine_accuracy(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
    ...
    fn check_ehlers_ecema_default_candles(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
    ...
    fn check_ehlers_ecema_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_ehlers_ecema_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_ehlers_ecema_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_ehlers_ecema_empty_input(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let empty: [f64; 0] = [];
    ...
    fn check_ehlers_ecema_invalid_gain_limit(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_ehlers_ecema_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let valid_count = second_result.values.iter()
            ...
            .count();
        assert!(valid_count > 0, "[{}] No valid values after reinput", test_name);
        
        Ok(())
    }
    
    fn check_ehlers_ecema_nan_handling(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
    ...
    fn check_ehlers_ecema_no_poison(_test_name: &str, _kernel: Kernel) -> Result<(), Box<dyn Error>> {
        Ok(())
    }
    
    #[cfg(feature = "proptest")]
    #[allow(clippy::float_cmp)]
    fn check_ehlers_ecema_property(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        use proptest::prelude::*;
        skip_if_unsupported!(kernel, test_name);
        ...
        proptest::test_runner::TestRunner::default()
            .run(&strat, |(data, length, gain_limit)| {
                let params = EhlersEcemaParams {
                    length: Some(length),
                    gain_limit: Some(gain_limit),
                    pine_compatible: Some(false),
                ...
                let EhlersEcemaOutput { values: out } = ehlers_ecema_with_kernel(&input, kernel).unwrap();
                let EhlersEcemaOutput { values: ref_out } = ehlers_ecema_with_kernel(&input, Kernel::Scalar).unwrap();
                
                // Check output length matches input
                assert_eq!(out.len(), data.len(), "[{}] Output length mismatch", test_name);
                assert_eq!(ref_out.len(), data.len(), "[{}] Reference output length mismatch", test_name);
                ...
                for i in (length - 1)..data.len() {
                    if out[i].is_finite() && ref_out[i].is_finite() {
                        ...
                        assert!(
                            ...
                            test_name, i, out[i], ref_out[i], diff
                        );
                    }
                }
                ...
                for i in 0..(length - 1) {
                    assert!(out[i].is_nan(), "[{}] Expected NaN during warmup at idx {}", test_name, i);
                }
    ...
    fn check_batch_no_poison(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for (cfg_idx, &(l_start, l_end, l_step, g_start, g_end, g_step)) in test_configs.iter().enumerate() {
            let output = EhlersEcemaBatchBuilder::new()
                .kernel(kernel)
