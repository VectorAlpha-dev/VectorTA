...
#[inline(always)]
fn pwma_compute_into(data: &[f64], weights: &[f64], period: usize, first: usize, kernel: Kernel, out: &mut [f64]) {
	unsafe {
		match kernel {
...
impl PwmaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<PwmaBatchOutput, PwmaError> {
		PwmaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
#[inline(always)]
pub fn pwma_batch_slice(data: &[f64], sweep: &PwmaBatchRange, kern: Kernel) -> Result<PwmaBatchOutput, PwmaError> {
	pwma_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn pwma_batch_par_slice(data: &[f64], sweep: &PwmaBatchRange, kern: Kernel) -> Result<PwmaBatchOutput, PwmaError> {
	pwma_batch_inner(data, sweep, kern, true)
}

#[inline(always)]
pub fn pwma_batch_with_kernel(data: &[f64], sweep: &PwmaBatchRange, k: Kernel) -> Result<PwmaBatchOutput, PwmaError> {
	let kernel = match k {
		Kernel::Auto => detect_best_batch_kernel(),
		other if other.is_batch() => other,
		_ => return Err(PwmaError::InvalidPeriod { period: 0, data_len: 0 }),
	};
...
mod tests {
	...
	fn check_pwma_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		assert!(res.is_err(), "[{}] PWMA should fail with zero period", test_name);
		Ok(())
	}

	fn check_pwma_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_pwma_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
		let single_point = [42.0];
		let params = PwmaParams { period: Some(5) };
		let input = PwmaInput::from_slice(&single_point, params);
		let res = pwma_with_kernel(&input, kernel);
		assert!(res.is_err(), "[{}] PWMA should fail with insufficient data", test_name);
		Ok(())
	}
...
impl PwmaStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = PwmaParams { period: Some(period) };
		let stream = PwmaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn pwma_batch_py<'py>(
	...
	let sweep = PwmaBatchRange { period: period_range };
...
pub fn pwma_js(data: &[f64], period: usize) -> Result<Vec<f64>, JsValue> {
	...
	pwma_into_slice(&mut output, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn pwma_batch_metadata_js(period_start: usize, period_end: usize, period_step: usize) -> Result<Vec<f64>, JsValue> {
	let sweep = PwmaBatchRange {
		period: (period_start, period_end, period_step),
	};
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn pwma_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, period: usize) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr {
			...
			pwma_into_slice(&mut temp, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
			...
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			pwma_into_slice(out, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
...
pub fn pwma_batch_into(
	...
	unsafe {
		...
		pwma_batch_inner_into(data, &sweep, Kernel::Auto, false, out).map_err(|e| JsValue::from_str(&e.to_string()))?;
