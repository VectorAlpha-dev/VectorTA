...
impl ZlemaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<ZlemaBatchOutput, ZlemaError> {
		ZlemaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
#[inline(always)]
pub fn zlema_batch_slice(data: &[f64], sweep: &ZlemaBatchRange, kern: Kernel) -> Result<ZlemaBatchOutput, ZlemaError> {
...
mod tests {
	...
	fn check_zlema_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let period = 14;
		let input = ZlemaInput::from_candles(&candles, "close", ZlemaParams { period: Some(period) });
		let batch_output = zlema_with_kernel(&input, kernel)?.values;

		let mut stream = ZlemaStream::try_new(ZlemaParams { period: Some(period) })?;
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = ZlemaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
impl ZlemaStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = ZlemaParams { period: Some(period) };
		let stream = ZlemaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
