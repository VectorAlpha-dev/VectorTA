...
impl<'a> KamaInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: KamaParams) -> Self {
		Self {
			data: KamaData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
...
impl KamaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<KamaBatchOutput, KamaError> {
		KamaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}

pub fn kama_batch_with_kernel(data: &[f64], sweep: &KamaBatchRange, k: Kernel) -> Result<KamaBatchOutput, KamaError> {
	let kernel = match k {
...
#[inline(always)]
pub fn kama_batch_slice(data: &[f64], sweep: &KamaBatchRange, kern: Kernel) -> Result<KamaBatchOutput, KamaError> {
	kama_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn kama_batch_par_slice(data: &[f64], sweep: &KamaBatchRange, kern: Kernel) -> Result<KamaBatchOutput, KamaError> {
...
pub fn kama_js(data: &[f64], period: usize) -> Result<Vec<f64>, JsValue> {
	...
	kama_into_slice(&mut output, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn kama_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, period: usize) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr {
			...
			kama_into_slice(&mut temp, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
			...
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			kama_into_slice(out, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
...
pub fn kama_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let output = kama_batch_inner(data, &sweep, Kernel::Auto, false).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn kama_batch_into(
	...
	unsafe {
		...
		kama_batch_inner_into(data, &sweep, Kernel::Auto, false, out).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn kama_batch_metadata_js(period_start: usize, period_end: usize, period_step: usize) -> Vec<f64> {
...
mod tests {
	...
	fn check_kama_accuracy(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		assert!(result.values.len() >= 5, "Expected at least 5 values to compare");
		assert_eq!(
			result.values.len(),
			candles.close.len(),
	...
	fn check_kama_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let candles = read_candles_from_csv(file_path)?;
		let period = 30;
		let input = KamaInput::from_candles(&candles, "close", KamaParams { period: Some(period) });
	...
	fn check_kama_no_poison(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		for period in &test_periods {
			for source in &test_sources {
				let input = KamaInput::from_candles(&candles, source, KamaParams { period: Some(*period) });
				let output = kama_with_kernel(&input, kernel)?;
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = KamaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
