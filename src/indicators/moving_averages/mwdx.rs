...
#[inline]
pub fn mwdx_batch_with_kernel(data: &[f64], sweep: &MwdxBatchRange, k: Kernel) -> Result<MwdxBatchOutput, MwdxError> {
	let kernel = match k {
...
impl MwdxBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<MwdxBatchOutput, MwdxError> {
		MwdxBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
#[inline(always)]
pub fn mwdx_batch_slice(data: &[f64], sweep: &MwdxBatchRange, kern: Kernel) -> Result<MwdxBatchOutput, MwdxError> {
...
mod tests {
	...
	fn check_mwdx_zero_factor(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		assert!(res.is_err(), "[{}] MWDX should fail with zero factor", test_name);
		Ok(())
	}
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let output = MwdxBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
impl MwdxStreamPy {
	...
	fn new(factor: f64) -> PyResult<Self> {
		let params = MwdxParams { factor: Some(factor) };
		let stream = MwdxStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn mwdx_js(data: &[f64], factor: f64) -> Result<Vec<f64>, JsValue> {
	...
	mwdx_into_slice(&mut output, &input, Kernel::Scalar).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn mwdx_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let output =
		mwdx_batch_inner(data, &sweep, Kernel::Scalar, false).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn mwdx_batch_js(data: &[f64], factor_start: f64, factor_end: f64, factor_step: f64) -> Result<Vec<f64>, JsValue> {
	let sweep = MwdxBatchRange {
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn mwdx_batch_metadata_js(factor_start: f64, factor_end: f64, factor_step: f64) -> Result<Vec<f64>, JsValue> {
	let sweep = MwdxBatchRange {
		factor: (factor_start, factor_end, factor_step),
	};
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn mwdx_batch_rows_cols_js(factor_start: f64, factor_end: f64, factor_step: f64, data_len: usize) -> Vec<usize> {
	let sweep = MwdxBatchRange {
		factor: (factor_start, factor_end, factor_step),
	};
	let combos = expand_grid(&sweep);
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn mwdx_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, factor: f64) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr as *const f64 {
			...
			mwdx_into_slice(&mut temp, &input, Kernel::Scalar).map_err(|e| JsValue::from_str(&e.to_string()))?;
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			out.copy_from_slice(&temp);
		} else {
			// Different pointers - compute directly
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			mwdx_into_slice(out, &input, Kernel::Scalar).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
...
pub fn mwdx_batch_into(
	...
	unsafe {
		...
		for (i, params) in combos.iter().enumerate() {
			...
			mwdx_into_slice(out_row, &input, Kernel::Scalar).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
