...
impl SrwmaBuilder {
	...
	pub fn apply(self, c: &Candles) -> Result<SrwmaOutput, SrwmaError> {
		let p = SrwmaParams { period: self.period };
...
pub fn srwma_with_kernel(input: &SrwmaInput, kernel: Kernel) -> Result<SrwmaOutput, SrwmaError> {
	...
	unsafe {
		match chosen {
			Kernel::Scalar | Kernel::ScalarBatch => srwma_scalar(data, &weights, period, first, inv_norm, &mut out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx2 | Kernel::Avx2Batch => srwma_avx2(data, &weights, period, first, inv_norm, &mut out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx512 | Kernel::Avx512Batch => srwma_avx512(data, &weights, period, first, inv_norm, &mut out),
			_ => unreachable!(),
		}
	}
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn srwma_avx512(data: &[f64], weights: &[f64], period: usize, first_valid: usize, inv_norm: f64, out: &mut [f64]) {
	if period <= 32 {
...
#[inline]
pub fn srwma_scalar(data: &[f64], weights: &[f64], period: usize, first_val: usize, inv_norm: f64, out: &mut [f64]) {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn srwma_avx2(data: &[f64], weights: &[f64], period: usize, first_valid: usize, inv_norm: f64, out: &mut [f64]) {
...
pub fn srwma_batch_with_kernel(
	...
	let kernel = match k {
		...
		_ => return Err(SrwmaError::InvalidPeriod { period: 0, data_len: 0 }),
	};
...
impl SrwmaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<SrwmaBatchOutput, SrwmaError> {
		SrwmaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
#[inline(always)]
pub fn srwma_batch_slice(data: &[f64], sweep: &SrwmaBatchRange, kern: Kernel) -> Result<SrwmaBatchOutput, SrwmaError> {
...
fn srwma_batch_inner(
	...
	if combos.iter().any(|c| (len - first) < (c.period.unwrap() + 1)) {
...
mod tests {
	...
	fn check_srwma_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		assert!(res.is_err(), "[{}] SRWMA should fail with zero period", test_name);
		Ok(())
	}
	...
	fn check_srwma_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let res = srwma_with_kernel(&input, kernel);
		assert!(res.is_err(), "[{}] SRWMA should fail with insufficient data", test_name);
		Ok(())
	}
	...
	fn check_srwma_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let period = 14;
		let input = SrwmaInput::from_candles(&candles, "close", SrwmaParams { period: Some(period) });
	...
	fn check_srwma_no_poison(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_srwma_no_poison(_test_name: &str, _kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		Ok(())
	}
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let output = SrwmaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
impl SrwmaStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = SrwmaParams { period: Some(period) };
		let stream = SrwmaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn srwma_into_slice(dst: &mut [f64], input: &SrwmaInput, kern: Kernel) -> Result<(), SrwmaError> {
	...
	unsafe {
		match chosen {
			Kernel::Scalar | Kernel::ScalarBatch => srwma_scalar(data, &weights, period, first, inv_norm, dst),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx2 | Kernel::Avx2Batch => srwma_avx2(data, &weights, period, first, inv_norm, dst),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx512 | Kernel::Avx512Batch => srwma_avx512(data, &weights, period, first, inv_norm, dst),
			_ => unreachable!(),
		}
	}
...
pub fn srwma_js(data: &[f64], period: usize) -> Result<Vec<f64>, JsValue> {
	...
	srwma_into_slice(&mut output, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn srwma_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, period: usize) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr {
			...
			srwma_into_slice(&mut temp, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
			...
			// Direct computation
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			srwma_into_slice(out, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
