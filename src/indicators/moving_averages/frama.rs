...
impl<'a> FramaInput<'a> {
	...
	#[inline]
	pub fn from_slices(high: &'a [f64], low: &'a [f64], close: &'a [f64], params: FramaParams) -> Self {
		Self {
...
impl FramaBuilder {
	...
	#[inline(always)]
	pub fn apply_slices(self, high: &[f64], low: &[f64], close: &[f64]) -> Result<FramaOutput, FramaError> {
		let p = FramaParams {
			window: self.window,
			sc: self.sc,
			fc: self.fc,
...
> {
	...
	if window == 0 || window > len {
		return Err(FramaError::InvalidWindow { window, data_len: len });
	}
...
unsafe fn frama_avx2_small<const WIN: usize>(
	...
	for i in (first + WIN)..len {
		...
		if unlikely(
			(*high.get_unchecked(i)).is_nan() || (*low.get_unchecked(i)).is_nan() || (*close.get_unchecked(i)).is_nan(),
		) {
		...
		*out.get_unchecked_mut(i) = (*close.get_unchecked(i)).mul_add(alpha, (1.0 - alpha) * *out.get_unchecked(i - 1));
	}
}
...
unsafe fn frama_avx512_small<const WIN: usize>(
	...
	for i in (first + WIN)..len {
		...
		if tail != 0 {
			...
			let h_tail = _mm512_mask_loadu_pd(_mm512_set1_pd(f64::MIN), mask, high.as_ptr().add(off));
			let l_tail = _mm512_mask_loadu_pd(_mm512_set1_pd(f64::MAX), mask, low.as_ptr().add(off));
...
impl FramaBatchBuilder {
	...
	pub fn apply_slices(self, high: &[f64], low: &[f64], close: &[f64]) -> Result<FramaBatchOutput, FramaError> {
	...
	pub fn with_default_slices(
		...
		FramaBatchBuilder::new().kernel(k).apply_slices(high, low, close)
	}
	...
	pub fn with_default_candles(c: &Candles) -> Result<FramaBatchOutput, FramaError> {
		FramaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c)
	}
}
#[derive(Clone, Debug)]
...
pub fn frama_batch_with_kernel(
	...
	let kernel = match k {
		...
		_ => return Err(FramaError::InvalidWindow { window: 0, data_len: 0 }),
	};
...
fn frama_batch_inner_into(
	...
	let max_even_w = combos.iter().map(|c| {
...
impl FramaStream {
	pub fn try_new(params: FramaParams) -> Result<Self, FramaError> {
		...
		if window == 0 {
			return Err(FramaError::InvalidWindow { window, data_len: 0 });
		}
		let mut n = window;
		if n % 2 == 1 {
			n += 1;
	...
	pub fn update(&mut self, high: f64, low: f64, close: f64) -> Option<f64> {
		...
		let new_n = ((self.sc as f64 - self.fc as f64) * ((old_n - 1.0) / (self.sc as f64 - 1.0))) + self.fc as f64;
...
mod tests {
	...
	fn check_frama_zero_window(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		assert!(res.is_err(), "[{}] FRAMA should fail with zero window", test_name);
		Ok(())
	}
	fn check_frama_window_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_frama_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
		...
		let res = frama_with_kernel(&input, kernel);
		assert!(res.is_err(), "[{}] FRAMA should fail with insufficient data", test_name);
		Ok(())
	}
	...
	fn check_frama_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let second_input = FramaInput::from_slices(&first_res.values, &first_res.values, &first_res.values, params);
...
pub fn frama_py<'py>(
	...
	let params = FramaParams { window: Some(window), sc: Some(sc), fc: Some(fc) };
	let input = FramaInput::from_slices(h, l, c, params);
	let kern = validate_kernel(kernel, false)?;

	let out: Vec<f64> = py.allow_threads(|| frama_with_kernel(&input, kern)
		.map(|o| o.values))
...
pub fn frama_batch_py<'py>(
	...
	let windows: Vec<u64> = combos_result.iter().map(|c| c.window.unwrap_or(10) as u64).collect();
	let scs: Vec<u64> = combos_result.iter().map(|c| c.sc.unwrap_or(300) as u64).collect();
	let fcs: Vec<u64> = combos_result.iter().map(|c| c.fc.unwrap_or(1) as u64).collect();
...
impl FramaStreamPy {
	...
	fn new(window: usize, sc: usize, fc: usize) -> PyResult<Self> {
		Ok(Self { 
			inner: FramaStream::try_new(FramaParams{window:Some(window),sc:Some(sc),fc:Some(fc)})
				.map_err(|e| PyValueError::new_err(e.to_string()))? 
		})
	}
...
pub fn frama_into_slice(dst: &mut [f64], input: &FramaInput, kern: Kernel) -> Result<(), FramaError> {
	let ((high, low, close), window, sc, fc, first, len, _warm_from_prepare, chosen) = frama_prepare(input, kern)?;
...
pub fn frama_js(
	...
	let input = FramaInput::from_slices(high, low, close, FramaParams{
		window: Some(window), sc: Some(sc), fc: Some(fc)
	});
...
pub fn frama_into_js(
	...
	unsafe {
		...
		let input = FramaInput::from_slices(h, l, c, FramaParams{ window: Some(window), sc: Some(sc), fc: Some(fc) });

		// handle aliasing
		if out_ptr as *const f64 == high_ptr || out_ptr as *const f64 == low_ptr || out_ptr as *const f64 == close_ptr {
...
pub fn frama_batch_unified_js(high: &[f64], low: &[f64], close: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	let config: FramaBatchConfig =
...
pub fn frama_batch_into_js(
	...
	unsafe {
		...
		let l = std::slice::from_raw_parts(low_ptr, len);
		let c = std::slice::from_raw_parts(close_ptr, len);
		let sweep = FramaBatchRange { window:(w0,w1,ws), sc:(s0,s1,ss), fc:(f0,f1,fs) };
