...
impl<'a> SmmaInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: SmmaParams) -> Self {
		Self {
			data: SmmaData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
...
impl SmmaStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = SmmaParams { period: Some(period) };
		let stream = SmmaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn smma_js(data: &[f64], period: usize) -> Result<Vec<f64>, JsValue> {
	...
	smma_into_slice(&mut output, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn smma_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let output = smma_batch_inner(data, &sweep, Kernel::Auto, false).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn smma_batch_metadata_js(period_start: usize, period_end: usize, period_step: usize) -> Vec<usize> {
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn smma_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, period: usize) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr as *const f64 {
			...
			smma_into_slice(&mut temp, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
			...
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			smma_into_slice(out, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
