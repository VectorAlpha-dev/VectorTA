...
mod tests {
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = HighPassBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
impl HighPassStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = HighPassParams { period: Some(period) };
		let stream = HighPassStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
#[inline]
pub fn highpass_into_slice(dst: &mut [f64], input: &HighPassInput, kern: Kernel) -> Result<(), HighPassError> {
	// Validate input
...
pub fn highpass_js(data: &[f64], period: usize) -> Result<Vec<f64>, JsValue> {
	...
	highpass_into_slice(&mut output, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(all(target_arch = "wasm32", feature = "wasm"))]
#[wasm_bindgen]
pub fn highpass_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, period: usize) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr {
			...
			highpass_into_slice(&mut temp, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
			...
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			highpass_into_slice(out, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
...
pub fn highpass_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	let config: HighPassBatchConfig =
	...
	let output =
		highpass_batch_with_kernel(data, &sweep, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(all(target_arch = "wasm32", feature = "wasm"))]
#[wasm_bindgen]
pub fn highpass_batch_metadata_js(period_start: usize, period_end: usize, period_step: usize) -> Vec<f64> {
