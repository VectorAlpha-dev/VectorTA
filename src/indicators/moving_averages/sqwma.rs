...
impl<'a> SqwmaInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: SqwmaParams) -> Self {
		Self {
			data: SqwmaData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
...
pub fn sqwma_with_kernel(input: &SqwmaInput, kernel: Kernel) -> Result<SqwmaOutput, SqwmaError> {
	...
	unsafe {
		match chosen {
			Kernel::Scalar | Kernel::ScalarBatch => sqwma_scalar(data, &weights, period, first, weight_sum, &mut out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx2 | Kernel::Avx2Batch => sqwma_avx2(data, &weights, period, first, weight_sum, &mut out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx512 | Kernel::Avx512Batch => sqwma_avx512(data, &weights, period, first, weight_sum, &mut out),
			_ => unreachable!(),
		}
	}
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn sqwma_avx512(data: &[f64], weights: &[f64], period: usize, first: usize, weight_sum: f64, out: &mut [f64]) {
	if period <= 32 {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn sqwma_avx2(data: &[f64], weights: &[f64], period: usize, first: usize, weight_sum: f64, out: &mut [f64]) {
...
impl SqwmaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<SqwmaBatchOutput, SqwmaError> {
		SqwmaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
fn sqwma_batch_inner_into(
	...
	if parallel {
		#[cfg(not(target_arch = "wasm32"))]
		out.par_chunks_mut(cols).enumerate().for_each(|(r, s)| work(r, s));
...
#[inline(always)]
pub fn sqwma_batch_slice(data: &[f64], sweep: &SqwmaBatchRange, kern: Kernel) -> Result<SqwmaBatchOutput, SqwmaError> {
...
fn sqwma_batch_inner(
	...
	let warm: Vec<usize> = combos.iter().map(|c| (first + c.period.unwrap() + 1).min(cols)).collect();
	...
	Ok(SqwmaBatchOutput { values, combos, rows, cols })
}
...
mod tests {
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let output = SqwmaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
pub fn sqwma_batch_py<'py>(
	...
	let slice_in = data.as_slice()?;
	let sweep = SqwmaBatchRange { period: period_range };
	let combos = expand_grid(&sweep);
	let rows = combos.len();
...
pub fn sqwma_js(data: &[f64], period: usize) -> Result<Vec<f64>, JsValue> {
	...
	sqwma_into_slice(&mut output, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn sqwma_into_slice(dst: &mut [f64], input: &SqwmaInput, kernel: Kernel) -> Result<(), SqwmaError> {
	let data: &[f64] = match &input.data {
		SqwmaData::Candles { candles, source } => source_type(candles, source),
		SqwmaData::Slice(sl) => sl,
	};
	...
	unsafe {
		match chosen {
			Kernel::Scalar | Kernel::ScalarBatch => sqwma_scalar(data, &weights, period, first, weight_sum, dst),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx2 | Kernel::Avx2Batch => sqwma_avx2(data, &weights, period, first, weight_sum, dst),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx512 | Kernel::Avx512Batch => sqwma_avx512(data, &weights, period, first, weight_sum, dst),
			_ => unreachable!(),
		}
	}
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn sqwma_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, period: usize) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr {
			...
			sqwma_into_slice(&mut temp, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
			...
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			sqwma_into_slice(out, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
...
pub fn sqwma_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let output = sqwma_batch_inner(data, &sweep, Kernel::Auto, false).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn sqwma_batch_into(
	...
	unsafe {
		let data = std::slice::from_raw_parts(in_ptr, len);
		let sweep = SqwmaBatchRange { period: (period_start, period_end, period_step) };
		let combos = expand_grid(&sweep);
		let rows = combos.len();
		if rows == 0 {
		...
		let warm: Vec<usize> = combos.iter().map(|c| (first + c.period.unwrap() + 1).min(cols)).collect();
		let mu = std::slice::from_raw_parts_mut(out.as_mut_ptr() as *mut MaybeUninit<f64>, out.len());
		init_matrix_prefixes(mu, cols, &warm);
