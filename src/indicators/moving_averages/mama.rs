...
impl<'a> MamaInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: MamaParams) -> Self {
		Self {
			data: MamaData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
	...
	unsafe {
		...
		#[cfg(all(target_arch = "wasm32", target_feature = "simd128"))]
		{
			if matches!(chosen, Kernel::Scalar | Kernel::ScalarBatch) {
				...
				for v in &mut mama_values[..WARM] { *v = f64::NAN; }
				for v in &mut fama_values[..WARM] { *v = f64::NAN; }
		...
		match chosen {
			...
			Kernel::Scalar | Kernel::ScalarBatch => {
				mama_scalar_inplace(data, fast_limit, slow_limit, &mut mama_values, &mut fama_values);
			}

			// ---- AVX2 ---------------------------------------------
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx2 | Kernel::Avx2Batch => {
				mama_avx2_inplace(data, fast_limit, slow_limit, &mut mama_values, &mut fama_values);
			}

			// ---- AVX-512 ------------------------------------------
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx512 | Kernel::Avx512Batch => {
				mama_avx512_inplace(data, fast_limit, slow_limit, &mut mama_values, &mut fama_values);
			}
...
pub fn mama_into_slice(
	...
	if dst_mama.len() != data.len() || dst_fama.len() != data.len() {
		return Err(MamaError::NotEnoughData { needed: data.len(), found: dst_mama.len().min(dst_fama.len()) });
	}
	mama_compute_into(input, kern, dst_mama, dst_fama)?;

	const WARM: usize = 10;
	let warm = WARM.min(data.len());
	for v in &mut dst_mama[..warm] { *v = f64::NAN; }
	for v in &mut dst_fama[..warm] { *v = f64::NAN; }
	Ok(())
}
...
#[inline]
pub fn mama_scalar_inplace(data: &[f64], fast_limit: f64, slow_limit: f64, out_mama: &mut [f64], out_fama: &mut [f64]) {
	debug_assert_eq!(data.len(), out_mama.len());
	debug_assert_eq!(data.len(), out_fama.len());
	let len = data.len();
	let warm = 10;
...
impl MamaStream {
	...
	pub fn update(&mut self, value: f64) -> Option<(f64, f64)> {
		...
		if !self.filled && self.pos == 0 { self.filled = true; }
...
impl MamaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<MamaBatchOutput, MamaError> {
		MamaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
pub fn mama_batch_with_kernel(data: &[f64], sweep: &MamaBatchRange, k: Kernel) -> Result<MamaBatchOutput, MamaError> {
	let kernel = match k {
...
#[inline(always)]
pub fn mama_batch_slice(data: &[f64], sweep: &MamaBatchRange, kern: Kernel) -> Result<MamaBatchOutput, MamaError> {
	mama_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn mama_batch_par_slice(data: &[f64], sweep: &MamaBatchRange, kern: Kernel) -> Result<MamaBatchOutput, MamaError> {
...
fn mama_batch_inner(
	...
	if parallel {
		...
		{
			for (row, (m_row, f_row)) in raw_mama.chunks_mut(cols).zip(raw_fama.chunks_mut(cols)).enumerate() {
	...
	let mama_values = unsafe {
		Vec::from_raw_parts(guard_m.as_mut_ptr() as *mut f64, guard_m.len(), guard_m.capacity())
	};
	let fama_values = unsafe {
		Vec::from_raw_parts(guard_f.as_mut_ptr() as *mut f64, guard_f.len(), guard_f.capacity())
	};
...
pub fn mama_batch_inner_into(
	...
	let out_mama_uninit =
		unsafe { std::slice::from_raw_parts_mut(out_mama.as_mut_ptr() as *mut MaybeUninit<f64>, out_mama.len()) };
	let out_fama_uninit =
		unsafe { std::slice::from_raw_parts_mut(out_fama.as_mut_ptr() as *mut MaybeUninit<f64>, out_fama.len()) };
...
mod tests {
			...
			.run(&strat, |(data, fast_limit, slow_limit)| {
				...
				for i in 0..data.len() {
					if i < WARMUP {
						prop_assert!(
							mama_out[i].is_nan(),
							"MAMA should have NaN warmup at index {}, got {}", i, mama_out[i]
						);
						prop_assert!(
							fama_out[i].is_nan(),
							"FAMA should have NaN warmup at index {}, got {}", i, fama_out[i]
						);
					} else {
						prop_assert!(
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = MamaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
mod python_bindings {
	...
	pub fn mama_py<'py>(
		...
		let params = MamaParams { fast_limit: Some(fast_limit), slow_limit: Some(slow_limit) };
	...
	impl MamaStreamPy {
		...
		pub fn new(fast_limit: f64, slow_limit: f64) -> PyResult<Self> {
			...
			let stream = MamaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn mama_js(data: &[f64], fast_limit: f64, slow_limit: f64) -> Result<JsValue, JsValue> {
	...
	let out = MamaResult { values, rows: 2, cols: data.len() };
...
pub fn mama_batch_js(
	data: &[f64], 
	fast_start: f64, fast_end: f64, fast_step: f64,
	slow_start: f64, slow_end: f64, slow_step: f64
	...
	mama_batch_inner_into(data, &sweep, kern, false, &mut mama_values, &mut fama_values)
		.map_err(|e| JsValue::from_str(&e.to_string()))?;

	let out = MamaBatchJsOutput { mama: mama_values, fama: fama_values, combos, rows, cols };
	serde_wasm_bindgen::to_value(&out).map_err(|e| JsValue::from_str(&format!("Serialization error: {}", e)))
}
...
pub fn mama_batch_into(
	...
	unsafe {
		...
		let batch_output =
			mama_batch_with_kernel(data, &range, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
