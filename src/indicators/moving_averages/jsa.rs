...
impl JsaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<JsaBatchOutput, JsaError> {
		JsaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}

pub fn jsa_batch_with_kernel(data: &[f64], sweep: &JsaBatchRange, k: Kernel) -> Result<JsaBatchOutput, JsaError> {
	let kernel = match k {
...
#[inline(always)]
pub fn jsa_batch_slice(data: &[f64], sweep: &JsaBatchRange, kern: Kernel) -> Result<JsaBatchOutput, JsaError> {
	jsa_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn jsa_batch_par_slice(data: &[f64], sweep: &JsaBatchRange, kern: Kernel) -> Result<JsaBatchOutput, JsaError> {
...
impl JsaStreamPy {
	...
	pub fn new(period: usize) -> PyResult<Self> {
		let params = JsaParams { period: Some(period) };
		let stream = JsaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn jsa_batch_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let output = jsa_batch_inner(data, &sweep, Kernel::Auto, false).map_err(|e| JsValue::from_str(&e.to_string()))?;

	let js_output = JsaBatchJsOutput {
		values: output.values,
		periods: output.combos.iter().map(|p| p.period.unwrap_or(30)).collect(),
		rows: output.rows,
		cols: output.cols,
	};
...
pub fn jsa_batch_into(
	...
	unsafe {
		...
		jsa_batch_inner_into(data, &sweep, Kernel::Auto, false, out).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
mod tests {
	...
	fn check_jsa_no_poison(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		for period in &test_periods {
			for source in &test_sources {
				let input = JsaInput::from_candles(&candles, source, JsaParams { period: Some(*period) });
				let output = jsa_with_kernel(&input, kernel)?;
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = JsaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
