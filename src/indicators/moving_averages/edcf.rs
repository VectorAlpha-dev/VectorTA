...
impl<'a> EdcfInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: EdcfParams) -> Self {
		Self {
			data: EdcfData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
...
impl EdcfBuilder {
	...
	pub fn apply(self, c: &Candles) -> Result<EdcfOutput, EdcfError> {
		let p = EdcfParams { period: self.period };
...
fn edcf_prepare<'a>(
	...
	let first = data.iter().position(|x| !x.is_nan()).ok_or(EdcfError::AllValuesNaN)?;
	let period = input.get_period();
	if period == 0 || period > len {
		return Err(EdcfError::InvalidPeriod { period, data_len: len });
	}
	let needed = 2 * period;
	if (len - first) < needed {
...
impl EdcfBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<EdcfBatchOutput, EdcfError> {
		EdcfBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}

pub fn edcf_batch_with_kernel(data: &[f64], sweep: &EdcfBatchRange, k: Kernel) -> Result<EdcfBatchOutput, EdcfError> {
	if data.is_empty() {
		return Err(EdcfError::NoData);
	}
	let kernel = match k {
...
fn expand_grid(r: &EdcfBatchRange) -> Vec<EdcfParams> {
	...
	periods.into_iter().map(|p| EdcfParams { period: Some(p) }).collect()
}

#[inline(always)]
pub fn edcf_batch_slice(data: &[f64], sweep: &EdcfBatchRange, kern: Kernel) -> Result<EdcfBatchOutput, EdcfError> {
	edcf_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn edcf_batch_par_slice(data: &[f64], sweep: &EdcfBatchRange, kern: Kernel) -> Result<EdcfBatchOutput, EdcfError> {
...
mod tests {
	...
	fn check_edcf_partial_params(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
	...
	fn check_edcf_accuracy_last_five(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		skip_if_unsupported!(kernel, test_name);
		let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
		let candles = read_candles_from_csv(file_path)?;
	...
	fn check_edcf_with_default_candles(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		skip_if_unsupported!(kernel, test_name);
		let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
		let candles = read_candles_from_csv(file_path)?;
	...
	fn check_edcf_with_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_edcf_with_no_data(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		skip_if_unsupported!(kernel, test_name);
		let data: [f64; 0] = [];
	...
	fn check_edcf_very_small_data_set(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		skip_if_unsupported!(kernel, test_name);
		let data = [42.0];
		let input = EdcfInput::from_slice(&data, EdcfParams { period: Some(15) });
		let result = edcf_with_kernel(&input, kernel);
	...
	fn check_edcf_with_slice_data_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		skip_if_unsupported!(kernel, test_name);
		let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
		let candles = read_candles_from_csv(file_path)?;
		let first_input = EdcfInput::from_candles(&candles, "close", EdcfParams { period: Some(15) });
		...
		if second_result.values.len() > 240 {
			for (i, &val) in second_result.values.iter().enumerate().skip(240) {
				assert!(!val.is_nan(), "Found NaN in second EDCF output at index {}", i);
			}
		}
		Ok(())
	}

	fn check_edcf_accuracy_nan_check(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		skip_if_unsupported!(kernel, test_name);
		let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
		let candles = read_candles_from_csv(file_path)?;
		let period = 15;
		let input = EdcfInput::from_candles(&candles, "close", EdcfParams { period: Some(period) });
		let result = edcf_with_kernel(&input, kernel)?;
		assert_eq!(result.values.len(), candles.close.len());
	...
	fn check_edcf_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		skip_if_unsupported!(kernel, test_name);
		let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
		let candles = read_candles_from_csv(file_path)?;
	...
	fn check_edcf_no_poison(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		for period in &test_periods {
			for source in &test_sources {
				let input = EdcfInput::from_candles(&candles, source, EdcfParams { period: Some(*period) });
	...
	fn check_edcf_property(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		use proptest::prelude::*;
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_edcf_invalid_kernel(test_name: &str, _kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		let data = [1.0, 2.0, 3.0];
		let range = EdcfBatchRange::default();
		let res = edcf_batch_with_kernel(&data, &range, Kernel::Avx2);
		assert!(matches!(res, Err(EdcfError::InvalidKernel)), "{}", test_name);
		Ok(())
	}
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let output = EdcfBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
impl EdcfStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = EdcfParams { period: Some(period) };
		let stream = EdcfStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn edcf_batch_py<'py>(
	...
	let sweep = EdcfBatchRange { period: period_range };
...
pub fn edcf_js(data: &[f64], period: usize) -> Result<Vec<f64>, JsValue> {
	...
	edcf_into_slice(&mut output, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn edcf_batch_metadata_js(period_start: usize, period_end: usize, period_step: usize) -> Result<Vec<f64>, JsValue> {
	let sweep = EdcfBatchRange {
		period: (period_start, period_end, period_step),
	};
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn edcf_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, period: usize) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr {
			...
			edcf_into_slice(&mut temp, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
			...
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			edcf_into_slice(out, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
...
pub fn edcf_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let output = edcf_batch_inner(data, &sweep, Kernel::Auto, false).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn edcf_batch_into(
	...
	unsafe {
		...
		edcf_batch_inner_into(data, &sweep, Kernel::Auto, false, out).map_err(|e| JsValue::from_str(&e.to_string()))?;
