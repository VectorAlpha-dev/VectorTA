...
//! CUDA: Wrapper present (FP32 rolling/tiled); Python interop returns CAI v3 + DLPack handles and holds the CUDA context alive.
...
// =============== Python: Device VRAM handle (CAI v3 + DLPack) ===============
...
impl DeviceArrayF32Py {
    ...
    fn __cuda_array_interface__<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyDict>> {
        ...
        let ptr: usize = self.inner.buf.as_device_ptr().as_raw() as usize;
        d.set_item("data", (ptr, false))?;
        if self.stream_handle != 0 { d.set_item("stream", self.stream_handle)?; }
        d.set_item("version", 3)?;
        Ok(d)
    }

    fn __dlpack_device__(&self) -> PyResult<(i32, i32)> { Ok((2, self._device_id as i32)) }
    ...
    fn __dlpack__<'py>(&mut self, py: Python<'py>, _stream: Option<i64>) -> PyResult<PyObject> {
        ...
        unsafe extern "C" fn capsule_destructor(capsule: *mut pyo3::ffi::PyObject) {
            ...
            if !ptr.is_null() {
                ...
                if let Some(deleter) = (*mt).deleter { deleter(mt) }
            }
        }

        // Move VRAM buffer into holder to tie lifetime to capsule
        let dummy = unsafe { cust::memory::DeviceBuffer::<f32>::uninitialized(0) }
...
pub fn edcf_cuda_batch_dev_py(
    ...
    let (inner, ctx, dev_id) = py.allow_threads(|| {
        ...
        let out = cuda
            ...
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
        Ok::<_, PyErr>((out, ctx, dev_id))
    })?;

    Ok(DeviceArrayF32Py::new_from_rust(inner, 0, ctx, dev_id))
}
