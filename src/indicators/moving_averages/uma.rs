...
impl<'a> UmaInput<'a> {
    ...
    pub fn from_candles(c: &'a Candles, s: &'a str, p: UmaParams) -> Self {
        Self {
            data: UmaData::Candles { candles: c, source: s },
            params: p,
...
impl UmaStream {
    ...
    pub fn update_with_volume(&mut self, value: f64, volume: Option<f64>) -> Option<f64> {
        ...
        if self.buffer.len() > min_needed * 2 {
            ...
            self.volume_buffer.drain(0..self.volume_buffer.len() - min_needed * 2);
        }
...
pub enum UmaError {
    ...
    MinLengthGreaterThanMaxLength { min_length: usize, max_length: usize },
...
fn uma_prepare<'a>(input: &'a UmaInput) -> Result<(&'a [f64], usize, usize, usize, f64), UmaError> {
    ...
    let len = data.len();
    if len == 0 { return Err(UmaError::EmptyInputData); }

    let first = data.iter().position(|x| !x.is_nan()).ok_or(UmaError::AllValuesNaN)?;
    ...
    if max_length == 0 || max_length > len {
        return Err(UmaError::InvalidMaxLength { max_length, data_len: len });
    }
    if min_length == 0 { return Err(UmaError::InvalidMinLength { min_length }); }
    if min_length > max_length { return Err(UmaError::MinLengthGreaterThanMaxLength { min_length, max_length }); }
    if smooth_length == 0 { return Err(UmaError::InvalidSmoothLength { smooth_length }); }
    if accelerator < 1.0 { return Err(UmaError::InvalidAccelerator { accelerator }); }
    if len - first < max_length {
        return Err(UmaError::NotEnoughValidData { needed: max_length, valid: len - first });
    }
...
fn uma_core_into(
    ...
    let mean = sma(&SmaInput::from_slice(data, SmaParams { period: Some(max_length) }))
        .map_err(|e| UmaError::DependencyError(e.to_string()))?
        .values;
    let std_dev = deviation(&DeviationInput::from_slice(data, DeviationParams { period: Some(max_length), devtype: Some(0) }))
        .map_err(|e| UmaError::DependencyError(e.to_string()))?
        .values;
    ...
    for i in warmup_end..len {
        ...
        if mean_val.is_nan() || std_val.is_nan() { continue; }
        ...
        let mf = if let Some(vol) = input.volume {
            ...
            if v_i == 0.0 || v_i.is_nan() {
                ...
                let window_end = i + 1;
                let rsi_input = RsiInput::from_slice(&data[window_start..window_end], RsiParams { period: Some(len_r) });
                rsi(&rsi_input).ok().and_then(|r| r.values.last().copied()).unwrap_or(50.0)
            } else {
                ...
                match &input.data {
                    UmaData::Candles { candles, .. } => {
                        ...
                        mfi(&MfiInput::from_slices(&tp, &vv, MfiParams { period: Some(len_r) }))
                            .ok().and_then(|r| r.values.last().copied()).unwrap_or(50.0)
                    }
                    UmaData::Slice(_) => {
...
pub fn uma_with_kernel(input: &UmaInput, kernel: Kernel) -> Result<UmaOutput, UmaError> {
    ...
    if smooth > 1 {
        let w = wma(&WmaInput::from_slice(&out, WmaParams { period: Some(smooth) }))
            .map_err(|e| UmaError::DependencyError(e.to_string()))?
            .values;
...
pub fn uma_into_slice(dst: &mut [f64], input: &UmaInput, kern: Kernel) -> Result<(), UmaError> {
    let (data, first, min_len, max_len, accel) = uma_prepare(input)?;
    if dst.len() != data.len() {
        return Err(UmaError::InvalidMaxLength { max_length: dst.len(), data_len: data.len() });
    }
    ...
    if smooth > 1 {
        ...
        let tmp = wma(&WmaInput::from_slice(dst, WmaParams { period: Some(smooth) }))
            .map_err(|e| UmaError::DependencyError(e.to_string()))?
            .values;
...
impl UmaBatchBuilder {
    ...
    #[inline]
    pub fn apply_candles(self, candles: &Candles, source: &str) -> Result<UmaBatchOutput, UmaError> {
    ...
    #[inline]
    pub fn apply_slice(self, data: &[f64], volume: Option<&[f64]>) -> Result<UmaBatchOutput, UmaError> {
        let sweep = UmaBatchRange {
            accelerator: self.accelerator_range,
            min_length: self.min_length_range,
            max_length: self.max_length_range,
    ...
    pub fn with_default_slice(data: &[f64], volume: Option<&[f64]>, k: Kernel) -> Result<UmaBatchOutput, UmaError> {
        UmaBatchBuilder::new().kernel(k).apply_slice(data, volume)
    }

    pub fn with_default_candles(c: &Candles) -> Result<UmaBatchOutput, UmaError> {
        UmaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
    }
}
...
pub fn expand_grid_uma(r: &UmaBatchRange) -> Vec<UmaParams> {
    ...
    for &a in &accs {
        for &min in &mins {
            for &max in &maxs {
                for &s in &smooths {
                    if min <= max { // Only valid combinations
                        out.push(UmaParams {
                            accelerator: Some(a),
                            min_length: Some(min),
                            max_length: Some(max),
...
pub fn uma_batch_with_kernel(
    ...
    let kernel = match k {
        ...
        _ => return Err(UmaError::InvalidMaxLength { max_length: 0, data_len: 0 }),
    };
...
fn uma_batch_inner_into(
    ...
    if combos.is_empty() { return Err(UmaError::InvalidMaxLength { max_length: 0, data_len: 0 }); }
    ...
    if parallel {
        use rayon::prelude::*;
        out.par_chunks_mut(cols).enumerate().try_for_each(|(r, s)| do_row(r, s))?;
    } else {
...
fn uma_batch_inner(
    ...
    let cols = data.len();
    let rows = combos.len();
    if cols == 0 { return Err(UmaError::EmptyInputData); }
    ...
    Ok(UmaBatchOutput { values, combos, rows, cols })
}
...
pub struct UmaStreamPy { stream: UmaStream }

#[cfg(feature = "python")]
#[pymethods]
impl UmaStreamPy {
    #[new]
    pub fn new(accelerator: f64, min_length: usize, max_length: usize, smooth_length: usize) -> PyResult<Self> {
        let params = UmaParams {
            accelerator: Some(accelerator),
            min_length: Some(min_length),
            max_length: Some(max_length),
        ...
        let stream = UmaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
        Ok(Self { stream })
    }
    ...
    pub fn reset(&mut self) { self.stream.reset(); }
}
...
pub fn uma_batch_py<'py>(
    ...
    dict.set_item("accelerators",
        combos.iter().map(|c| c.accelerator.unwrap_or(1.0)).collect::<Vec<_>>().into_pyarray(py))?;
    dict.set_item("min_lengths",
        combos.iter().map(|c| c.min_length.unwrap_or(5) as u64).collect::<Vec<_>>().into_pyarray(py))?;
    dict.set_item("max_lengths",
        combos.iter().map(|c| c.max_length.unwrap_or(50) as u64).collect::<Vec<_>>().into_pyarray(py))?;
    dict.set_item("smooth_lengths",
        combos.iter().map(|c| c.smooth_length.unwrap_or(4) as u64).collect::<Vec<_>>().into_pyarray(py))?;
    
    // Add combos list for compatibility with tests
    let combo_list: Vec<Bound<'py, PyDict>> = combos.iter().map(|c| {
        let d = PyDict::new(py);
        d.set_item("accelerator", c.accelerator.unwrap_or(1.0)).unwrap();
        d.set_item("min_length", c.min_length.unwrap_or(5)).unwrap();
        d.set_item("max_length", c.max_length.unwrap_or(50)).unwrap();
        d.set_item("smooth_length", c.smooth_length.unwrap_or(4)).unwrap();
        d.into()
    }).collect();
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn uma_js(data: &[f64], accelerator: f64, min_length: usize, max_length: usize, smooth_length: usize, volume: Option<Vec<f64>>) -> Result<JsValue, JsValue> {
    let params = UmaParams {
        accelerator: Some(accelerator),
        min_length: Some(min_length),
        max_length: Some(max_length),
...
pub fn uma_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
    ...
    let js = UmaBatchJsOutput { values: out.values, combos: out.combos, rows: out.rows, cols: out.cols };
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn uma_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, accelerator: f64, min_length: usize, max_length: usize, smooth_length: usize) -> Result<(), JsValue> {
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn uma_stream_new(accelerator: f64, min_length: usize, max_length: usize, smooth_length: usize) -> *mut UmaStream {
    let params = UmaParams {
        accelerator: Some(accelerator),
        min_length: Some(min_length),
        max_length: Some(max_length),
...
pub fn uma_stream_update(stream: *mut UmaStream, value: f64) -> Option<f64> {
    ...
    unsafe {
        (*stream).update_with_volume(value, None)
    }
}

#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn uma_stream_update_with_volume(stream: *mut UmaStream, value: f64, volume: f64) -> Option<f64> {
    if stream.is_null() {
        return None;
    }
    unsafe {
        (*stream).update_with_volume(value, Some(volume))
    }
}
...
pub fn uma_get_view(ptr: *mut f64, len: usize) -> js_sys::Float64Array {
    unsafe {
        js_sys::Float64Array::view(std::slice::from_raw_parts(ptr, len))
    }
}

#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn uma_update(ptr: *mut f64, len: usize, accelerator: f64, min_length: usize, max_length: usize, smooth_length: usize) -> Result<(), JsValue> {
    if ptr.is_null() {
...
pub fn uma_batch_js(
    ...
    if accelerator_range.len() != 3 || min_length_range.len() != 3 || 
       max_length_range.len() != 3 || smooth_length_range.len() != 3 {
        return Err(JsValue::from_str("All range arrays must have exactly 3 elements: [start, end, step]"));
    }
    
    let sweep = UmaBatchRange {
        accelerator: (accelerator_range[0], accelerator_range[1], accelerator_range[2]),
        min_length: (min_length_range[0], min_length_range[1], min_length_range[2]),
        max_length: (max_length_range[0], max_length_range[1], max_length_range[2]),
        smooth_length: (smooth_length_range[0], smooth_length_range[1], smooth_length_range[2]),
    };
    ...
    js_sys::Reflect::set(&obj, &JsValue::from_str("rows"), &JsValue::from_f64(result.rows as f64))?;
    js_sys::Reflect::set(&obj, &JsValue::from_str("cols"), &JsValue::from_f64(result.cols as f64))?;
...
mod tests {
    ...
    fn check_uma_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        assert!(res.is_err(), "[{}] UMA should fail with zero max_length", test_name);
        Ok(())
    }

    fn check_uma_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        skip_if_unsupported!(kernel, test_name);
    ...
    fn check_uma_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let res = uma_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] UMA should fail with insufficient data", test_name);
        Ok(())
    }
    ...
    fn check_uma_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let valid_count = second_result.values.iter()
            .filter(|&&v| !v.is_nan())
            .count();
        assert!(valid_count > 0, "[{}] UMA reinput should produce valid values", test_name);
    ...
    fn check_uma_nan_handling(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let valid_count = res.values[60..].iter()
            .filter(|&&v| !v.is_nan())
            .count();
        assert!(valid_count > 0, "[{}] UMA should handle NaN prefix", test_name);
    ...
    fn check_uma_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let batch_valid: Vec<f64> = batch_output.iter()
            .filter(|&&v| !v.is_nan())
            .copied()
            .collect();
        let stream_valid: Vec<f64> = stream_values.iter()
            .filter(|&&v| !v.is_nan())
            .copied()
            .collect();
    ...
    fn check_uma_property(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        use proptest::prelude::*;
        skip_if_unsupported!(kernel, test_name);

        let strat = (5usize..=20, 20usize..=50, 2usize..=8, 1.0f64..3.0)
            .prop_flat_map(|(min_len, max_len, smooth_len, acc)| {
                (
                    prop::collection::vec(
        ...
        proptest::test_runner::TestRunner::default()
            .run(&strat, |(data, min_len, max_len, smooth_len, acc)| {
                ...
                let UmaOutput { values: ref_out } = uma_with_kernel(&input, Kernel::Scalar).unwrap();
                ...
                for i in 0..data.len() {
                    ...
                    if !y.is_finite() || !r.is_finite() {
                        prop_assert!(y.to_bits() == r.to_bits(), "finite/NaN mismatch idx {i}: {y} vs {r}");
                        continue;
                    }
    ...
    fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let output = UmaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
        ...
        assert!(valid_count > 0, "[{}] Batch should produce valid values", test);
    ...
    fn check_batch_no_poison(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for (cfg_idx, &(a_start, a_end, a_step, min_start, min_end, min_step, 
                       max_start, max_end, max_step, s_start, s_end, s_step)) in
