...
#[inline(always)]
fn swma_compute_into(data: &[f64], weights: &[f64], period: usize, first: usize, kernel: Kernel, out: &mut [f64]) {
	unsafe {
		match kernel {
...
#[inline]
pub fn swma_scalar(data: &[f64], weights: &[f64], period: usize, first_val: usize, out: &mut [f64]) {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn swma_avx512(data: &[f64], weights: &[f64], period: usize, first_valid: usize, out: &mut [f64]) {
	if period <= 32 {
...
unsafe fn swma_avx2(data: &[f64], weights: &[f64], period: usize, first_valid: usize, out: &mut [f64]) {
	swma_scalar(data, weights, period, first_valid, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f,fma")]
unsafe fn swma_avx512_short(data: &[f64], weights: &[f64], period: usize, first_valid: usize, out: &mut [f64]) {
	swma_scalar(data, weights, period, first_valid, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f,avx512dq,fma")]
unsafe fn swma_avx512_long(data: &[f64], weights: &[f64], period: usize, first_valid: usize, out: &mut [f64]) {
	swma_scalar(data, weights, period, first_valid, out)
}
...
impl SwmaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<SwmaBatchOutput, SwmaError> {
		SwmaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}

pub fn swma_batch_with_kernel(data: &[f64], sweep: &SwmaBatchRange, k: Kernel) -> Result<SwmaBatchOutput, SwmaError> {
	let kernel = match k {
		Kernel::Auto => detect_best_batch_kernel(),
		other if other.is_batch() => other,
		_ => return Err(SwmaError::InvalidPeriod { period: 0, data_len: 0 }),
	};
...
#[inline(always)]
pub fn swma_batch_slice(data: &[f64], sweep: &SwmaBatchRange, kern: Kernel) -> Result<SwmaBatchOutput, SwmaError> {
	swma_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn swma_batch_par_slice(data: &[f64], sweep: &SwmaBatchRange, kern: Kernel) -> Result<SwmaBatchOutput, SwmaError> {
...
#[inline(always)]
unsafe fn swma_row_scalar(data: &[f64], first: usize, period: usize, w_ptr: *const f64, out: &mut [f64]) {
	let p4 = period & !3;
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[target_feature(enable = "avx2,fma")]
unsafe fn swma_row_avx2(data: &[f64], first: usize, period: usize, w_ptr: *const f64, out: &mut [f64]) {
	swma_row_scalar(data, first, period, w_ptr, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f,avx512dq,fma")]
pub unsafe fn swma_row_avx512(data: &[f64], first: usize, period: usize, w_ptr: *const f64, out: &mut [f64]) {
	if period <= 32 {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f,fma")]
unsafe fn swma_row_avx512_short(data: &[f64], first: usize, period: usize, w_ptr: *const f64, out: &mut [f64]) {
	swma_row_scalar(data, first, period, w_ptr, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f,avx512dq,fma")]
unsafe fn swma_row_avx512_long(data: &[f64], first: usize, period: usize, w_ptr: *const f64, out: &mut [f64]) {
	swma_row_scalar(data, first, period, w_ptr, out)
}
...
mod tests {
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let output = SwmaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
impl SwmaStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = SwmaParams { period: Some(period) };
		let stream = SwmaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn swma_batch_py<'py>(
	...
	let sweep = SwmaBatchRange { period: period_range };
...
pub fn swma_js(data: &[f64], period: usize) -> Result<Vec<f64>, JsValue> {
	...
	swma_into_slice(&mut output, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn swma_batch_metadata_js(period_start: usize, period_end: usize, period_step: usize) -> Result<Vec<f64>, JsValue> {
	let sweep = SwmaBatchRange {
		period: (period_start, period_end, period_step),
	};
...
pub fn swma_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let output = swma_batch_with_kernel(data, &sweep, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn swma_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, period: usize) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr {
			...
			swma_into_slice(&mut temp, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
			...
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			swma_into_slice(out, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
...
pub fn swma_batch_into(
	...
	unsafe {
		...
		swma_batch_inner_into(data, &sweep, Kernel::Auto, false, out).map_err(|e| JsValue::from_str(&e.to_string()))?;
