...
fn alma_compute_into(
	...
	unsafe {
		...
		match kernel {
			Kernel::Scalar | Kernel::ScalarBatch => alma_scalar(data, weights, period, first, inv_n, out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx2 | Kernel::Avx2Batch => alma_avx2(data, weights, period, first, inv_n, out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx512 | Kernel::Avx512Batch => alma_avx512(data, weights, period, first, inv_n, out),
...
#[inline]
#[target_feature(enable = "avx512f")]
pub fn alma_avx512(data: &[f64], weights: &[f64], period: usize, first_valid: usize, inv_norm: f64, out: &mut [f64]) {
	if period <= 32 {
...
impl AlmaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<AlmaBatchOutput, AlmaError> {
		AlmaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}

pub fn alma_batch_with_kernel(data: &[f64], sweep: &AlmaBatchRange, k: Kernel) -> Result<AlmaBatchOutput, AlmaError> {
	let kernel = match k {
...
#[inline(always)]
pub fn alma_batch_slice(data: &[f64], sweep: &AlmaBatchRange, kern: Kernel) -> Result<AlmaBatchOutput, AlmaError> {
	alma_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn alma_batch_par_slice(data: &[f64], sweep: &AlmaBatchRange, kern: Kernel) -> Result<AlmaBatchOutput, AlmaError> {
...
mod tests {
	...
	fn check_alma_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		assert!(res.is_err(), "[{}] ALMA should fail with zero period", test_name);
		Ok(())
	}

	fn check_alma_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_alma_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
		...
		let res = alma_with_kernel(&input, kernel);
		assert!(res.is_err(), "[{}] ALMA should fail with insufficient data", test_name);
		Ok(())
	}
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = AlmaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
impl AlmaStreamPy {
	...
	fn new(period: usize, offset: f64, sigma: f64) -> PyResult<Self> {
		...
		let stream = AlmaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
