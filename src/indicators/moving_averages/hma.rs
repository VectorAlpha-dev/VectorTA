...
impl HmaBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<HmaBatchOutput, HmaError> {
		HmaBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}

pub fn hma_batch_with_kernel(data: &[f64], sweep: &HmaBatchRange, k: Kernel) -> Result<HmaBatchOutput, HmaError> {
	let kernel = match k {
...
#[inline(always)]
pub fn hma_batch_slice(data: &[f64], sweep: &HmaBatchRange, kern: Kernel) -> Result<HmaBatchOutput, HmaError> {
	hma_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn hma_batch_par_slice(data: &[f64], sweep: &HmaBatchRange, kern: Kernel) -> Result<HmaBatchOutput, HmaError> {
...
pub fn hma_batch_py<'py>(
	...
	let sweep = HmaBatchRange { period: period_range };
	...
	let combos = py
		.allow_threads(|| {
			...
			hma_batch_inner_into(slice_in, &sweep, simd, true, slice_out).map(|(combos, _, _)| combos)
		})
		.map_err(|e| PyValueError::new_err(e.to_string()))?;
...
impl HmaStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = HmaParams { period: Some(period) };
		let stream = HmaStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn hma_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let output = hma_batch_inner(data, &sweep, kernel, false).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn hma_batch_metadata_js(period_start: usize, period_end: usize, period_step: usize) -> Vec<f64> {
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn hma_into(in_ptr: *const f64, out_ptr: *mut f64, len: usize, period: usize) -> Result<(), JsValue> {
	...
	unsafe {
		...
		if in_ptr == out_ptr {
			...
			hma_into_slice(&mut temp, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
			...
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			hma_into_slice(out, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
...
pub fn hma_batch_into(
	...
	unsafe {
		...
		hma_batch_inner_into(data, &sweep, kernel, false, out).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
mod tests {
	...
	fn check_hma_empty_input(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		assert!(matches!(res, Err(HmaError::NoData)), "[{}] expected NoData", test_name);
		Ok(())
	}
	...
	fn check_hma_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let candles = read_candles_from_csv(file_path)?;
		let period = 5;
		let input = HmaInput::from_candles(&candles, "close", HmaParams { period: Some(period) });
		let batch_output = hma_with_kernel(&input, kernel)?.values;

		let mut stream = HmaStream::try_new(HmaParams { period: Some(period) })?;
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = HmaBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
