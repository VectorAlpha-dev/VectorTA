...
impl<'a> WildersInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: WildersParams) -> Self {
		Self {
			data: WildersData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
...
#[inline]
#[target_feature(enable = "avx512f")]
pub unsafe fn wilders_avx512_short(data: &[f64], period: usize, first_valid: usize, out: &mut [f64]) {
	wilders_scalar(data, period, first_valid, out)
}
...
#[inline]
#[target_feature(enable = "avx512f")]
pub unsafe fn wilders_avx512_long(data: &[f64], period: usize, first_valid: usize, out: &mut [f64]) {
	wilders_scalar(data, period, first_valid, out)
}
...
mod tests {
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = WildersBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
impl WildersStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = WildersParams { period: Some(period) };
		let stream = WildersStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn wilders_batch_py<'py>(
	...
	let sweep = WildersBatchRange { period: period_range };
