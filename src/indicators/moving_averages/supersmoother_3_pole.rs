...
pub fn supersmoother_3_pole_with_kernel(
	...
	unsafe {
		match chosen {
			Kernel::Scalar | Kernel::ScalarBatch => supersmoother_3_pole_scalar(data, period, first, &mut out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx2 | Kernel::Avx2Batch => supersmoother_3_pole_avx2(data, period, first, &mut out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx512 | Kernel::Avx512Batch => supersmoother_3_pole_avx512(data, period, first, &mut out),
			_ => unreachable!(),
		}
	}
...
pub fn supersmoother_3_pole_batch_inner_into(
	...
	let mut raw =
		unsafe { std::slice::from_raw_parts_mut(output.as_mut_ptr() as *mut std::mem::MaybeUninit<f64>, output.len()) };
	unsafe { init_matrix_prefixes(&mut raw, cols, warm) };
...
mod tests {
	...
	fn check_supersmoother_3_pole_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let candles = read_candles_from_csv(file_path)?;
		let first_input =
			SuperSmoother3PoleInput::from_candles(&candles, "close", SuperSmoother3PoleParams { period: Some(14) });
		let first_result = supersmoother_3_pole_with_kernel(&first_input, kernel)?;
		let second_input =
			SuperSmoother3PoleInput::from_slice(&first_result.values, SuperSmoother3PoleParams { period: Some(7) });
	...
	fn check_supersmoother_3_pole_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let period = 14;
		let input =
			SuperSmoother3PoleInput::from_candles(&candles, "close", SuperSmoother3PoleParams { period: Some(period) });
...
impl SuperSmoother3PoleStreamPy {
	...
	fn new(period: usize) -> PyResult<Self> {
		let params = SuperSmoother3PoleParams { period: Some(period) };
		let stream = SuperSmoother3PoleStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn supersmoother_3_pole_batch_py<'py>(
	...
	let sweep = SuperSmoother3PoleBatchRange { period: period_range };
	...
	py.allow_threads(|| {
		...
		supersmoother_3_pole_batch_inner_into(slice_in, &combos, first, &warm, cols, simd, true, slice_out);
	});
...
pub fn supersmoother_3_pole_into_slice(
	...
	unsafe {
		match chosen {
			Kernel::Scalar | Kernel::ScalarBatch => supersmoother_3_pole_scalar(data, period, first, dst),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx2 | Kernel::Avx2Batch => supersmoother_3_pole_avx2(data, period, first, dst),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx512 | Kernel::Avx512Batch => supersmoother_3_pole_avx512(data, period, first, dst),
			_ => unreachable!(),
		}
	}
...
pub fn supersmoother_3_pole_batch(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	Reflect::set(&result, &JsValue::from_str("rows"), &JsValue::from_f64(rows as f64))?;
	Reflect::set(&result, &JsValue::from_str("cols"), &JsValue::from_f64(cols as f64))?;
	Reflect::set(
		&result,
