...
pub fn correl_hl_with_kernel(input: &CorrelHlInput, kernel: Kernel) -> Result<CorrelHlOutput, CorrelHlError> {
    let (high, low) = match &input.data {
        CorrelHlData::Candles { candles } => {
            let high = candles.select_candle_field("high").map_err(|_e| CorrelHlError::EmptyData)?;
            let low = candles.select_candle_field("low").map_err(|_e| CorrelHlError::EmptyData)?;
            (high, low)
        }
    ...
    let first_valid_idx = match high.iter().zip(low.iter()).position(|(&h, &l)| !h.is_nan() && !l.is_nan()) {
        Some(idx) => idx,
    ...
    unsafe {
        match chosen {
            Kernel::Scalar | Kernel::ScalarBatch => {
                correl_hl_scalar(high, low, period, first_valid_idx, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 | Kernel::Avx2Batch => {
                correl_hl_avx2(high, low, period, first_valid_idx, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                correl_hl_avx512(high, low, period, first_valid_idx, &mut out)
            }
            _ => unreachable!(),
        }
    }
...
mod tests {
    ...
    fn check_correl_hl_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let input = CorrelHlInput::from_slices(&high, &low, params);
        let result = correl_hl_with_kernel(&input, kernel);
        assert!(result.is_err(), "[{}] correl_hl should fail with zero period", test_name);
        Ok(())
    }

    fn check_correl_hl_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        skip_if_unsupported!(kernel, test_name);
        let high = [1.0, 2.0, 3.0];
        let low = [1.0, 2.0, 3.0];
        ...
        let input = CorrelHlInput::from_slices(&high, &low, params);
        let result = correl_hl_with_kernel(&input, kernel);
        assert!(result.is_err(), "[{}] correl_hl should fail with period exceeding length", test_name);
        Ok(())
    }
    ...
    fn check_correl_hl_data_length_mismatch(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let input = CorrelHlInput::from_slices(&high, &low, params);
        let result = correl_hl_with_kernel(&input, kernel);
        assert!(result.is_err(), "[{}] correl_hl should fail on length mismatch", test_name);
        Ok(())
    }
