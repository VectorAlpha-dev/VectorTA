...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub unsafe fn ui_avx512_short(data: &[f64], period: usize, scalar: f64, first: usize, out: &mut [f64]) {
	ui_scalar(data, period, scalar, first, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub unsafe fn ui_avx512_long(data: &[f64], period: usize, scalar: f64, first: usize, out: &mut [f64]) {
	ui_scalar(data, period, scalar, first, out)
}
...
#[inline(always)]
pub fn ui_batch_slice(data: &[f64], sweep: &UiBatchRange, kern: Kernel) -> Result<UiBatchOutput, UiError> {
	ui_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn ui_batch_par_slice(data: &[f64], sweep: &UiBatchRange, kern: Kernel) -> Result<UiBatchOutput, UiError> {
...
mod tests {
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		let output = UiBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
