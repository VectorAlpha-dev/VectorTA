...
impl<'a> FisherInput<'a> {
    ...
    pub fn from_candles(candles: &'a Candles, params: FisherParams) -> Self {
        Self { data: FisherData::Candles { candles }, params }
    }
    ...
    pub fn from_slices(high: &'a [f64], low: &'a [f64], params: FisherParams) -> Self {
        Self { data: FisherData::Slices { high, low }, params }
    }
    #[inline]
...
pub fn fisher_with_kernel(input: &FisherInput, kernel: Kernel) -> Result<FisherOutput, FisherError> {
    ...
    let first = merged.iter().position(|x| !x.is_nan()).ok_or(FisherError::AllValuesNaN)?;
    if (data_len - first) < period {
        return Err(FisherError::NotEnoughValidData { needed: period, valid: data_len - first });
    }
...
#[inline]
pub fn fisher_scalar(merged: &[f64], period: usize, first: usize, data_len: usize) -> Result<FisherOutput, FisherError> {
    ...
    Ok(FisherOutput { fisher: fisher_vals, signal: signal_vals })
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn fisher_avx512(merged: &[f64], period: usize, first: usize, data_len: usize) -> Result<FisherOutput, FisherError> {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn fisher_avx512_short(merged: &[f64], period: usize, first: usize, data_len: usize) -> Result<FisherOutput, FisherError> {
    fisher_avx512(merged, period, first, data_len)
}
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn fisher_avx512_long(merged: &[f64], period: usize, first: usize, data_len: usize) -> Result<FisherOutput, FisherError> {
    fisher_avx512(merged, period, first, data_len)
}

#[inline]
pub fn fisher_batch_with_kernel(high: &[f64], low: &[f64], sweep: &FisherBatchRange, k: Kernel) -> Result<FisherBatchOutput, FisherError> {
    let kernel = match k {
...
fn fisher_batch_inner(
    ...
    if data_len - first < max_p {
        return Err(FisherError::NotEnoughValidData { needed: max_p, valid: data_len - first });
    }
    let rows = combos.len();
    let cols = data_len;
    ...
    Ok(FisherBatchOutput { fisher, signal, combos, rows, cols })
}

#[inline(always)]
unsafe fn fisher_row_scalar(
    merged: &[f64],
    first: usize,
    period: usize,
    out_fish: &mut [f64],
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn fisher_row_avx512_short(merged: &[f64], first: usize, period: usize, out_fish: &mut [f64], out_signal: &mut [f64]) {
    fisher_row_avx512(merged, first, period, out_fish, out_signal)
}
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn fisher_row_avx512_long(merged: &[f64], first: usize, period: usize, out_fish: &mut [f64], out_signal: &mut [f64]) {
    fisher_row_avx512(merged, first, period, out_fish, out_signal)
}
...
impl FisherStream {
    ...
    fn dot_ring(&mut self) -> (f64, f64) {
        ...
        if self.val1 > 0.99 { self.val1 = 0.999; }
        else if self.val1 < -0.99 { self.val1 = -0.999; }
