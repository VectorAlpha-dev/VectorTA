...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub unsafe fn ppo_avx2(data: &[f64], fast: usize, slow: usize, ma_type: &str, first: usize, out: &mut [f64]) {
	ppo_scalar(data, fast, slow, ma_type, first, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub unsafe fn ppo_avx512(data: &[f64], fast: usize, slow: usize, ma_type: &str, first: usize, out: &mut [f64]) {
	if slow <= 32 {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub unsafe fn ppo_avx512_short(data: &[f64], fast: usize, slow: usize, ma_type: &str, first: usize, out: &mut [f64]) {
	ppo_scalar(data, fast, slow, ma_type, first, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub unsafe fn ppo_avx512_long(data: &[f64], fast: usize, slow: usize, ma_type: &str, first: usize, out: &mut [f64]) {
	ppo_scalar(data, fast, slow, ma_type, first, out)
}
...
#[inline(always)]
pub fn ppo_batch_with_kernel(data: &[f64], sweep: &PpoBatchRange, k: Kernel) -> Result<PpoBatchOutput, PpoError> {
	let kernel = match k {
...
#[inline(always)]
pub fn ppo_batch_slice(data: &[f64], sweep: &PpoBatchRange, kern: Kernel) -> Result<PpoBatchOutput, PpoError> {
	ppo_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn ppo_batch_par_slice(data: &[f64], sweep: &PpoBatchRange, kern: Kernel) -> Result<PpoBatchOutput, PpoError> {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn ppo_row_avx2(data: &[f64], first: usize, fast: usize, slow: usize, ma_type: &str, out: &mut [f64]) {
	ppo_scalar(data, fast, slow, ma_type, first, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn ppo_row_avx512(data: &[f64], first: usize, fast: usize, slow: usize, ma_type: &str, out: &mut [f64]) {
	if slow <= 32 {
...
mod tests {
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = PpoBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
...
impl PpoStreamPy {
	#[new]
	fn new(fast_period: Option<usize>, slow_period: Option<usize>, ma_type: Option<&str>) -> PyResult<Self> {
		let params = PpoParams {
			fast_period,
			slow_period,
			ma_type: ma_type.map(|s| s.to_string()),
		};
		let stream = PpoStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn ppo_js(data: &[f64], fast_period: usize, slow_period: usize, ma_type: &str) -> Result<Vec<f64>, JsValue> {
	let params = PpoParams {
