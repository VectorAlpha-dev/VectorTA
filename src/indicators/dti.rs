...
pub fn dti_with_kernel(input: &DtiInput, kernel: Kernel) -> Result<DtiOutput, DtiError> {
	...
	let warmup_period = r.max(s).max(u) + first_valid_idx;
	let mut out = alloc_with_nan_prefix(len, warmup_period);
	
	#[cfg(all(target_arch = "wasm32", target_feature = "simd128"))]
	{
		if matches!(chosen, Kernel::Scalar | Kernel::ScalarBatch) {
			unsafe { dti_simd128(high, low, r, s, u, first_valid_idx, &mut out) };
	...
	unsafe {
		match chosen {
			...
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx2 | Kernel::Avx2Batch => dti_avx2(high, low, r, s, u, first_valid_idx, &mut out),
			#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
			Kernel::Avx512 | Kernel::Avx512Batch => dti_avx512(high, low, r, s, u, first_valid_idx, &mut out),
			_ => unreachable!(),
		}
	}
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn dti_avx2(high: &[f64], low: &[f64], r: usize, s: usize, u: usize, first_valid_idx: usize, out: &mut [f64]) {
	dti_scalar(high, low, r, s, u, first_valid_idx, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub fn dti_avx512(high: &[f64], low: &[f64], r: usize, s: usize, u: usize, first_valid_idx: usize, out: &mut [f64]) {
	if r.max(s).max(u) <= 32 {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub fn dti_row_avx2(high: &[f64], low: &[f64], r: usize, s: usize, u: usize, first_valid_idx: usize, out: &mut [f64]) {
...
fn dti_batch_inner(
	...
	let warm: Vec<usize> = combos
		.iter()
		.map(|c| first_valid + c.r.unwrap().max(c.s.unwrap()).max(c.u.unwrap()))
		.collect();
	init_matrix_prefixes(&mut buf_mu, cols, &warm);
	
	let mut buf_guard = mem::ManuallyDrop::new(buf_mu);
	let values: &mut [f64] = 
...
mod tests {
	...
	fn check_dti_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		assert!(res.is_err(), "[{}] DTI should fail with zero period", test_name);
		Ok(())
	}
	fn check_dti_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		skip_if_unsupported!(kernel, test_name);
		let high = [10.0, 11.0];
		let low = [9.0, 10.0];
		let params = DtiParams {
