...
impl<'a> AroonOscInput<'a> {
    ...
    pub fn from_candles(candles: &'a Candles, params: AroonOscParams) -> Self {
        Self { data: AroonOscData::Candles { candles }, params }
    }
    #[inline]
    pub fn from_slices_hl(high: &'a [f64], low: &'a [f64], params: AroonOscParams) -> Self {
        Self { data: AroonOscData::SlicesHL { high, low }, params }
    }
    #[inline]
    pub fn with_default_candles(candles: &'a Candles) -> Self {
        Self { data: AroonOscData::Candles { candles }, params: AroonOscParams::default() }
    }
    #[inline]
...
pub fn aroon_osc_with_kernel(input: &AroonOscInput, kernel: Kernel) -> Result<AroonOscOutput, AroonOscError> {
    ...
    let (high, low) = match &input.data {
        AroonOscData::Candles { candles } => {
            if candles.close.is_empty() { return Err(AroonOscError::NoCandles); }
            let high = candles.select_candle_field("high")?;
            let low = candles.select_candle_field("low")?;
            (high, low)
        }
        AroonOscData::SlicesHL { high, low } => {
            if high.is_empty() || low.is_empty() { return Err(AroonOscError::EmptySlices); }
            if high.len() != low.len() {
                return Err(AroonOscError::SlicesLengthMismatch {
                    high_len: high.len(), low_len: low.len()
                });
            }
            (*high, *low)
        }
    ...
    let len = low.len();
    if len < length {
        return Err(AroonOscError::NotEnoughData { required: length, found: len });
    }
    ...
    unsafe {
        match chosen {
            Kernel::Scalar | Kernel::ScalarBatch => {
                aroon_osc_scalar(high, low, length, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 | Kernel::Avx2Batch => {
                aroon_osc_avx2(high, low, length, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                aroon_osc_avx512(high, low, length, &mut out)
            }
            _ => unreachable!(),
        }
    }
...
pub fn aroon_osc_scalar(high: &[f64], low: &[f64], length: usize, out: &mut [f64]) {
    ...
    for i in (window - 1)..len {
        ...
        for j in (start + 1)..=i {
            let h_val = high[j];
            if h_val > highest_val { highest_val = h_val; highest_idx = j; }
            let l_val = low[j];
            if l_val < lowest_val { lowest_val = l_val; lowest_idx = j; }
        }
...
pub fn aroon_osc_batch_with_kernel(
    high: &[f64], low: &[f64], sweep: &AroonOscBatchRange, k: Kernel
) -> Result<AroonOscBatchOutput, AroonOscError> {
    let kernel = match k {
        Kernel::Auto => detect_best_batch_kernel(),
        other if other.is_batch() => other,
        _ => { return Err(AroonOscError::InvalidLength { length: 0 }); }
    };
...
pub fn aroon_osc_batch_slice(
    high: &[f64], low: &[f64], sweep: &AroonOscBatchRange, kern: Kernel
) -> Result<AroonOscBatchOutput, AroonOscError> {
...
fn aroon_osc_batch_inner(
    high: &[f64], low: &[f64], sweep: &AroonOscBatchRange, kern: Kernel, parallel: bool
) -> Result<AroonOscBatchOutput, AroonOscError> {
    let combos = expand_grid(sweep);
    if combos.is_empty() { return Err(AroonOscError::InvalidLength { length: 0 }); }
    let len = high.len();
    if high.len() != low.len() { return Err(AroonOscError::SlicesLengthMismatch { high_len: high.len(), low_len: low.len() }); }
    let max_l = combos.iter().map(|c| c.length.unwrap()).max().unwrap();
    if len < max_l { return Err(AroonOscError::NotEnoughData { required: max_l, found: len }); }
    let rows = combos.len();
    let cols = len;
    ...
    Ok(AroonOscBatchOutput { values, combos, rows, cols })
}
...
impl AroonOscStream {
    pub fn try_new(params: AroonOscParams) -> Result<Self, AroonOscError> {
        ...
        if length == 0 { return Err(AroonOscError::InvalidLength { length }); }
        Ok(Self {
            length,
    ...
    fn calc_ring(&self) -> f64 {
        ...
        for i in 1..window {
            ...
            if h_val > highest_val { highest_val = h_val; highest_idx = i; }
            let l_val = self.low_buffer[idx];
            if l_val < lowest_val { lowest_val = l_val; lowest_idx = i; }
        }
...
mod tests {
    ...
    fn check_aroonosc_accuracy(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (i, &value) in last_five.iter().enumerate() {
            assert!((value - expected_last_five[i]).abs() < 1e-2, "Aroon Osc mismatch at index {}: expected {}, got {}", i, expected_last_five[i], value);
        }
        let length = 14;
    ...
    fn check_aroonosc_default_candles(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let candles = read_candles_from_csv(file_path)?;
        let input = AroonOscInput::with_default_candles(&candles);
        match input.data { AroonOscData::Candles { .. } => {}, _ => panic!("Expected AroonOscData::Candles variant"), }
        assert!(input.params.length.is_some());
        Ok(())
    }
    fn check_aroonosc_with_slices_data_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        skip_if_unsupported!(kernel, test_name);
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
    ...
    fn check_aroonosc_nan_handling(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if result.values.len() > 50 {
            for i in 50..result.values.len() {
                assert!(!result.values[i].is_nan(), "Expected no NaN after index {}, but found NaN", i);
            }
        }
        Ok(())
    }
