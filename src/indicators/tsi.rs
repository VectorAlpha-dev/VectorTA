...
//!
use crate::indicators::ema::{ema, EmaInput, EmaOutput, EmaParams, EmaError};
use crate::indicators::mom::{mom, MomInput, MomOutput, MomParams, MomError};
...
pub enum TsiError {
    ...
    InvalidPeriod { long_period: usize, short_period: usize, data_len: usize },
    ...
    unsafe {
        match chosen {
            ...
            Kernel::Avx2 | Kernel::Avx2Batch => {
                tsi_avx2(data, long, short, first)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                tsi_avx512(data, long, short, first)
            }
            _ => unreachable!(),
        }
    }
...
pub unsafe fn tsi_scalar(data: &[f64], long: usize, short: usize, first: usize) -> Result<TsiOutput, TsiError> {
    ...
    let abs_mom: Vec<f64> = mom_output.values.iter().map(|&v| if v.is_nan() { f64::NAN } else { v.abs() }).collect();
...
impl TsiStream {
    ...
    pub fn update(&mut self, value: f64) -> Option<f64> {
        let mom = if self.idx == 0 { f64::NAN } else { value - self.buffer[0] };
        self.buffer[0] = value;
        self.idx += 1;
        if mom.is_nan() {
            return None;
...
fn expand_grid(r: &TsiBatchRange) -> Vec<TsiParams> {
    ...
    for &l in &longs {
        for &s in &shorts {
            out.push(TsiParams { long_period: Some(l), short_period: Some(s) });
        }
    }
    out
}
...
fn tsi_batch_inner(
    ...
    if combos.is_empty() {
        return Err(TsiError::InvalidPeriod { long_period: 0, short_period: 0, data_len: 0 });
    }
    ...
    if data.len() - first < max_needed {
        return Err(TsiError::NotEnoughValidData { needed: max_needed, valid: data.len() - first });
    }
    ...
    Ok(TsiBatchOutput { values, combos, rows, cols })
}
...
mod tests {
    ...
    fn check_tsi_partial_params(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let default_params = TsiParams { long_period: None, short_period: None };
    ...
    fn check_tsi_accuracy(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (i, &val) in tsi_result.values[start..].iter().enumerate() {
            ...
            assert!(diff < 1e-7, "[{}] TSI {:?} mismatch at idx {}: got {}, expected {}", test_name, kernel, i, val, expected_last_five[i]);
        }
        Ok(())
    }
    ...
    fn check_tsi_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let params = TsiParams { long_period: Some(0), short_period: Some(13) };
    ...
    fn check_tsi_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let res = tsi_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] TSI should fail with period exceeding length", test_name);
        Ok(())
    }
    ...
    fn check_tsi_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let first_params = TsiParams { long_period: Some(25), short_period: Some(13) };
        let first_input = TsiInput::from_candles(&candles, "close", first_params);
        let first_result = tsi_with_kernel(&first_input, kernel)?;

        let second_params = TsiParams { long_period: Some(25), short_period: Some(13) };
    ...
    fn check_tsi_nan_handling(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let input = TsiInput::from_candles(&candles, "close", TsiParams { long_period: Some(25), short_period: Some(13) });
        let res = tsi_with_kernel(&input, kernel)?;
    ...
    fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (i, &v) in row[start..].iter().enumerate() {
            assert!((v - expected[i]).abs() < 1e-7, "[{test}] default-row mismatch at idx {i}: {v} vs {expected:?}");
        }
        Ok(())
    }
