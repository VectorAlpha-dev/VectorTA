...
impl DpoBuilder {
	...
	pub fn apply(self, c: &Candles) -> Result<DpoOutput, DpoError> {
		let p = DpoParams { period: self.period };
...
#[inline]
pub fn dpo_batch_with_kernel(data: &[f64], sweep: &DpoBatchRange, k: Kernel) -> Result<DpoBatchOutput, DpoError> {
	let kernel = match k {
		Kernel::Auto => detect_best_batch_kernel(),
		other if other.is_batch() => other,
		_ => return Err(DpoError::InvalidPeriod { period: 0, data_len: 0 }),
	};
...
impl DpoBatchBuilder {
	...
	pub fn with_default_candles(c: &Candles) -> Result<DpoBatchOutput, DpoError> {
		DpoBatchBuilder::new().kernel(Kernel::Auto).apply_candles(c, "close")
	}
}
...
#[inline(always)]
pub fn dpo_batch_slice(data: &[f64], sweep: &DpoBatchRange, kern: Kernel) -> Result<DpoBatchOutput, DpoError> {
	dpo_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn dpo_batch_par_slice(data: &[f64], sweep: &DpoBatchRange, kern: Kernel) -> Result<DpoBatchOutput, DpoError> {
...
mod tests {
	...
	fn check_batch_default_row(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
		...
		let output = DpoBatchBuilder::new().kernel(kernel).apply_candles(&c, "close")?;
