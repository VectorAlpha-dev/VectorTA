    ...
    unsafe {
        match chosen {
            ...
            Kernel::Avx2 | Kernel::Avx2Batch => {
                rsx_avx2(data, period, first, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                rsx_avx512(data, period, first, &mut out)
            }
            _ => unreachable!(),
        }
    }
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline]
pub unsafe fn rsx_avx512_short(
    data: &[f64],
    period: usize,
    first: usize,
    out: &mut [f64],
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
unsafe fn rsx_row_avx512_short(
    data: &[f64],
    first: usize,
    period: usize,
    out: &mut [f64],
...
mod tests {
    ...
    fn check_rsx_partial_params(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let output_period_10 =
            rsx_with_kernel(&input_period_10, kernel)?;
