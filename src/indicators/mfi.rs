...
pub fn mfi_with_kernel(input: &MfiInput, kernel: Kernel) -> Result<MfiOutput, MfiError> {
    let (high, low, close, volume): (&[f64], &[f64], &[f64], &[f64]) = match &input.data {
        ...
        MfiData::Slices { high, low, close, volume } => (*high, *low, *close, *volume),
    };
    ...
    if period == 0 || period > length {
        return Err(MfiError::InvalidPeriod { period, data_len: length });
    }
    if (length - first_valid_idx) < period {
        return Err(MfiError::NotEnoughValidData { needed: period, valid: length - first_valid_idx });
    }
    ...
    unsafe {
        match chosen {
            ...
            Kernel::Avx2 | Kernel::Avx2Batch => {
                mfi_avx2(high, low, close, volume, period, first_valid_idx, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                mfi_avx512(high, low, close, volume, period, first_valid_idx, &mut out)
            }
            _ => unreachable!(),
        }
    }
...
impl MfiStream {
    ...
    pub fn update(&mut self, high: f64, low: f64, close: f64, volume: f64) -> Option<f64> {
        ...
        Some(if total < 1e-14 { 0.0 } else { 100.0 * (self.pos_sum / total) })
    }
}
...
impl MfiBatchBuilder {
    ...
    pub fn apply_slices(
        self, high: &[f64], low: &[f64], close: &[f64], volume: &[f64]
...
pub fn mfi_batch_slice(
    high: &[f64], low: &[f64], close: &[f64], volume: &[f64],
    sweep: &MfiBatchRange, kern: Kernel
) -> Result<MfiBatchOutput, MfiError> {
...
fn mfi_batch_inner(
    high: &[f64], low: &[f64], close: &[f64], volume: &[f64],
    sweep: &MfiBatchRange, kern: Kernel, parallel: bool
) -> Result<MfiBatchOutput, MfiError> {
...
mod tests {
    ...
    fn check_mfi_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let second_input = MfiInput::from_slices(
            &high_values, &low_values, &close_values, &volume_values, second_params
        );
