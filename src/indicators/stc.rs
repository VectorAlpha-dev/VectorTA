...
impl<'a> StcInput<'a> {
	...
	pub fn from_candles(c: &'a Candles, s: &'a str, p: StcParams) -> Self {
		Self {
			data: StcData::Candles { candles: c, source: s },
			params: p,
		}
	}
	#[inline]
...
pub fn stc_batch_with_kernel(data: &[f64], sweep: &StcBatchRange, k: Kernel) -> Result<StcBatchOutput, StcError> {
	let kernel = match k {
...
#[inline(always)]
pub fn stc_batch_slice(data: &[f64], sweep: &StcBatchRange, kern: Kernel) -> Result<StcBatchOutput, StcError> {
	stc_batch_inner(data, sweep, kern, false)
}

#[inline(always)]
pub fn stc_batch_par_slice(data: &[f64], sweep: &StcBatchRange, kern: Kernel) -> Result<StcBatchOutput, StcError> {
...
fn stc_batch_inner(
	...
	if parallel {
		#[cfg(not(target_arch = "wasm32"))]
		{
			values_slice.par_chunks_mut(cols).enumerate().for_each(|(row, slice)| {
...
#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn stc_row_avx2(data: &[f64], first: usize, prm: &StcParams, out: &mut [f64]) -> Result<(), StcError> {
	stc_row_scalar(data, first, prm, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn stc_row_avx512(data: &[f64], first: usize, prm: &StcParams, out: &mut [f64]) -> Result<(), StcError> {
...
impl StcStreamPy {
	#[new]
	fn new(fast_period: usize, slow_period: usize, k_period: usize, d_period: usize) -> PyResult<Self> {
		let params = StcParams {
			fast_period: Some(fast_period),
			slow_period: Some(slow_period),
			k_period: Some(k_period),
		...
		let stream = StcStream::try_new(params).map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn stc_js(
	...
	stc_into_slice(&mut output, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn stc_into(
	...
	unsafe {
		...
		if in_ptr == out_ptr {
			...
			stc_into_slice(&mut temp, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			out.copy_from_slice(&temp);
		} else {
			let out = std::slice::from_raw_parts_mut(out_ptr, len);
			stc_into_slice(out, &input, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
		}
...
pub fn stc_batch_unified_js(data: &[f64], config: JsValue) -> Result<JsValue, JsValue> {
	...
	let result = stc_batch_with_kernel(data, &sweep, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;
