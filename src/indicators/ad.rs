...
impl<'a> AdInput<'a> {
	...
	#[inline]
	pub fn from_slices(high: &'a [f64], low: &'a [f64], close: &'a [f64], volume: &'a [f64], params: AdParams) -> Self {
		Self {
			data: AdData::Slices {
				high,
				low,
...
impl AdBuilder {
	...
	#[inline(always)]
	pub fn apply_slices(self, high: &[f64], low: &[f64], close: &[f64], volume: &[f64]) -> Result<AdOutput, AdError> {
...
fn ad_batch_inner(data: &AdBatchInput, kern: Kernel, parallel: bool) -> Result<AdBatchOutput, AdError> {
...
fn ad_batch_inner_into(data: &AdBatchInput, kern: Kernel, parallel: bool, out: &mut [f64]) -> Result<(), AdError> {
	let rows = data.highs.len();
	let cols = if rows > 0 { data.highs[0].len() } else { 0 };
		...
		unsafe {
			match actual {
				Kernel::Scalar | Kernel::ScalarBatch => {
					ad_row_scalar(data.highs[row], data.lows[row], data.closes[row], data.volumes[row], dst)
				}
				#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
				...
				_ => ad_row_scalar(data.highs[row], data.lows[row], data.closes[row], data.volumes[row], dst),
			}
		}
	};
...
#[inline(always)]
pub unsafe fn ad_row_scalar(high: &[f64], low: &[f64], close: &[f64], volume: &[f64], out: &mut [f64]) {
	ad_scalar(high, low, close, volume, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn ad_row_avx2(high: &[f64], low: &[f64], close: &[f64], volume: &[f64], out: &mut [f64]) {
	ad_row_scalar(high, low, close, volume, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn ad_row_avx512(high: &[f64], low: &[f64], close: &[f64], volume: &[f64], out: &mut [f64]) {
	ad_row_scalar(high, low, close, volume, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn ad_row_avx512_short(high: &[f64], low: &[f64], close: &[f64], volume: &[f64], out: &mut [f64]) {
	ad_row_scalar(high, low, close, volume, out)
}

#[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
#[inline(always)]
pub unsafe fn ad_row_avx512_long(high: &[f64], low: &[f64], close: &[f64], volume: &[f64], out: &mut [f64]) {
	ad_row_scalar(high, low, close, volume, out)
}
...
pub fn ad_batch_py<'py>(
	...
	for i in 0..rows {
		...
		if l.len()? != n || c.len()? != n || v.len()? != n {
			return Err(PyValueError::new_err("Rows must have equal lengths across OHLCV arrays"));
		}
		high_arrays.push(h);
		low_arrays.push(l);
		close_arrays.push(c);
	...
	let volume_slices: Vec<&[f64]> = volume_arrays.iter().map(|a| a.as_slice().unwrap()).collect();
...
pub fn ad_batch_unified_js(
	...
	if rows == 0 { return Err(JsValue::from_str("rows must be > 0")); }
	if highs_flat.is_empty() { return Err(JsValue::from_str("empty inputs")); }
	let cols = highs_flat.len() / rows;
	let check = rows * cols;
	...
	let batch = AdBatchInput { highs: &highs, lows: &lows, closes: &closes, volumes: &volumes };
	let out = ad_batch_with_kernel(&batch, Kernel::Auto).map_err(|e| JsValue::from_str(&e.to_string()))?;

	let packed = AdBatchJsOutput { values: out.values, rows: out.rows, cols: out.cols };
	serde_wasm_bindgen::to_value(&packed).map_err(|e| JsValue::from_str(&format!("Serialization error: {}", e)))
}
...
pub fn ad_batch_into(
	...
	if highs_ptr.is_null() || lows_ptr.is_null() || closes_ptr.is_null() || volumes_ptr.is_null() || out_ptr.is_null() {
		return Err(JsValue::from_str("null pointer"));
	}
	unsafe {
		let check = rows.checked_mul(cols).ok_or_else(|| JsValue::from_str("rows*cols overflow"))?;
...
mod tests {
	...
	fn check_ad_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		...
		for i in 0..candles.close.len() {
			let val = stream.update(candles.high[i], candles.low[i], candles.close[i], candles.volume[i]);
	...
	fn check_ad_no_poison(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		skip_if_unsupported!(kernel, test_name);
	...
	fn check_ad_no_poison(_test_name: &str, _kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
		Ok(())
	}
