    ...
    unsafe {
        match chosen {
            ...
            Kernel::Avx2 | Kernel::Avx2Batch => {
                ift_rsi_avx2(data, rsi_period, wma_period, first, &mut vec![f64::NAN; len])
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
...
pub fn ift_rsi_scalar(
    ...
    let mut rsi_values = rsi(&RsiInput::from_slice(
        sliced,
        RsiParams { period: Some(rsi_period) },
    ))
    ...
    let wma_values = wma(&WmaInput::from_slice(
        &rsi_values,
        WmaParams { period: Some(wma_period) },
    ))
...
impl IftRsiStream {
    ...
    pub fn update(&mut self, value: f64) -> Option<f64> {
        ...
        let rsi_val = match rsi(&RsiInput::from_slice(rsi_slice, RsiParams { period: Some(self.rsi_period) })) {
        ...
        let wma_val = match wma(&WmaInput::from_slice(wma_slice, WmaParams { period: Some(self.wma_period) })) {
            Ok(res) => res.values.last().cloned().unwrap_or(f64::NAN),
            Err(_) => return None,
        };
        if wma_val.is_nan() {
...
mod tests {
    ...
    fn check_ift_rsi_partial_params(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let default_params = IftRsiParams { rsi_period: None, wma_period: None };
    ...
    fn check_ift_rsi_accuracy(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for (i, &val) in result.values[start..].iter().enumerate() {
            ...
            assert!(diff < 1e-8, "[{}] IFT_RSI {:?} mismatch at idx {}: got {}, expected {}", test_name, kernel, i, val, expected_last_five[i]);
        }
        Ok(())
    }
    ...
    fn check_ift_rsi_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let params = IftRsiParams { rsi_period: Some(0), wma_period: Some(9) };
    ...
    fn check_ift_rsi_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let params = IftRsiParams { rsi_period: Some(10), wma_period: Some(9) };
        let input = IftRsiInput::from_slice(&data_small, params);
        let res = ift_rsi_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] IFT_RSI should fail with period exceeding length", test_name);
        Ok(())
    }
    ...
    fn check_ift_rsi_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let params = IftRsiParams { rsi_period: Some(5), wma_period: Some(9) };
        let input = IftRsiInput::from_slice(&single_point, params);
        let res = ift_rsi_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] IFT_RSI should fail with insufficient data", test_name);
        Ok(())
    }
    ...
    fn check_ift_rsi_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
        let first_params = IftRsiParams { rsi_period: Some(5), wma_period: Some(9) };
        ...
        let second_params = IftRsiParams { rsi_period: Some(5), wma_period: Some(9) };
