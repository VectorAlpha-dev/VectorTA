...
pub fn qstick_with_kernel(input: &QstickInput, kernel: Kernel) -> Result<QstickOutput, QstickError> {
    ...
    let first = diff.iter().position(|&x| !x.is_nan()).ok_or(QstickError::AllValuesNaN)?;
    ...
    unsafe {
        match chosen {
            Kernel::Scalar | Kernel::ScalarBatch => {
                qstick_scalar(&diff, period, first, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 | Kernel::Avx2Batch => {
                qstick_avx2(&diff, period, first, &mut out)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                qstick_avx512(&diff, period, first, &mut out)
            }
            _ => unreachable!(),
        }
    }
...
fn qstick_batch_inner(
    ...
    let first = diffs.iter().position(|&x| !x.is_nan()).ok_or(QstickError::AllValuesNaN)?;
...
mod tests {
    ...
    fn check_qstick_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let input = QstickInput::from_slices(&open_data, &close_data, params);
        let res = qstick_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] Qstick should fail with period exceeding length", test_name);
        Ok(())
    }
    fn check_qstick_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let input = QstickInput::from_slices(&open_data, &close_data, params);
        let res = qstick_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] Qstick should fail with insufficient data", test_name);
        Ok(())
    }
