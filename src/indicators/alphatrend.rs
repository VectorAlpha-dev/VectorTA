...
impl<'a> AlphaTrendInput<'a> {
    ...
    pub fn from_slices(
        ...
        Self {
            data: AlphaTrendData::Slices { open, high, low, close, volume },
            params: p,
        }
    }
...
pub fn alphatrend_with_kernel(input: &AlphaTrendInput, kernel: Kernel) -> Result<AlphaTrendOutput, AlphaTrendError> {
    let (open, high, low, close, volume, coeff, period, no_volume, first, chosen) = alphatrend_prepare(input, kernel)?;
...
pub fn alphatrend_into_slices(
    ...
    let (open, high, low, close, volume, coeff, period, no_volume, first, chosen) = alphatrend_prepare(input, kern)?;
    ...
    for v in &mut dst_k1[..k1_warm_end] { *v = f64::NAN; }
    for v in &mut dst_k2[..k2_warm_end] { *v = f64::NAN; }
...
fn alphatrend_prepare<'a>(
    ...
    kernel: Kernel,
) -> Result<(&'a [f64], &'a [f64], &'a [f64], &'a [f64], &'a [f64], f64, usize, bool, usize, Kernel), AlphaTrendError> {
    let (open, high, low, close, volume) = match &input.data {
        ...
        AlphaTrendData::Slices { open, high, low, close, volume } => {
            (*open, *high, *low, *close, *volume)
        }
    };
    ...
    let first = close.iter().position(|x| !x.is_nan())
    ...
    Ok((open, high, low, close, volume, coeff, period, no_volume, first, chosen))
}
    ...
    unsafe {
        #[cfg(all(target_arch = "wasm32", target_feature = "simd128"))]
        {
            if matches!(kernel, Kernel::Scalar | Kernel::ScalarBatch) {
                return alphatrend_simd128(open, high, low, close, volume, coeff, period, no_volume, first, out_k1, out_k2);
            }
        }
        
        match kernel {
            Kernel::Scalar | Kernel::ScalarBatch => {
                alphatrend_scalar(open, high, low, close, volume, coeff, period, no_volume, first, out_k1, out_k2)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx2 | Kernel::Avx2Batch => {
                alphatrend_avx2(open, high, low, close, volume, coeff, period, no_volume, first, out_k1, out_k2)
            }
            #[cfg(all(feature = "nightly-avx", target_arch = "x86_64"))]
            Kernel::Avx512 | Kernel::Avx512Batch => {
                alphatrend_avx512(open, high, low, close, volume, coeff, period, no_volume, first, out_k1, out_k2)
            }
...
pub fn alphatrend_scalar(
    ...
    let tr: &mut [f64] = unsafe {
        core::slice::from_raw_parts_mut(tr_mu.as_mut_ptr() as *mut f64, len)
    };
    // first bar
    if first_val < len {
    ...
    let atr: &mut [f64] = unsafe {
        core::slice::from_raw_parts_mut(atr_mu.as_mut_ptr() as *mut f64, len)
    };
    for i in warmup..len {
    ...
    let alpha_trend: &mut [f64] = unsafe {
        core::slice::from_raw_parts_mut(at_mu.as_mut_ptr() as *mut f64, len)
    };
    ...
    for i in warmup..len {
        ...
        if i == warmup {
            ...
            alpha_trend[i] = if m_check {
                if up_t < prev { prev } else { up_t }
            } else {
                if down_t > prev { prev } else { down_t }
            };
        }
    }
    ...
    for v in &mut out_k1[..warmup] { *v = f64::NAN; }
...
unsafe fn alphatrend_avx2(
    ...
    alphatrend_scalar(open, high, low, close, volume, coeff, period, no_volume, first_val, out_k1, out_k2)
}
...
pub fn alphatrend_py<'py>(
    ...
    let input = AlphaTrendInput::from_slices(
        open_slice, high_slice, low_slice, close_slice, volume_slice, params
    );
    ...
    Ok((
        result.k1.into_pyarray(py),
        result.k2.into_pyarray(py),
    ))
}
...
impl AlphaTrendStreamPy {
    ...
    fn new(coeff: f64, period: usize, no_volume: bool) -> PyResult<Self> {
        ...
        let stream = AlphaTrendStream::try_new(params)
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
...
pub fn alphatrend_js(
    ...
    let out = AlphaTrendJsOutput { values, rows: 2, cols: close.len() };
...
pub fn alphatrend_free_flat(ptr: *mut f64, n: usize) {
    unsafe { let _ = Vec::from_raw_parts(ptr, 2 * n, 2 * n); }
}

#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn alphatrend_into_flat(
    open_ptr: *const f64, high_ptr: *const f64, low_ptr: *const f64,
    close_ptr: *const f64, volume_ptr: *const f64,
    out_flat_ptr: *mut f64, len: usize,
    coeff: f64, period: usize, no_volume: bool,
) -> Result<(), JsValue> {
    if [open_ptr, high_ptr, low_ptr, close_ptr, volume_ptr].iter().any(|&p| p.is_null()) || out_flat_ptr.is_null() {
    ...
    unsafe {
        ...
        let params = AlphaTrendParams { coeff: Some(coeff), period: Some(period), no_volume: Some(no_volume) };
        let input  = AlphaTrendInput::from_slices(open, high, low, close, volume, params);
...
pub fn alphatrend_alloc(_len: usize) -> *mut f64 { core::ptr::null_mut() }
...
pub fn alphatrend_batch_with_kernel(
    ...
    let kernel = match k {
        ...
        _ => return Err(AlphaTrendError::InvalidPeriod { period: 0, data_len: 0 }),
    };
...
fn alphatrend_batch_inner_from_slices(
    ...
    let cols = close.len();
    let rows = combos.len();
    if cols == 0 { return Err(AlphaTrendError::EmptyInputData); }

    let first = close.iter().position(|x| !x.is_nan())
        .ok_or(AlphaTrendError::AllValuesNaN)?;
    let warm_k1: Vec<usize> = combos.iter().map(|p| first + p.period.unwrap_or(14) - 1).collect();
    ...
    let out_k1: &mut [f64] = unsafe { core::slice::from_raw_parts_mut(k1_guard.as_mut_ptr() as *mut f64, k1_guard.len()) };
    let out_k2: &mut [f64] = unsafe { core::slice::from_raw_parts_mut(k2_guard.as_mut_ptr() as *mut f64, k2_guard.len()) };

    let actual = match kern { Kernel::Auto => detect_best_batch_kernel(), k => k };
    alphatrend_batch_inner_into_slices(open, high, low, close, volume, sweep, actual, parallel, out_k1, out_k2)?;

    let values_k1 = unsafe { Vec::from_raw_parts(k1_guard.as_mut_ptr() as *mut f64, k1_guard.len(), k1_guard.capacity()) };
    let values_k2 = unsafe { Vec::from_raw_parts(k2_guard.as_mut_ptr() as *mut f64, k2_guard.len(), k2_guard.capacity()) };
    core::mem::forget(k1_guard);
    core::mem::forget(k2_guard);

    Ok(AlphaTrendBatchOutput { values_k1, values_k2, combos, rows, cols })
}
...
fn alphatrend_batch_inner(
    ...
    let first = candles.close.iter().position(|x| !x.is_nan())
        .ok_or(AlphaTrendError::AllValuesNaN)?;
    let warm_k1: Vec<usize> = combos.iter().map(|p| first + p.period.unwrap_or(14) - 1).collect();
    let warm_k2: Vec<usize> = warm_k1.iter().map(|&w| w.saturating_add(2)).collect();
    ...
    if parallel {
        ...
        out_k1.par_chunks_mut(cols)
            .zip(out_k2.par_chunks_mut(cols))
            .enumerate()
            .try_for_each(|(row, (k1r, k2r))| do_row(row, k1r, k2r))?;
    } else {
        for (row, (k1r, k2r)) in out_k1.chunks_mut(cols).zip(out_k2.chunks_mut(cols)).enumerate() {
            do_row(row, k1r, k2r)?;
        }
    }
    ...
    let values_k1 = unsafe {
        Vec::from_raw_parts(k1_guard.as_mut_ptr() as *mut f64, k1_guard.len(), k1_guard.capacity())
    };
    let values_k2 = unsafe {
        Vec::from_raw_parts(k2_guard.as_mut_ptr() as *mut f64, k2_guard.len(), k2_guard.capacity())
    };
...
pub fn alphatrend_batch_inner_into_slices(
    ...
    if parallel {
        ...
        k1_slice.par_chunks_mut(cols)
            .zip(k2_slice.par_chunks_mut(cols))
            .enumerate()
            .try_for_each(|(row, (k1r, k2r))| do_row(row, k1r, k2r))?;
    } else {
        for (row, (k1r, k2r)) in k1_slice.chunks_mut(cols).zip(k2_slice.chunks_mut(cols)).enumerate() {
            do_row(row, k1r, k2r)?;
        }
    }
...
pub fn alphatrend_batch_py<'py>(
    ...
    let (o, h, l, c, v) = (open.as_slice()?, high.as_slice()?, low.as_slice()?, close.as_slice()?, volume.as_slice()?);
    let len = c.len();
    ...
    let rows = {
        fn axis_usize((s,e,st):(usize,usize,usize))->usize {
            if st==0 || s==e {1} else { (e - s)/st + 1 }
        }
        fn axis_f64((s,e,st):(f64,f64,f64))->usize {
            if st.abs()<1e-12 || (s-e).abs()<1e-12 {1} else { ((e - s)/st).floor() as usize + 1 }
        }
    ...
    let combo_list = PyList::new(
        ...
        combos.iter().map(|c| {
            ...
            d.set_item("no_volume", c.no_volume.unwrap_or(false)).unwrap();
            d
        }),
    )?;
...
pub fn alphatrend_batch_js(
    ...
    alphatrend_batch_inner_into_slices(
        open, high, low, close, volume,
        &sweep, detect_best_batch_kernel(), true,
        &mut k1, &mut k2,
    ).map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    let js = AlphaTrendBatchJsOutput { values, combos, rows: rows * 2, cols };
...
pub fn alphatrend_batch_into_flat(
    open_ptr: *const f64, high_ptr: *const f64, low_ptr: *const f64, close_ptr: *const f64, volume_ptr: *const f64,
    out_ptr: *mut f64, len: usize,
    coeff_start: f64, coeff_end: f64, coeff_step: f64,
    period_start: usize, period_end: usize, period_step: usize,
    no_volume: bool,
) -> Result<usize, JsValue> {
    if [open_ptr,high_ptr,low_ptr,close_ptr,volume_ptr].iter().any(|&p| p.is_null()) || out_ptr.is_null() {
        return Err(JsValue::from_str("null pointer"));
    }
    unsafe {
        let (open,high,low,close,volume) = (
        ...
        alphatrend_batch_inner_into_slices(
            open, high, low, close, volume,
            &sweep, detect_best_batch_kernel(), false,
            k1, k2,
        ).map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn alphatrend_batch_unified_js(
    ...
    let config: AlphaTrendBatchConfig =
    ...
    let output = alphatrend_batch_slice(open, high, low, close, volume, &sweep, detect_best_kernel())
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn alphatrend_into(
    ...
    if in_ptr.is_null() || out_k1_ptr.is_null() || out_k2_ptr.is_null() ||
       open_ptr.is_null() || high_ptr.is_null() || low_ptr.is_null() || volume_ptr.is_null() {
...
impl AlphaTrendContext {
    ...
    pub fn new(coeff: f64, period: usize, no_volume: bool) -> Result<AlphaTrendContext, JsValue> {
        ...
        if coeff <= 0.0 || !coeff.is_finite() {
            return Err(JsValue::from_str(&format!("Invalid coefficient: {}", coeff)));
        }
...
mod tests {
    ...
    fn check_alphatrend_partial_params(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
    ...
    fn check_alphatrend_default_candles(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
    ...
    fn check_alphatrend_zero_period(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        assert!(res.is_err(), "[{}] AlphaTrend should fail with zero period", test_name);
        Ok(())
    }
    ...
    fn check_alphatrend_all_nan(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let params = AlphaTrendParams::default();
        let input = AlphaTrendInput::from_slices(&nan_data, &nan_data, &nan_data, &nan_data, &nan_data, params);
        let res = alphatrend_with_kernel(&input, kernel);
        assert!(
            res.is_err(),
    ...
    fn check_alphatrend_period_exceeds_length(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let input = AlphaTrendInput::from_slices(&data_small, &data_small, &data_small, &data_small, &data_small, params);
        let res = alphatrend_with_kernel(&input, kernel);
        assert!(
            res.is_err(),
    ...
    fn check_alphatrend_very_small_dataset(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let input = AlphaTrendInput::from_slices(&single_point, &single_point, &single_point, &single_point, &single_point, params);
        let res = alphatrend_with_kernel(&input, kernel);
        assert!(res.is_err(), "[{}] AlphaTrend should fail with insufficient data", test_name);
        Ok(())
    }
    
    fn check_alphatrend_invalid_coeff(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
    ...
    fn check_alphatrend_inconsistent_lengths(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        let open = vec![10.0, 20.0, 30.0];
    ...
    fn check_alphatrend_reinput(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        let synthetic_high: Vec<f64> = k1.iter().map(|&v| if v.is_nan() { v } else { v + 10.0 }).collect();
        let synthetic_low: Vec<f64> = k1.iter().map(|&v| if v.is_nan() { v } else { v - 10.0 }).collect();
        let synthetic_volume = vec![1000.0; k1.len()];
        
        let second_input = AlphaTrendInput::from_slices(k1, &synthetic_high, &synthetic_low, k1, &synthetic_volume, second_params);
    ...
    fn check_alphatrend_nan_handling(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        let file_path = "src/data/2018-09-01-2024-Bitfinex_Spot-4h.csv";
        let candles = read_candles_from_csv(file_path)?;
    ...
    fn check_alphatrend_streaming(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for i in 0..30 {
            ...
            if i >= 13 {
                ...
                assert!(result.is_none(), "[{}] Streaming not yet fully implemented", test_name);
            }
        }
        Ok(())
    }
    ...
    fn check_alphatrend_property(test_name: &str, kernel: Kernel) -> Result<(), Box<dyn std::error::Error>> {
        ...
        proptest::test_runner::TestRunner::default()
            .run(&strat, |(close_data, period, coeff, no_volume)| {
                ...
                let input = AlphaTrendInput::from_slices(&open, &high, &low, &close_data, &volume, params);
                ...
                for i in 0..close_data.len() {
                    ...
                    if !y.is_finite() || !r.is_finite() {
                        prop_assert!(y.to_bits() == r.to_bits(), "K1 finite/NaN mismatch idx {i}: {y} vs {r}");
                        continue;
                    }
                ...
                for i in 0..close_data.len() {
                    ...
                    if !y.is_finite() || !r.is_finite() {
                        prop_assert!(y.to_bits() == r.to_bits(), "K2 finite/NaN mismatch idx {i}: {y} vs {r}");
                        continue;
                    }
    ...
    fn check_batch_no_poison(test: &str, kernel: Kernel) -> Result<(), Box<dyn Error>> {
        ...
        for (cfg_idx, &(c_start, c_end, c_step, p_start, p_end, p_step, no_vol)) in test_configs.iter().enumerate() {
            let sweep = AlphaTrendBatchRange {
            ...
            for (idx, &val) in output.values_k1.iter().chain(output.values_k2.iter()).enumerate() {
                if val.is_nan() {
                    continue;
                }
