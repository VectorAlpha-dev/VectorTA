...
mod tests {
	...
	fn test_fast_sin_cos() {
		let angles = [0.0, PI * 0.25, PI * 0.5, PI * 0.75, PI, -PI * 0.5, -PI, 10.0, -10.0];
		for &ang in &angles {
			...
			assert!(approx_eq(fs, rs, 0.05), "fast_sin_f64({ang}) => {fs} vs std => {rs}");
			assert!(approx_eq(fc, rc, 0.05), "fast_cos_f64({ang}) => {fc} vs std => {rc}");
		}
	}
	...
	fn test_atan_approx() {
		...
		for &v in &vals {
			let app = atan64(v);
			let real = v.atan();
			assert!(approx_eq(app, real, 0.1), "atan64({v}) => {app}, real => {real}");
		}
	}
	...
	fn test_ln_approx() {
		...
		for &v in &vals {
			...
			let real = v.ln();
			assert!(approx_eq(app, real, 0.2), "ln_approx_f64({v}) => {app}, real => {real}");
		}
	}
