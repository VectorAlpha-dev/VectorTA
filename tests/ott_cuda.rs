...
#[cfg(feature = "cuda")]
#[test]
fn ott_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[ott_cuda_batch_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    let len = 4096usize;
    ...
    let sweep = OttBatchRange {
        period: (2, 3, 1),
        percent: (1.0, 1.4, 0.4),
        ma_types: vec!["VAR".to_string(), "EMA".to_string()],
    };
...
#[cfg(feature = "cuda")]
#[test]
fn ott_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[ott_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    let cols = 8usize;
    let rows = 1024usize;
