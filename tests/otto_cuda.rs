...
fn otto_cuda_batch_matches_cpu_var_only() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (hott_dev, lott_dev, _combos) = cuda
        .otto_batch_dev(&price_f32, &sweep)
        .expect("otto_batch_dev");
    ...
    for idx in 0..(len * combos.len()) {
        ...
        assert!(
            approx_eq(cpu_lott[idx], g_lott[idx] as f64, tol),
            "lott mismatch at {}",
            idx
        );
    }
...
fn otto_cuda_many_series_one_param_matches_cpu_var_only() -> Result<(), Box<dyn std::error::Error>>
{
    if !cuda_available() {
        eprintln!(
            "[otto_cuda_many_series_one_param_matches_cpu_var_only] skipped - no CUDA device"
        );
        return Ok(());
    }
    ...
    for s in 0..rows {
        for t in 0..cols {
            let x = (t as f64) + (s as f64) * 0.07;
            tm[t * rows + s] = (x * 0.002).sin() + 0.00021 * x;
        }
    }
    ...
    let (hott_tm, lott_tm) = cuda
        ...
        .expect("ms otto dev");
    assert_eq!(hott_tm.rows, rows);
    assert_eq!(hott_tm.cols, cols);
    ...
    for idx in 0..(cols * rows) {
        assert!(
            approx_eq(cpu_hott_tm[idx], g_hott_tm[idx] as f64, tol),
            "hott tm mismatch at {}",
            idx
        );
        assert!(
            approx_eq(cpu_lott_tm[idx], g_lott_tm[idx] as f64, tol),
            "lott tm mismatch at {}",
            idx
        );
    }
