...
fn voss_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 4..len { let x = i as f64; data[i] = (x * 0.00123).sin() + 0.00037 * x; }
    let sweep = VossBatchRange { period: (10, 34, 4), predict: (1, 4, 1), bandwidth: (0.10, 0.40, 0.10) };

    let cpu = voss_batch_with_kernel(&data, &sweep, Kernel::ScalarBatch)?;

    let data_f32: Vec<f32> = data.iter().map(|&v| v as f32).collect();
    let cuda = CudaVoss::new(0).expect("CudaVoss::new");
    let (dev_voss, dev_filt, combos) = cuda.voss_batch_dev(&data_f32, &sweep).expect("voss_batch_dev");
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        ...
        assert!(approx_eq(cv, gv, tol), "voss mismatch at {}: cpu={} gpu={}", idx, cv, gv);
        assert!(approx_eq(cf, gf, tol), "filt mismatch at {}: cpu={} gpu={}", idx, cf, gf);
    }
    Ok(())
}
...
fn voss_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use my_project::indicators::voss::{voss_with_kernel, VossInput, VossParams, VossData};

    let cols = 8usize; // series
    let rows = 2048usize; // time
    let mut tm = vec![f64::NAN; cols * rows];
    for s in 0..cols {
        for t in (2 + s)..rows { // stagger valid starts
    ...
    for s in 0..cols {
        ...
        let input = VossInput { data: VossData::Slice(&series), params: params.clone() };
    ...
    for idx in 0..voss_g.len() {
        assert!(approx_eq(voss_cpu_tm[idx], voss_g[idx] as f64, tol), "voss mismatch at {}", idx);
        assert!(approx_eq(filt_cpu_tm[idx], filt_g[idx] as f64, tol), "filt mismatch at {}", idx);
    }
    Ok(())
}
