...
fn bandpass_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 4..len {
        let x = i as f64;
        price[i] = (x * 0.0031).sin() + 0.00013 * x;
    }
    ...
    let res = cuda
        ...
        .expect("cuda bandpass_batch_dev");
    let out = res.outputs;
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(
            approx_eq(cpu.bp[idx], bp_host[idx] as f64, tol),
            "bp mismatch at {}",
            idx
        );
        assert!(
        ...
        assert!(
            approx_eq(cpu.signal[idx], sig_host[idx] as f64, 1e-3),
            "sig mismatch at {}",
            idx
        );
        assert!(
            approx_eq(cpu.trigger[idx], trg_host[idx] as f64, tol),
            "trg mismatch at {}",
            idx
        );
    }
...
fn bandpass_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[bandpass_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for idx in 0..g_bp.len() {
        assert!(
            approx_eq(cpu_bp_tm[idx], g_bp[idx] as f64, tol),
            "bp mismatch at {}",
            idx
        );
        assert!(
            approx_eq(cpu_bpn_tm[idx], g_bpn[idx] as f64, tol),
            "bpn mismatch at {}",
            idx
        );
        assert!(
            approx_eq(cpu_sig_tm[idx], g_sig[idx] as f64, 1e-3),
            "sig mismatch at {}",
            idx
        );
        assert!(
            approx_eq(cpu_trg_tm[idx], g_trg[idx] as f64, tol),
            "trg mismatch at {}",
            idx
        );
    }
