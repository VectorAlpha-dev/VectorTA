...
fn vwmacd_cuda_one_series_many_params_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..macd_gpu.len() {
        assert!(approx_eq(cpu_macd[i] as f64, macd_gpu[i] as f64, tol), "MACD mismatch at {}", i);
        assert!(approx_eq(cpu_signal[i] as f64, signal_gpu[i] as f64, tol), "Signal mismatch at {}", i);
        assert!(approx_eq(cpu_hist[i] as f64, hist_gpu[i] as f64, tol), "Hist mismatch at {}", i);
    }

    // Sanity: single-series matches a selected row
    let target = VwmacdParams { fast_period: Some(12), slow_period: Some(24), signal_period: Some(9), fast_ma_type: Some("sma".into()), slow_ma_type: Some("sma".into()), signal_ma_type: Some("ema".into()) };
    let cpu_single = VwmacdBuilder::new().fast(12).slow(24).signal(9).apply_slices(&close, &volume)?.macd;
    let row = combos.iter().position(|p| p.fast_period==target.fast_period && p.slow_period==target.slow_period && p.signal_period==target.signal_period).unwrap();
    let gpu_row = &macd_gpu[row * series_len..(row+1) * series_len];
    for j in 0..series_len { assert!(approx_eq(cpu_single[j], gpu_row[j] as f64, 1e-4)); }
...
fn vwmacd_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols {
        ...
        for r in 0..rows { let idx=r*cols+s; c[r]=prices_tm[idx]; v[r]=volumes_tm[idx]; }
        let out = VwmacdBuilder::new().fast(f).slow(sl).signal(g).apply_slices(&c, &v)?;
        for r in 0..rows {
            let idx = r*cols + s;
    ...
    let prm = VwmacdParams { fast_period: Some(f), slow_period: Some(sl), signal_period: Some(g), fast_ma_type: Some("sma".into()), slow_ma_type: Some("sma".into()), signal_ma_type: Some("ema".into()) };
    let dev = cuda.vwmacd_many_series_one_param_time_major_dev(&p32, &v32, cols, rows, &prm)
        .map_err(|e| Box::<dyn std::error::Error>::from(e))?;
