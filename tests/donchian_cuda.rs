...
fn donchian_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(
            approx_eq(cpu.upper[idx], g_u[idx] as f64, tol),
            "upper mismatch at {}",
            idx
        );
        assert!(
            approx_eq(cpu.middle[idx], g_m[idx] as f64, tol),
            "middle mismatch at {}",
            idx
        );
        assert!(
            approx_eq(cpu.lower[idx], g_l[idx] as f64, tol),
            "lower mismatch at {}",
            idx
        );
    }
...
fn donchian_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[donchian_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for s in 0..cols {
        ...
        let input = DonchianInput {
            data: DonchianData::Slices { high: &h, low: &l },
            params,
        };
    ...
    let cuda = CudaDonchian::new(0).expect("CudaDonchian::new");
    let triplet = cuda
        .donchian_many_series_one_param_time_major_dev(
            &high_tm_f32,
            &low_tm_f32,
            cols,
            rows,
    ...
    for i in 0..(rows * cols) {
        assert!(
            approx_eq(up_tm[i], g_u[i] as f64, tol),
            "upper mismatch at {}",
            i
        );
        assert!(
            approx_eq(mid_tm[i], g_m[i] as f64, tol),
            "middle mismatch at {}",
            i
        );
        assert!(
            approx_eq(lo_tm[i], g_l[i] as f64, tol),
            "lower mismatch at {}",
            i
        );
    }
