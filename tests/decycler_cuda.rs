...
use my_project::indicators::decycler::{
    decycler_batch_with_kernel, decycler_with_kernel, DecyclerBatchRange, DecyclerInput, DecyclerParams,
};
...
fn decycler_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sweep = DecyclerBatchRange { hp_period: (6, 64, 7), k: (0.2, 0.9, 0.15) };
    ...
    let gpu_handle = cuda.decycler_batch_dev(&data_f32, &sweep).expect("cuda decycler batch dev");

    assert_eq!(cpu.rows, gpu_handle.rows);
    assert_eq!(cpu.cols, gpu_handle.cols);

    let mut gpu_host = vec![0f32; gpu_handle.len()];
    gpu_handle.buf.copy_to(&mut gpu_host).expect("copy decycler cuda batch result to host");
...
fn decycler_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let params = DecyclerParams { hp_period: Some(48), k: Some(0.707) };

    // CPU reference: per series
    let mut cpu_tm = vec![f64::NAN; num_series * series_len];
    for s in 0..num_series {
        let mut one = vec![f64::NAN; series_len];
        for t in 0..series_len { one[t] = data_tm[t * num_series + s]; }
        let input = DecyclerInput::from_slice(&one, params.clone());
        let out = decycler_with_kernel(&input, Kernel::Scalar)?.values;
        for t in 0..series_len { cpu_tm[t * num_series + s] = out[t]; }
    }

    let cuda = CudaDecycler::new(0).expect("CudaDecycler::new");
    let data_tm_f32: Vec<f32> = data_tm.iter().map(|&v| v as f32).collect();
    let handle = cuda
        .decycler_many_series_one_param_time_major_dev(&data_tm_f32, num_series, series_len, &params)
    ...
    handle.buf.copy_to(&mut gpu_tm).expect("copy decycler cuda many-series result to host");
