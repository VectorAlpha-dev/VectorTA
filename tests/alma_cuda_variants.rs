// Integration tests covering ALMA CUDA kernel variants via explicit policy.
...
fn gen_time_major_f64(cols: usize, rows: usize) -> Vec<f64> {
    ...
    for j in 0..cols {
        for t in j..rows { // stagger first_valids
...
#[cfg(feature = "cuda")]
fn compare_batch(policy: CudaAlmaPolicy, series_len: usize, start_period: usize, end_period: usize) {
    if !cuda_available() { eprintln!("[compare_batch] skipped - no CUDA device"); return; }
    ...
    let mut cuda = CudaAlma::new_with_policy(0, policy).expect("cuda alma");
    let data_f32: Vec<f32> = data.iter().map(|&v| v as f32).collect();
    let dev = cuda.alma_batch_dev(&data_f32, &sweep).expect("gpu alma batch");
...
fn compare_many_series(policy: CudaAlmaPolicy, cols: usize, rows: usize, period: usize) {
    if !cuda_available() { eprintln!("[compare_many_series] skipped - no CUDA device"); return; }

    let tm = gen_time_major_f64(cols, rows);
    let params = AlmaParams { period: Some(period), offset: Some(0.85), sigma: Some(6.0) };
    ...
    for j in 0..cols {
        ...
        let out = AlmaBuilder::default().period(period).offset(0.85).sigma(6.0).apply_slice(&s).expect("cpu alma series");
        for t in 0..rows { cpu_tm[t * cols + j] = out.values[t]; }
    }

    let mut cuda = CudaAlma::new_with_policy(0, policy).expect("cuda alma");
    let tm_f32: Vec<f32> = tm.iter().map(|&v| v as f32).collect();
    let dev = cuda.alma_multi_series_one_param_time_major_dev(&tm_f32, cols, rows, &params).expect("gpu many-series");
...
fn alma_batch_plain_matches_cpu() {
    let policy = CudaAlmaPolicy { batch: BatchKernelPolicy::Plain { block_x: 256 }, many_series: ManySeriesKernelPolicy::Auto };
...
fn alma_batch_tiled_1x_matches_cpu() {
    let policy = CudaAlmaPolicy { batch: BatchKernelPolicy::Tiled { tile: 256, per_thread: BatchThreadsPerOutput::One }, many_series: ManySeriesKernelPolicy::Auto };
...
fn alma_batch_tiled_2x_matches_cpu() {
    let policy = CudaAlmaPolicy { batch: BatchKernelPolicy::Tiled { tile: 256, per_thread: BatchThreadsPerOutput::Two }, many_series: ManySeriesKernelPolicy::Auto };
...
fn alma_many_series_1d_matches_cpu() {
    let policy = CudaAlmaPolicy { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::OneD { block_x: 128 } };
...
fn alma_many_series_2d_ty4_matches_cpu() {
    let policy = CudaAlmaPolicy { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::Tiled2D { tx: 128, ty: 4 } };
    // cols divisible by 4 for vectorized path
...
fn alma_many_series_2d_ty2_matches_cpu() {
    let policy = CudaAlmaPolicy { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::Tiled2D { tx: 128, ty: 2 } };
