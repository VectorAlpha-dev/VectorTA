...
use my_project::indicators::fisher::{
    fisher_batch_with_kernel, fisher_with_kernel, FisherBatchBuilder, FisherBatchRange, FisherInput,
    FisherParams,
};
...
fn fisher_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(approx_eq(cpu.fisher[idx], g_fish[idx] as f64, tol), "fisher mismatch at {}", idx);
        assert!(approx_eq(cpu.signal[idx], g_sig[idx] as f64, tol), "signal mismatch at {}", idx);
    }
    Ok(())
}
...
fn fisher_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!(
            "[fisher_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device"
        );
        return Ok(());
    }
    ...
    for s in 0..cols {
        ...
        for r in 0..rows { high[r] = high_tm[r * cols + s]; low[r] = low_tm[r * cols + s]; }
        let input = FisherInput::from_slices(&high, &low, FisherParams { period: Some(period) });
        let out = fisher_with_kernel(&input, Kernel::Scalar)?;
        for r in 0..rows { cpu_fish_tm[r * cols + s] = out.fisher[r]; cpu_sig_tm[r * cols + s] = out.signal[r]; }
    }
    ...
    for idx in 0..g_fish.len() {
        assert!(approx_eq(cpu_fish_tm[idx], g_fish[idx] as f64, tol), "fisher mismatch at {}", idx);
        assert!(approx_eq(cpu_sig_tm[idx],  g_sig[idx]  as f64, tol),  "signal mismatch at {}", idx);
    }
