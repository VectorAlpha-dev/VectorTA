...
#[cfg(feature = "cuda")]
#[test]
fn reverse_rsi_cuda_device_inputs_match_legacy_batch() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[reverse_rsi_cuda_device_inputs_match_legacy_batch] skipped - no CUDA device");
        return Ok(());
    }
    ...
    let (legacy, _) = cuda
        .reverse_rsi_batch_dev(&price_f32, &sweep)
        .expect("legacy reverse_rsi");
    let (device, _) = cuda
        .reverse_rsi_batch_dev_from_device_prices(d_price.buffer(), len, 8, &sweep)
        .expect("device reverse_rsi");
