...
fn adx_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols {
        ...
        let sweep = AdxBatchRange { period: (period, period, 0) };
        let out = adx_batch_with_kernel(&h, &l, &c, &sweep, Kernel::ScalarBatch)?;
        let row = 0usize;
        for t in 0..rows { cpu_tm[t*cols+s] = out.values[row*rows + t]; }
    }
    ...
    let cuda = CudaAdx::new(0).expect("CudaAdx::new");
    let dev = cuda
        .adx_many_series_one_param_time_major_dev(&high_tm_f32, &low_tm_f32, &close_tm_f32, cols, rows, period)
