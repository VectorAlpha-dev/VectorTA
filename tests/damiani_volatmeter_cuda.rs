...
fn damiani_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (row, params) in cpu.combos.iter().enumerate() {
        ...
        let gpu_warm_vol = gpu_vol_slice
            .iter()
            .position(|v| v.is_finite())
            .unwrap_or(gpu_vol_slice.len());
        let gpu_warm_anti = gpu_anti_slice
            .iter()
            .position(|v| v.is_finite())
            .unwrap_or(gpu_anti_slice.len());
        assert_eq!(
            gpu_warm_vol, cpu_warm_vol,
            "warmup prefix mismatch (vol) for row {}",
            row
        );
        assert_eq!(
            gpu_warm_anti, cpu_warm_anti,
        ...
        for i in (cpu.cols.saturating_sub(64))..cpu.cols {
            assert!(
                ...
                "GPU vol should be finite where CPU is finite"
            );
            assert!(
                gpu_anti_slice[i].is_finite() || anti[i].is_nan(),
