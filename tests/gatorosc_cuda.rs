...
use my_project::indicators::gatorosc::{
    gatorosc_batch_with_kernel, gatorosc_with_kernel, GatorOscBatchRange, GatorOscInput, GatorOscParams,
};
...
fn gatorosc_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[gatorosc_cuda_batch_matches_cpu] skipped - no CUDA device"); return Ok(()); }

    let len = 8192usize;
    let mut close = vec![f64::NAN; len];
    for i in 10..len { let x = i as f64; close[i] = (x * 0.00123).sin() * 3.0 + 0.00017 * x; }
    let sweep = GatorOscBatchRange {
        jaws_length: (10, 14, 2), jaws_shift: (4, 8, 2),
        teeth_length: (7, 9, 1), teeth_shift: (3, 5, 1),
        lips_length: (4, 6, 1), lips_shift: (1, 3, 1),
    };
    ...
    let dev = cuda.gatorosc_batch_dev(&close_f32, &sweep).expect("gatorosc_batch_dev");
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(approx_eq(cpu.upper[idx], u[idx] as f64, tol), "upper mismatch at {}", idx);
        assert!(approx_eq(cpu.lower[idx], l[idx] as f64, tol), "lower mismatch at {}", idx);
        assert!(approx_eq(cpu.upper_change[idx], uc[idx] as f64, tol), "upper_change mismatch at {}", idx);
        assert!(approx_eq(cpu.lower_change[idx], lc[idx] as f64, tol), "lower_change mismatch at {}", idx);
    }
    Ok(())
}
...
fn gatorosc_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[gatorosc_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device"); return Ok(()); }

    let cols = 8usize; // series
    let rows = 2048usize; // time
    let mut close_tm = vec![f64::NAN; cols * rows];
    for s in 0..cols { for t in s..rows { let x = (t as f64) + (s as f64) * 0.37; close_tm[t * cols + s] = (x * 0.002).cos() * 1.7 + 0.0003 * x; } }
    ...
    for s in 0..cols {
        ...
        for t in 0..rows { series[t] = close_tm[t * cols + s]; }
        let params = GatorOscParams { jaws_length: Some(jl), jaws_shift: Some(js), teeth_length: Some(tl), teeth_shift: Some(ts), lips_length: Some(ll), lips_shift: Some(ls) };
    ...
    let dev = cuda
        .gatorosc_many_series_one_param_time_major_dev(
            &close_tm_f32, cols, rows, jl, js, tl, ts, ll, ls,
        )
    ...
    let tol = 1e-3;
    for idx in 0..u.len() {
        assert!(approx_eq(cpu_u[idx], u[idx] as f64, tol), "upper mismatch at {}", idx);
        assert!(approx_eq(cpu_l[idx], l[idx] as f64, tol), "lower mismatch at {}", idx);
        assert!(approx_eq(cpu_uc[idx], uc[idx] as f64, tol), "upper_change mismatch at {}", idx);
        assert!(approx_eq(cpu_lc[idx], lc[idx] as f64, tol), "lower_change mismatch at {}", idx);
    }
    Ok(())
}
