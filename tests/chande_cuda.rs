...
fn chande_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sweep = ChandeBatchRange { period: (10, 30, 5), mult: (2.0, 4.0, 1.0) };
    let dir = "long";
...
fn chande_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols { for t in 0..rows { let v = close_tm[t * cols + s]; if v.is_nan() { continue; } let x = (t as f64) * 0.002; let off = (0.004 * x.cos()).abs() + 0.11; high_tm[t * cols + s] = v + off; low_tm[t * cols + s] = v - off; } }

    // CPU baseline per series using scalar kernel
    let period = 22usize; let mult = 3.0f64; let direction = "long";
    ...
    for s in 0..cols {
        ...
        let params = ChandeParams { period: Some(period), mult: Some(mult), direction: Some(direction.into()) };
    ...
    let mut g_tm = vec![0f32; dev.len()]; dev.buf.copy_to(&mut g_tm)?;
