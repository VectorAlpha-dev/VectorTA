...
use my_project::indicators::dec_osc::{dec_osc, DecOscInput, DecOscParams, DecOscBatchBuilder};

#[test]
fn test_dec_osc_verification() {
    // Test data
    let data = vec![
        2761.7, 2740.0, 2763.0, 2800.0, 2750.0,
        2780.0, 2820.0, 2810.0, 2795.0, 2830.0,
        2850.0, 2840.0, 2860.0, 2880.0, 2875.0
    ];
    
    // Test 1: Basic dec_osc with suitable params for small data
    let params = DecOscParams { hp_period: Some(10), k: Some(1.0) };
    ...
    for (idx, combo) in batch_result.combos.iter().enumerate() {
        ...
        for i in 0..data.len() {
            ...
            assert!(diff < 1e-10, 
                ...
                idx, i, single_result.values[i], batch_row[i]);
        }
    }
}
...
fn test_dec_osc_api_completeness() {
    use my_project::indicators::dec_osc::{
        DecOscBuilder, DecOscStream, dec_osc_with_kernel, dec_osc_into_slice
    };
    ...
    // Test Streaming API
    let stream_params = DecOscParams { hp_period: Some(10), k: Some(1.0) };
...
fn test_dec_osc_error_handling() {
    ...
    let params = DecOscParams { hp_period: Some(0), k: Some(1.0) };
    let input = DecOscInput::from_slice(&data, params);
    let result = dec_osc(&input);
    ...
    let params = DecOscParams { hp_period: Some(10), k: Some(0.0) };
    let input = DecOscInput::from_slice(&long_data, params);
    let result = dec_osc(&input);
    assert!(matches!(result, Err(DecOscError::InvalidK { k }) if k == 0.0), 
        "Expected InvalidK error for k=0.0, got: {:?}", result);
    ...
    assert!(matches!(result, Err(DecOscError::InvalidPeriod { .. })), 
        "Expected InvalidPeriod error for period > data_len, got: {:?}", result);
}