...
#[cfg(feature = "cuda")]
fn compare_batch(policy: CudaEhmaPolicy, series_len: usize, start_period: usize, end_period: usize) {
    if !cuda_available() { eprintln!("[ehma compare_batch] skipped - no CUDA device"); return; }

    let data = gen_series_f64(series_len, 3);
    let sweep = EhmaBatchRange { period: (start_period, end_period, 1) };
    ...
    let dev = cuda.ehma_batch_dev(&data_f32, &sweep).expect("gpu ehma batch");
...
fn compare_many_series(policy: CudaEhmaPolicy, cols: usize, rows: usize, period: usize) {
    if !cuda_available() { eprintln!("[ehma compare_many_series] skipped - no CUDA device"); return; }
    ...
    for j in 0..cols {
        ...
        let out = EhmaBuilder::default().period(period).apply_slice(&s).expect("cpu ehma series");
        for t in 0..rows { cpu_tm[t * cols + j] = out.values[t]; }
    }

    let mut cuda = CudaEhma::new_with_policy(0, policy).expect("cuda ehma");
    let tm_f32: Vec<f32> = tm.iter().map(|&v| v as f32).collect();
    let dev = cuda.ehma_multi_series_one_param_time_major_dev(&tm_f32, cols, rows, &params).expect("gpu many-series");
...
fn ehma_batch_plain_matches_cpu() {
    let policy = CudaEhmaPolicy { batch: BatchKernelPolicy::Plain { block_x: 256 }, many_series: ManySeriesKernelPolicy::Auto };
...
fn ehma_batch_tiled_2x_matches_cpu() {
    let policy = CudaEhmaPolicy { batch: BatchKernelPolicy::Tiled { tile: 256, per_thread: BatchThreadsPerOutput::Two }, many_series: ManySeriesKernelPolicy::Auto };
...
fn ehma_many_series_1d_matches_cpu() {
    let policy = CudaEhmaPolicy { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::OneD { block_x: 128 } };
...
fn ehma_many_series_2d_ty4_matches_cpu() {
    let policy = CudaEhmaPolicy { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::Tiled2D { tx: 128, ty: 4 } };
...
fn ehma_many_series_2d_ty2_matches_cpu() {
    let policy = CudaEhmaPolicy { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::Tiled2D { tx: 128, ty: 2 } };
