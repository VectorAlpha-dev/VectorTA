...
fn halftrend_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[halftrend_cuda_batch_matches_cpu] skipped - no CUDA device"); return Ok(()); }

    let len = 8192usize;
    let mut close = vec![f64::NAN; len];
    for i in 4..len { let x = i as f64; close[i] = (x*0.0023).sin() + 0.0004*x; }
    let mut high = close.clone();
    let mut low = close.clone();
    for i in 0..len {
        if !close[i].is_nan() { let x = i as f64 * 0.0025; let off = (0.002 * x.sin()).abs() + 0.15; high[i] = close[i] + off; low[i] = close[i] - off; }
    }

    let sweep = HalfTrendBatchRange { amplitude: (2, 16, 2), channel_deviation: (2.0, 2.0, 0.0), atr_period: (14, 14, 0) };
    ...
    let cpu = halftrend_batch_with_kernel_slices_internal(&hq, &lq, &cq, &sweep, Kernel::ScalarBatch)?;
    ...
    let dev = cuda.halftrend_batch_dev(&high_f32, &low_f32, &close_f32, &sweep).expect("halftrend batch");
    ...
    for idx in 0..need {
        assert!(approx_eq(cpu.halftrend[idx], g_ht[idx] as f64, tol), "halftrend mismatch at {}", idx);
        assert!(approx_eq(cpu.trend[idx], g_tr[idx] as f64, 1e-3), "trend mismatch at {}", idx);
        assert!(approx_eq(cpu.atr_high[idx], g_ah[idx] as f64, tol), "atr_high mismatch at {}", idx);
        assert!(approx_eq(cpu.atr_low[idx], g_al[idx] as f64, tol), "atr_low mismatch at {}", idx);
        // buy/sell are sparse; compare only where one is finite
        let cb = cpu.buy_signal[idx]; let gb = g_bs[idx] as f64; if !(cb.is_nan() && gb.is_nan()) { assert!(approx_eq(cb, gb, 5e-2), "buy mismatch at {}", idx); }
        let cs = cpu.sell_signal[idx]; let gs = g_ss[idx] as f64; if !(cs.is_nan() && gs.is_nan()) { assert!(approx_eq(cs, gs, 5e-2), "sell mismatch at {}", idx); }
    }
    Ok(())
}
...
fn halftrend_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[halftrend_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device"); return Ok(()); }
    let cols = 16usize; let rows = 2048usize;
    ...
    let mut high_tm = close_tm.clone(); let mut low_tm = close_tm.clone();
    ...
    let amplitude = 2usize; let atr_period = 14usize; let ch = 2.0f64;
    ...
    for s in 0..cols {
        let mut h = vec![f64::NAN; rows]; let mut l = vec![f64::NAN; rows]; let mut c = vec![f64::NAN; rows];
        for t in 0..rows { let idx = t*cols+s; h[t] = high_tm[idx]; l[t] = low_tm[idx]; c[t] = close_tm[idx]; }
        let range = HalfTrendBatchRange { amplitude: (amplitude, amplitude, 0), channel_deviation: (ch, ch, 0.0), atr_period: (atr_period, atr_period, 0) };
        ...
        let out = halftrend_batch_with_kernel_slices_internal(&hq, &lq, &cq, &range, Kernel::ScalarBatch)?;
        for t in 0..rows { let idx = t*cols+s; cpu_ht_tm[idx] = out.halftrend[t]; cpu_tr_tm[idx] = out.trend[t]; }
    }
    ...
    let cuda = CudaHalftrend::new(0).expect("CudaHalftrend::new");
    let dev = cuda.halftrend_many_series_one_param_time_major_dev(&high_tm_f32, &low_tm_f32, &close_tm_f32, cols, rows, amplitude, ch, atr_period).expect("many-series");
    assert_eq!(dev.halftrend.rows, rows); assert_eq!(dev.halftrend.cols, cols);
    let mut g_ht = vec![0f32; cols*rows]; let mut g_tr = vec![0f32; cols*rows];
    dev.halftrend.buf.copy_to(&mut g_ht)?; dev.trend.buf.copy_to(&mut g_tr)?;
    let tol = 1e-3;
    for i in 0..g_ht.len() {
        assert!(approx_eq(cpu_ht_tm[i], g_ht[i] as f64, tol), "ht mismatch at {}", i);
        assert!(approx_eq(cpu_tr_tm[i], g_tr[i] as f64, tol), "tr mismatch at {}", i);
    }
    Ok(())
}
...
fn halftrend_batch_with_kernel_slices_internal(
    high: &[f64], low: &[f64], close: &[f64], sweep: &HalfTrendBatchRange, kern: Kernel,
    ...
    Ok(HalfTrendBatchOutputLite { halftrend: out.halftrend, trend: out.trend, atr_high: out.atr_high, atr_low: out.atr_low, buy_signal: out.buy_signal, sell_signal: out.sell_signal, rows: out.rows, cols: out.cols })
}

#[derive(Clone)]
struct HalfTrendBatchOutputLite { halftrend: Vec<f64>, trend: Vec<f64>, atr_high: Vec<f64>, atr_low: Vec<f64>, buy_signal: Vec<f64>, sell_signal: Vec<f64>, rows: usize, cols: usize }
...
fn halftrend_batch_with_kernel_slices_public(
    ...
    let combos_out = my_project::indicators::halftrend::HalfTrendBatchBuilder::new()
        ...
        .channel_deviation_range(sweep.channel_deviation.0, sweep.channel_deviation.1, sweep.channel_deviation.2)
