...
use my_project::indicators::rsmk::{rsmk_batch_with_kernel, rsmk_with_kernel, RsmkBatchRange, RsmkInput, RsmkParams, RsmkData};
...
fn rsmk_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (pair, _combos) = cuda.rsmk_batch_dev(&main_f32, &comp_f32, &sweep).expect("rsmk batch dev");
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(approx_eq(cpu.indicator[idx], ind_host[idx] as f64, tol), "indicator mismatch at {}", idx);
        assert!(approx_eq(cpu.signal[idx], sig_host[idx] as f64, tol), "signal mismatch at {}", idx);
    }
    Ok(())
}
...
fn rsmk_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let lookback = 30usize; let period = 3usize; let sigp = 10usize;
    ...
    for s in 0..cols {
        ...
        for t in 0..rows { main[t] = main_tm[t * cols + s]; comp[t] = comp_tm[t * cols + s]; }
        let input = RsmkInput { data: RsmkData::Slices { main: &main, compare: &comp }, params: RsmkParams { lookback: Some(lookback), period: Some(period), signal_period: Some(sigp), matype: Some("ema".into()), signal_matype: Some("ema".into()) } };
        let out = rsmk_with_kernel(&input, Kernel::Scalar).unwrap();
        for t in 0..rows { cpu_ind_tm[t*cols+s] = out.indicator[t]; cpu_sig_tm[t*cols+s] = out.signal[t]; }
    }
    ...
    let cuda = CudaRsmk::new(0).expect("CudaRsmk::new");
    let pair = cuda
        .rsmk_many_series_one_param_time_major_dev(&main_tm_f32, &comp_tm_f32, cols, rows, &RsmkParams { lookback: Some(lookback), period: Some(period), signal_period: Some(sigp), matype: Some("ema".into()), signal_matype: Some("ema".into()) })
        .expect("rsmk many series");

    assert_eq!(pair.a.rows, rows); assert_eq!(pair.a.cols, cols);
    assert_eq!(pair.b.rows, rows); assert_eq!(pair.b.cols, cols);
    ...
    for idx in 0..g_ind_tm.len() {
        assert!(approx_eq(cpu_ind_tm[idx], g_ind_tm[idx] as f64, tol), "indicator mismatch at {}", idx);
