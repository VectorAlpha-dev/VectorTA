...
fn percentile_nearest_rank_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 3..len { let x = i as f64; price[i] = (x * 0.00131).sin() + 0.00017 * x; }

    let sweep = pnr::PercentileNearestRankBatchRange { length: (5, 25, 5), percentage: (25.0, 75.0, 25.0) };
    ...
    let (dev, combos) = cuda.pnr_batch_dev(&price_f32, &sweep).expect("pnr_batch_dev");
...
#[cfg(feature = "cuda")]
#[test]
fn percentile_nearest_rank_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
    ...
    for s in 0..cols { for t in (s + 2)..rows { let x = t as f64 + s as f64 * 0.2; price_tm[t * cols + s] = (x * 0.002).sin() + 0.0003 * x; } }
    ...
    for s in 0..cols {
        ...
        for t in 0..rows { col[t] = price_tm[t * cols + s]; }
        let params = pnr::PercentileNearestRankParams { length: Some(length), percentage: Some(percentage) };
    ...
    let tol = 1e-4;
    for idx in 0..host.len() { assert!(approx_eq(cpu_tm[idx], host[idx] as f64, tol), "mismatch at {}", idx); }
    Ok(())
}
