...
use my_project::indicators::dm::{dm_batch_with_kernel, dm_with_kernel, DmBatchRange, DmInput, DmParams};
...
fn dm_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pair = cuda.dm_batch_dev(&high_f32, &low_f32, &sweep).expect("dm_batch_dev").0;
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(approx_eq(cpu.plus[idx], plus_host[idx] as f64, tol), "plus mismatch at {}", idx);
        assert!(approx_eq(cpu.minus[idx], minus_host[idx] as f64, tol), "minus mismatch at {}", idx);
    }
    Ok(())
}
...
fn dm_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols {
        ...
        for t in 0..rows { cpu_plus_tm[t * cols + s] = out.plus[t]; cpu_minus_tm[t * cols + s] = out.minus[t]; }
    }
