// Integration tests for CUDA APO kernels

use my_project::indicators::apo::{apo_batch_with_kernel, apo_with_kernel, ApoBatchRange, ApoData, ApoInput, ApoParams};
...
#[cfg(feature = "cuda")]
use my_project::cuda::moving_averages::CudaApo;
...
fn apo_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dev = cuda.apo_batch_dev(&price_f32, &sweep).expect("apo_batch_dev");
...
fn apo_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols {
        ...
        let params = ApoParams { short_period: Some(short), long_period: Some(long) };
        let input = ApoInput { data: ApoData::Slice(&p), params };
    ...
    let cuda = CudaApo::new(0).expect("CudaApo::new");
    let dev_tm = cuda
        .apo_many_series_one_param_time_major_dev(&price_tm_f32, cols, rows, &ApoParams { short_period: Some(short), long_period: Some(long) })
