...
use my_project::indicators::di::{di_batch_with_kernel, di_with_kernel, DiBatchRange, DiInput, DiParams, DiData};
...
fn di_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 6..len { let x = i as f64; close[i] = (x * 0.00137).sin() + 0.00009 * x; }
    let mut high = close.clone();
    let mut low  = close.clone();
    for i in 0..len {
        if close[i].is_nan() { continue; }
    ...
    let (plus_dev, minus_dev, combos) = cuda.di_batch_dev(&h_f32, &l_f32, &c_f32, &sweep).expect("di cuda batch dev");
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        ...
        assert!(approx_eq(c_pl, g_pl, tol), "plus mismatch at {}: cpu={} gpu={}", idx, c_pl, g_pl);
        assert!(approx_eq(c_mi, g_mi, tol), "minus mismatch at {}: cpu={} gpu={}", idx, c_mi, g_mi);
    }
    Ok(())
}
...
fn di_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols { for t in s..rows { let x = (t as f64) + (s as f64) * 0.3; close_tm[t*cols+s] = (x * 0.0023).sin() + 0.0002 * x; } }
    let mut high_tm = close_tm.clone();
    let mut low_tm  = close_tm.clone();
    for s in 0..cols { for t in 0..rows { let idx = t*cols+s; if close_tm[idx].is_nan() { continue; } let off = 0.1 + ((t as f64) * 0.0029).cos().abs() * 0.02; high_tm[idx] = close_tm[idx] + off; low_tm[idx] = close_tm[idx] - off; } }
    ...
    for s in 0..cols {
        ...
        let input = DiInput { data: DiData::Slices { high: &h, low: &l, close: &c }, params: DiParams { period: Some(period) } };
        let out = di_with_kernel(&input, Kernel::Scalar)?;
        for t in 0..rows { let idx = t*cols + s; cpu_plus_tm[idx] = out.plus[t]; cpu_minus_tm[idx] = out.minus[t]; }
    }
    ...
    for idx in 0..(cols*rows) {
        assert!(approx_eq(cpu_plus_tm[idx], g_plus_tm[idx] as f64, tol), "plus mismatch at {}", idx);
        assert!(approx_eq(cpu_minus_tm[idx], g_minus_tm[idx] as f64, tol), "minus mismatch at {}", idx);
    }
    Ok(())
}
