...
use my_project::indicators::coppock::{
    coppock_with_kernel, CoppockBatchRange, CoppockInput, CoppockParams,
...
fn coppock_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[coppock_cuda_batch_matches_cpu] skipped - no CUDA device"); return Ok(()); }
    let len = 8192usize;
    let mut close = vec![f64::NAN; len];
    for i in 50..len { let x = i as f64; close[i] = 100.0 + (x * 0.001).sin() * 2.0 + 0.001 * x; }

    let sweep = CoppockBatchRange { short: (8, 16, 4), long: (14, 28, 7), ma: (8, 12, 2) };
    ...
    let dev = cuda.coppock_batch_dev(&price_f32, &sweep).expect("coppock_batch_dev");
...
fn coppock_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[coppock_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device"); return Ok(()); }
    let cols = 6usize; // series
    let rows = 2048usize; // time
    let mut tm = vec![f64::NAN; cols * rows];
    for s in 0..cols { for t in s..rows { let x = (t as f64) + (s as f64) * 0.3; tm[t * cols + s] = 100.0 + (x * 0.0023).sin() + 0.0007 * x; } }
    ...
    for s in 0..cols {
        ...
        let params = CoppockParams { short_roc_period: Some(short), long_roc_period: Some(long), ma_period: Some(ma_p), ma_type: Some("wma".to_string()) };
    ...
    let tol = 8e-4;
    for idx in 0..got.len() { assert!(approx_eq(cpu[idx], got[idx] as f64, tol), "mismatch at {}", idx); }
    Ok(())
}
