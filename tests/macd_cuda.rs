...
fn macd_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cpu = my_project::indicators::macd::macd_batch_with_kernel(&price, &sweep, Kernel::ScalarBatch)?;

    let price_f32: Vec<f32> = price.iter().map(|&v| v as f32).collect();
    let cuda = CudaMacd::new(0).expect("CudaMacd::new");
    let (dev_triplet, combos) = cuda.macd_batch_dev(&price_f32, &sweep).expect("macd_batch_dev");
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(approx_eq(cpu.macd[idx], g_macd[idx] as f64, tol), "macd mismatch at {}", idx);
        assert!(approx_eq(cpu.signal[idx], g_signal[idx] as f64, tol), "signal mismatch at {}", idx);
        assert!(approx_eq(cpu.hist[idx], g_hist[idx] as f64, tol), "hist mismatch at {}", idx);
    }
    Ok(())
}
...
fn macd_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols { for t in s..rows { let x = (t as f64) + (s as f64)*0.3; tm[t*cols + s] = (x*0.002).sin() + 0.0002*x; } }
    let fast=12usize; let slow=26usize; let signal=9usize;
    ...
    for s in 0..cols {
        ...
        for t in 0..rows { p[t] = tm[t * cols + s]; }
        let input = MacdInput::from_slice(&p, MacdParams { fast_period: Some(fast), slow_period: Some(slow), signal_period: Some(signal), ma_type: Some("ema".to_string()) });
        let out = macd(&input)?;
        for t in 0..rows {
    ...
    let cuda = CudaMacd::new(0).expect("CudaMacd::new");
    let params = MacdParams { fast_period: Some(fast), slow_period: Some(slow), signal_period: Some(signal), ma_type: Some("ema".to_string()) };
    let dev = cuda.macd_many_series_one_param_time_major_dev(&tm_f32, cols, rows, &params).expect("macd many series");
    assert_eq!(dev.macd.rows, rows); assert_eq!(dev.macd.cols, cols);
    ...
    for i in 0..(cols*rows) {
        assert!(approx_eq(cpu_macd[i], g_macd[i] as f64, tol), "macd mismatch at {}", i);
        assert!(approx_eq(cpu_signal[i], g_signal[i] as f64, tol), "signal mismatch at {}", i);
        assert!(approx_eq(cpu_hist[i], g_hist[i] as f64, tol), "hist mismatch at {}", i);
    }
    Ok(())
}
