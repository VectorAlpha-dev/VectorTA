// Integration tests for CUDA KAMA kernels

use my_project::indicators::moving_averages::kama::{
...
#[cfg(feature = "cuda")]
use my_project::cuda::moving_averages::CudaKama;
...
#[cfg(feature = "cuda")]
#[test]
fn kama_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[kama_cuda_batch_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    let cpu = kama_batch_with_kernel(&data_quant, &sweep, Kernel::ScalarBatch)?;

    let cuda = CudaKama::new(0)?;
    let handle = cuda.kama_batch_dev(&data_f32, &sweep)?;
...
#[cfg(feature = "cuda")]
#[test]
fn kama_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[kama_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for series in 0..num_series {
        for t in (series + 12)..series_len {
            let time = t as f64;
            let base = (time * 0.0031 + series as f64 * 0.17).sin();
            let drift = 0.00023 * time + 0.014 * series as f64;
    ...
    let params = KamaParams { period: Some(28) };
    ...
    for series in 0..num_series {
        ...
        let input = KamaInput::from_slice(&slice, params.clone());
        let out = kama_with_kernel(&input, Kernel::Scalar)?;
        for t in 0..series_len {
    ...
    let cuda = CudaKama::new(0)?;
    let handle = cuda.kama_many_series_one_param_time_major_dev(
        &data_tm_f32,
        num_series,
        series_len,
        &params,
