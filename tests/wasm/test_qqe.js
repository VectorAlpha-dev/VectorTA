...
test('QQE default params', () => {
    // Test with default parameters - mirrors check_qqe_default_params
...
test('QQE partial params', () => {
    // Test with partial parameters - mirrors check_qqe_partial_params
    const close = new Float64Array(testData.close);
...
test('QQE accuracy', async () => {
    ...
    // The result is now a flat structure with values, rows, cols
    assert.ok(result.values, 'Result should have values array');
    assert.strictEqual(result.rows, 2, 'Should have 2 rows (fast and slow)');
    assert.strictEqual(result.cols, close.length, 'Columns should match data length');
    
    // Extract fast and slow from flattened values
...
test('QQE unified output', async () => {
    // Test the unified output version
    const close = new Float64Array(testData.close);
    ...
    // Should return interleaved fast and slow values
    assert.strictEqual(result.length, close.length * 2);
    
    // Check that values alternate between fast and slow
...
test('QQE custom parameters', async () => {
    // Test with custom parameters - mirrors check_qqe_custom_params
    const close = new Float64Array(testData.close);
    ...
    // Run with default parameters for comparison
    const defaultResult = wasm.qqe_js(close, 14, 5, 4.236);
    
    // Extract fast values from both results
    const fast = result.values.slice(0, result.cols);
...
test('QQE reinput', () => {
    ...
    const warmupFirst = 17;  // 0 + 14 + 5 - 2
    const warmupSecond = warmupFirst + 17;  // Double application
...
test('QQE batch single parameter set', () => {
    // Skip if batch function not available
    ...
    // Using batch API for single parameter set (defaults)
    const batchConfig = {
...
test('QQE batch multiple parameters', () => {
    ...
    // Multiple parameters: 3 * 2 * 2 = 12 combinations
    const batchConfig = {
        rsi_period_range: [10, 14, 2],      // 10, 12, 14
        smoothing_factor_range: [3, 4, 1],  // 3, 4
        fast_factor_range: [3.0, 3.5, 0.5]  // 3.0, 3.5
    };
    
    const batchResult = wasm.qqe_batch_unified_js(close, batchConfig);
    
    // Should have 3 * 2 * 2 = 12 rows
    ...
    for (let i = 0; i < Math.min(3, batchResult.rows); i++) {
        ...
        // Calculate single for comparison
...
test('QQE constant data', () => {
    ...
    // After warmup, values should stabilize
    const warmup = 17; // 0 + 14 + 5 - 2
    
    if (fast.length > warmup + 10) {
        // Calculate standard deviation of last 10 values
