...
#[cfg(feature = "cuda")]
#[test]
fn ehma_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[ehma_cuda_batch_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    let cpu = ehma_batch_with_kernel(&data, &sweep, Kernel::ScalarBatch)?;

    let cuda = CudaEhma::new(0).expect("CudaEhma::new");
    let data_f32: Vec<f32> = data.iter().map(|&v| v as f32).collect();
    let gpu = cuda
        .ehma_batch_dev(&data_f32, &sweep)
        .expect("cuda ehma_batch_dev");
...
#[cfg(feature = "cuda")]
#[test]
fn ehma_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[ehma_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for j in 0..num_series {
        ...
        let out = EhmaBuilder::new().period(period).apply_slice(&series)?;
        for t in 0..series_len {
    ...
    let gpu = cuda
        .ehma_multi_series_one_param_time_major_dev(&data_tm_f32, num_series, series_len, &params)
        .expect("cuda ehma_multi_series_one_param_time_major_dev");
