...
use my_project::indicators::rocp::{rocp_batch_with_kernel, RocpBatchRange, RocpInput, RocpParams, rocp_with_kernel};
...
fn rocp_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[rocp_cuda_batch_matches_cpu] skipped - no CUDA device"); return Ok(()); }

    let len = 65536usize;
    let mut data = vec![f64::NAN; len];
    for i in 100..len { let x = i as f64; data[i] = (x * 0.00123).sin() + 0.00017 * x; }
    let sweep = RocpBatchRange { period: (4, 128, 5) };

    let cpu = rocp_batch_with_kernel(&data, &sweep, Kernel::ScalarBatch)?;

    let data_f32: Vec<f32> = data.iter().map(|&v| v as f32).collect();
    let cuda = CudaRocp::new(0).expect("CudaRocp::new");
    let (dev, combos) = cuda.rocp_batch_dev(&data_f32, &sweep).expect("rocp_batch_dev");
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(approx_eq(cpu.values[idx], gpu[idx] as f64, tol), "mismatch at {}", idx);
    }
    Ok(())
}
...
fn rocp_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[rocp_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device"); return Ok(()); }

    let cols = 16usize; // series
    let rows = 4096usize; // time
    let mut tm = vec![f64::NAN; cols * rows];
    for s in 0..cols { for t in s..rows { let x = t as f64 + (s as f64) * 0.25; tm[t*cols + s] = (x*0.002).sin() + 0.0003 * x; } }
    let period = 14usize;
    ...
    for s in 0..cols {
        ...
        let input = RocpInput::from_slice(&series, RocpParams { period: Some(period) });
