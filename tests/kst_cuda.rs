...
#[cfg(feature = "cuda")]
use vector_ta::cuda::oscillators::CudaKst;
...
fn kst_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (pair, _combos) = cuda
        .kst_batch_dev(&price_f32, &sweep)
        .expect("kst_batch_dev");
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        ...
        assert!(
            approx_eq(c_l, g_l, tol),
            "line mismatch at {}: cpu={} gpu={}",
            idx,
            c_l,
            g_l
        );
        assert!(
            approx_eq(c_s, g_s, tol),
