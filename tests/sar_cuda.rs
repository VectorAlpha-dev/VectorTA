...
use my_project::indicators::sar::{
    sar_with_kernel, SarBatchBuilder, SarInput, SarParams,
};
...
fn sar_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 4..len { let x = i as f64; base[i] = (x * 0.0013).sin() + 0.0002 * x; }
    ...
    let sweep = my_project::indicators::sar::SarBatchRange { acceleration: (0.01, 0.05, 0.01), maximum: (0.1, 0.3, 0.1) };
    let cpu = SarBatchBuilder::new().kernel(Kernel::ScalarBatch).apply_slices(&high, &low)?;
    ...
    let (dev, _combos) = cuda.sar_batch_dev(&h_f32, &l_f32, &sweep).expect("sar_batch_dev");
...
fn sar_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols {
        ...
        let out = sar_with_kernel(&SarInput::from_slices(&h, &l, params.clone()), Kernel::Scalar)?.values;
    ...
    let tol = 3e-3;
    for idx in 0..g.len() {
        assert!(approx_eq(cpu_tm[idx], g[idx] as f64, tol), "mismatch at {}", idx);
    }
    Ok(())
}
