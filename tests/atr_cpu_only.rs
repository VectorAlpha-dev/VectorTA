// Focused ATR CPU tests to validate the indicator in isolation from crate unit tests
use my_project::indicators::atr::{atr, AtrData, AtrInput, AtrParams};

#[test]
fn atr_basic_smoke() {
    // Simple synthetic OHLC where TR is deterministic
    ...
    // Warmup prefix should be NaN for first_valid + length - 1
...
fn atr_errors_mapped() {
    // Empty input -> EmptyInputData or equivalent
    let empty: [f64;0] = [];
