...
fn mab_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (trip, combos) = cuda
        .mab_batch_dev(&price_f32, &sweep)
        .expect("cuda mab batch");
    ...
    for i in 0..(cpu.rows * cpu.cols) {
        assert!(
            approx_eq(cpu.upperbands[i], up_g[i] as f64, tol),
            "upper mismatch at {}",
            i
        );
        assert!(
            approx_eq(cpu.middlebands[i], mid_g[i] as f64, tol),
            "middle mismatch at {}",
            i
        );
        assert!(
            approx_eq(cpu.lowerbands[i], lo_g[i] as f64, tol),
            "lower mismatch at {}",
            i
        );
    }
...
fn mab_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols {
        ...
        for t in 0..rows {
            let idx = t * cols + s;
            up_cpu[idx] = out.upperband[t];
            mid_cpu[idx] = out.middleband[t];
            lo_cpu[idx] = out.lowerband[t];
        }
    }
    ...
    for i in 0..up_g.len() {
        assert!(
            approx_eq(up_cpu[i], up_g[i] as f64, tol),
            "upper mismatch at {}",
            i
        );
        assert!(
            approx_eq(mid_cpu[i], mid_g[i] as f64, tol),
            "middle mismatch at {}",
            i
        );
        assert!(
            approx_eq(lo_cpu[i], lo_g[i] as f64, tol),
            "lower mismatch at {}",
            i
        );
    }
