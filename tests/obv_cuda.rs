...
fn obv_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cpu = obv(&ObvInput {
        data: ObvData::Slices {
            close: &close32_as_f64,
            volume: &volume32_as_f64,
        },
...
fn obv_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[obv_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for idx in 0..g_tm.len() {
        assert!(
            approx_eq_absrel(cpu_tm[idx], g_tm[idx] as f64, 1e-3, 1e-7),
            "mismatch at {}",
            idx
        );
    }
