...
try:
    import cupy as cp  # optional, used to wrap VRAM handles
...
def _cuda_available() -> bool:
    ...
    try:
        ...
        _ = cp.asarray(handle)  # CuPy should accept the handle
        return True
...
class TestChandelierExitCuda:
    ...
    def test_ce_cuda_batch_matches_cpu(self, test_data):
        ...
        # Row 0 = long, Row 1 = short for this single combo
    ...
    def test_ce_cuda_many_series_one_param_matches_cpu(self, test_data):
        ...
        gpu = cp.asnumpy(cp.asarray(handle))  # shape: (2*T, N)
        long_tm = gpu[:T, :]
        short_tm = gpu[T:, :]
