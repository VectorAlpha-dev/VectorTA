...
class TestAo:
    ...
    def test_ao_with_different_params(self, test_data):
        ...
        result = vector_ta.ao(high, low, 3, 10)
    ...
    def test_ao_invalid_periods(self, test_data):
        ...
        with pytest.raises(ValueError, match="Invalid periods"):
            vector_ta.ao(high, low, 0, 34)


        with pytest.raises(ValueError, match="Invalid periods"):
            vector_ta.ao(high, low, 5, 0)


        with pytest.raises(ValueError, match="Short period must be less than long period"):
            vector_ta.ao(high, low, 34, 34)

        with pytest.raises(ValueError, match="Short period must be less than long period"):
            vector_ta.ao(high, low, 35, 34)
    ...
    def test_ao_all_nan(self):
        ...
        with pytest.raises(ValueError, match="All values are NaN"):
            vector_ta.ao(all_nan, all_nan, 5, 34)
    ...
    def test_ao_kernel_selection(self, test_data):
        ...
        result_scalar = vector_ta.ao(high, low, 5, 34, kernel="scalar")


        result_auto = vector_ta.ao(high, low, 5, 34)
    ...
    def test_ao_streaming(self, test_data):
        ...
        batch_result = vector_ta.ao(high, low, 5, 34)


        stream = vector_ta.AoStream(5, 34)
        stream_results = []
    ...
    def test_ao_batch_single_params(self, test_data):
        ...
        single_result = vector_ta.ao(high, low, 5, 34)
    ...
    def test_ao_error_coverage(self, test_data):
        ...
        with pytest.raises(ValueError, match="All values are NaN"):
            vector_ta.ao(np.full(10, np.nan), np.full(10, np.nan), 5, 34)


        with pytest.raises(ValueError, match="Invalid periods"):
            vector_ta.ao(high[:50], low[:50], 0, 34)


        with pytest.raises(ValueError, match="Short period must be less than long period"):
            vector_ta.ao(high[:50], low[:50], 34, 34)


        with pytest.raises(ValueError, match="empty|no data|No data"):
            vector_ta.ao(np.array([]), np.array([]), 5, 34)


        with pytest.raises(ValueError, match="Not enough valid data"):
            vector_ta.ao(high[:10], low[:10], 5, 34)
