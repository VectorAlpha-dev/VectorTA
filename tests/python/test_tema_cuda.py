"""Python binding tests for TEMA CUDA kernels."""
import pytest
import numpy as np
...
def _cuda_available() -> bool:
    if cp is None:
        return False
    if not hasattr(ti, 'tema_cuda_batch_dev'):
        return False
    try:
        sample = np.array([np.nan, 1.0, 2.0, 3.0, 4.0], dtype=np.float32)
        handle = ti.tema_cuda_batch_dev(sample, (3, 3, 0))
        _ = cp.asarray(handle)
        return True
...
class TestTemaCuda:
    ...
    def test_tema_cuda_batch_matches_cpu(self, test_data):
        close = test_data['close']
        period = 21

        cpu = ti.tema(close, period)

        handle = ti.tema_cuda_batch_dev(
    ...
    def test_tema_cuda_many_series_one_param_matches_cpu(self, test_data):
        T = 512
        N = 4
