...
try:
    import cupy as cp
except ImportError:  # pragma: no cover - optional dep for CUDA verification
    cp = None
...
def _cuda_available() -> bool:
    ...
    try:
        ...
        handle = ti.nama_cuda_batch_dev(x, period_range=(4, 4, 0))
        _ = cp.asarray(handle)
        return True
...
@pytest.mark.skipif(not _cuda_available(), reason="CUDA not available or NAMA CUDA bindings missing")
class TestNamaCuda:
    ...
    def test_nama_cuda_batch_matches_cpu(self, test_data):
        ...
        cpu = ti.nama(close.astype(np.float64), period)
    ...
    def test_nama_cuda_many_series_one_param_matches_cpu(self, test_data):
        ...
        for j in range(N):
            cpu_tm[:, j] = ti.nama(data_tm[:, j], period)
        ...
        assert_close(
            ...
            rtol=1e-5,
            atol=1e-6,
            msg="NAMA CUDA many-series vs CPU mismatch",
        )
