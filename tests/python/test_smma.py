...
class TestSmma:
    ...
    def test_smma_nan_handling(self, test_data):
        ...
        assert np.all(np.isnan(result[:6])), "Expected NaN in warmup period"
        # First valid value should be at index period-1
        assert not np.isnan(result[6]), "Expected valid value at index period-1"
    ...
    def test_smma_edge_case_period_one(self):
        ...
        assert_close(result, data, rtol=1e-12, 
