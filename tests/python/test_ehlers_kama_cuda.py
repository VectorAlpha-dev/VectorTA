"""Python binding tests for Ehlers KAMA CUDA kernels."""
import pytest
import numpy as np
...
def _cuda_available() -> bool:
    if cp is None:
        return False
    if not hasattr(ti, "ehlers_kama_cuda_batch_dev"):
        return False
    try:
        probe = np.array([np.nan, np.nan, 1.0, 2.0, 3.0, 4.0], dtype=np.float64)
        handle = ti.ehlers_kama_cuda_batch_dev(probe, (6, 6, 0))
        _ = cp.asarray(handle)
        return True
...
class TestEhlersKamaCuda:
    ...
    def test_ehlers_kama_cuda_batch_matches_cpu(self, price_series):
        sweep = (5, 125, 5)

        cpu = ti.ehlers_kama_batch(price_series, sweep)
