"""Python binding tests for TRIMA CUDA kernels."""
import pytest
import numpy as np
...
def _cuda_available() -> bool:
    if cp is None:
        return False
    if not hasattr(ti, "trima_cuda_batch_dev"):
        return False
    try:
        probe = np.array([np.nan, np.nan, 1.0, 2.0, 3.0, 4.0], dtype=np.float64)
        handle = ti.trima_cuda_batch_dev(probe, (6, 6, 0))
        _ = cp.asarray(handle)
        return True
...
class TestTrimaCuda:
    ...
    def test_trima_cuda_batch_matches_cpu(self, price_series):
        sweep = (10, 64, 6)

        cpu = ti.trima_batch(price_series, sweep)
        cpu_values = np.asarray(cpu["values"], dtype=np.float64)

        handle = ti.trima_cuda_batch_dev(price_series, sweep)
    ...
    def test_trima_cuda_many_series_one_param_matches_cpu(self, price_series):
        T = 1024
        N = 4
