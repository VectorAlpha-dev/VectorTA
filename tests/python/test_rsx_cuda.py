"""
Python binding tests for RSX CUDA kernels.
...
def _cuda_available() -> bool:
    if cp is None:
        return False
    if not hasattr(ti, 'rsx_cuda_batch_dev'):
        return False
    try:
...
class TestRsxCuda:
    def test_rsx_cuda_batch_matches_cpu(self):
        close = load_test_data()['close'].astype(np.float64)
        period = 14
        cpu = ti.rsx(close, period=period)

        handle = ti.rsx_cuda_batch_dev(close.astype(np.float32), (period, period, 0))
        gpu_row = cp.asnumpy(cp.asarray(handle))[0]

        assert_close(gpu_row, cpu, rtol=1e-3, atol=5e-3, msg="RSX CUDA batch vs CPU mismatch")

    def test_rsx_cuda_many_series_one_param_matches_cpu(self):
        T = 2048
        N = 6
