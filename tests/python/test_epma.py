...
class TestEpma:
    ...
    def test_epma_accuracy(self, test_data):
        ...
        # Match Rust tolerance exactly: absolute 1e-1, no relative tolerance
        assert_close(
            out[-5:],
            exp['last_5_values'],
            rtol=0.0,
        ...
        # Full-series parity vs Rust reference
    ...
    def test_epma_streaming(self, test_data):
        ...
        # Compare where both are finite (skip warmup Nones/NaNs)
    ...
    def test_epma_batch(self, test_data):
        ...
        # Single-combo batch (default row)
        ...
        # Multi-combo batch and consistency
        # Use offsets that are valid for all chosen periods (offset < period)
        batch2 = ti.epma_batch(close[:200], (5, 11, 3), (1, 3, 2))  # periods: 5,8,11; offsets:1,3
        rows = len(batch2['periods'])
        assert batch2['values'].shape[0] == rows
        assert rows == len(batch2['offsets'])
        # Verify each row equals a single-run result
        for i in range(rows):
