"""Python binding tests for LINREG CUDA kernels."""
import pytest
import numpy as np
...
def _cuda_available() -> bool:
    if cp is None:
        return False
    if not hasattr(ti, "linreg_cuda_batch_dev"):
        return False
    try:
        sample = np.array([np.nan, 1.0, 2.0, 3.0, 4.0], dtype=np.float32)
        handle, meta = ti.linreg_cuda_batch_dev(sample, period_range=(10, 14, 2))
        _ = cp.asarray(handle)
...
class TestLinregCuda:
    @pytest.fixture(scope="class")
    def test_data(self):
        return load_test_data()

    def test_linreg_cuda_batch_matches_cpu(self, test_data):
        close = test_data["close"].astype(np.float64)
        sweep = (6, 30, 6)

        cpu = ti.linreg_batch(close, sweep)
        cpu_values = cpu["values"]
        cpu_periods = cpu["periods"]

        handle, meta = ti.linreg_cuda_batch_dev(close.astype(np.float32), sweep)
        ...
        assert_close(
            ...
            rtol=8e-4,
            atol=2.5e-3,
            msg="CUDA LINREG mismatch vs CPU baseline",
        )

    def test_linreg_cuda_many_series_one_param_matches_cpu(self, test_data):
        close = test_data["close"].astype(np.float64)
        period = 18
        cols = 4
        rows = close.shape[0]
        ...
        assert_close(
            ...
            rtol=8e-4,
            atol=3e-3,
            msg="CUDA LINREG many-series mismatch vs CPU baseline",
        )
