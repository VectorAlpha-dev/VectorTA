"""Python binding tests for SWMA CUDA kernels."""
import pytest
import numpy as np
...
def _cuda_available() -> bool:
    if cp is None:
        return False
    if not hasattr(ti, "swma_cuda_batch_dev"):
        return False
    try:
        probe = np.array([np.nan, np.nan, 1.0, 2.0, 3.0, 4.0], dtype=np.float64)
        handle = ti.swma_cuda_batch_dev(probe, (6, 6, 0))
        _ = cp.asarray(handle)
        return True
...
class TestSwmaCuda:
    ...
    def test_swma_cuda_batch_matches_cpu(self, price_series):
        sweep = (5, 120, 5)

        cpu = ti.swma_batch(price_series, sweep)
        cpu_values = np.asarray(cpu["values"], dtype=np.float64)

        handle = ti.swma_cuda_batch_dev(price_series, sweep)
        gpu = cp.asnumpy(cp.asarray(handle)).astype(np.float64)

        assert gpu.shape == cpu_values.shape
        assert_close(gpu, cpu_values, rtol=2e-4, atol=5e-4, msg="CUDA batch vs CPU mismatch")

    def test_swma_cuda_many_series_one_param_matches_cpu(self, price_series):
        T = 1024
        N = 4
        ...
        assert_close(gpu_tm, cpu_tm, rtol=2e-4, atol=5e-4, msg="CUDA many-series vs CPU mismatch")
