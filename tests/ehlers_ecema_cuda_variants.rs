...
fn ecema_cuda_variants_batch_plain_and_tiled_match_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[ecema variants] skipped - no CUDA device"); return Ok(()); }
    ...
    let sweep = EhlersEcemaBatchRange { length: (6, 30, 6), gain_limit: (10, 50, 20) };
    let cpu = ehlers_ecema_batch_with_kernel(&data_quant, &sweep, Kernel::ScalarBatch)?;

    // Plain: use legacy one-block-per-combo
    let mut cuda_plain = CudaEhlersEcema::new_with_policy(0, CudaEhlersEcemaPolicy {
        batch: BatchKernelPolicy::Plain { block_x: 1 },
        many_series: ManySeriesKernelPolicy::Auto,
    })?;
    let params = EhlersEcemaParams { length: None, gain_limit: None, pine_compatible: Some(false), confirmed_only: Some(false) };
    ...
    let mut cuda_tiled = CudaEhlersEcema::new_with_policy(0, CudaEhlersEcemaPolicy {
        batch: BatchKernelPolicy::Tiled { tile: 128, per_thread: BatchThreadsPerOutput::One },
        many_series: ManySeriesKernelPolicy::Auto,
    })?;
    ...
    for i in 0..cpu.values.len() {
        ...
        assert!(approx_eq(c, p, tol_plain), "plain mismatch at {}: {} vs {}", i, c, p);
        assert!(approx_eq(c, t, tol_tiled), "tiled mismatch at {}: {} vs {}", i, c, t);
    }
...
fn ecema_cuda_variants_many_series_1d_and_2d_match_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols { for t in (s + 8)..rows { let time = t as f64; tm[t * cols + s] = (time * 0.0023 + s as f64 * 0.41).sin() + (time * 0.0009).cos() * (0.2 + 0.05 * s as f64) + 0.0007 * time; }}
    let tm_f32: Vec<f32> = tm.iter().map(|&v| v as f32).collect();
    let tm_quant: Vec<f64> = tm_f32.iter().map(|&v| v as f64).collect();

    let params = EhlersEcemaParams { length: Some(24), gain_limit: Some(40), pine_compatible: Some(false), confirmed_only: Some(false) };

    // CPU
    let mut cpu_tm = vec![f64::NAN; cols * rows];
    for s in 0..cols {
        let mut series = vec![f64::NAN; rows];
        for t in 0..rows { series[t] = tm_quant[t * cols + s]; }
    ...
    // 1D
    let mut cuda_1d = CudaEhlersEcema::new_with_policy(0, CudaEhlersEcemaPolicy {
        batch: BatchKernelPolicy::Auto,
        many_series: ManySeriesKernelPolicy::OneD { block_x: 128 },
    })?;
    let handle_1d = cuda_1d.ehlers_ecema_many_series_one_param_time_major_dev(&tm_f32, cols, rows, &params)?;
    let mut gpu_1d = vec![0f32; handle_1d.len()];
    handle_1d.buf.copy_to(&mut gpu_1d)?;

    // 2D
    let mut cuda_2d = CudaEhlersEcema::new_with_policy(0, CudaEhlersEcemaPolicy {
        batch: BatchKernelPolicy::Auto,
        many_series: ManySeriesKernelPolicy::Tiled2D { tx: 128, ty: 2 },
    })?;
    let handle_2d = cuda_2d.ehlers_ecema_many_series_one_param_time_major_dev(&tm_f32, cols, rows, &params)?;
