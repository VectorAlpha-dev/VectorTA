...
use my_project::indicators::correlation_cycle::{
    correlation_cycle_batch_with_kernel, correlation_cycle_with_kernel,
    CorrelationCycleBatchRange, CorrelationCycleInput, CorrelationCycleParams,
};
...
fn correlation_cycle_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sweep = CorrelationCycleBatchRange { period: (16, 64, 8), threshold: (9.0, 9.0, 0.0) };
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(approx_eq(cpu.real[idx], g_real[idx] as f64, tol), "real mismatch at {}", idx);
        assert!(approx_eq(cpu.imag[idx], g_imag[idx] as f64, tol), "imag mismatch at {}", idx);
        assert!(approx_eq(cpu.angle[idx], g_ang[idx] as f64, 0.1), "angle mismatch at {}", idx);
        // state is discrete, tolerate exact or NaN warmup
        let cs = cpu.state[idx];
        let gs = g_st[idx] as f64;
        if !(cs.is_nan() && gs.is_nan()) {
            assert!(approx_eq(cs, gs, 1e-3), "state mismatch at {}: cpu={} gpu={}", idx, cs, gs);
        }
    }
    Ok(())
}
...
#[cfg(feature = "cuda")]
#[test]
fn correlation_cycle_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[correlation_cycle_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    let cols = 7usize; let rows = 2048usize;
    let mut price_tm = vec![f64::NAN; cols * rows];
    for s in 0..cols { for t in s..rows {
        let x = (t as f64) + (s as f64) * 0.2; price_tm[t*cols+s] = (x*0.002).sin() + 0.0003*x;
    }}
    let period = 32usize; let threshold = 9.0f64;
    ...
    for idx in 0..(cols*rows) {
        assert!(approx_eq(cpu_real[idx], g_real[idx] as f64, tol), "real mismatch at {}", idx);
        assert!(approx_eq(cpu_imag[idx], g_imag[idx] as f64, tol), "imag mismatch at {}", idx);
        if !(cpu_ang[idx].is_nan() && (g_ang[idx] as f64).is_nan()) {
            assert!(approx_eq(cpu_ang[idx], g_ang[idx] as f64, 0.15), "angle mismatch at {}", idx);
        }
        let cs = cpu_st[idx]; let gs = g_st[idx] as f64;
        if !(cs.is_nan() && gs.is_nan()) {
