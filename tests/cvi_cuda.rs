...
use my_project::indicators::cvi::{cvi_batch_with_kernel, cvi_with_kernel, CviBatchRange, CviInput, CviParams, CviData};
...
fn cvi_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 5..len { let x = i as f64; close[i] = (x * 0.00123).sin() + 0.00017 * x; }
    let mut high = close.clone();
    let mut low  = close.clone();
    for i in 0..len { let v = close[i]; if v.is_nan() { continue; } let x = i as f64 * 0.002; let off = (0.004 * x.sin()).abs() + 0.12; high[i] = v + off; low[i] = v - off; }
    ...
    let dev = cuda.cvi_batch_dev(&high_f32, &low_f32, &sweep).expect("cvi_batch_dev");
...
fn cvi_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols { for t in s..rows { let x = (t as f64) + (s as f64) * 0.2; base[t * cols + s] = (x * 0.002).sin() + 0.0003 * x; } }
    let mut high_tm = base.clone();
    let mut low_tm  = base.clone();
    for s in 0..cols { for t in 0..rows { let v = base[t * cols + s]; if v.is_nan() { continue; } let x = (t as f64) * 0.002; let off = (0.004 * x.cos()).abs() + 0.11; high_tm[t * cols + s] = v + off; low_tm[t * cols + s] = v - off; } }
    ...
    for s in 0..cols {
        ...
        let input = CviInput { data: CviData::Slices { high: &h, low: &l }, params };
