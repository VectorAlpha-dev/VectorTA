...
#[cfg(feature = "cuda")]
use my_project::cuda::moving_averages::{BatchKernelPolicy, BatchThreadsPerOutput, CudaCwmaPolicy, ManySeriesKernelPolicy};
...
fn cwma_cuda_batched_tiled_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut data = vec![f64::NAN; series_len];
    for i in 7..series_len { let x = i as f64; data[i] = (x * 0.001).cos() + 0.0001 * x; }
    let sweep = CwmaBatchRange { period: (8, 64, 3) };

    let cpu = match cwma_batch_with_kernel(&data, &sweep, Kernel::ScalarBatch) { Ok(v) => v, Err(e) => return Err(Box::new(e)), };

    let mut cuda = CudaCwma::new_with_policy(0, CudaCwmaPolicy { batch: BatchKernelPolicy::Tiled { tile: 256, per_thread: BatchThreadsPerOutput::Two }, many_series: ManySeriesKernelPolicy::Auto })
        .map_err(|e| Box::<dyn std::error::Error>::from(e))?;
    let data_f32: Vec<f32> = data.iter().map(|&v| v as f32).collect();
    let gpu_handle = cuda.cwma_batch_dev(&data_f32, &sweep).map_err(|e| Box::<dyn std::error::Error>::from(e))?;

    assert_eq!(cpu.rows, gpu_handle.rows);
    assert_eq!(cpu.cols, gpu_handle.cols);
    let mut gpu_host = vec![0f32; gpu_handle.len()];
    gpu_handle.buf.copy_to(&mut gpu_host).map_err(|e| Box::<dyn std::error::Error>::from(e))?;
    let tol = 5e-5;
    for idx in 0..(cpu.rows * cpu.cols) {
        let a = cpu.values[idx];
        let b = gpu_host[idx] as f64;
...
fn cwma_cuda_many_series_tiled2d_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let series_len = 4096usize;
    let mut data_tm = vec![f64::NAN; num_series * series_len];
    for j in 0..num_series { for t in (j + 3)..series_len { let x = (t as f64) + (j as f64) * 0.1; data_tm[t * num_series + j] = (x * 0.0009).sin(); } }
    ...
    for j in 0..num_series {
        ...
        let out = match CwmaBuilder::default().period(params.period.unwrap()).apply_slice(&series) { Ok(v) => v, Err(e) => return Err(Box::new(e)), };
        for t in 0..series_len { cpu_tm[t * num_series + j] = out.values[t]; }
    }

    let mut cuda = CudaCwma::new_with_policy(0, CudaCwmaPolicy { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::Tiled2D { tx: 128, ty: 4 } }).map_err(|e| Box::<dyn std::error::Error>::from(e))?;
    let data_tm_f32: Vec<f32> = data_tm.iter().map(|&v| v as f32).collect();
    let gpu_handle = cuda.cwma_multi_series_one_param_time_major_dev(&data_tm_f32, num_series, series_len, &params).map_err(|e| Box::<dyn std::error::Error>::from(e))?;
    assert_eq!(gpu_handle.rows, series_len);
    assert_eq!(gpu_handle.cols, num_series);
    let mut gpu_tm = vec![0f32; gpu_handle.len()];
    gpu_handle.buf.copy_to(&mut gpu_tm).map_err(|e| Box::<dyn std::error::Error>::from(e))?;
    let tol = 5e-5;
    for idx in 0..(num_series * series_len) {
        let a = cpu_tm[idx];
        let b = gpu_tm[idx] as f64;
