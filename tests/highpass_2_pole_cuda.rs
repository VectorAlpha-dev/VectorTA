// Integration tests for CUDA highpass_2_pole kernels
...
#[cfg(feature = "cuda")]
use my_project::cuda::moving_averages::CudaHighPass2;
...
#[cfg(feature = "cuda")]
#[test]
fn highpass2_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[highpass2_cuda_batch_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for i in 5..series_len {
        let x = i as f64;
        data[i] = (x * 0.00097).sin() + 0.00013 * x.cos();
    }
    ...
    let cpu = highpass_2_pole_batch_with_kernel(&data, &sweep, Kernel::ScalarBatch)?;

    let cuda = CudaHighPass2::new(0).expect("CudaHighPass2::new");
    let data_f32: Vec<f32> = data.iter().map(|&v| v as f32).collect();
    let gpu_handle = cuda
        .highpass2_batch_dev(&data_f32, &sweep)
        .expect("cuda highpass2 batch dev");
    ...
    gpu_handle
        ...
        .expect("copy highpass2 cuda batch result to host");
...
#[cfg(feature = "cuda")]
#[test]
fn highpass2_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[highpass2_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for series in 0..num_series {
        for t in (series * 2)..series_len {
            ...
            data_tm[t * num_series + series] = (base * 0.0023).cos() + 0.00041 * base.sin();
        }
    }
    ...
    for series in 0..num_series {
        ...
        let input = HighPass2Input::from_slice(&single, params.clone());
        let cpu_vals = highpass_2_pole_with_kernel(&input, Kernel::Scalar)?;
        for t in 0..series_len {
    ...
    let handle = cuda
        ...
        .expect("cuda highpass2 many-series dev");
    ...
    handle
        .buf
        .copy_to(&mut gpu_tm)
        .expect("copy highpass2 cuda many-series result to host");
