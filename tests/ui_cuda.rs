...
use my_project::indicators::ui::{
    ui_batch_slice, ui_with_kernel, UiBatchRange, UiInput, UiParams,
};
...
fn ui_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sweep = UiBatchRange { period: (10, 28, 3), scalar: (50.0, 150.0, 25.0) };
...
fn ui_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols {
        ...
        let input = UiInput { data: my_project::indicators::ui::UiData::Slice(&series), params };
    ...
    for idx in 0..gpu_tm.len() {
        assert!(approx_eq(cpu_tm[idx], gpu_tm[idx] as f64, tol), "many-series mismatch at {}", idx);
    }
    Ok(())
}
