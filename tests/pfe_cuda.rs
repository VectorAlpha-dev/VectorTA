...
#[cfg(feature = "cuda")]
#[test]
fn pfe_cuda_device_inputs_match_legacy_batch() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[pfe_cuda_device_inputs_match_legacy_batch] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for i in 0..len {
        let x = i as f64;
        price[i] = (x * 0.0017).sin() + 0.00011 * x;
    }
    ...
    let legacy = cuda.pfe_batch_dev(&price_f32, &sweep).expect("legacy pfe");
    let device = cuda
        .pfe_batch_dev_from_device_prices(d_price.buffer(), len, 0, &sweep)
        .expect("device pfe");
