// Integration tests for CUDA Reflex kernels

use my_project::indicators::moving_averages::reflex::{
    reflex_batch_with_kernel, ReflexBatchRange, ReflexBuilder,
};
...
#[cfg(feature = "cuda")]
use my_project::cuda::moving_averages::CudaReflex;
...
#[cfg(feature = "cuda")]
#[test]
fn reflex_cuda_one_series_many_params_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[reflex_cuda_one_series_many_params_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for i in 5..series_len {
        let x = i as f64;
        data[i] = (x * 0.0023).sin() + (x * 0.0007).cos() * 0.5;
    }
    ...
    gpu_handle
        ...
        .expect("copy reflex batch result to host");
...
#[cfg(feature = "cuda")]
#[test]
fn reflex_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[reflex_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for j in 0..num_series {
        for t in 0..series_len {
            ...
            data_tm[t * num_series + j] = (x * 0.0014).cos() + 0.0008 * x;
        }
    }
    ...
    gpu_handle
        .buf
        .copy_to(&mut gpu_tm)
        .expect("copy reflex many-series result to host");
