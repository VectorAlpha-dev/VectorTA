...
use my_project::indicators::ao::{ao_with_kernel, AoInput, AoParams, AoBatchRange, ao_batch_with_kernel};
...
fn ao_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[ao_cuda_batch_matches_cpu] skipped - no CUDA device"); return Ok(()); }

    let len = 8192usize;
    let mut hl2 = vec![f64::NAN; len];
    for i in 10..len { let x = i as f64; hl2[i] = (x * 0.00123).sin() + 0.00017 * x; }
    let sweep = AoBatchRange { short_period: (4, 28, 4), long_period: (32, 128, 16) };
...
fn ao_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[ao_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device"); return Ok(()); }

    let cols = 8usize; // series
    let rows = 1024usize; // time
    let mut hl2_tm = vec![f64::NAN; cols * rows];
    for s in 0..cols { for t in s..rows { let x = (t as f64) + (s as f64) * 0.2; hl2_tm[t * cols + s] = (x * 0.002).sin() + 0.0003 * x; } }
    ...
    for s in 0..cols {
        ...
        for t in 0..rows { series[t] = hl2_tm[t * cols + s]; }
        let params = AoParams { short_period: Some(short), long_period: Some(long) };
