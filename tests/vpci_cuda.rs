// Integration tests for CUDA VPCI kernels

use my_project::utilities::enums::Kernel;
use my_project::indicators::vpci::{vpci_batch_with_kernel, VpciBatchRange, VpciInput, VpciParams, vpci_with_kernel};

#[cfg(feature = "cuda")]
use my_project::cuda::cuda_available;
#[cfg(feature = "cuda")]
use my_project::cuda::vpci_wrapper::CudaVpci;
#[cfg(feature = "cuda")]
...
fn vpci_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sweep = VpciBatchRange { short_range: (5, 13, 2), long_range: (16, 40, 4) };
    ...
    let (pair, combos) = cuda.vpci_batch_dev(&c32, &v32, &sweep).expect("vpci_batch_dev");
    ...
    let tol = 5e-2; // prefix-sum parity in f32
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(approx_eq(cpu.vpci[idx],  vpci_g[idx]  as f64, tol),  "vpci mismatch at {}", idx);
        assert!(approx_eq(cpu.vpcis[idx], vpcis_g[idx] as f64, tol), "vpcis mismatch at {}", idx);
    }
    Ok(())
}
...
fn vpci_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols {
        for r in (s + 10)..rows { // stagger first valid a bit
            let idx = r * cols + s;
    ...
    let short = 7usize; let long = 24usize;
    ...
    for s in 0..cols {
        ...
        for r in 0..rows { let idx = r * cols + s; c[r] = close_tm[idx]; v[r] = volume_tm[idx]; }
        let params = VpciParams { short_range: Some(short), long_range: Some(long) };
        let input = VpciInput::from_slices(&c, &v, params);
        let out = vpci_with_kernel(&input, Kernel::Scalar)?;
        for r in 0..rows { let idx = r * cols + s; vpci_cpu_tm[idx] = out.vpci[r]; vpcis_cpu_tm[idx] = out.vpcis[r]; }
    }
    ...
    let cuda = CudaVpci::new(0).expect("CudaVpci::new");
    let pair = cuda
        .vpci_many_series_one_param_time_major_dev(&c32, &v32, cols, rows, &VpciParams { short_range: Some(short), long_range: Some(long) })
    ...
    for idx in 0..(rows*cols) {
        assert!(approx_eq(vpci_cpu_tm[idx],  vpci_g[idx]  as f64, tol),  "vpci mismatch at {}", idx);
        assert!(approx_eq(vpcis_cpu_tm[idx], vpcis_g[idx] as f64, tol), "vpcis mismatch at {}", idx);
    }
    Ok(())
}
