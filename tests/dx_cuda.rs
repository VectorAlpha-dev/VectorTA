...
#[cfg(feature = "cuda")]
#[test]
fn dx_cuda_device_inputs_match_legacy_batch() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[dx_cuda_device_inputs_match_legacy_batch] skipped - no CUDA device");
        return Ok(());
    }
    ...
    let sweep = DxBatchRange { period: (6, 18, 6) };
