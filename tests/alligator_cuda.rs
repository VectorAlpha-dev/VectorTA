...
fn alligator_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sweep = AlligatorBatchRange {
        jaw_period: (10, 18, 4), jaw_offset: (3, 6, 1),
        teeth_period: (6, 14, 4), teeth_offset: (2, 5, 1),
        lips_period: (3, 9, 3), lips_offset: (1, 3, 1),
    };
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(approx_eq(cpu.jaw[idx], jaw_host[idx] as f64, tol), "jaw mismatch at {}", idx);
        assert!(approx_eq(cpu.teeth[idx], teeth_host[idx] as f64, tol), "teeth mismatch at {}", idx);
        assert!(approx_eq(cpu.lips[idx], lips_host[idx] as f64, tol), "lips mismatch at {}", idx);
    }
...
fn alligator_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for t in 12..rows { for j in 0..cols { let idx = t * cols + j; let x = (t as f64) + (j as f64) * 0.09; data_tm[idx] = (x * 0.0023).cos() + 0.0007 * x; } }
    ...
    for j in 0..cols {
        ...
        for t in 0..rows { series[t] = data_tm[t * cols + j]; }
        let out = AlligatorBuilder::new().apply_slice(&series)?;
        for t in 0..rows { let idx = t * cols + j; jaw_tm[idx] = out.jaw[t]; teeth_tm[idx] = out.teeth[t]; lips_tm[idx] = out.lips[t]; }
    }
    ...
    for idx in 0..(cols * rows) {
        assert!(approx_eq(jaw_tm[idx], jaw_gpu[idx] as f64, tol), "jaw mismatch at {}", idx);
        assert!(approx_eq(teeth_tm[idx], teeth_gpu[idx] as f64, tol), "teeth mismatch at {}", idx);
        assert!(approx_eq(lips_tm[idx], lips_gpu[idx] as f64, tol), "lips mismatch at {}", idx);
    }
