...
use my_project::indicators::rvi::{rvi_batch_with_kernel, RviBatchRange, RviInput, RviParams, rvi_with_kernel};
...
fn rvi_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[rvi_cuda_batch_matches_cpu] skipped - no CUDA device"); return Ok(()); }

    let len = 131072usize;
    let mut data = vec![f64::NAN; len];
    for i in 200..len { let x = i as f64; data[i] = (x * 0.0011).sin() + 0.00021 * x; }
    // Keep to devtype=0 (StdDev) and matype=1 (EMA) for CUDA parity
    let sweep = RviBatchRange { period: (10, 24, 2), ma_len: (14, 14, 0), matype: (1, 1, 0), devtype: (0, 0, 0) };

    let cpu = rvi_batch_with_kernel(&data, &sweep, Kernel::ScalarBatch)?;

    let data_f32: Vec<f32> = data.iter().map(|&v| v as f32).collect();
    let cuda = CudaRvi::new(0).expect("CudaRvi::new");
    let (dev, combos) = cuda.rvi_batch_dev(&data_f32, &sweep).expect("rvi_batch_dev");
...
fn rvi_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[rvi_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device"); return Ok(()); }
