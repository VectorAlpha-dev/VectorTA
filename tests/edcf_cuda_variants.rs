...
fn gen_time_major_f64(cols: usize, rows: usize) -> Vec<f64> {
    ...
    for j in 0..cols {
        for t in (j + 1)..rows { // stagger first_valids
...
#[cfg(feature = "cuda")]
fn compare_batch(policy: CudaEdcfPolicy, series_len: usize, start_period: usize, end_period: usize) {
    if !cuda_available() { eprintln!("[edcf compare_batch] skipped - no CUDA device"); return; }

    let data = gen_series_f64(series_len, 3);
    let sweep = EdcfBatchRange { period: (start_period, end_period, 1) };
    ...
    let dev = cuda.edcf_batch_dev(&data_f32, &sweep).expect("gpu edcf batch");
...
fn compare_many_series(policy: CudaEdcfPolicy, cols: usize, rows: usize, period: usize) {
    if !cuda_available() { eprintln!("[edcf compare_many_series] skipped - no CUDA device"); return; }
    ...
    for j in 0..cols {
        ...
        let out = EdcfBuilder::default().period(period).apply_slice(&s).expect("cpu edcf series");
        for t in 0..rows { cpu_tm[t * cols + j] = out.values[t]; }
    }

    let mut cuda = CudaEdcf::new_with_policy(0, policy).expect("cuda edcf");
    let tm_f32: Vec<f32> = tm.iter().map(|&v| v as f32).collect();
    let dev = cuda.edcf_many_series_one_param_time_major_dev(&tm_f32, cols, rows, &params).expect("gpu many-series");
...
fn edcf_batch_plain_matches_cpu() {
    let policy = CudaEdcfPolicy { batch: BatchKernelPolicy::Plain { block_x: 256 }, many_series: ManySeriesKernelPolicy::Auto };
...
fn edcf_batch_tiled_matches_cpu() {
    let policy = CudaEdcfPolicy { batch: BatchKernelPolicy::Tiled { tile: 256 }, many_series: ManySeriesKernelPolicy::Auto };
...
fn edcf_many_series_1d_matches_cpu() {
    let policy = CudaEdcfPolicy { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::OneD { block_x: 128 } };
...
fn edcf_many_series_2d_ty4_matches_cpu() {
    let policy = CudaEdcfPolicy { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::Tiled2D { tx: 128, ty: 4 } };
...
fn edcf_many_series_2d_ty2_matches_cpu() {
    let policy = CudaEdcfPolicy { batch: BatchKernelPolicy::Auto, many_series: ManySeriesKernelPolicy::Tiled2D { tx: 128, ty: 2 } };
