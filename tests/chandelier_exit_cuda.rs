...
use my_project::indicators::chandelier_exit::{
    ce_batch_with_kernel, ChandelierExitBuilder, ChandelierExitData, ChandelierExitInput,
    ChandelierExitParams, CeBatchRange,
};
...
fn chandelier_exit_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 4..len { let x = i as f64; close[i] = (x * 0.002).sin() + 0.0003 * x; }
    ...
    let sweep = CeBatchRange { period: (10, 30, 10), mult: (2.0, 3.0, 1.0), use_close: (true, true, false) };
    ...
    for r in 0..cpu.rows {
        let base = r * cpu.cols;
        for i in 0..cpu.cols {
            assert!(approx_eq(cpu.values[base + i], got[base + i] as f64, tol),
                "mismatch at row {}, col {}: cpu={} gpu={}", r, i, cpu.values[base+i], got[base+i]);
        }
    }
    Ok(())
}
...
fn chandelier_exit_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[chandelier_exit_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for s in 0..cols {
        for t in s..rows { let x = (t as f64) + (s as f64) * 0.2; close_tm[t*cols+s] = (x*0.002).sin() + 0.0003*x; }
    }
    ...
    let period = 22usize; let mult = 3.0f64; let use_close = true;
    ...
    for s in 0..cols {
        ...
        let input = ChandelierExitInput { data: ChandelierExitData::Slices { high: &h, low: &l, close: &c }, params };
        let out = ChandelierExitBuilder::new().period(period).mult(mult).use_close(use_close).apply_slices(&h,&l,&c).unwrap();
        for t in 0..rows { cpu_long_tm[t*cols+s] = out.long_stop[t]; cpu_short_tm[t*cols+s] = out.short_stop[t]; }
    }
    ...
    let cuda = CudaChandelierExit::new(0).expect("cuda ce");
    let dev = cuda
        .chandelier_exit_many_series_one_param_time_major_dev(&high_tm_f32, &low_tm_f32, &close_tm_f32, cols, rows, period, mult as f32, use_close)
    ...
    for idx in 0..(rows*cols) {
        assert!(approx_eq(cpu_long_tm[idx], got_long[idx] as f64, tol), "long mismatch at {}", idx);
        assert!(approx_eq(cpu_short_tm[idx], got_short[idx] as f64, tol), "short mismatch at {}", idx);
    }
    Ok(())
}
