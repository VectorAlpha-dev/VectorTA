...
fn emd_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sweep = EmdBatchRange { period: (10, 22, 4), delta: (0.3, 0.7, 0.2), fraction: (0.05, 0.15, 0.05) };
    ...
    let res = cuda.emd_batch_dev(&high_f32, &low_f32, &sweep).expect("cuda emd batch");
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(approx_eq(cpu.upperband[idx], g_upper[idx] as f64, tol), "upper mismatch at {}", idx);
        assert!(approx_eq(cpu.middleband[idx], g_middle[idx] as f64, tol), "middle mismatch at {}", idx);
        assert!(approx_eq(cpu.lowerband[idx], g_lower[idx] as f64, tol), "lower mismatch at {}", idx);
    }
    Ok(())
}
...
fn emd_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let period = 18usize; let delta = 0.5f64; let fraction = 0.1f64;
    ...
    for s in 0..cols {
        ...
        for t in 0..rows { cpu_upper_tm[t * cols + s] = out.upperband[t]; cpu_middle_tm[t * cols + s] = out.middleband[t]; cpu_lower_tm[t * cols + s] = out.lowerband[t]; }
    }

    let mid_tm_f32: Vec<f32> = mid_tm.iter().map(|&v| v as f32).collect();
    // Compute first_valids per series for GPU wrapper
    let mut first_valids = vec![0i32; cols];
    for s in 0..cols { let mut fv = 0i32; for t in 0..rows { if mid_tm_f32[t * cols + s].is_finite() { fv = t as i32; break; } } first_valids[s] = fv; }

    let cuda = CudaEmd::new(0).expect("CudaEmd::new");
    let trio = cuda
        .emd_many_series_one_param_time_major_dev(&mid_tm_f32, cols, rows, &EmdParams { period: Some(period), delta: Some(delta), fraction: Some(fraction) }, &first_valids)
    ...
    for idx in 0..(rows * cols) {
        assert!(approx_eq(cpu_upper_tm[idx], g_upper[idx] as f64, tol), "upper mismatch at {}", idx);
        assert!(approx_eq(cpu_middle_tm[idx], g_middle[idx] as f64, tol), "middle mismatch at {}", idx);
        assert!(approx_eq(cpu_lower_tm[idx], g_lower[idx] as f64, tol), "lower mismatch at {}", idx);
    }
    Ok(())
}
