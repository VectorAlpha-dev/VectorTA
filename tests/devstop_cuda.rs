...
fn devstop_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sweep = DevStopBatchRange { period: (10, 30, 5), mult: (0.0, 2.0, 0.5), devtype: (0, 0, 0) };
    ...
    let (dev, meta) = cuda.devstop_batch_dev(&high_f32, &low_f32, &sweep, true).expect("devstop batch");
...
fn devstop_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cols = 16usize; let rows = 8192usize;
    ...
    let period = 20usize; let mult = 1.5f64;
    ...
    for s in 0..cols {
        ...
        let params = DevStopParams { period: Some(period), mult: Some(mult), devtype: Some(0), direction: Some("long".into()), ma_type: Some("sma".into()) };
        let input = DevStopInput { data: DevStopData::SliceHL(&h, &l), params };
    ...
    let cuda = CudaDevStop::new(0).expect("CudaDevStop::new");
    let dev = cuda
        .devstop_many_series_one_param_time_major_dev(&high_f32, &low_f32, cols, rows, period, mult as f32, true)
        .expect("devstop many-series");
    assert_eq!(dev.rows, rows); assert_eq!(dev.cols, cols);
    let mut host = vec![0f32; dev.len()]; dev.buf.copy_to(&mut host)?;
