...
#[cfg(feature = "cuda")]
#[test]
fn fwma_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[fwma_cuda_batch_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }

    let series_len = 4096usize;
    let mut data = vec![f64::NAN; series_len];
    for i in 30..series_len {
        let x = i as f64;
        data[i] = (x * 0.0013).cos() + 0.0007 * x;
    }
    ...
    let cpu = fwma_batch_with_kernel(&data, &sweep, Kernel::ScalarBatch)?;

    let cuda = CudaFwma::new(0).expect("CudaFwma::new");
    let data_f32: Vec<f32> = data.iter().map(|&v| v as f32).collect();
    let gpu = cuda
        .fwma_batch_dev(&data_f32, &sweep)
        .expect("cuda fwma_batch_dev");
    ...
    gpu.buf
        ...
        .expect("copy cuda fwma batch result");
...
#[cfg(feature = "cuda")]
#[test]
fn fwma_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() {
        eprintln!("[fwma_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device");
        return Ok(());
    }
    ...
    for j in 0..num_series {
        for t in j..series_len {
            ...
            data_tm[idx] = (x * 0.0021).sin() + 0.0005 * x;
        }
    }
    ...
    for j in 0..num_series {
        ...
        let out = FwmaBuilder::new().period(period).apply_slice(&series)?;
        for t in 0..series_len {
    ...
    let gpu = cuda
        .fwma_multi_series_one_param_time_major_dev(&data_tm_f32, num_series, series_len, &params)
        .expect("cuda fwma_multi_series_one_param_time_major_dev");
    ...
    gpu.buf
        .copy_to(&mut gpu_tm)
        .expect("copy cuda fwma many-series result");
