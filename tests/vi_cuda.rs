...
use my_project::indicators::vi::{vi_batch_with_kernel, ViBatchRange, ViInput, ViParams, vi_with_kernel};
...
fn vi_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 3..len { let x = i as f64; high[i] = (x*0.00123).sin() + 0.0007*x; low[i] = high[i] - 0.6; close[i] = (0.5*x).cos() + 0.0009*x; }
    ...
    let (pair, combos) = cuda.vi_batch_dev(&h32, &l32, &c32, &sweep).expect("vi_batch_dev");
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        assert!(approx_eq(cpu.plus[idx], plus_g[idx] as f64, tol), "plus mismatch at {}", idx);
        assert!(approx_eq(cpu.minus[idx], minus_g[idx] as f64, tol), "minus mismatch at {}", idx);
    }
    Ok(())
}
...
fn vi_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for s in 0..cols {
        ...
        for r in 0..rows { let idx = r*cols + s; h[r]=high_tm[idx]; l[r]=low_tm[idx]; c[r]=close_tm[idx]; }
    ...
    let cuda = CudaVi::new(0).expect("CudaVi::new");
    let pair = cuda
        .vi_many_series_one_param_time_major_dev(&h32, &l32, &c32, cols, rows, &ViParams { period: Some(period) })
    ...
    for idx in 0..(rows*cols) {
        assert!(approx_eq(plus_cpu_tm[idx],  plus_g[idx]  as f64, tol), "plus mismatch at {}", idx);
        assert!(approx_eq(minus_cpu_tm[idx], minus_g[idx] as f64, tol), "minus mismatch at {}", idx);
    }
    Ok(())
}
