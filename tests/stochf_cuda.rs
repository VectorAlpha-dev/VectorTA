...
fn stochf_cuda_batch_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[stochf_cuda_batch_matches_cpu] skipped - no CUDA device"); return Ok(()); }
    ...
    let sweep = StochfBatchRange { fastk_period: (5, 11, 2), fastd_period: (3, 3, 0) };
    ...
    for idx in 0..(cpu.rows * cpu.cols) {
        ...
        assert!(approx_eq(ck, gk, tol), "K mismatch at {}: cpu={} gpu={}", idx, ck, gk);
        assert!(approx_eq(cd, gd, tol), "D mismatch at {}: cpu={} gpu={}", idx, cd, gd);
    }
...
fn stochf_cuda_many_series_one_param_matches_cpu() -> Result<(), Box<dyn std::error::Error>> {
    if !cuda_available() { eprintln!("[stochf_cuda_many_series_one_param_matches_cpu] skipped - no CUDA device"); return Ok(()); }
    ...
    let fk = 9usize; let fd = 3usize; let mt = 0usize;
    ...
    for s in 0..cols {
        ...
        let input = StochfInput::from_slices(&h, &l, &c, StochfParams{ fastk_period: Some(fk), fastd_period: Some(fd), fastd_matype: Some(mt)});
        let out = my_project::indicators::stochf::stochf(&input)?;
        for t in 0..rows { let idx = t*cols+s; cpu_k_tm[idx] = out.k[t]; cpu_d_tm[idx] = out.d[t]; }
    }
    ...
    let cuda = CudaStochf::new(0).expect("CudaStochf::new");
    let params = StochfParams { fastk_period: Some(fk), fastd_period: Some(fd), fastd_matype: Some(mt) };
    let (k_dev_tm, d_dev_tm) = cuda
        .stochf_many_series_one_param_time_major_dev(&high_tm_f32, &low_tm_f32, &close_tm_f32, cols, rows, &params)
        .expect("stochf_many_series_one_param_time_major_dev");

    assert_eq!(k_dev_tm.rows, rows); assert_eq!(k_dev_tm.cols, cols);
    assert_eq!(d_dev_tm.rows, rows); assert_eq!(d_dev_tm.cols, cols);

    let mut gk_tm = vec![0f32; k_dev_tm.len()];
    let mut gd_tm = vec![0f32; d_dev_tm.len()];
    k_dev_tm.buf.copy_to(&mut gk_tm)?; d_dev_tm.buf.copy_to(&mut gd_tm)?;

    let tol = 1e-3; // small float32 tolerance
    for idx in 0..gk_tm.len() {
        assert!(approx_eq(cpu_k_tm[idx], gk_tm[idx] as f64, tol), "K mismatch at {}", idx);
        assert!(approx_eq(cpu_d_tm[idx], gd_tm[idx] as f64, tol), "D mismatch at {}", idx);
    }
