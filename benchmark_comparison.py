...
# Python benchmark with same methodology
...
for ma_type in ["sma", "ema", "wma", "alma", "hma"]:
    times = []
    for _ in range(10):  # Match Rust's 10 iterations
        start = time.perf_counter()
        _ = ta.ma(data, ma_type, 14)
        times.append((time.perf_counter() - start) * 1000)
    
    times.sort()
    python_times[ma_type] = times[len(times) // 2]  # Median
