...
def _import_module():
    ...
    if not hasattr(mod, 'alma_cuda_batch'):
        try:
            import my_project as alt
            if hasattr(alt, 'alma_cuda_batch'):
                return alt
        except Exception:
            pass
        try:
...
def main():
    ...
    series_len = 50_000
    x = np.full(series_len, np.nan, dtype=np.float64)
    ...
    def run_many_series():
        ti.alma_cuda_many_series_one_param(tm, 14, 0.85, 6.0)

    # Multi-stream batch with moderate combos
    X2 = 100_000
    x2 = np.full(X2, np.nan, dtype=np.float64)
    for i in range(3, X2):
        v = float(i)
    ...
    bench("alma_cuda_batch(50k x ~4k params)", run_batch)
    bench("alma_cuda_many_series_one_param(256 x 50k)", run_many_series)
    # multi-stream variant removed

    # New: 1,000,000 x 240 params (period sweep only)
    X3 = 1_000_000
    x3 = np.full(X3, np.nan, dtype=np.float64)
    for i in range(3, X3):
        v = float(i)
    ...
    bench("alma_cuda_batch(1M x 240 params)", run_batch_1m_240)

    # Optional very large case: 250k x ~4k params (may require >= 6-8GB VRAM)
    try:
        X4 = 250_000
        x4 = np.full(X4, np.nan, dtype=np.float64)
        for i in range(3, X4):
            v = float(i)
        ...
        bench("alma_cuda_batch(250k x ~4k params)", run_batch_250k_4k)
