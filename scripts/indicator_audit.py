...
def find_indicator_file(indicator: str) -> Path:
    ...
    # choose shortest path (prefer top-level over deeper if duplicates)
...
def detect_python_tests(indicator: str) -> Tuple[List[Path], List[Path]]:
    ...
    # Broader scan for secondary files that still call this indicator
...
def detect_rust_cuda_test(indicator: str) -> Optional[Path]:
    ...
    # Prefer file name match like <indicator>_cuda.rs
    name_matches = list(tests_dir.glob(f"{indicator}_cuda.rs"))
    if name_matches:
        return name_matches[0]
    # Fallback: content search for the indicator in *_cuda.rs
...
def compute_mod_path(indicator_file: Path) -> str:
    # Convert src/indicators/[subdirs]/<ind>.rs -> indicators::[subdirs]::<ind>
...
def strip_comments_keep_header(src: str) -> str:
    # Identify the leading comment/header block: lines that are empty or comment-only
    ...
    # Remove block comments from body
    body = re.sub(r"/\*.*?\*/", "", body, flags=re.S)
    # Remove line comments from body
    body = re.sub(r"(^|\s)//.*?$", r"\1", body, flags=re.M)
    # Collapse >2 consecutive blank lines to at most one
    body = re.sub(r"\n{3,}", "\n\n", body)
...
def ensure_python_env(auto_install: bool) -> bool:
    try:
        ...
        # try maturin import path; not strictly required if available as command
        try:
...
def run_rust_cuda_test(test_file: Path, use_cuda_stub: bool) -> bool:
    name = test_file.stem  # <indicator>_cuda
    env = os.environ.copy()
    if use_cuda_stub and not have_nvcc():
        env["NVCC"] = str(REPO_ROOT / "nvcc_stub.sh")
        env["CUDA_PLACEHOLDER_ON_FAIL"] = "1"
...
def main() -> None:
    ...
    # Decide which test families to run by default
    ...
    # (reserved spot) could add CUDA-specific prechecks here if needed
    ...
    if run_python:
        ...
        # Build Python module for CPU tests (and possibly CUDA if requested)
